...
import { collections as collectionSpecs, migrateArchiveShares } from '../src/db/arango-migrate';
...
if (!['localhost', '127.0.0.1', '::1'].includes(hostname)) throw new Error(`Refusing Gallery E2E against non-local Arango host ${hostname}.`);
const system = new Database({ url, auth: { username: process.env.ARANGO_USERNAME ?? 'root', password: process.env.ARANGO_ROOT_PASSWORD ?? '' } });
const databaseName = `gallery_e2e_${crypto.randomUUID().replaceAll('-', '')}`;
...
try {
  ...
  const [{ createGalleryRepository }, { createGalleryService }, { processImage }, { imageSchema }] = await Promise.all([
    import('../src/lib/gallery/repository'), import('../src/lib/gallery/service'), import('../src/lib/ai/image-processing'), import('../src/lib/db/images.node'),
  ]);
  ...
  const repository = createGalleryRepository(database, (operation) => withDatabaseTransaction(database, transactionCollections, (transaction) => operation(transaction)));
  let tokenSequence = 0;
  const service = createGalleryService({ repository, token: () => `gallery-e2e-token-${String(++tokenSequence).padStart(32, '0')}` });
  ...
  await database.collection('scopes').save({ _key: scopeKey, organizationKey: 'gallery-e2e-org', deletedAt: null });
  await database.collection('userOrganizations').import([{ _key: actorKey, organizationId: 'gallery-e2e-org', userId: newId(), orgRole: 'member', status: 'active' }, { _key: strangerKey, organizationId: 'gallery-e2e-org', userId: newId(), orgRole: 'member', status: 'active' }]);
  ...
  await database.collection('images').save({ _key: imageKey, scopeKey, ownerKey: actorKey, filename: 'launch.png', caption: 'A launch vehicle', storageKey: 'gallery/e2e.png', mimeType: 'image/png', sizeBytes: 24, width: 1, height: 1, requestHash: 'a'.repeat(64), embedding: Array(4096).fill(0), isFavorite: false, deletedAt: null, createdAt: now, updatedAt: now });
  ...
  const shareInput = { scopeKey, sourceType: 'image' as const, sourceKey: imageKey, ownerKey: actorKey, password: 'gallery password', now, idempotencyKey: 'share-e2e' };
  ...
  await database.collection('collectionMembers').save({ _key: destinationOwnerMembershipKey, scopeKey, collectionKey: destinationCollectionKey, memberKey: actorKey, role: 'owner', createdAt: now });
  if (membership.imageKey !== imageKey || assignment.sourceKey !== imageKey || 'tokenHash' in projected.share || 'passwordHash' in projected.share) throw new Error('Gallery projection verification failed.');
  const accessed = await service.accessGlobalShare({ token: projected.token, password: 'gallery password', at: now });
  if (accessed.share.key !== projected.share.key || 'tokenHash' in accessed.share || 'passwordHash' in accessed.share) throw new Error('Gallery active share lookup leaked secrets or returned the wrong source.');
  await database.collection('shares').update(projected.share.key, { revokedAt: now, updatedAt: now });
  let revokedRejected = false;
  try { await service.accessGlobalShare({ token: projected.token, password: 'gallery password', at: now }); } catch { revokedRejected = true; }
  if (!revokedRejected) throw new Error('Revoked Gallery share remained active.');
  ...
  await database.collection('documentShares').save({ _key: legacyKey, scopeKey, documentKey, token: 'legacy-gallery-e2e-token', permission: 'edit', createdAt: now, updatedAt: now });
  await migrateArchiveShares(database);
  ...
  const { createArchivePersistence } = await import('../src/lib/db/archive-persistence.node');
  const archive = createArchivePersistence(database as never);
  const dualKey = newId();
  await archive.insertShare({ key: dualKey, scopeKey, documentKey, permission: 'read', tokenHash: 'e'.repeat(64), createdAt: now, updatedAt: now });
  await archive.updateShare(scopeKey, dualKey, { revokedAt: now, updatedAt: now });
  ...
  if (!await archive.deleteShare(scopeKey, dualKey) || await database.collection('shares').documentExists(dualKey) || await database.collection('documentShares').documentExists(dualKey)) throw new Error('Dual-mode delete did not remove both records atomically.');
  ...
  try { await migrateArchiveShares(database); } catch { mismatchBlockedDrop = true; }
  ...
  const postDrop = await archive.insertShare({ key: newId(), scopeKey, documentKey, permission: 'read', tokenHash: 'f'.repeat(64), createdAt: now, updatedAt: now });
  const archivedPostDrop = await archive.updateShare(scopeKey, postDrop.key, { deletedAt: now, updatedAt: now });
  const restoredPostDrop = await archive.updateShare(scopeKey, postDrop.key, { deletedAt: null, updatedAt: now });
  const listed = await archive.listShares(scopeKey, [documentKey]);
  if (postDrop.documentKey !== documentKey || archivedPostDrop?.deletedAt !== now || restoredPostDrop?.deletedAt !== null || !listed.some((share) => share.key === postDrop.key) || 'sourceType' in postDrop || 'sourceKey' in postDrop) throw new Error('Post-drop Archive share compatibility failed.');
  console.log('Gallery E2E passed: Gallery authorization/shares, phased catch-up/cutover/drop, and post-drop Archive shares.');
} finally {
