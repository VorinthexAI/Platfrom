...
try {
  ...
  for (const source of sources) {
    ...
    await insertPreparedDocument({ key, scopeKey: source.scopeKey, folderKey: folderByScope.get(source.scopeKey), name: source.name, html: `<p>${source.content}</p>`, content: source.content, embedding, deletedAt: null, createdAt: now, updatedAt: now });
  }
