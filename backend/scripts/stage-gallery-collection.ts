...
function sellingCopy(image: GalleryImage, index: number) {
  if (SELLING_COPY[index]) return SELLING_COPY[index];
  const description = `${image.filename} ${image.caption ?? ""}`.toLowerCase();
  if (description.includes("archive") || description.includes("document") || description.includes("folder"))
    return "Keep your important ideas organized, connected, and ready when you need them.";
  if (description.includes("gallery") || description.includes("image") || description.includes("highlight") || description.includes("memories"))
    return "Keep the visual ideas behind your work together in one place.";
  if (description.includes("compass") || description.includes("city") || description.includes("place") || description.includes("trip"))
    return "Make every destination part of a journey you can keep building.";
  if (description.includes("signal") || description.includes("email") || description.includes("inbox"))
    return "Turn incoming messages into focused work with the right context.";
  if (description.includes("ascend") || description.includes("book") || description.includes("audio"))
    return "Build a library of knowledge that is ready to read or hear.";
  return "Bring the parts of your digital life together in one intelligent workspace.";
}
...
async function main() {
  ...
  const cursor = await db.query(
    `FOR collection IN collections
      FILTER LOWER(collection.name) == LOWER(@collectionName)
      LET images = (
        FOR relation IN collectionImages
          FILTER relation.collectionKey == collection._key
          SORT relation.createdAt ASC, relation._key ASC
          LET image = DOCUMENT(images, relation.imageKey)
          FILTER image != null
          RETURN image
      )
      RETURN { key: collection._key, scopeKey: collection.scopeKey, images }`,
    { collectionName },
  );
  const collections = await cursor.all() as Array<{
    key: string;
    scopeKey: string;
    images: GalleryImage[];
  }>;
  if (collections.length !== 1)
    throw new Error(`Expected one collection named "${collectionName}", found ${collections.length}.`);

  const [collection] = collections;
  if (collection.images.length === 0)
    throw new Error(`Collection "${collectionName}" has no images.`);

  await mkdir(outputDirectory, { recursive: true });
  await Promise.all(collection.images.map(async (image, index) => {
    const stored = await documentStorage.download(image.storageKey);
    const filename = `${String(index + 1).padStart(2, "0")}-${image.filename.replace(/[^a-zA-Z0-9._-]/g, "-")}`;
    const source = sharp(stored.bytes);
    const metadata = await source.metadata();
    if (!metadata.width || !metadata.height || metadata.height <= 128)
      throw new Error(`Invalid screenshot dimensions for ${image.filename}.`);
    // The supplied Android captures include system navigation controls below the app.
    const rendered = await source.extract({ left: 0, top: 0, width: metadata.width, height: metadata.height - 128 }).png().toBuffer();
    await writeFile(path.join(outputDirectory, filename), rendered);
    image.filename = filename;
  }));
  await writeFile(
    path.join(outputDirectory, "collection.json"),
    `${JSON.stringify({ collectionKey: collection.key, scopeKey: collection.scopeKey, images: collection.images }, null, 2)}\n`,
  );
  await writeFile(
    ...
    `${JSON.stringify({
      version: 1,
      defaults: {
        logo: path.resolve(import.meta.dir, "../../web/app/public/logos/vorinthex-mark.png"),
      },
      screenshots: collection.images.map((image, index) => ({
        id: String(index + 1).padStart(2, "0"),
        title: "Vorinthex AI",
        description: sellingCopy(image, index),
        screenshot: image.filename,
      })),
    }, null, 2)}\n`,
  );
  console.log(`Staged ${collection.images.length} images from ${collectionName} in ${outputDirectory}.`);
}
