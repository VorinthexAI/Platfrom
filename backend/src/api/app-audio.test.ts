...
import { createAppAudioHandler } from './app-audio';
...
describe('app audio HTTP API', () => {
  test('registers the authenticated strict route', async () => {
    const routes = new Hono(); registerRoutes(routes);
    expect((await routes.request('/app/audio', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })).status).toBe(401);
    const app = new Hono().post('/app/audio', createAppAudioHandler({ getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ input: { organizationKey, scopeKey }, context }), service: { generateDocument: async () => output, generateForTarget: async () => { throw new Error('unexpected'); } } }));
    const response = await app.request('/app/audio', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ organizationKey, scopeKey, input: { documentKey, text: 'forged' } }) });
    expect(response.status).toBe(400);
  });

  test('HTTP and Core converge on app.audio with trusted identity and scope', async () => {
    const calls: unknown[][] = [];
    ...
    await runTool('app.audio', '', { documentKey, voice: 'calm' }, { contentContext: context, appAudioService: service });
    const app = new Hono().post('/app/audio', createAppAudioHandler({ getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ input: { organizationKey, scopeKey }, context }), service }));
    const response = await app.request('/app/audio', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ organizationKey, scopeKey, input: { documentKey, voice: 'calm' } }) });
