...
import { userNotificationMarkReadInputSchema } from '@/lib/user-notifications/schemas';
import { InvalidUserNotificationCursorError, UserNotificationAccessError, UserNotificationNotFoundError, userNotificationService, type UserNotificationService } from '@/lib/user-notifications/service';
...
const markReadHttpSchema = z.object({ teamKey: z.string().cuid(), scopeKey: z.string().cuid(), read: userNotificationMarkReadInputSchema.shape.read.optional() }).strict();

function errorResponse(c: Context, error: unknown) {
  if (error instanceof AppNotificationAccessError || error instanceof UserNotificationAccessError) return c.json({ success: false, error: error.message }, 403);
  if (error instanceof UserNotificationNotFoundError) return c.json({ success: false, error: error.message }, 404);
  if (error instanceof InvalidUserNotificationCursorError) return c.json({ success: false, error: error.message }, 400);
...
export function createAppNotificationHandlers(dependencies: { service?: AppNotificationService; userNotifications?: UserNotificationService; getIdentity?: typeof getAuthIdentity; getInstallationIdentifier?: typeof currentEventIdentifier; authorize?: typeof authorizeContentExecution } = {}) {
  const service = dependencies.service ?? appNotificationService;
  const notifications = dependencies.userNotifications ?? userNotificationService;
  ...
  return {
    ...
    list: async (c: Context) => { try { const auth = await authenticated(c); if ('response' in auth) return auth.response; const body = await parseJson(c, listHttpSchema); const { teamKey, scopeKey, ...input } = body; const { context } = await (dependencies.authorize ?? authorizeContentExecution)({ teamKey, scopeKey }, authenticatedTeamContext(auth.identity)); const result = await runTool('notification.list', '', notificationListInputSchema.parse(input), { contentContext: context, userNotificationService: notifications }); return c.json({ success: true, data: result }); } catch (error) { return errorResponse(c, error); } },
    markRead: async (c: Context) => {
      try {
        const auth = await authenticated(c); if ('response' in auth) return auth.response;
        const body = await parseJson(c, markReadHttpSchema);
        const { teamKey, scopeKey, ...input } = body;
        const { context } = await (dependencies.authorize ?? authorizeContentExecution)({ teamKey, scopeKey }, authenticatedTeamContext(auth.identity));
        const parsed = userNotificationMarkReadInputSchema.parse({ ...input, notificationKey: c.req.param('notificationKey') });
        return c.json({ success: true, data: await runTool('notification.mark-read', '', parsed, { contentContext: context, userNotificationService: notifications }) });
