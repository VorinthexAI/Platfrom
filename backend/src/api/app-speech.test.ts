...
describe('app speech HTTP API', () => {
  test('registers the authenticated strict route', async () => {
    ...
    const app = new Hono().post('/app/speech', createAppSpeechHandler({ getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ input: { teamKey, scopeKey }, context }), service: { generateDocument: async () => output, generateForTarget: async () => { throw new Error('unexpected'); } } }));
    const response = await app.request('/app/speech', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, input: { documentKey, text: 'forged' } }) });
  ...
  test('HTTP and Core converge on app.speech with trusted identity and scope', async () => {
    ...
    const app = new Hono().post('/app/speech', createAppSpeechHandler({ getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ input: { teamKey, scopeKey }, context }), service, appScopeKey: newId() }));
    const response = await app.request('/app/speech', { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'speech-request' }, body: JSON.stringify({ teamKey, scopeKey, input: { documentKey, voice: 'calm' } }) });
  ...
  test('HTTP and Core share action debit semantics and HTTP preserves 402', async () => {
    ...
    await runTool('app.speech', '', { documentKey, voice: 'calm' }, { contentContext: context, appSpeechService: service, requestKey: 'core-speech', recordEvent: async () => {}, appScopeKey: 'cmrnlzf640001qc7kazsr96k5', billing });
    const app = new Hono().post('/app/speech', createAppSpeechHandler({ getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ input: { teamKey, scopeKey }, context }), service, recordEvent: async () => {}, appScopeKey: newId(), billing }));
    expect((await app.request('/app/speech', { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'http-speech' }, body: JSON.stringify({ teamKey, scopeKey, input: { documentKey, voice: 'calm' } }) })).status).toBe(200);
    expect(charges).toHaveLength(2);
    expect(charges.every((charge) => charge.actionSlug === 'speech' && charge.microSparks === 10_000)).toBe(true);

    const insufficient = new Hono().post('/app/speech', createAppSpeechHandler({ getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ input: { teamKey, scopeKey }, context }), service, recordEvent: async () => {}, appScopeKey: newId(), billing: { charge: async () => { throw new SparkRepositoryError('INSUFFICIENT_BALANCE', 'private'); } } }));
    expect((await insufficient.request('/app/speech', { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'insufficient' }, body: JSON.stringify({ teamKey, scopeKey, input: { documentKey, voice: 'calm' } }) })).status).toBe(402);
  });
});
