...
const teamKey = newId(), scopeKey = newId(), userKey = newId(), documentKey = newId(), messageKey = newId();
...
const authorize = async () => ({ input: { teamKey, scopeKey }, context });

function appWith(dependencies: Parameters<typeof createAppTransformationHandlers>[0]) {
  const handlers = createAppTransformationHandlers({ appScopeKey: newId(), ...dependencies });
...
describe('app transformation HTTP API', () => {
  test('registers both authenticated routes and rejects unknown model input', async () => {
    ...
    const response = await app.request('/app/enhance', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, input: { text: 'Draft', userKey } }) });
  ...
  test('HTTP and unified tools share raw text transformation services', async () => {
    ...
    const response = await appWith({ getIdentity: identity, authorize, service }).request('/app/enhance', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, input: { text: 'Draft.' } }) });
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ success: true, data: { text: 'Clear draft.' } });
    expect(calls.map((call) => call.slice(0, 3))).toEqual([
      ['enhance', { text: 'Draft.', instruction: undefined }, teamKey],
      ['enhance', { text: 'Draft.', instruction: undefined }, teamKey],
    ]);
  });
  ...
  test('Archive selectors converge on preview-only Content operations', async () => {
    ...
    const response = await appWith({ getIdentity: identity, authorize, executeContent: executeContent as never }).request('/app/translate', { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'preview-request' }, body: JSON.stringify({ teamKey, scopeKey, input: { documentKey, targetLanguage: 'French', save: false } }) });
    expect(response.status).toBe(200);
  ...
  test('Signal selectors converge on persisted email translation', async () => {
    ...
    const response = await appWith({ getIdentity: identity, authorize, email }).request('/app/translate', { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'http-request' }, body: JSON.stringify({ teamKey, scopeKey, input: { messageKey, targetLanguage: 'French' } }) });
    expect(response.status).toBe(200);
    expect(calls).toEqual([
      [{ userKey, teamKey, scopeKey }, { messageKey, targetLanguage: 'French', sourceLanguage: undefined }, 'tool-request'],
      [{ userKey, teamKey, scopeKey }, { messageKey, targetLanguage: 'French', sourceLanguage: undefined }, 'http-request'],
    ]);
  });
  ...
  test('HTTP and Core meter translation actions identically and preserve 402', async () => {
    ...
    await runTool('app.translate', '', { text: 'Draft', targetLanguage: 'French' }, { contentContext: context, appTransformationService: service, requestKey: 'core-translate', recordEvent: async () => {}, appScopeKey: 'cmrnlzf640001qc7kazsr96k5', billing });
