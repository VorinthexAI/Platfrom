...
const toolDomain = (teamKey: string, scopeKey: string, userKey: string) => ({ teamKey, runtimeScopeKey: scopeKey, principal: { kind: 'member', user: { key: userKey }, userTeam: { key: newId(), teamKey: teamKey, userId: userKey, status: 'active' } } }) as unknown as ToolContext;
...
describe('book HTTP handlers', () => {
  ...
  test('keeps POST /books as a thin call to BookService.create', async () => {
    const userKey = newId();
    const body = { teamKey: 'team', scopeKey: newId(), generationRequestKey: 'request-1', topic: 'Decision making', goal: 'Decide well', currentKnowledge: 'Basic familiarity', writingTone: 'Clear', language: 'English', archiveDocumentKeys: [], narratorVoiceKey: 'clear', narrationPace: 1 };
    const calls: unknown[][] = [];
    const app = new Hono();
    app.post('/books', createBookHandlers({ ...billingFixture, service: { create: async (...args: unknown[]) => { calls.push(args); return { key: newId() }; } } as never, getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ context: toolDomain(body.teamKey, body.scopeKey, userKey) }) }).create);
  ...
  test('HTTP and Core share book acceptance debits and insufficient-balance behavior', async () => {
    ...
    const body = { teamKey, scopeKey, generationRequestKey: 'http-book', topic: 'Decision making', goal: 'Decide well', currentKnowledge: 'Basic familiarity', writingTone: 'Clear', language: 'English', archiveDocumentKeys: [], narratorVoiceKey: 'clear', narrationPace: 1 };
    ...
    const context = toolDomain(teamKey, scopeKey, userKey);
    ...
    await runTool('book.create', '', modelInput, { contentContext: context, bookService: service, requestKey: 'core-book', recordEvent: async () => {}, appScopeKey: 'cmrnlzf640001qc7kazsr96k5', billing });
    ...
    await expect(runTool('book.create', '', modelInput, { contentContext: context, bookService: service, requestKey: 'core-insufficient', recordEvent: async () => {}, appScopeKey: 'cmrnlzf640001qc7kazsr96k5', billing: insufficientBilling })).rejects.toMatchObject({ code: 'INSUFFICIENT_BALANCE' });
    const insufficient = new Hono().post('/books', createBookHandlers({ service, getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ context }), recordEvent: async () => {}, appScopeKey: 'cmrnlzf640001qc7kazsr96k5', billing: insufficientBilling }).create);
  ...
  test('keeps topic suggestion HTTP and Core callers on the same canonical service method', async () => {
    ...
    const body = { teamKey, scopeKey, excludeTopics: ['Old idea'] };
  ...
  test('keeps goal suggestion HTTP and Core callers on the same canonical service method', async () => {
    ...
    const body = { teamKey, scopeKey, topic: 'Decision making', excludeGoals: ['Old goal'] };
  ...
  test('maps generation request key conflicts to HTTP 409', async () => {
    ...
    const body = { teamKey: 'team', scopeKey: newId(), generationRequestKey: 'request-1', topic: 'Decision making', goal: 'Decide well', currentKnowledge: 'Basic familiarity', writingTone: 'Clear', language: 'English', archiveDocumentKeys: [], narratorVoiceKey: 'clear', narrationPace: 1 };
    const app = new Hono();
    app.post('/books', createBookHandlers({ ...billingFixture, service: { create: async () => { throw new BookRepositoryError('conflict', 'Generation request key was reused with a different brief.'); } } as never, getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ context: toolDomain(body.teamKey, body.scopeKey, userKey) }) }).create);
    const response = await app.request('/books', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });
  ...
  test('keeps strict favorite HTTP and Core callers on the same canonical service method', async () => {
    const teamKey = newId(); const scopeKey = newId(); const userKey = newId(); const bookKey = newId(); const calls: unknown[][] = [];
    ...
    const body = { teamKey, scopeKey, isFavorite: true };
  ...
  test('keeps extension HTTP and Core callers on the canonical service with trusted context and request key injection', async () => {
    ...
    const handlers = createBookHandlers({ ...billingFixture, service, getIdentity: async () => ({ key: userKey, identityType: 'user' }), authorize: async () => ({ context: toolDomain(teamKey, scopeKey, userKey) }) }); const app = new Hono(); app.post('/books/:bookKey/extension/preview', handlers.extensionPreview); app.post('/books/:bookKey/extension', handlers.extensionGenerate);
  ...
  test('keeps HTTP and Core retry, cancel, and delete on the same authorized service methods', async () => {
    ...
    const serviceContext = { teamKey, scopeKey }; const domain = { teamKey, runtimeScopeKey: scopeKey, principal: { kind: 'member', user: { key: userKey }, userTeam: { key: newId(), teamKey: teamKey, userId: userKey, status: 'active' } } } as unknown as ToolContext;
  ...
  test('rejects non-user HTTP and inactive Core lifecycle callers before service execution', async () => {
    ...
    const teamKey = newId(); const userKey = newId(); const domain = { teamKey, runtimeScopeKey: newId(), principal: { kind: 'member', user: { key: userKey }, userTeam: { key: newId(), teamKey: teamKey, userId: userKey, status: 'inactive' } } } as unknown as ToolContext;
