...
import { MAX_DOCUMENT_SCAN_PAGE_BYTES } from '@/lib/ai/document-processing';
...
function normalizeDocumentUpload(input: unknown, maxBytes: number) {
  ...
  if (record.pages !== undefined) return normalizeDocumentPages(input);
  ...
  if (!isCanonicalUploadBase64(upload.content)) throw new ContentError('CONTENT_INVALID_INPUT', 'Document content must be canonical base64.', 'document.parse', { action: 'parse' });
...
function normalizeDocumentPages(input: unknown) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return input;
  const record = input as Record<string, unknown>;
  ...
  const normalized = pages.map((page) => {
    if (page.content.length > Math.ceil(MAX_DOCUMENT_SCAN_PAGE_BYTES / 3) * 4 || !isCanonicalUploadBase64(page.content)) throw new ContentError('CONTENT_INVALID_INPUT', 'Page content must be canonical base64 within the page size limit.', 'document.parse', { action: 'parse' });
    const bytes = new Uint8Array(Buffer.from(page.content, 'base64'));
    if (bytes.byteLength !== page.sizeBytes) throw new ContentError('CONTENT_INVALID_INPUT', 'Page size does not match its content.', 'document.parse', { action: 'parse' });
...
export function createContentToolHandler(dependencies: ContentToolHandlerDependencies = {}) {
  return async (c: Context) => {
    ...
    try {
      ...
      // A validated body key is an explicit caller key, not a missing key. Use
      // it for both billing and persistence before considering a hash fallback.
      if (!idempotencyKey && mutation && input && typeof input === 'object' && !Array.isArray(input)) idempotencyKey = (input as { idempotencyKey?: string }).idempotencyKey;
