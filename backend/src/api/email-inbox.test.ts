...
const overviewOutput = { accounts: [], selectedAccount: null, threads: [], drafts: [], tones: [], unassignedDrafts: [], counts: { all: 0, important: 0, urgent: 0, purchases: 0, needsAction: 0, filtered: 0, unread: 0, favorite: 0, trash: 0 }, nextCursor: null };
...
function appWith(overrides: Parameters<typeof createEmailHandlers>[0]) {
  const handlers = createEmailHandlers({
    authorize: async ({ teamKey: authorizedTeamKey, scopeKey: authorizedScopeKey }) => ({ context: { teamKey: authorizedTeamKey, runtimeScopeKey: authorizedScopeKey, principal: { kind: 'member', user: { key: userKey }, userTeam: { key: userKey, teamKey: authorizedTeamKey, userId: userKey, status: 'active' }, scopeMember: null } } as unknown as ToolContext }),
    recordEvent: async () => {},
...
describe('email inbox handlers', () => {
  test('passes authenticated team and scope context to overview', async () => {
    ...
    const response = await app.request('/email/overview', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, connectorKey, filter: 'urgent' }) });
    expect(response.status).toBe(200);
    expect(received).toEqual({ actor: { userKey, teamKey, scopeKey }, input: { connectorKey, filter: 'urgent', search: undefined, cursor: undefined, limit: undefined } });
  });
  ...
  test('passes composite overview input unchanged to the canonical service and rejects ambiguous transport input', async () => {
    ...
    const composite = { teamKey, scopeKey, connectorKey, readState: 'unread', facets: ['favorite', 'urgent', 'urgent'], search: 'plan', limit: 10 };
    ...
    expect(calls).toEqual([[{ userKey, teamKey, scopeKey }, { connectorKey, readState: 'unread', facets: ['favorite', 'urgent', 'urgent'], search: 'plan', limit: 10 }]]);
  ...
  test('routes strict inbox and tone semantic searches to their canonical services', async () => {
    ...
    const body = { teamKey, scopeKey, query: '  leadership  ', recordHistory: false };
    ...
    expect(calls.map((call: any) => call.slice(0, 3))).toEqual([
      ['inboxes', { userKey, teamKey, scopeKey }, { query: 'leadership', minimumScore: 0.55, limit: 50, recordHistory: false }],
      ['tones', { userKey, teamKey, scopeKey }, { query: 'leadership', minimumScore: 0.55, limit: 50, recordHistory: false }],
    ]);
  ...
  test('rejects unknown input and unauthenticated requests', async () => {
    ...
    const invalid = await app.request('/email/overview', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, unexpected: true }) });
    expect(invalid.status).toBe(400);
    const unauthorized = appWith({ getIdentity: (async () => null) as never, service: { overview: async () => ({}) } as never, oauth: {} as never });
    expect((await unauthorized.request('/email/overview', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey }) })).status).toBe(401);
  });
  ...
  test('maps stable email idempotency states to HTTP 409 with retryability', async () => {
    for (const [code, retryable] of [['EMAIL_IDEMPOTENCY_CONFLICT', false], ['EMAIL_IDEMPOTENCY_PENDING', true], ['EMAIL_IDEMPOTENCY_INDETERMINATE', false], ['EMAIL_IDEMPOTENCY_FAILED', false]] as const) {
      ...
      const response = await app.request('/email/overview', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey }) });
  ...
  test('keeps deployed sync and subscribe aliases strict and projects their legacy response shapes', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const body = { teamKey, scopeKey, connectorKey };
    ...
    expect(calls).toEqual([
      ['sync', { userKey, teamKey, scopeKey }, connectorKey],
      ['registerWatch', { userKey, teamKey, scopeKey }, connectorKey],
    ]);
  });
  ...
  test('keeps POST /email/sync and inbox.refresh on canonical EmailService.sync', async () => {
    ...
    const contentContext = {
      ...
      principal: { kind: 'member', user: { key: userKey }, userTeam: { key: connectorKey, teamKey: teamKey, userId: userKey, status: 'active' } },
    } as unknown as ToolContext;

    const response = await app.request('/email/sync', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, connectorKey }) });
    expect(response.status).toBe(200);
    ...
    expect(calls).toEqual([
      [{ userKey, teamKey, scopeKey }, connectorKey],
      [{ userKey, teamKey, scopeKey }, connectorKey],
    ]);
  });
  ...
  test('omits new connector fields from legacy strict mobile transport responses', async () => {
    ...
    const request = (path: string, body: object, method = 'POST') => app.request(path, { method, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, ...body }) });
    ...
    const current = await app.request('/email/overview', { method: 'POST', headers: { 'content-type': 'application/json', 'x-vorinthex-email-transport': '2' }, body: JSON.stringify({ teamKey, scopeKey }) });
  ...
  test('routes similarity, trash, and summaries through canonical service operations', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const context = { teamKey, scopeKey };
  ...
  test('routes strict bulk generated-email deletion with the trusted Idempotency-Key', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const request = (path: string, body: object) => app.request(path, { method: 'DELETE', headers: { 'content-type': 'application/json', 'idempotency-key': 'delete-request' }, body: JSON.stringify({ teamKey, scopeKey, ...body }) });
    ...
    expect(calls).toEqual([
      ['translations', { userKey, teamKey, scopeKey }, { messageKey: userKey, translationKeys: [translationKey] }, 'delete-request'],
      ['summaries', { userKey, teamKey, scopeKey }, { messageKey: userKey, summaryKeys: [summaryKey] }, 'delete-request'],
    ]);
  });
  ...
  test('requires one-time connection grants and strict drafting tones', async () => {
    ...
    const exchange = await app.request('/email/connect/exchange', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, code: `vrtx_email_grant_${'a'.repeat(20)}` }) });
    expect(exchange.status).toBe(401);
    const draft = await app.request('/email/drafts', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, threadKey: userKey, tone: '' }) });
    expect(draft.status).toBe(400);
    const valid = await app.request('/email/drafts', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, threadKey: userKey, tone: 'warm', replyMode: 'reply_all' }) });
    expect(valid.status).toBe(201);
    const invalidMode = await app.request('/email/drafts', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, threadKey: userKey, tone: 'warm', replyMode: 'all' }) });
  ...
  test('accepts only the Gmail connection transport', async () => {
    ...
    const context = { teamKey, scopeKey, name: 'Work' };
    ...
    expect(calls).toEqual([
      ['oauth', { userKey, teamKey, scopeKey, provider: 'gmail', name: 'Work', description: undefined, returnUri: 'vorinthexcore://capability/signal' }],
    ]);
  });
  ...
  test('routes strict disconnect through the authorized protocol boundary only', async () => {
    ...
    expect(calls).toEqual([[{ userKey, teamKey, scopeKey }, connectorKey]]);
  });
  ...
  test('routes Gmail-backed favorite through the same canonical operation as Core', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const response = await app.request(`/email/threads/${userKey}/favorite`, { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'request-1' }, body: JSON.stringify({ teamKey, scopeKey, isFavorite: true }) });
    expect(response.status).toBe(200);
    expect(calls).toEqual([[{ userKey, teamKey, scopeKey }, { threadKey: userKey, isFavorite: true }, false, 'request-1']]);
  });
  ...
  test('keeps canonical HTTP thread reads viewer-safe and rejects legacy compound markRead input', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const request = (body: object) => app.request(`/email/threads/${userKey}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, ...body }) });
    expect((await request({})).status).toBe(200);
    expect((await request({ cursor: 'next-page' })).status).toBe(200);
    expect((await request({ markRead: true })).status).toBe(400);
    expect(calls).toEqual([
      [{ userKey, teamKey, scopeKey }, userKey, undefined],
      [{ userKey, teamKey, scopeKey }, userKey, 'next-page'],
    ]);
  });
  ...
  test('routes strict new drafts and tone listing through the canonical service', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const input = { teamKey, scopeKey, connectorKey, to: ['recipient@example.com'], subject: 'Planning', tone: 'direct', attachments: [{ type: 'document', key: userKey }] };
    ...
    const preserved = { teamKey, scopeKey, connectorKey, to: ['recipient@example.com'], subject: '', authoredBody: '', generationMode: 'preserve' };
    ...
    expect((await app.request('/email/drafts', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey, threadKey: userKey, tone: 'direct', senderIdentity: { displayName: 'Untrusted' } }) })).status).toBe(400);
    expect((await app.request('/email/tones/list', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teamKey, scopeKey }) })).status).toBe(200);
    expect(calls).toEqual([
      ['draftNew', { userKey, teamKey, scopeKey }, { connectorKey, to: input.to, generationMode: 'generate', subject: input.subject, tone: input.tone, attachments: input.attachments }, expect.any(String)],
      ['draftNew', { userKey, teamKey, scopeKey }, { connectorKey, to: preserved.to, generationMode: 'preserve', subject: '', authoredBody: '' }, expect.any(String)],
      ['tones', { userKey, teamKey, scopeKey }],
    ]);
  ...
  test('routes draft and custom tone hard deletion with trusted request keys', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const init = { method: 'DELETE', headers: { 'content-type': 'application/json', 'idempotency-key': 'delete-1' }, body: JSON.stringify({ teamKey, scopeKey }) };
    ...
    expect(calls).toEqual([
      ['draft', { userKey, teamKey, scopeKey }, { draftKey: userKey }, 'delete-1'],
      ['tone', { userKey, teamKey, scopeKey }, { toneKey: userKey }, 'delete-1'],
    ]);
  });
  ...
  test('routes strict legacy draft assignment through the canonical service', async () => {
    ...
    expect(calls).toEqual([[{ userKey, teamKey, scopeKey }, { draftKey: userKey, connectorKey }]]);
  });
  ...
  test('routes strict body and attachment draft patches through the canonical service', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const request = (body: object) => app.request(`/email/drafts/${userKey}`, { method: 'PATCH', headers: { 'content-type': 'application/json', 'idempotency-key': 'update-1' }, body: JSON.stringify({ teamKey, scopeKey, ...body }) });
    ...
    expect(calls).toEqual([
      [{ userKey, teamKey, scopeKey }, { draftKey: userKey, finalContent: 'Reviewed', attachments }, 'update-1'],
      [{ userKey, teamKey, scopeKey }, { draftKey: userKey, attachments: [] }, 'update-1'],
    ]);
  });
  ...
  test('routes an optional send-time reply mode through the canonical service', async () => {
    ...
    const app = appWith({ getIdentity: identity as never, service: service as never, oauth: {} as never });
    const body = { teamKey, scopeKey, replyMode: 'reply_all' };
    ...
    expect(calls).toEqual([[{ userKey, teamKey, scopeKey }, userKey, undefined, 'send-1', 'reply_all']]);
  });
  ...
  test('routes strict inbox and custom tone mutations through the canonical service', async () => {
    ...
    const toolContext = { teamKey, runtimeScopeKey: scopeKey, principal: { kind: 'member', user: { key: userKey }, userTeam: { key: connectorKey, teamKey: teamKey, userId: userKey, status: 'active' } } } as unknown as ToolContext;
    const app = appWith({
