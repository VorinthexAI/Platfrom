...
async function deliverPrivateBetaTicketEmail(input: { email: string; name?: string | null }) {
  ...
  await sendBrandedEmail({
    ...
    preheader: 'Your ticket is confirmed. Receipt will arrive separately.',
    label: 'Ticket',
    ...
    bodyHtml: [
      ...
      '<p style="margin:0 0 16px;">Thank you for believing in Vorinthex early.</p>',
      '<p style="margin:0;">Your receipt will arrive separately. More signals soon.</p>',
    ].join(''),
    actionUrl: appUrl('/'),
    actionLabel: 'Open Vorinthex',
    supportingHtml: 'Keep this email for your records.',
