...
const assurance = { teamMembershipKey: newId(), teamMfaVersion: 4 };
...
const dependencies = (service: TagHandlerDependencies['service']): TagHandlerDependencies => ({
  service,
  getIdentity: async () => ({ key: userKey, identityType: 'user', teamMembershipKey: assurance.teamMembershipKey, teamMfaVersion: assurance.teamMfaVersion }),
  authorize: async (selector, options) => {
    expect(selector).toEqual({ teamKey, scopeKey });
    expect(options.authenticatedUserKey).toBe(userKey);
    expect(options.teamAssurance).toEqual(assurance);
    return { context };
  },
});
...
describe('tag HTTP and unified integrations', () => {
  test('routes strict CRUD requests to the canonical service with trusted context', async () => {
    ...
    expect((await request(app, '/tags/list', { teamKey, scopeKey, limit: 10 })).status).toBe(200);
    expect((await request(app, '/tags', { teamKey, scopeKey, key: tagKey, name: ' Plan ' })).status).toBe(201);
    expect((await request(app, `/tags/${tagKey}`, { teamKey, scopeKey, name: 'Roadmap' }, 'PATCH')).status).toBe(200);
    expect((await request(app, `/tags/${tagKey}`, { teamKey, scopeKey }, 'DELETE')).status).toBe(200);
  ...
  test('rejects unknown, forged, duplicate, invalid-action, and oversized assignment input', async () => {
    ...
    const app = appWith(dependencies(service));
    expect((await request(app, '/tags/list', { teamKey, scopeKey, teamMembershipKey: newId() })).status).toBe(400);
    expect((await request(app, '/tags', { teamKey, scopeKey, name: 'x', userKey })).status).toBe(400);
    expect((await request(app, `/tags/${tagKey}`, { teamKey, scopeKey, name: 'x', tagKey }, 'PATCH')).status).toBe(400);
    expect((await request(app, `/tags/${tagKey}`, { teamKey, scopeKey }, 'PATCH')).status).toBe(400);
    expect((await request(app, `/tags/${tagKey}`, { teamKey, scopeKey, name: 42 }, 'PATCH')).status).toBe(400);
    expect((await request(app, `/tags/${tagKey}`, { teamKey, scopeKey, source: 'ai' }, 'DELETE')).status).toBe(400);
    const target = { type: 'document', key: targetKey };
    expect((await request(app, '/tags/assignments?action=tag', { teamKey, scopeKey, targets: [target, target], tagKeys: [tagKey] })).status).toBe(400);
    expect((await request(app, '/tags/assignments?action=tag', { teamKey, scopeKey, targets: [target], tagKeys: [tagKey, tagKey] })).status).toBe(400);
    expect((await request(app, '/tags/assignments?action=tag', { teamKey, scopeKey, targets: [target], tagKeys: [tagKey], source: 'ai' })).status).toBe(400);
    expect((await request(app, '/tags/assignments?action=replace', { teamKey, scopeKey, targets: [target], tagKeys: [tagKey] })).status).toBe(400);
    expect((await request(app, '/tags/assignments?action=tag&source=ai', { teamKey, scopeKey, targets: [target], tagKeys: [tagKey] })).status).toBe(400);
    expect((await request(app, '/tags/assignments?action=tag', { teamKey, scopeKey, targets: Array.from({ length: 11 }, () => ({ type: 'document', key: newId() })), tagKeys: Array.from({ length: 10 }, () => newId()) })).status).toBe(400);
    expect(calls).toBe(0);
  });
  ...
  test('expands HTTP desired state and preserves HTTP/Core source parity', async () => {
    ...
    expect((await request(app, '/tags/assignments?action=untag', { teamKey, scopeKey, targets: [document, image], tagKeys: [tagKey, secondTagKey] })).status).toBe(200);
  ...
  test('applies one Archive assignment across heterogeneous folder, document, and file targets', async () => {
    ...
    const response = await request(appWith(dependencies(service)), '/tags/assignments?action=tag', { teamKey, scopeKey, targets: [folder, authoredDocument, uploadedFile], tagKeys: [tagKey] });
  ...
  test('routes distinct batch-list targets and preserves the additive response shape', async () => {
    ...
    const response = await request(appWith(dependencies(service)), '/tags/list', { teamKey, scopeKey, targets: [document, book], limit: 25 });
    expect(response.status).toBe(200);
  ...
  test('strictly rejects duplicate, ambiguous, empty, oversized, and unknown batch-list transport input', async () => {
    ...
    const target = { type: 'document', key: targetKey };
    expect((await request(app, '/tags/list', { teamKey, scopeKey, targets: [target, target] })).status).toBe(400);
    expect((await request(app, '/tags/list', { teamKey, scopeKey, target, targets: [target] })).status).toBe(400);
    expect((await request(app, '/tags/list', { teamKey, scopeKey, targets: [] })).status).toBe(400);
    expect((await request(app, '/tags/list', { teamKey, scopeKey, targets: Array.from({ length: 101 }, () => ({ type: 'document', key: newId() })) })).status).toBe(400);
    expect((await request(app, '/tags/list', { teamKey, scopeKey, targets: [target], assignmentMode: 'direct' })).status).toBe(400);
    expect(calls).toBe(0);
  });
  ...
  test('returns nondisclosing not-found for an unauthorized batch-list target', async () => {
    ...
    const response = await request(appWith(dependencies(service)), '/tags/list', { teamKey, scopeKey, targets: [{ type: 'book', key: newId() }] });
