...
describe('MFA team selection HTTP boundary', () => {
  test('uses strict transport input and the canonical selection service', async () => {
    const calls: unknown[] = [];
    const service = { select: async (input: unknown, selected: ToolContext) => { calls.push(['select', input, selected]); return { status: 'selected' }; } } as never;
    const handlers = createTeamHandlers({ service, getIdentity: async () => ({ key: 'user-1', identityType: 'user' }), resolveContext: async () => context });
    const app = new Hono(); app.post('/teams/select', handlers.select);
    ...
    expect(calls.map((call) => (call as unknown[])[0])).toEqual(['select']);
  ...
  test('rejects unauthenticated requests', async () => {
    ...
    const app = new Hono(); app.post('/teams/select', handlers.select);
    expect((await app.request('/teams/select', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ targetTeamKey: 'team-1' }) })).status).toBe(401);
  });
