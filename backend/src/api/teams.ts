...
import { teamSelectInputSchema, teamService, TeamServiceError, type TeamService } from '@/lib/teams';
...
export function createTeamHandlers(dependencies: TeamHandlerDependencies = {}) {
  ...
  const select = async (c: Context) => {
    ...
    try {
      const input = await parseJson(c, teamHttpSelectInputSchema);
      const context = await (dependencies.resolveContext ?? defaultContext)(identity);
      const result = await service.select(teamSelectInputSchema.parse(input), context);
