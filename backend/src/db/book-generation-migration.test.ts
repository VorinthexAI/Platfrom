...
describe('canonical book persistence migration', () => {
  ...
  test('backfills durable legacy defaults without inventing resumable input', () => {
    ...
    expect(legacyBookChapterPatch({ title: 'Opening', description: 'Brief', objective: 'Orient' })).toMatchObject({ evidenceKeyPoints: ['Orient'], targetWordMin: 400, targetWordMax: 450 });
