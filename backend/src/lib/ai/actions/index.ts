import { ACTION_IDS, actionDefinitionSchema, isValidActionIdFormat, type ActionDefinition, type ActionId } from './types';
...
export { ACTION_IDS, actionIdSchema, actionDefinitionSchema, type ActionDefinition, type ActionId } from './types';
...
/**
 * Verifies the registry against `ACTION_IDS`: every id present, no unknown
...
export function assertActionRegistryIntegrity(): void {
  ...
  for (const id of ACTION_IDS) {
    if (seen.has(id)) throw new Error(`Duplicate action id in ACTION_IDS: ${id}`);
    seen.add(id);
