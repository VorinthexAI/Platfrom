...
test('defines mode-routed web search with strict grounded-text contracts', () => {
  expect(webSearchAction.models).toEqual([{ provider: 'openrouter', model: 'google.gemini-2.5-flash-lite', priority: 100 }, { provider: 'openai', model: 'openai.gpt-5.6-luna', priority: 90 }]);
