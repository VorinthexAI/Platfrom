...
describe('split agent execution storage', () => {
  test('agentRuns contains summary fields only', () => {
    ...
    expect(Object.keys(run)).toEqual(['key', 'organizationKey', 'scopeKey', 'agentKey', 'principalType', 'userOrganizationKey', 'authorization', 'status', 'reason', 'score', 'startedAt', 'endedAt', 'elapsedMs', 'createdAt']);
    expect(run).toMatchObject({ principalType: 'system', userOrganizationKey: null, authorization: null });
