...
function fixture(role: 'owner' | 'admin' = 'owner') {
  ...
  const genesis = agentSchema.parse({ key: newId(), slug: 'genesis', name: 'Genesis', title: 'Agent Architect', scopeKey: beaconHome.key });
  ...
  const runtimeData = {
    async getAgent(key: string) { return key === beacon.key ? beacon : null; }, async getScope(key: string) { return key === beaconHome.key ? beaconHome : null; }, async getOrganization(key: string) { return key === organization.key ? organization : null; },
  ...
  return { organization, scope, user, membership, beacon, genesis, runtimeData, runs, steps, runStore, stepStore, events };
}
