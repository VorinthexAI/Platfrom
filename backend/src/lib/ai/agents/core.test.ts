...
describe('internal agents', () => {
  ...
  test('loads selected definitions only, validates props in dispatcher, and returns failed status to the AI', async () => {
    const inputs: CoreChatInput[] = []; const dispatchSchema = z.object({ name: z.string() }).strict();
  ...
  test('rejects invalid calls and forces a final answer after four executions', async () => {
    ...
    let executions = 0;
    await expect(executeCoreAgent(request(), { toolContext }, { stream: queue(repeated), tools: { names: ['folder.create'], definitions: [definition('folder.create')], execute: async () => { executions += 1; return {}; } } })).resolves.toMatchObject({ message: 'Execution limit reached.', tools: expect.any(Array) });
  ...
  test('propagates abort and safely omits oversized successful results', async () => {
    ...
    const result = await runAgent(coreAgent, request(), { toolContext }, { stream: queue([[text('{"tools":["folder.create"],"message":""}'), done], [call('x', 'folder.create', {}), done], [text('{"tools":[],"message":"Too large."}'), done]]), tools: { names: ['folder.create'], definitions: [definition('folder.create')], execute: async () => ({ data: 'x'.repeat(50_000) }) } });
