...
import { AGENT_ARCHITECT_SKILL_KEY, GENESIS_AGENT_KEY, GENESIS_AGENT_SKILL_KEY, GenesisSeedPrerequisiteError, loadGenesisSeedFiles, seedGenesis, type GenesisSeedDataSource } from './seed';

function fixture(organizationKey = newId()) {
  const now = '2026-07-16T00:00:00.000Z';
  const organization = organizationSchema.parse({ key: organizationKey, name: 'Vorinthex', createdAt: now, updatedAt: now });
  const scope = scopeSchema.parse({ key: newId(), organizationKey: organization.key, slug: 'launch', name: 'Launch', summary: 'Build agents.', description: 'Build agents.', position: 1 });
  const calls: string[] = [];
  const source: GenesisSeedDataSource = {
    async requireOrganization() { calls.push('organization'); return organization; }, async findScope() { calls.push('scope'); return scope; },
    ...
    async removeAgentTools() { calls.push('removeTools'); },
...
describe('Genesis canonical seed', () => {
  test('contains the exact skill and validated fixed CUIDs', async () => {
    ...
    expect(seed.seed.skills).toHaveLength(1); expect(seed.seed.agents).toHaveLength(1); expect(seed.seed.agentSkills).toHaveLength(1); expect(seed.seed.agentTools).toHaveLength(0);
    expect(seed.seed.skills[0]?.key).toBe(AGENT_ARCHITECT_SKILL_KEY);
    expect(seed.seed.agents[0]?.key).toBe(GENESIS_AGENT_KEY);
    expect(seed.seed.agents[0]?.scopeSlug).toBe('launch');
  ...
  test('seeds Agent Architect with no tool grants', async () => {
    ...
    expect(f.calls).toEqual(['organization', 'scope', 'skill', 'agent', 'agentSkill', 'removeTools', 'runtime']);
    expect(f.calls.some((call) => call.includes('ask'))).toBe(false);
  });
  test('accepts the preserved pre-CUID root organization key', async () => {
    const f = fixture('legacy-root-key');
    expect((await seedGenesis(f.organization.key, f.source)).organizationKey).toBe('legacy-root-key');
  });
  test('refuses to seed without the canonical organization', async () => {
    const f = fixture();
    f.source.requireOrganization = async () => null;
