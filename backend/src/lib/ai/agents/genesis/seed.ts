...
const defaultSeedDataSource: GenesisSeedDataSource = {
  ...
  async verifyRuntime(agentKey) {
    ...
    if (runtime.tools.length !== 0) throw new GenesisSeedPrerequisiteError('Genesis must not expose tools');
  },
};
...
/** Idempotently seeds Genesis with exactly one skill and no tools. */
