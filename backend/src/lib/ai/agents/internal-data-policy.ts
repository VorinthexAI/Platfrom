const disclosureIntent = /\b(what|which|who|show|list|tell|reveal|describe|give|dump|print|expose|how is|how are|visa|vilka|vilken|vem|vad ar|beratta|avsloja|lista|beskriv|muestra|cuales|revela|describe|dime|montre|quels|revele|decris|zeige|welche|enthulle|beschreibe)\b/;
const protectedTarget = /\b(system prompt|developer prompt|hidden instructions?|tool schemas?|source code|codebase|api keys?|credentials?|environment variables?|database (?:fields?|schema|structure|tables?|collections?)|collection (?:fields?|schema|structure)|table (?:fields?|schema|structure)|internal fields?|underlying (?:ai )?model|base (?:ai )?model|model (?:name|identity|version|provider|vendor|architecture)|(?:ai |language )?model (?:are you|do you use|powers you)|who (?:trained|built|made|created|developed|provides|powers) you|(?:trained|built|made|created|developed|provided|powered) by|databas(?:ens)? (?:falt|schema|struktur|tabeller|samlingar)|kallkod|systemprompt|dolda instruktioner|vilken (?:ai )?modell (?:ar|anvander) du|vem (?:tranade|byggde|skapade|utvecklade) dig|base de datos|datenbank)\b/;
const platformOwner = /\b(vorinthex|platform|plattform(?:en|ens)?|internal|intern(?:a|t)?|our|vara|vart)\b/;
const refusal = /\b(cannot|can't|will not|won't|do not|don't|refuse|kan inte|kommer inte|far inte)\b.{0,80}\b(reveal|provide|show|disclose|share|avsl(?:o|ö)ja|visa|lamna ut)\b/;
const explicitSecret = /\b(?:sk-[a-z0-9_-]{16,}|akia[0-9a-z]{16}|(?:api[_ -]?key|secret|password|token)\s*[:=]\s*["']?[a-z0-9_./+=-]{12,})\b|-----begin (?:rsa |ec |openssh )?private key-----/i;
const internalIdentifier = /\bc[a-z0-9]{24}\b/gi;
const modelIdentityDisclosure = /\b(?:i am|i'm|i use|my (?:model|provider)|core (?:uses|is)|this assistant (?:uses|is)|powered by|trained by)\b.{0,120}\b(?:ai|language model|model|provider|vendor|google|openai|anthropic|gemini|gpt|claude|llama|mistral)\b/i;

function normalized(value: string) {
  return value.normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase().replace(/[_`'"()[\]{}:;,.!?/\\-]+/g, ' ').replace(/\s+/g, ' ').trim();
}

/** High-precision deterministic backstop; broader cases remain covered by the model policy. */
export function requestsPlatformInternals(message: string) {
  const value = normalized(message);
  return disclosureIntent.test(value) && protectedTarget.test(value);
}

/** Rejects high-confidence generated disclosures without blocking ordinary user-owned content. */
export function disclosesPlatformInternals(message: string) {
  if (explicitSecret.test(message)) return true;
  const value = normalized(message);
  return !refusal.test(value) && ((platformOwner.test(value) && protectedTarget.test(value)) || modelIdentityDisclosure.test(value));
}

export function protectPlatformOutput(message: string) {
  const redacted = message
    .replace(/\s*\(\s*(?:id|key)\s*:\s*c[a-z0-9]{24}\s*\)/gi, '')
    .replace(internalIdentifier, '[internal identifier hidden]');
  return disclosesPlatformInternals(redacted) ? PLATFORM_INTERNALS_REFUSAL : redacted;
}

export const PLATFORM_INTERNALS_REFUSAL = 'I cannot provide Vorinthex internal implementation details.';
