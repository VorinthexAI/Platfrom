...
export const internalAgentRequestSchema = z.object({
  ...
  const documents = value.attachments.filter((attachment) => attachment.kind === 'document');
  if (documents.reduce((total, document) => total + document.bytes.byteLength, 0) > CORE_CHAT_MAX_FILE_BYTES_TOTAL) context.addIssue({ code: 'custom', path: ['attachments'], message: `Agent file bytes must not exceed ${CORE_CHAT_MAX_FILE_BYTES_TOTAL} bytes.` });
});
...
export const agentToolStatusSchema = agentToolInvocationSchema.extend({
  ...
  result: z.unknown().refine((value) => { try { return Buffer.byteLength(JSON.stringify(value) ?? '', 'utf8') <= 100_000; } catch { return false; } }, 'Tool evidence exceeds 100000 bytes.').optional(),
