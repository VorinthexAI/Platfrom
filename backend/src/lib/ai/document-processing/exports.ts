...
import type { ExtractedBlock } from './schemas';
...
const documentExportInputSchema = z.object({
  ...
  const sources = Number(input.html !== undefined) + Number(input.content !== undefined);
  if (sources !== 1) context.addIssue({ code: z.ZodIssueCode.custom, message: 'Exactly one of html or content is required for document export.' });
});
...
function documentXml(blocks: ExtractedBlock[]): string {
  ...
  const visit = (block: ExtractedBlock, listPrefix?: string) => {
    if (block.type === 'heading') addParagraph(block.text ?? '', `<w:outlineLvl w:val="${(block.level ?? 1) - 1}"/>`);
    else if (block.type === 'paragraph' || block.type === 'codeBlock' || block.type === 'blockquote') addParagraph(`${listPrefix ?? ''}${block.text ?? ''}`);
    else if (block.type === 'horizontalRule') addParagraph('---');
    else if (block.type === 'listItem') {
      addParagraph(`${listPrefix ?? '\u2022 '}${block.text ?? ''}`);
      (block.children ?? []).forEach((child) => visit(child));
    } else if (block.type === 'orderedList') (block.children ?? []).forEach((child, index) => visit(child, `${index + 1}. `));
    else if (block.type === 'bulletList') (block.children ?? []).forEach((child) => visit(child, '\u2022 '));
    else if (block.type === 'tableRow') addParagraph((block.children ?? []).map((child) => child.text ?? '').join('\t'));
    else (block.children ?? []).forEach((child) => visit(child));
  };
...
export function htmlToDocx(input: string): Uint8Array {
  ...
  return zip([
    ...
    { name: 'word/document.xml', content: documentXml(htmlToExtractedBlocks(html)) },
  ]);
}
...
export async function generateDocumentExport(input: DocumentExportInput, options: { pdfRenderer?: PdfRenderer } = {}): Promise<DocumentExportResult> {
  ...
  const html = sanitizeDocumentHtml(parsed.html ?? plainContentToHtml(parsed.content!));
  let bytes: Uint8Array;
  if (format === 'html') bytes = textEncoder.encode(html);
  else if (format === 'txt') bytes = textEncoder.encode(htmlToPlainText(html));
  else if (format === 'md') bytes = textEncoder.encode(htmlToMarkdown(html));
  else if (format === 'docx') bytes = htmlToDocx(html);
  else {
