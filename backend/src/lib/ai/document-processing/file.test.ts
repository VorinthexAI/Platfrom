...
test('dispatches document and scan inputs through the canonical AWS file executor', async () => {
  ...
  const document = await action.execute({ operation: 'document', storageKey: 'content/report.pdf', filename: 'report.pdf', mimeType: 'application/pdf', bytes: new Uint8Array([1]) }, 'team');
  const scan = await action.execute({ operation: 'scan', storageKey: 'content/page.png', mimeType: 'image/png', bytes: new Uint8Array([1]) }, 'team');
