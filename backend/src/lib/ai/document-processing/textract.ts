...
const textract = new TextractClient({ region: documentTextractRegion, endpoint: documentTextractEndpoint, credentials: documentTextractCredentials });
...
async function ensureDevelopmentBucket(bucket: string) {
  ...
  developmentBucketReady ??= (async () => {
    let exists = true;
    try { await textractStorage.send(new HeadBucketCommand({ Bucket: bucket })); }
    catch (error) {
    ...
    if (!exists) {
      try { await textractStorage.send(new CreateBucketCommand({ Bucket: bucket, CreateBucketConfiguration: { LocationConstraint: 'eu-west-1' } })); }
    ...
    await configure(() => textractStorage.send(new PutPublicAccessBlockCommand({ Bucket: bucket, PublicAccessBlockConfiguration: { BlockPublicAcls: true, BlockPublicPolicy: true, IgnorePublicAcls: true, RestrictPublicBuckets: true } })));
    await configure(() => textractStorage.send(new PutBucketEncryptionCommand({ Bucket: bucket, ServerSideEncryptionConfiguration: { Rules: [{ ApplyServerSideEncryptionByDefault: { SSEAlgorithm: 'AES256' } }] } })));
    await configure(() => textractStorage.send(new PutBucketLifecycleConfigurationCommand({ Bucket: bucket, LifecycleConfiguration: { Rules: [{ ID: 'expire-textract-inputs', Status: 'Enabled', Filter: { Prefix: 'textract/' }, Expiration: { Days: 1 }, AbortIncompleteMultipartUpload: { DaysAfterInitiation: 1 } }] } })));
  })().catch((error) => { developmentBucketReady = undefined; throw error; });
...
const wait = (milliseconds: number, signal: AbortSignal) => new Promise<void>((resolve, reject) => {
  ...
  function aborted() { clearTimeout(timeout); reject(signal.reason); }
  signal.addEventListener('abort', aborted, { once: true });
});

export interface DocumentOcr { extract(storageKey: string, bytes?: Uint8Array): Promise<ExtractionResult> }
export interface DocumentImageOcr { extract(storageKey: string, bytes: Uint8Array): Promise<ExtractionResult> }
...
export function textractBlocksToExtractionResult(blocks: Block[]): ExtractionResult {
  ...
  return { extractedText, metadata: { provider: 'aws-textract', pages: pages.size, ...(confidences.length ? { averageConfidence: confidences.reduce((sum, confidence) => sum + confidence, 0) / confidences.length, minimumConfidence: Math.min(...confidences) } : {}) } };
}
...
export function createAwsTextractDocumentOcr(options: DocumentOcrOptions = {}): DocumentOcr {
  ...
  const extract: DocumentOcr['extract'] = async (storageKey, bytes) => {
    const stagingBucket = options.stagingBucket ?? process.env.CONTENT_TEXTRACT_BUCKET ?? documentTextractBucket ?? (process.env.NODE_ENV === 'development' && documentTextractUsesAwsCredentials ? developmentTextractBucket : undefined);
    ...
    try {
      if (stagingBucket) {
        ...
        if (storageClient === textractStorage && documentTextractUsesAwsCredentials) await storageClient.send(new HeadObjectCommand({ Bucket: bucket, Key: objectKey }), { abortSignal: controller.signal });
      }
      const started = await textractClient.send(new StartDocumentTextDetectionCommand({ DocumentLocation: { S3Object: { Bucket: bucket, Name: objectKey } }, ClientRequestToken: createHash('sha256').update(bucket).update('\0').update(objectKey).digest('hex') }), { abortSignal: controller.signal });
      ...
      do {
        ...
        do {
          ...
          if (response.JobStatus === 'FAILED' || response.JobStatus === 'PARTIAL_SUCCESS') throw new Error('AWS Textract could not extract the document.');
      ...
      if (process.env.NODE_ENV === 'development' && error instanceof Error) throw new Error(`Textract staging ${bucket}/${objectKey} failed: ${error.message}`, { cause: error });
      throw error;
    } finally {
