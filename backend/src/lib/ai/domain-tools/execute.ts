...
export async function executeDomainTool(action: DomainActionSlug, rawInput: unknown, context: DomainToolContext, options: DomainToolExecutionOptions = {}): Promise<DomainToolResult> {
  ...
  if (action === 'scope.create') {
    ...
    const key = newId(), memberKey = newId(), relationKey = parent ? newId() : null; const slug = input.name.toLocaleLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''); const level = (parent?.level ?? 0) + 1;
    await withTransaction(['scopes', 'scopeScopes', 'scopeMembers'], async (trx) => { await trx.query('INSERT { _key: @key, organizationKey: @organizationKey, slug: @slug, name: @name, summary: @description, description: @description, position: @position, level: @level, deletedAt: null, embedding: [] } INTO scopes', { key, organizationKey: context.organizationKey, slug, name: input.name, description: input.description ?? input.name, position: input.position, level }); if (parent) await trx.query('INSERT { _key: @relationKey, parentKey: @parentKey, childKey: @key, level: @level, deletedAt: null } INTO scopeScopes', { relationKey, parentKey: parent._key, key, level }); await trx.query('INSERT { _key: @memberKey, scopeKey: @key, userOrganizationKey: @membershipKey, role: "owner", status: "active" } INTO scopeMembers', { memberKey, key, membershipKey: principal.userOrganization.key }); });
