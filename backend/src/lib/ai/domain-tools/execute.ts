...
type ScopeRow = RecordDoc & { _key: string; organizationKey: string; name: string; slug: string; description: string | null; position: number; deletedAt?: string | null };
...
async function effectiveScopeRole(context: DomainToolContext, scopeKey: string) {
  ...
  const cursor = await db.query<{ role: 'owner' | 'admin' | 'moderator' | 'viewer' }>('FOR member IN scopeMembers FILTER member.scopeKey == @scopeKey && member.userOrganizationKey == @membershipKey LIMIT 1 RETURN { role: member.role }', { scopeKey, membershipKey: principal.userOrganization.key });
  return (await cursor.next())?.role ?? null;
}
...
export async function executeDomainTool(action: DomainActionSlug, rawInput: unknown, context: DomainToolContext): Promise<DomainToolResult> {
  ...
  if (action.startsWith('organization.member.')) {
    ...
    if (action === 'organization.member.add') {
      ...
      await withTransaction(['userOrganizations'], async (trx) => { await trx.query('INSERT { _key: @key, organizationId: @organizationKey, userId: @userId, orgRole: @role, orgTitle: null, status: "active", joinedAt: @now, isMfaEnabled: false, totpSecret: null, lastTotpTimeStep: null, createdAt: @now, updatedAt: @now, embedding: [] } INTO userOrganizations', { key, organizationKey: context.organizationKey, userId: users[0]!._key, role: input.role, now }); });
      await emitDomainEvent(context, action, { userOrganizationKey: key }); return result(action, { userOrganizationKey: key, role: input.role, status: 'active' });
    }
    ...
    if (action === 'organization.member.role.update') await withTransaction(['userOrganizations'], async (trx) => { await trx.query('FOR membership IN userOrganizations FILTER membership._key IN @keys UPDATE membership WITH { orgRole: @role, updatedAt: @now } IN userOrganizations', { keys, role: input.role, now }); });
    if (action === 'organization.member.activate') await withTransaction(['userOrganizations'], async (trx) => { await trx.query('FOR membership IN userOrganizations FILTER membership._key IN @keys UPDATE membership WITH { status: "active", updatedAt: @now } IN userOrganizations', { keys, now }); });
    ...
    if (action === 'organization.member.remove') await withTransaction(['userOrganizations', 'scopeMembers', 'userSessions'], async (trx) => { await trx.query('FOR member IN scopeMembers FILTER member.userOrganizationKey IN @keys REMOVE member IN scopeMembers', { keys }); await trx.query('FOR session IN userSessions FILTER session.userId IN @userIds && session.disconnectedAt == null UPDATE session WITH { disconnectedAt: @now, updatedAt: @now } IN userSessions', { userIds: resolved.map((row) => row.user._key), now }); await trx.query('FOR membership IN userOrganizations FILTER membership._key IN @keys REMOVE membership IN userOrganizations', { keys }); });
    await emitDomainEvent(context, action, { userOrganizationKeys: keys }); return result(action, { userOrganizationKeys: keys });
  }
  ...
  if (action === 'scope.create') {
    ...
    const key = newId(), memberKey = newId(), relationKey = parent ? newId() : null; const slug = input.name.toLocaleLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    await withTransaction(['scopes', 'scopeScopes', 'scopeMembers'], async (trx) => { await trx.query('INSERT { _key: @key, organizationKey: @organizationKey, slug: @slug, name: @name, summary: @description, description: @description, position: @position, deletedAt: null, embedding: [] } INTO scopes', { key, organizationKey: context.organizationKey, slug, name: input.name, description: input.description ?? input.name, position: input.position }); if (parent) await trx.query('INSERT { _key: @relationKey, parentKey: @parentKey, childKey: @key, deletedAt: null } INTO scopeScopes', { relationKey, parentKey: parent._key, key }); await trx.query('INSERT { _key: @memberKey, scopeKey: @key, userOrganizationKey: @membershipKey, role: "owner" } INTO scopeMembers', { memberKey, key, membershipKey: principal.userOrganization.key }); });
    await emitDomainEvent(context, action, { scopeKey: key }); return result(action, { key, slug, parentScopeKey: parent?._key ?? null, role: 'owner' });
  }
  ...
  if (action === 'scope.move') { const scope = targets[0]!; await assertScopeRole(context, scope._key, ['owner', 'admin']); if (!graph.parentByChild.has(scope._key)) throw new DomainToolExecutionError('root_scope_protected', 'Root scopes cannot be moved'); const parent = input.parentScope === undefined ? graph.byKey.get(graph.parentByChild.get(scope._key)!) ?? null : input.parentScope === null ? null : resolveScopes(graph, [input.parentScope], true)[0]!.results[0]!; if (parent?.deletedAt) throw new DomainToolExecutionError('parent_archived', 'Destination parent is archived'); let cursor = parent?._key; while (cursor) { if (cursor === scope._key) throw new DomainToolExecutionError('scope_cycle', 'Move would create a cycle'); cursor = graph.parentByChild.get(cursor); } await withTransaction(['scopes', 'scopeScopes'], async (trx) => { if (input.parentScope !== undefined) { await trx.query('FOR relation IN scopeScopes FILTER relation.childKey == @scopeKey REMOVE relation IN scopeScopes', { scopeKey: scope._key }); if (parent) await trx.query('INSERT { _key: @key, parentKey: @parentKey, childKey: @scopeKey, deletedAt: null } INTO scopeScopes', { key: newId(), parentKey: parent._key, scopeKey: scope._key }); } if (input.position !== undefined) await trx.query('UPDATE @scopeKey WITH { position: @position } IN scopes', { scopeKey: scope._key, position: input.position }); }); await emitDomainEvent(context, action, { scopeKey: scope._key }); return result(action, { scopeKey: scope._key, parentScopeKey: parent?._key ?? null, position: input.position ?? scope.position }); }
