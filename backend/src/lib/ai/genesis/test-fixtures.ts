...
export function buildGenesisFixture() {
  ...
  const scope = scopeSchema.parse({ key: newId(), organizationKey: organization.key, slug: 'agent-builder', name: 'Agent Builder', summary: 'Build validated agents.', description: 'Build validated agents.', position: 2, embedding: [0, 1] });
