...
describe('AI metadata model contract', () => {
  test('uses the canonical collection names', () => {
    expect([
      ...
      SCOPES_COLLECTION, SCOPE_SCOPES_COLLECTION, SCOPE_MEMBERS_COLLECTION, SCOPE_AGENTS_COLLECTION, AGENT_MEMBERS_COLLECTION,
       SKILLS_COLLECTION, AGENTS_COLLECTION, AGENT_SKILLS_COLLECTION,
  ...
  test('locks the complete organization, registry, runtime, execution, and history layers', () => {
    ...
    expect(AGENT_ARCHITECTURE.registries).toEqual({ agents: 'agents', skills: 'skills', actions: 'actions', models: 'models', providers: 'providers' });
    expect(AGENT_ARCHITECTURE.linkingNodes).toEqual({ userOrganizations: 'userOrganizations', scopeScopes: 'scopeScopes', scopeMembers: 'scopeMembers', scopeAgents: 'scopeAgents', agentMembers: 'agentMembers', agentSkills: 'agentSkills', modelActions: 'modelActions', modelProviders: 'modelProviders', agentRunSources: 'agentRunSources' });
    expect(AGENT_ARCHITECTURE.runtime.agentContext).toEqual(['organization', 'scope', 'agent', 'skills', 'knowledge', 'guardrails', 'currentTask']);
    expect(AGENT_ARCHITECTURE.execution).toEqual(['action', 'router', 'model', 'provider']);
    ...
    expect(AGENT_EXECUTION_SEQUENCE).toEqual(['agentContext', 'action', 'router', 'model', 'provider', 'response', 'agentRun', 'agentRunSteps', 'agentRunCalls']);
  });
});
