...
const MOMENTUM_COLLECTIONS = [PROJECTS_COLLECTION, MILESTONES_COLLECTION, TASKS_COLLECTION, FOLDERS_COLLECTION, SCOPES_COLLECTION] as const;
...
export interface MomentumRepository {
  ...
  updateArchiveFolder(key: string, patch: Partial<Folder> & Pick<Folder, 'embedding' | 'updatedAt'>, expectedUpdatedAt?: string): Promise<Folder>;
...
export function createMomentumRepository(
  ...
  const repository: MomentumRepository = {
    ...
    updateArchiveFolder: (key, patch, expectedUpdatedAt) => update(FOLDERS_COLLECTION, folderSchema, folderSchema.shape.key, key, patch, expectedUpdatedAt),
...
export class MomentumConcurrentUpdateError extends Error {
  constructor(collection: string, key: string) {
    super(`${collection} document changed during mutation preparation: ${key}`);
    this.name = 'MomentumConcurrentUpdateError';
  }
}
