...
const definitions: Array<{
  ...
  { operation: 'search', name: 'search_images', description: 'Search Gallery by visible content, optionally within a collection, or find images visually similar to one image.', schema: z.object({ query: z.string().trim().min(1).max(12_000).optional(), imageKey: key.optional(), collectionKey: key.optional(), threshold: z.number().min(-1).max(1).optional(), limit: z.number().int().min(1).max(50).default(50) }).strict().refine((value) => Boolean(value.query) !== Boolean(value.imageKey), 'Provide exactly one of query or imageKey').refine((value) => !(value.imageKey && value.collectionKey), 'Collection search requires a text query') },
