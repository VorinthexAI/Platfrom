...
export const compassCapabilities = [
  ...
  capability('place.find', 'Find bounded factual country or place information for a detail sheet without saving it.', z.object({ query: z.string().trim().min(2).max(200) }).strict(), async (input, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).findPlace({ ...actor.serviceContext, ...input }, actor.userKey, { signal: context.signal, timeoutMs: context.timeoutMs }); }),
