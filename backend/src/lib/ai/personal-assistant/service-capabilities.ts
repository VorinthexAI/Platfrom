...
export const archiveCapabilities = [
  ...
  archive('archive_document_summary_audio', 'Generate durable audio for one or more saved Archive document summaries.', z.object({ summaryKeys: z.array(key).min(1).max(100), voice: z.string().trim().min(1).max(120).optional(), language: z.string().trim().regex(/^[a-z]{2,3}(?:-[A-Z]{2})?$/).optional() }).strict(), 'document.summary.audio.generate', (input) => input, true),
