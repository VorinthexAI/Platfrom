...
export const signalCapabilities = [
  capability('email.overview', 'List and search email threads with stable cursor pagination.', z.object({ filter: z.enum(['all', 'important', 'urgent', 'needs_action', 'filtered', 'unread', 'favorite']).optional(), search: z.string().trim().max(200).optional(), cursor: z.string().min(1).max(2_000).optional(), limit: z.number().int().min(1).max(50).optional() }).strict(), async (input, context) => { const actor = identity(context); return (context.email ?? createEmailService()).overview(actor.emailActor, input); }),
  capability('inbox.sync', 'Synchronize the connected email inbox.', z.object({}).strict(), async (_input, context) => { const actor = identity(context); return (context.email ?? createEmailService()).sync(actor.emailActor); }, 'signal'),
  capability('inbox.subscribe', 'Ensure change notifications are registered for the connected inbox.', z.object({}).strict(), async (_input, context) => { const actor = identity(context); return (context.email ?? createEmailService()).subscribe(actor.emailActor); }, 'signal'),
