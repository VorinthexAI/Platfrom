...
export const archiveCapabilities = [
  ...
  archive('document.list', 'List Archive documents at the root or in a folder.', z.object({ folderKey: key.optional(), includeArchived: z.boolean().optional(), limit: z.number().int().min(1).max(100).optional() }).strict(), 'document.list', (input, context) => ({ scopeKey: context.domain.runtimeScopeKey, ...input })),
  archive('document.find', 'Find an Archive document by key.', z.object({ documentKey: key, includeArchived: z.boolean().optional(), includeContent: z.boolean().optional() }).strict(), 'document.find', ({ documentKey, includeContent, ...input }) => ({ documentKeys: [documentKey], ...input, ...(includeContent ? { include: ['content'] } : {}) })),
...
export const compassCapabilities = [
  capability('place.list', 'List saved places, visits, trips, and itineraries.', z.object({}).strict(), async (_input, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).overview(actor.serviceContext, actor.userKey); }),
  capability('place.create', 'Create a saved place in Compass.', z.object({ kind: z.enum(['country', 'place']).optional(), name: z.string().trim().min(1).max(200), latitude: z.number().finite().min(-90).max(90), longitude: z.number().finite().min(-180).max(180), countryCode: z.string().trim().length(2), country: serviceText, continent: serviceText, region: serviceText, city: serviceText, wishlist: z.boolean().optional() }).strict(), async (input, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).createPlace({ ...actor.serviceContext, ...input }, actor.userKey); }, 'compass'),
  capability('place.visit.create', 'Record a visit to a saved Compass place.', z.object({ placeKey: key, tripKey: key.optional(), arrivedAt: z.string().date().optional(), departedAt: z.string().date().optional() }).strict(), async ({ placeKey, ...input }, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).createVisit(placeKey, { ...actor.serviceContext, ...input }, actor.userKey); }, 'compass'),
  capability('trip.create', 'Create a Compass trip.', z.object({ name: z.string().trim().min(1).max(200), description: z.string().trim().min(1).max(2_000).optional(), startDate: z.string().date().optional(), endDate: z.string().date().optional() }).strict(), async (input, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).createTrip({ ...actor.serviceContext, ...input }, actor.userKey); }, 'compass'),
  capability('trip.place.add', 'Add a saved place to a Compass trip.', z.object({ tripKey: key, placeKey: key, arrivalDate: z.string().date().optional(), departureDate: z.string().date().optional() }).strict(), async ({ tripKey, ...input }, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).appendPlace(tripKey, { ...actor.serviceContext, ...input }, actor.userKey); }, 'compass'),
  capability('trip.place.remove', 'Remove a saved place from a Compass trip.', z.object({ tripKey: key, placeKey: key }).strict(), async ({ tripKey, placeKey }, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).removePlace(tripKey, placeKey, actor.serviceContext, actor.userKey); }, 'compass'),
];
