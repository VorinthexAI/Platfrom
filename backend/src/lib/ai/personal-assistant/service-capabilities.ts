...
export const signalCapabilities = [
  capability('email.overview', 'List Gmail accounts, or list one selected account with stable cursor pagination.', z.object({ connectorKey: key.optional(), filter: z.enum(['all', 'important', 'urgent', 'needs_action', 'filtered', 'unread', 'favorite']).optional(), search: z.string().trim().max(200).optional(), cursor: z.string().min(1).max(2_000).optional(), limit: z.number().int().min(1).max(50).optional() }).strict(), async (input, context) => { const actor = identity(context); return (context.email ?? createEmailService()).overview(actor.emailActor, input); }),
  capability('inbox.sync', 'Synchronize one connected email inbox.', z.object({ connectorKey: key }).strict(), async ({ connectorKey }, context) => { const actor = identity(context); return (context.email ?? createEmailService()).sync(actor.emailActor, connectorKey); }, 'signal'),
  capability('inbox.subscribe', 'Ensure change notifications are registered for one connected inbox.', z.object({ connectorKey: key }).strict(), async ({ connectorKey }, context) => { const actor = identity(context); return (context.email ?? createEmailService()).subscribe(actor.emailActor, connectorKey); }, 'signal'),
  ...
  capability('email.draft.create', 'Generate a reply draft for a Signal email thread without sending it.', z.object({ threadKey: key, tone: z.enum(['concise', 'warm', 'formal', 'direct']), instruction: z.string().trim().max(1_000).optional(), profileKey: key.optional(), attachments: emailAttachmentRefsSchema.optional() }).strict(), async (input, context) => { const actor = identity(context); return (context.email ?? createEmailService()).draft(actor.emailActor, input); }, 'signal'),
  capability('email.draft.compose', 'Generate and save a new email draft in a selected inbox.', z.object({ connectorKey: key.optional(), to: z.array(z.string().email()).min(1).max(50), cc: z.array(z.string().email()).max(50).optional(), bcc: z.array(z.string().email()).max(50).optional(), subject: z.string().trim().min(1).max(998), tone: z.enum(['concise', 'warm', 'formal', 'direct']), instruction: z.string().trim().max(1_000).optional(), attachments: emailAttachmentRefsSchema.optional() }).strict(), async (input, context) => { const actor = identity(context); return (context.email ?? createEmailService()).draftNew(actor.emailActor, input); }, 'signal'),
  ...
  capability('email.draft.assign', 'Assign a legacy unassigned new email draft to one connected inbox.', z.object({ draftKey: key, connectorKey: key }).strict(), async (input, context) => { const actor = identity(context); return (context.email ?? createEmailService()).assignDraft(actor.emailActor, input); }, 'signal'),
  capability('email.draft.send', 'Send a reviewed Signal email draft. connectorKey is only needed for a legacy unassigned new draft.', z.object({ draftKey: key, connectorKey: key.optional() }).strict(), async ({ draftKey, connectorKey }, context) => { const actor = identity(context); return (context.email ?? createEmailService()).sendDraft(actor.emailActor, draftKey, connectorKey); }, 'signal'),
  capability('email.disconnect', 'Disconnect one Gmail inbox only when the user explicitly asks.', z.object({ connectorKey: key }).strict(), async ({ connectorKey }, context) => { const actor = identity(context); return (context.email ?? createEmailService()).disconnect(actor.emailActor, connectorKey); }, 'signal'),
];
