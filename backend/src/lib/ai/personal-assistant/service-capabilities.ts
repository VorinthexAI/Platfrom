...
export const ascendCapabilities = [
  ...
  capability('book.delete', 'Hard-delete an audiobook and generated Archive and storage dependents.', z.object({ bookKey: key }).strict(), async ({ bookKey }, context) => { const actor = identity(context); return (context.books ?? defaultBookService).delete(bookKey, actor.serviceContext, actor.userKey); }, 'ascend'),
];
