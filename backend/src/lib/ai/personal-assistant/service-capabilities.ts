...
export const archiveCapabilities = [
  ...
  archive('scope.content.search-history.delete', 'Delete one entry from the current user\'s global search history.', z.object({ normalizedQuery: z.string().trim().min(1).max(12_000) }).strict(), 'scope.content.search-history.delete', (input, context) => ({ scopeKey: context.domain.runtimeScopeKey, ...input }), true),
];
