...
const toolNames: Record<string, string> = {
  ...
  archive_document_list: 'document.list', archive_document_find: 'document.find', archive_document_create: 'document.create', archive_document_update: 'document.update', archive_document_rename: 'document.rename', archive_document_move: 'document.move', archive_document_copy: 'document.copy', archive_document_summarize: 'document.summarize', archive_document_topics: 'document.topics', archive_document_summaries: 'document.list-summaries', archive_document_summary: 'document.find-summary', archive_document_summary_audio: 'document.summary.audio.generate', archive_document_audio_playback_update: 'document.audio.playback.update', archive_document_audio_playback_clear: 'document.audio.playback.clear', archive_document_enhance: 'document.enhance', archive_document_translate: 'document.translate', archive_document_versions: 'document.list-versions', archive_document_version_restore: 'document.restore-version', archive_document_download: 'document.download',
  content_neighbors: 'content.neighbors',
...
export const archiveCapabilities = [
  ...
  archive('archive_document_summarize', 'Create and save an immutable summary of the open Archive document, or an explicitly identified document.', z.object({ documentKey: key.optional(), topic: z.string().trim().min(1).max(500).optional(), style: z.enum(['brief', 'detailed', 'executive', 'bullet-points', 'technical']).optional(), language: z.string().trim().min(1).max(100).optional() }).strict(), 'document.summarize', ({ documentKey, ...input }, context) => ({ documentKeys: [currentDocumentKey(documentKey, context)], ...input, persist: true }), true),
  archive('archive_document_topics', 'Identify up to ten concise topics in the open Archive document, or an explicitly identified document.', z.object({ documentKey: key.optional() }).strict(), 'document.topics', ({ documentKey }, context) => ({ documentKey: currentDocumentKey(documentKey, context) })),
  archive('archive_document_summaries', 'List saved summaries of the open Archive document, or an explicitly identified document.', z.object({ documentKey: key.optional(), cursor: z.string().optional(), limit: z.number().int().min(1).max(100).optional() }).strict(), 'document.list-summaries', ({ documentKey, ...input }, context) => ({ documentKeys: [currentDocumentKey(documentKey, context)], ...input })),
  archive('archive_document_summary', 'Open one saved Archive document summary by key.', z.object({ summaryKey: key }).strict(), 'document.find-summary', ({ summaryKey }) => ({ summaryKeys: [summaryKey] })),
  archive('archive_document_summary_audio', 'Generate durable male-voice audio for one or more saved Archive document summaries.', z.object({ summaryKeys: z.array(key).min(1).max(100), voice: z.literal('Matthew').optional(), language: z.string().trim().regex(/^[a-z]{2,3}(?:-[A-Z]{2})?$/).optional() }).strict(), 'document.summary.audio.generate', (input) => input, true),
  archive('archive_document_audio_playback_update', 'Select an Archive document audio version and save its playback position.', z.object({ audioVersionKey: key, playbackPositionMs: z.number().int().nonnegative() }).strict(), 'document.audio.playback.update', (input) => input, true),
  archive('archive_document_audio_playback_clear', 'Dismiss the selected audio version for an Archive document.', z.object({ documentKey: key }).strict(), 'document.audio.playback.clear', (input) => input, true),
  archive('archive_document_enhance', 'Enhance the open Archive document, or an explicitly identified document, in place while preserving meaning and formatting.', z.object({ documentKey: key.optional() }).strict(), 'document.enhance', ({ documentKey }, context) => ({ documentKeys: [currentDocumentKey(documentKey, context)], mode: 'replace' }), true),
  ...
  archive('content_neighbors', 'Find semantically similar active folders, documents, and files for an Archive folder or document.', z.object({ folderKey: key.optional(), documentKey: key.optional() }).strict().refine((input) => Number(input.folderKey !== undefined) + Number(input.documentKey !== undefined) === 1, 'exactly one source key is required'), 'content.neighbors', (input) => input),
  archive('scope.content.search-history.delete', 'Delete one saved Content search from the current user\'s history.', z.object({ normalizedQuery: z.string().trim().min(1).max(8_000), folderKey: key.optional(), includeDescendants: z.boolean().optional() }).strict(), 'scope.content.search-history.delete', (input, context) => ({ scopeKey: context.domain.runtimeScopeKey, ...input }), true),
];
