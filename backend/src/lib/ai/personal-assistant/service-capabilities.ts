...
import { createTravelService, travelChildrenFindInputSchema, travelCityFindInputSchema, travelPlaceCreateInputSchema, travelPlaceFindInputSchema, travelPlaceOpenInputSchema, travelPlaceSearchInputSchema, travelTripCreateInputSchema, travelTripListInputSchema } from '@/lib/travel/service';
...
export const compassCapabilities = [
  ...
  capability('place.search', 'Suggest ten countries and cities matching a travel query without saving them.', travelPlaceSearchInputSchema.omit({ organizationKey: true, scopeKey: true }), async (input, context) => { const actor = identity(context); return (context.travel ?? createTravelService()).searchPlaces({ ...actor.serviceContext, ...input }, actor.userKey, { signal: context.signal, timeoutMs: context.timeoutMs }); }),
