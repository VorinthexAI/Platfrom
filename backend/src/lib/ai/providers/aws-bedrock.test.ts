...
describe('AWS Bedrock provider', () => {
  test('invokes Titan embeddings for core.embedd', async () => {
    let url = '';
    globalThis.fetch = (async (input) => {
      url = String(input);
      return new Response(JSON.stringify({ embedding: [0.25, 0.75], inputTextTokenCount: 3 }), { status: 200 });
    }) as typeof fetch;
    const result = await provider().execute({ actionId: 'core.embedd', modelId: 'amazon.titan-embed-text-v2', externalModelId: 'amazon.titan-embed-text-v2:0', input: { text: 'hello' }, organizationKey: 'organization' });
