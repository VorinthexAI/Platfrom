...
describe('AWS Polly provider', () => {
  test('synthesizes core.speak audio with the configured engine', async () => {
    let body = '';
    ...
    const result = await provider.execute({ actionId: 'core.speak', modelId: 'amazon.polly-generative', externalModelId: 'generative', input: { text: 'Hello', voice: 'Joanna', format: 'mp3' }, organizationKey: 'organization' });
