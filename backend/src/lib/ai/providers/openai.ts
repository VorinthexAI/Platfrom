...
async function executeWebSearch<TInput, TOutput>(client: OpenAI, request: ProviderExecuteRequest<TInput>): Promise<ProviderExecuteResponse<TOutput>> {
  ...
  try {
    ...
    if (!searchCompleted || sourceUrls.size === 0 || citations.length === 0 || citations.some(({ url }) => !sourceUrls.has(url))) throw new ProviderError(PROVIDER_ID, 'response_invalid', 'openai web search did not return grounded citations');
...
async function captionImages<TInput, TOutput>(client: OpenAI, request: ProviderExecuteRequest<TInput>): Promise<ProviderExecuteResponse<TOutput>> {
  ...
  const instruction = input.purpose === 'document-transcription'
    ...
    : input.purpose === 'document-reconciliation'
      ...
      : input.purpose === 'artwork-compliance'
        ? `Inspect each of the ${input.imageUrls.length} illustrations below for a strict publishing rule. An image is compliant only when it contains no person or human-like subject, no visible writing or text-like mark, and no botanical, fungal, or vegetation imagery. Treat any human, face, head, body, hand, limb, silhouette, crowd, portrait, character, mannequin, statue, humanoid robot, or anthropomorphic figure as noncompliant, including partial, distant, reflected, abstract, or shadowed depictions. Treat any word, letter, initial, number, caption, title, label, sign, logo, brand, watermark, signature, interface text, pseudo-writing, glyph, rune, or readable symbol as noncompliant. Treat any plant, tree, leaf, flower, grass, vine, forest, wetland, mushroom, fungus, mold, or mycelium as noncompliant. For each image, put a concise factual description of any violation in caption, or exactly "compliant" when none exists. Set score to exactly 100 only when compliant and exactly 1 when any prohibited element exists. When uncertain, mark the image noncompliant.`
  ...
  try {
    const completion = await client.chat.completions.create({
      ...
      max_completion_tokens: Math.min(input.imageUrls.length * (input.purpose === 'caption' ? 300 : input.purpose === 'artwork-compliance' ? 1_000 : 1_500), 16_000),
