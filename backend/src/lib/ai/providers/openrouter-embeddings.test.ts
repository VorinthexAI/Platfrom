...
describe('OpenRouter OpenAI embeddings', () => {
  test('strictly routes direct embeddings to Azure and restores indexed order', async () => {
    let body: Record<string, unknown> = {};
    globalThis.fetch = (async (_input, init) => {
      body = JSON.parse(String(init?.body));
      return Response.json({ provider: 'Azure', data: [{ index: 1, embedding: vector(2) }, { index: 0, embedding: vector(1) }], usage: { prompt_tokens: 4, total_tokens: 4 } });
    }) as typeof fetch;
  ...
  test('supports the routed embed action and rejects malformed cardinality', async () => {
    ...
    globalThis.fetch = (async () => Response.json({ provider: 'Azure', data: [{ index: 1, embedding: vector(1) }] })) as unknown as typeof fetch;
  ...
  test('rejects an embedding served by any upstream other than Azure', async () => {
    globalThis.fetch = (async () => Response.json({ provider: 'Other Provider', data: [{ index: 0, embedding: vector(1) }] })) as unknown as typeof fetch;
    await expect(createOpenRouterProvider({ apiKey: 'test-key' }).embed!({ externalModelId: EXTERNAL_EMBEDDING_MODEL_ID, input: 'private' }))
      .rejects.toMatchObject({ code: 'response_invalid', message: 'openrouter embeddings were not served by Azure' });
  });
  ...
  test('retries normalized transient failures and rejects invalid dimensions', async () => {
    ...
    globalThis.fetch = (async () => {
      ...
      return Response.json({ provider: 'Azure', data: [{ index: 0, embedding: vector(1) }] });
    }) as unknown as typeof fetch;
    const provider = createOpenRouterProvider({ apiKey: 'test-key' });
    await expect(provider.embed!({ externalModelId: EXTERNAL_EMBEDDING_MODEL_ID, input: 'retry' })).resolves.toBeDefined();
    expect(attempts).toBe(2);
    globalThis.fetch = (async () => Response.json({ provider: 'Azure', data: [{ index: 0, embedding: [1] }] })) as unknown as typeof fetch;
