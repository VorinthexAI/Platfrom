...
export function createScopeRepository(
  ...
  return {
    ...
    async removeScope(scopeKey) {
      ...
      const teardown = await database.query(`
        ...
        LET cleanupToneShares = (FOR share IN shares FILTER share.scopeKey == @scopeKey && share.sourceType == "document" && share.sourceKey IN toneDocumentKeys REMOVE share IN shares RETURN 1)
        LET cleanupToneBindings = (FOR binding IN generatedDocumentBindings FILTER binding.scopeKey == @scopeKey && binding.documentKey IN toneDocumentKeys REMOVE binding IN generatedDocumentBindings RETURN 1)
        LET cleanupToneTags = (FOR assignment IN tagAssignments FILTER assignment.scopeKey == @scopeKey && assignment.sourceType == "document" && assignment.sourceKey IN toneDocumentKeys REMOVE assignment IN tagAssignments RETURN 1)
        LET cleanupToneHiddens = (FOR hidden IN userHiddens FILTER hidden.source == "document" && hidden.sourceKey IN toneDocumentKeys REMOVE hidden IN userHiddens RETURN 1)
        LET mailDeletionJobs = (FOR storageKey IN mailStorageKeys UPSERT { storageKey } INSERT { storageKey, createdAt: @now } UPDATE {} IN storageDeletionJobs RETURN 1)
        ...
        RETURN true
      `, { scopeKey, toneFolderKey, toneDocuments, now: new Date().toISOString() });
