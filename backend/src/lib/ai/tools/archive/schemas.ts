...
import { archiveErrorSchema } from './errors';
...
export const archiveDocumentShareSchema = z.object({
  key: keySchema,
  scopeKey: keySchema,
...
export const archiveDocumentVersionSchema = z.object({
  key: keySchema,
  scopeKey: keySchema,
  documentKey: keySchema,
...
export const archiveProjectedDocumentSchema = archiveDocumentSchema.extend({
  ...
  shares: z.array(archiveDocumentShareSchema).optional(),
  latestVersion: archiveDocumentVersionSchema.optional(),
}).strict();

export const archiveBatchSummarySchema = z.object({
...
export function archiveBatchResultSchema<T extends z.ZodTypeAny>(dataSchema: T) {
  return z.object({
    key: keySchema,
    success: z.boolean(),
    data: dataSchema.optional(),
    error: archiveErrorSchema.optional(),
...
export function archiveBatchOutputSchema<T extends z.ZodTypeAny>(dataSchema: T) {
  return z.object({ results: z.array(archiveBatchResultSchema(dataSchema)), summary: archiveBatchSummarySchema }).strict().superRefine((value, context) => {
...
const folderDataSchema = z.object({ folder: archiveFolderSchema }).strict();
const documentDataSchema = z.object({ document: archiveDocumentSchema }).strict();
const projectedDocumentDataSchema = z.object({ document: archiveProjectedDocumentSchema }).strict();
const shareDataSchema = z.object({ share: archiveDocumentShareSchema }).strict();
const unsharedDataSchema = z.union([
  shareDataSchema,
  z.object({ documentKey: keySchema, shares: z.array(archiveDocumentShareSchema) }).strict(),
]);
const createdShareDataSchema = z.object({
  share: archiveDocumentShareSchema,
  token: z.string().min(32),
}).strict();
const copiedDocumentDataSchema = z.object({
  document: archiveDocumentSchema,
  shares: z.array(createdShareDataSchema).optional(),
}).strict();
const versionDataSchema = z.object({ version: archiveDocumentVersionSchema }).strict();
const projectedVersionDataSchema = z.object({ version: archiveProjectedDocumentVersionSchema }).strict();
...
export const archiveSearchFiltersSchema = z.object(commonSearchFilterShape).strict();
export const organizationArchiveSearchFiltersSchema = z.object({
...
const searchInputShape = {
  ...
  sources: z.array(archiveSearchSourceSchema).min(1).optional(),
  filters: archiveSearchFiltersSchema.optional(),
...
export const archiveSearchOutputSchema = z.object({ query: textSchema, results: z.array(archiveSearchResultSchema), totalCandidates: z.number().int().nonnegative().optional() }).strict();
...
export const archiveToolContracts = {
  ...
  'folder.rename': { description: 'Rename folders.', input: z.object({ renames: z.array(z.object({ folderKey: keySchema, name: nameSchema }).strict()).min(1).max(100), atomic: atomicSchema, ...idempotencyShape }).strict(), output: archiveBatchOutputSchema(folderDataSchema) },
  'folder.move': { description: 'Move folders to another parent or the scope root.', input: z.object({ moves: z.array(z.object({ folderKey: keySchema, targetParentFolderKey: keySchema.optional() }).strict()).min(1).max(100), atomic: atomicSchema, ...idempotencyShape }).strict(), output: archiveBatchOutputSchema(folderDataSchema) },
  ...
  'document.download': { description: 'Download documents.', input: z.object({ documentKeys: keysSchema, format: z.enum(['original', 'html', 'txt', 'md']).default('original') }).strict(), output: archiveBatchOutputSchema(fileDataSchema) },
  'document.export': { description: 'Export documents to selected formats.', input: z.object({ exports: z.array(z.object({ documentKey: keySchema, format: z.enum(['html', 'txt', 'md', 'pdf', 'docx']) }).strict()).min(1).max(100), atomic: atomicSchema }).strict(), output: archiveBatchOutputSchema(fileDataSchema) },
  'document.share': { description: 'Create document shares.', input: z.object({ shares: z.array(z.object({ documentKey: keySchema, permission: z.enum(['read', 'comment']), expiresAt: dateTimeSchema.optional(), password: z.string().min(1).max(256).optional() }).strict()).min(1).max(100), atomic: atomicSchema, ...idempotencyShape }).strict(), output: archiveBatchOutputSchema(createdShareDataSchema) },
  'document.unshare': { description: 'Revoke document shares.', input: z.object({ shareKeys: keysSchema.optional(), documentKeys: keysSchema.optional(), atomic: atomicSchema, ...idempotencyShape }).strict().refine((value) => Number(value.shareKeys !== undefined) + Number(value.documentKeys !== undefined) === 1, 'exactly one of shareKeys or documentKeys is required'), output: archiveBatchOutputSchema(unsharedDataSchema) },
  'document.list-shares': { description: 'List shares for documents.', input: z.object({ documentKeys: keysSchema, includeExpired: z.boolean().optional(), includeRevoked: z.boolean().optional() }).strict(), output: archiveBatchOutputSchema(z.object({ documentKey: keySchema, shares: z.array(archiveDocumentShareSchema) }).strict()) },
  ...
  'scope.document.search': { description: 'Search documents available from a scope.', input: z.object({ scopeKey: keySchema, ...searchInputShape }).strict(), output: archiveSearchOutputSchema },
  'organization.document.search': { description: 'Search documents across an organization.', input: z.object({ organizationKey: keySchema, ...searchInputShape, filters: organizationArchiveSearchFiltersSchema.optional(), include: organizationSearchIncludeSchema.optional() }).strict(), output: archiveSearchOutputSchema },
} as const satisfies Record<string, { description: string; input: z.ZodTypeAny; output: z.ZodTypeAny }>;

export type ArchiveToolName = keyof typeof archiveToolContracts;
export type ArchiveToolInput<Name extends ArchiveToolName> = z.input<(typeof archiveToolContracts)[Name]['input']>;
export type ArchiveToolOutput<Name extends ArchiveToolName> = z.output<(typeof archiveToolContracts)[Name]['output']>;
