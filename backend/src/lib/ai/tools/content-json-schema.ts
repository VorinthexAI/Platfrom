...
export function contentZodToJsonSchema(schema: z.ZodTypeAny): ContentJsonSchema {
  ...
  if (typeName === 'ZodOptional') return contentZodToJsonSchema(definition.innerType);
  if (typeName === 'ZodDefault') return { ...contentZodToJsonSchema(definition.innerType), default: definition.defaultValue() };
  if (typeName === 'ZodEffects') return contentZodToJsonSchema(definition.schema);
  ...
  if (typeName === 'ZodUnion' || typeName === 'ZodDiscriminatedUnion') {
    ...
    return { oneOf: options.map(contentZodToJsonSchema) };
  }
  if (typeName === 'ZodRecord') return { type: 'object', additionalProperties: contentZodToJsonSchema(definition.valueType) };
