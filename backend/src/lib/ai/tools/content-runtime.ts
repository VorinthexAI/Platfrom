...
import {
  ...
  generateDocumentTranslation,
  generateTextEnhancement,
} from "@/lib/ai/actions/document-text-generation";
import type { ToolContext } from "./tool-context";
import {
  prepareDocumentRepresentation,
...
export interface ContentRepository {
  ...
  role(scopeKey: string, teamMembershipKey: string): Promise<Role | null>;
  allowedScopeKeys(
    teamKey: string,
  ...
  createSummary?(
    summary: Omit<DocumentSummary, "version">,
  ): Promise<DocumentSummary>;
  ...
  semanticSearch(input: {
    ...
    Array<{ score: number; document: Document; matchedContent?: string }>
  >;
  semanticSearchFolders?(input: {
    embedding: number[];
    authorizedScopeKeys: string[];
    folderKeys?: string[];
    createdFrom?: string;
    ...
    minScore: number;
    limit: number;
  }): Promise<Array<{ score: number; folder: Folder }>>;
  semanticNeighbors?(input: {
    embedding: number[];
    scopeKey: string;
    ...
    folders: Array<{ score: number; folder: Folder }>;
    documents: Array<{ score: number; document: Document }>;
    files: Array<{ score: number; document: Document }>;
  }>;
  generatedDocumentBindings?(
    documentKeys: string[],
  ): Promise<GeneratedDocumentBinding[]>;
  transaction?<T>(
    operation: (repository: ContentRepository) => Promise<T>,
  ): Promise<T>;
}
...
const CONTENT_SEARCH_CACHE_VERSION = 7;
...
function fail(
  ...
  throw new ContentError(code, message, tool, {
    action,
    resourceKey,
    cause,
    retryable,
...
function mappedError(
  ...
  if (error instanceof DocumentProcessingError) {
    ...
    return new ContentError(
      codeByAction[error.action] ?? "DOCUMENT_PROCESSING_FAILED",
      error.message,
      tool,
      {
        action: error.action,
  ...
  const validation =
    error &&
    typeof error === "object" &&
    ("issues" in error || ("name" in error && error.name === "ZodError"));
  return new ContentError(
    validation ? "CONTENT_INVALID_INPUT" : "CONTENT_CONFLICT",
    validation
      ? "Content tool input or output was invalid."
      : "Content operation failed.",
    tool,
    { action, resourceKey, cause: error, retryable: !validation },
  );
}
...
async function folderView(
  ...
  dependencies: Pick<
    RuntimeDefaults,
    "getFolderCoverImage" | "signFolderCoverUrl"
  >,
) {
  const {
  ...
  return {
    ...projected,
    ...(image
      ? { coverUrl: await dependencies.signFolderCoverUrl(image.storageKey) }
      : {}),
  };
}
...
function documentView(document: Document) {
  const {
    ...
    chunkEmbeddings: _chunkEmbeddings,
    semanticChunkCount: _semanticChunkCount,
    semanticContentHash: _semanticContentHash,
    emailToneEmbeddingVersion: _emailToneEmbeddingVersion,
...
function publicDocumentContent(document: Document) {
  ...
  return raw &&
    typeof raw === "object" &&
    "kind" in raw &&
    raw.kind === "mail-message"
    ? emailMessagePayloadSchema.parse(raw).data.body
    : document.content;
}

function withinCreationDateRange(
  value: { createdAt: string },
  createdFrom?: string,
  createdTo?: string,
) {
  return (
    (!createdFrom || value.createdAt >= createdFrom) &&
    (!createdTo || value.createdAt <= createdTo)
  );
}
...
function isSystemManagedMailDocument(document: Document) {
  ...
  try {
    return MAIL_ENVELOPE_KINDS.has(JSON.parse(document.content)?.kind);
  } catch {
    return false;
  }
}
...
function versionView(version: DocumentVersion, include: string[] = []) {
  ...
  return {
    ...
    ...(include.includes("embedding") ? { embedding } : {}),
  };
}
...
async function summaryAudioView(
  audio: DocumentSummaryAudio,
...
async function projectedSummaryView(
  summary: DocumentSummary,
  audio: DocumentSummaryAudio | undefined,
  signUrl: (storageKey: string) => Promise<string>,
) {
  return {
    ...summaryView(summary),
    ...(audio ? { audio: await summaryAudioView(audio, signUrl) } : {}),
  };
}
...
function plainGeneratedSummary(value: unknown) {
  return z
    ...
    .parse(value)
    .replace(
      /<(?:analysis|thinking|reasoning)>[\s\S]*?<\/(?:analysis|thinking|reasoning)>/gi,
      "",
    )
...
async function audioVersionView(
  version: DocumentAudioVersion,
  current: Document,
  signUrl: (storageKey: string) => Promise<string>,
) {
  return {
    ...
    current:
      version.sourceContentHash === documentSemanticHash(current.content) &&
      (!version.includeTitle || version.sourceTitle === current.name),
...
async function productionRepository(): Promise<ContentRepository> {
  ...
  const allowedScopeKeys = async (
    teamKey: string,
    teamMembershipKey: string,
  ): Promise<string[]> => {
    const cursor = await client.db.query<{
      teamRole?: string;
      scopes: string[];
      members: string[];
      relations: Array<{ parentKey: string; childKey: string }>;
    }>(
      'LET membership = DOCUMENT(userTeams, @teamMembershipKey) RETURN { teamRole: membership.teamRole, scopes: (FOR scope IN scopes FILTER scope.teamKey == @teamKey RETURN scope._key), members: (FOR member IN scopeMembers FILTER member.userTeamKey == @teamMembershipKey && member.status == "active" RETURN member.scopeKey), relations: (FOR relation IN scopeScopes RETURN { parentKey: relation.parentKey, childKey: relation.childKey }) }',
      { teamKey, teamMembershipKey },
    );
    const data = await cursor.next();
    if (!data) return [];
    if (data.teamRole === "owner" || data.teamRole === "admin")
      return data.scopes;
    ...
    while (changed) {
      ...
      for (const relation of data.relations) {
        if (
          accessible.has(relation.parentKey) &&
          !accessible.has(relation.childKey)
        ) {
  ...
  const makeRepository = (persistence: Persistence): ContentRepository => ({
    ...
    async listFolders(scopeKey, includePendingDeletion) {
      return persistence.listFolders(scopeKey, includePendingDeletion);
    },
    ...
    async updateFolder(key, patch) {
      ...
      const updated = await persistence.updateFolder(
        current.scopeKey,
        key,
        patch,
      );
    ...
    async setFolderDeletion(key, marker, owner) {
      ...
      return persistence.setFolderDeletion(
        current.scopeKey,
        key,
        marker,
        owner,
    ...
    async deleteFolder(key) {
      const current = await persistence.getFolder(key);
      if (!current || !(await persistence.deleteFolder(current.scopeKey, key)))
    ...
    async updateDocument(key, patch, options) {
      ...
      if (!current)
        throw new Error("Document was not found for scoped update.");
      ...
      if (!updated)
        throw new Error(
          options?.expectedUpdatedAt
            ...
            : "Document scope changed during update.",
        );
      return updated;
    },
    async setDocumentDeletion(key, marker, owner) {
      const current = await persistence.getDocument(key);
      if (!current) return null;
      return persistence.setDocumentDeletion(
        current.scopeKey,
        key,
        marker,
        owner,
    ...
    async deleteDocument(key) {
      ...
      if (
        !current ||
        !(await persistence.deleteDocument(current.scopeKey, key))
      )
        throw new Error("Document was not found for scoped deletion.");
    },
    ...
    async deleteVersion(key) {
      ...
      if (!current || !(await persistence.deleteVersion(current.scopeKey, key)))
        throw new Error("Version was not found for scoped deletion.");
    },
    ...
    async deleteAudioVersion(key) {
      ...
      if (
        !current ||
        !(await persistence.deleteAudioVersion(current.scopeKey, key))
      )
        throw new Error("Audio version was not found for scoped deletion.");
    },
    ...
    async deleteSummary(key) {
      ...
      if (!current || !(await persistence.deleteSummary(current.scopeKey, key)))
        throw new Error("Summary was not found for scoped deletion.");
    },
    ...
    async deleteSummaryAudio(summaryKey) {
      ...
      if (
        !current ||
        !(await persistence.deleteSummaryAudio(current.scopeKey, current.key))
      )
        throw new Error("Summary audio was not found for scoped deletion.");
    },
    ...
    async generatedDocumentBindings(documentKeys) {
      ...
      const cursor = await client.db.query(
        "FOR binding IN generatedDocumentBindings FILTER binding.documentKey IN @documentKeys RETURN binding",
        { documentKeys },
      );
      const { generatedDocumentBindingSchema } =
        await import("@/lib/db/generated-document-bindings.node");
      return (await cursor.all()).map((value) =>
        generatedDocumentBindingSchema.parse(
          withArangoKey(value as Record<string, unknown>),
        ),
      );
    },
    transaction: (operation) =>
      content.withContentPersistenceTransaction((bound) =>
        operation(makeRepository(bound)),
      ),
  });
...
export async function authorizeDocumentParseLocation(
  input: { scopeKey: string; folderKey?: string },
  context: ToolContext,
  repository?: ContentRepository,
): Promise<void> {
  if (context.principal.kind !== "member")
    throw new ContentError(
      "CONTENT_FORBIDDEN",
      "A member principal is required.",
      "document.parse",
      { action: "authorization" },
    );
  const repo = repository ?? (await productionRepository());
  const scope = await repo.getScope(input.scopeKey);
  if (!scope || scope.teamKey !== context.teamKey)
    throw new ContentError(
      "CONTENT_NOT_FOUND",
      "Scope was not found in this team.",
      "document.parse",
      { action: "resolution" },
    );
  const teamRole = context.principal.userTeam.teamRole;
  const role: Role | null =
    teamRole === "owner" || teamRole === "admin"
      ? teamRole
      : await repo.role(input.scopeKey, context.principal.userTeam.key);
  if (!role || rank[role] < rank.moderator)
    throw new ContentError(
      "CONTENT_FORBIDDEN",
      "The principal lacks the required scope role.",
      "document.parse",
      { action: "authorization" },
    );
  ...
  while (folderKey) {
    if (visited.has(folderKey))
      throw new ContentError(
        ...
        "document.parse",
        { action: "resolution", resourceKey: folderKey },
      );
    ...
    if (folder.scopeKey !== input.scopeKey)
      throw new ContentError(
        "CONTENT_FORBIDDEN",
        "Folder does not belong to the requested scope.",
        "document.parse",
...
interface RuntimeDefaults {
  ...
  idempotency: ContentIdempotencyStore;
  generateExport: typeof generateDocumentExport;
  getFolderCoverImage: NonNullable<
    ContentToolDependencies["getFolderCoverImage"]
  >;
  signFolderCoverUrl: NonNullable<
    ContentToolDependencies["signFolderCoverUrl"]
  >;
...
async function defaults(
  ...
  const embedding = deps.embed
    ? (text: string) => deps.embed!(text)
    : async (text: string, purpose: "document" | "query" = "document") => {
        ...
        return z
          ...
          .parse(response.output.embedding);
      };
    ...
    : deps.embed
      ...
      : async (texts: string[], purpose: "document" | "query" = "document") => {
          ...
          for (
            let start = 0;
            start < texts.length;
            start += ROUTED_EMBEDDING_CONCURRENCY
          ) {
            values.push(
              ...(await Promise.all(
                texts
                  .slice(start, start + ROUTED_EMBEDDING_CONCURRENCY)
                  .map((text) => embedding(text, purpose)),
              )),
            );
          }
          return values;
  ...
  return {
    repository: deps.repository ?? (await productionRepository()),
    storage: deps.storage ?? storage.documentStorage,
    parseDocument: deps.parseDocument ?? processing.parseDocument,
    id: deps.id ?? newId,
    ...
    runAction:
      ...
      (async (
        ...
        if (operation === "document-embed")
          return processing.documentEmbed(input as never, {
            embedBatch: ({ texts }) => embeddingBatch(texts),
            dimensions: deps.ingestion?.embeddingDimensions,
          }) as Promise<ContentActionResult>;
...
function principal(context: ToolContext, tool: ContentToolName) {
  if (context.principal.kind !== "member")
    fail(
      ...
      "A resolved human principal is required.",
      tool,
      "authorization",
    );
  const member = context.principal;
  if (
    member.userTeam.teamKey !== context.teamKey ||
    member.userTeam.status !== "active"
  )
    fail(
      "CONTENT_FORBIDDEN",
      "Active membership in the requested team is required.",
      tool,
      "authorization",
    );
  return member;
...
async function observe(deps: ContentToolDependencies, event: SafeEvent) {
  try {
    await deps.observer?.(event);
  } catch {
    /* Telemetry cannot alter behavior. */
  }
}
...
async function batch<T>(
  ...
  atomic: boolean,
  initialRepository: ContentRepository,
) {
  let repo = initialRepository;
  if (atomic && !repo.transaction)
    fail(
      "CONTENT_CONFLICT",
      "Atomic mode is unavailable for this operation.",
      tool,
      "transaction",
    );
  ...
  const execute = async () => {
    ...
    for (const item of items) {
      try {
        ...
        if (!atomic && item.transactional && !initialRepository.transaction)
          fail(
            "CONTENT_CONFLICT",
            "Transaction-bound item execution is unavailable.",
            tool,
            "transaction",
            item.key,
          );
        const data =
          !atomic && item.transactional
            ? await initialRepository.transaction!((transactionRepository) =>
                item.run(transactionRepository, true),
              )
...
async function fingerprintInput(input: unknown): Promise<string> {
  ...
  return createHash("sha256")
    .update(JSON.stringify(await normalize(input)))
    .digest("hex");
}
...
export function isContentMutation(tool: ContentToolName, input: any): boolean {
  ...
  if (tool === "document.rewrite")
    return (
      Array.isArray(input?.rewrites) &&
      input.rewrites.some((item: any) => item?.mode !== "preview")
    );
  return MUTATIONS.has(tool);
}

export async function runContentTool<Name extends ContentToolName>(
  name: Name,
  rawInput: unknown,
  context: ToolContext,
  dependencies: ContentToolDependencies = {},
): Promise<ContentToolOutput<Name>> {
  ...
  const tool = name;
  const member = principal(context, tool);
  let input: any;
  try {
    input = contentToolInputSchemas[tool].parse(rawInput);
  } catch (error) {
    throw mappedError(error, tool, "parse");
  }
  ...
  const nextUpdatedAt = (current: string) =>
    new Date(
      Math.max(d.clock().getTime(), Date.parse(current) + 1),
    ).toISOString();
  const event = (
    type: SafeEvent["type"],
  ...
  const observeRepository = (target: ContentRepository): ContentRepository =>
    new Proxy(target, {
      get(repository, property, receiver) {
        const value = Reflect.get(repository, property, receiver);
        if (property === "transaction" && typeof value === "function") {
          return (operation: (bound: ContentRepository) => Promise<unknown>) =>
            value.call(repository, (bound: ContentRepository) =>
              operation(observeRepository(bound)),
            );
        }
        if (typeof value !== "function") return value;
        return async (...args: unknown[]) => {
          const method = String(property);
          const primitive = method.startsWith("get")
            ? "read"
            : method.startsWith("list") ||
                method === "allowedScopeKeys" ||
                method === "semanticSearch"
              ? "traverse"
              : method.startsWith("insert") || method.startsWith("create")
                ? "insert"
  ...
  const embed = async (
    ...
    try {
      const embedding = z
        .array(z.number().finite())
        .min(1)
        .parse(await d.embed(text, purpose));
      await event(
        "embedding",
        "succeeded",
        "embed",
  ...
  const storageOperation = async <T>(
    slug: string,
  ...
  const deleteStorageKeys = async (
    ...
    for (const storageKey of new Set(
      ...
      try {
        await storageOperation("delete", resourceKey, scopeKey, () =>
          d.storage.delete(storageKey),
        );
      } catch (error) {
        throw new ContentError(
          "CONTENT_CONFLICT",
          "Storage deletion failed; metadata pointers were retained for retry.",
          tool,
          { action: "storage", resourceKey, cause: error, retryable: true },
        );
      }
    }
  };
  const roleFor = async (
    ...
    try {
      ...
      const role: Role | null =
        member.userTeam.teamRole === "owner" ||
        member.userTeam.teamRole === "admin"
          ? member.userTeam.teamRole
          : await repository.role(scopeKey, member.userTeam.key);
  ...
  const folder = async (
    ...
    if (!value || !archiveVisible(value))
      fail("CONTENT_NOT_FOUND", "Folder was not found.", tool, "read", key);
    await roleFor(value.scopeKey, minimum, key, repository);
    if (!pendingDeletion && value._internalDeletion)
      fail("CONTENT_NOT_FOUND", "Folder was not found.", tool, "read", key);
    ...
    while (parentKey) {
      ...
      if (
        !parent ||
        parent.scopeKey !== value.scopeKey ||
        !archiveVisible(parent) ||
        (!pendingDeletion && parent._internalDeletion)
      )
        fail("CONTENT_NOT_FOUND", "Folder was not found.", tool, "read", key);
  ...
  const folderAncestors = async (
    ...
    while (currentKey) {
      ...
      if (!current || current.scopeKey !== scopeKey)
        fail(
          "CONTENT_CONFLICT",
          "Folder ancestor left the requested scope.",
          tool,
          "resolution",
          currentKey,
        );
  ...
  const document = async (
    ...
    if (!value || !archiveVisible(value))
      fail("CONTENT_NOT_FOUND", "Document was not found.", tool, "read", key);
    await roleFor(value.scopeKey, minimum, key, repository);
    if (!pendingDeletion && value._internalDeletion)
      fail("CONTENT_NOT_FOUND", "Document was not found.", tool, "read", key);
    ...
    while (parentKey) {
      if (visited.has(parentKey))
        fail(
          "FOLDER_CYCLE_DETECTED",
          "The document folder hierarchy contains a cycle.",
          tool,
          "resolution",
          parentKey,
        );
      ...
      if (
        ...
        fail("CONTENT_NOT_FOUND", "Document was not found.", tool, "read", key);
      if (!pendingDeletion && parent._internalDeletion)
        fail("CONTENT_NOT_FOUND", "Document was not found.", tool, "read", key);
      parentKey = parent.parentFolderKey;
    }
    return value;
    ...
    return all.filter((item) => {
      ...
      while (current) {
        if (
          ...
          !archiveVisible(current) ||
          (!includePendingDeletion && current._internalDeletion)
        )
          return false;
  ...
  const descendants = (all: Folder[], key: string) => {
    const out: Folder[] = [];
    const pending = [key];
    const seen = new Set(pending);
    while (pending.length) {
      const parentKey = pending.shift()!;
      for (const child of all.filter((f) => f.parentFolderKey === parentKey))
  ...
  const forbidManagedDocumentMutation = (
    current: Document,
    action: string,
    resourceKey = current.key,
  ) => {
    if (managedDocument(current))
      fail(
        "CONTENT_FORBIDDEN",
        "Managed documents are read-only.",
        tool,
        action,
        resourceKey,
      );
    if (isSystemManagedMailDocument(current))
      fail(
        "CONTENT_FORBIDDEN",
        "Mail documents are managed through their canonical email capabilities.",
        tool,
        action,
        resourceKey,
      );
  };
  const forbidManagedInFolders = async (
    scopeKey: string,
    folderKeys: Set<string>,
    action: "move" | "copy" | "delete",
    repository = repo,
  ) => {
    const folders = await repository.listFolders(scopeKey, true);
    const currentFolder = folders.find((item) => folderKeys.has(item.key) && managedFolder(item));
    if (currentFolder)
      fail(
        "CONTENT_FORBIDDEN",
        "Managed folders are read-only.",
        tool,
        action,
        currentFolder.key,
      );
  ...
  const generated = async (
    ...
    const output = await action(
      slug,
      {
        ...
        options: {
          ...
          maxTokens: Math.min(
            5_000,
            Math.max(256, Math.ceil(doc.content.length / 3)),
          ),
        },
      },
      doc.key,
    ...
    if (!output.text?.trim())
      fail(
        "CONTENT_CONFLICT",
        "The generation action returned invalid output.",
        tool,
        slug,
        doc.key,
      );
  ...
  const parsedSemantics = (
    ...
    const chunkEmbeddings = embedded.chunkEmbeddings
      ? z
          ...
          .parse(embedded.chunkEmbeddings)
      : contentChunks.length === 1
        ? [embedding]
        : fail(
            "CONTENT_CONFLICT",
            "Document embedding action did not return every semantic chunk.",
            tool,
            "document-embed",
            resourceKey,
          );
    return {
      ...
      contentChunks,
      chunkEmbeddings,
      semanticChunkCount: z
        .number()
        .int()
        .positive()
        .parse(embedded.semanticChunkCount ?? contentChunks.length),
      semanticContentHash: z
        .string()
        .regex(/^[a-f0-9]{64}$/)
        .parse(
          embedded.semanticContentHash ??
  ...
  const representations = async (
    ...
    return prepareDocumentRepresentation(
      { content: source, name: documentName },
      {
        documentEmbed: (input) =>
          action("document-embed", input, resourceKey, scopeKey),
        embeddingDimensions: EMBEDDING_DIMENSIONS,
      },
    );
  };
  const isCurrentEmbedding = (embedding: readonly number[]) =>
    embedding.length === EMBEDDING_DIMENSIONS &&
    embedding.every(Number.isFinite);
  const hasCurrentSemantics = (
    source: Pick<
      Document,
      "embedding" | "content" | "contentChunks" | "chunkEmbeddings"
    >,
  ) => {
    const expectedChunks = chunkDocumentContent(source.content);
    return (
      isCurrentEmbedding(source.embedding) &&
      source.contentChunks?.length === expectedChunks.length &&
      source.contentChunks.every(
  ...
  const generatedSemantics = async (
    name: string,
    content: string,
    resourceKey: string,
    scopeKey: string,
  ) => {
    const embedded = await action(
      "document-embed",
      { name, content },
      resourceKey,
      scopeKey,
  ...
  const currentDocumentSemantics = (
    source: Pick<
      Document,
      | "embedding"
      | "name"
    ...
    name === source.name && hasCurrentSemantics(source)
      ? Promise.resolve({
          ...
          contentChunks: source.contentChunks!,
          chunkEmbeddings: source.chunkEmbeddings!,
          semanticChunkCount: source.contentChunks!.length,
          semanticContentHash: createHash("sha256")
            .update(source.content)
            .digest("hex"),
        })
      : generatedSemantics(name, source.content, source.key, source.scopeKey);
  const currentVersionSemantics = async (
    source:
      | Pick<DocumentVersion, "content" | "key" | "scopeKey">
      | Pick<Document, "content" | "key" | "scopeKey">,
    documentName: string,
  ) => {
    const {
      embedding,
  ...
  const restoredVersionSemantics = (
    ...
    if (
      !isCurrentEmbedding(version.embedding) ||
  ...
  const persistGenerated = async (
    ...
    const finalName =
      mode === "copy" ? `${source.name} (${suffix})` : source.name;
    ...
    if (mode === "replace") {
      ...
      try {
        await repo.updateDocument(source.key, {
          ...transformed,
        ...
        try {
          await repo.deleteVersion(backup.key);
        } catch (cleanupError) {
          throw new ContentError(
            "CONTENT_CONFLICT",
            "AI replacement failed and backup compensation requires retry.",
            tool,
            {
              action: "cleanup",
  ...
  const idempotencyIdentity =
    mutation && input.idempotencyKey
      ...
      : undefined;
  const requestHash = idempotencyIdentity
    ? await fingerprintInput(input)
    : undefined;
  let ownsIdempotencyClaim = false;
  let idempotencyExecutionStarted = false;
  if (idempotencyIdentity && requestHash) {
    const claim = await d.idempotency.claim(
      idempotencyIdentity,
      requestHash,
      invocationKey,
      now(),
    ...
    if (claim.status === "replay")
      return contentToolOutputSchemas[tool].parse(
        claim.response,
      ) as ContentToolOutput<Name>;
    ...
    if (claim.status === "pending")
      throw new ContentError(
        ...
        "An invocation with this idempotency key is still active.",
        tool,
        { action: "idempotency", retryable: true },
      );
    if (claim.status === "indeterminate")
      throw new ContentError(
        "CONTENT_IDEMPOTENCY_INDETERMINATE",
        "The prior invocation may have produced effects and cannot be executed again.",
        tool,
    ...
    if (claim.status === "failed") {
      throw new ContentError(
        ...
        tool,
        { action: "idempotency", retryable: claim.failure.retryable },
      );
    }
    ownsIdempotencyClaim = true;
    if (
      !(await d.idempotency.start(
        idempotencyIdentity,
        requestHash,
        invocationKey,
        now(),
      ...
      throw new ContentError(
        "CONTENT_IDEMPOTENCY_PENDING",
        "Idempotency execution ownership was lost.",
        tool,
        { action: "idempotency", retryable: true },
      );
    }
  ...
  const linkedMutationKeys = (() => {
    ...
    const visit = (value: unknown, field?: string) => {
      ...
      else if (field === "documentKeys" && Array.isArray(value))
  ...
  try {
    linkedBindings =
      linkedMutationKeys.length > 0 && repo.generatedDocumentBindings
        ? await repo.generatedDocumentBindings(linkedMutationKeys)
        : [];
    const managedChapter = linkedBindings.find(
      ({ subjectType, kind }) =>
        subjectType === "chapter" && kind === "chapter",
    );
    if (managedChapter)
      fail(
        "CONTENT_FORBIDDEN",
        "Published audiobook chapters are managed by their canonical book capability.",
        tool,
        "update",
        managedChapter.documentKey,
      );
    if (tool === "folder.create") {
      const creates = input.folders.map((item: any) => ({
        ...item,
        key: item.key ?? d.id(),
      }));
      result = await batch(
        ...
        creates.map((item: any) => ({
          ...
          run: async () => {
            ...
            if (item.parentFolderKey) {
              ...
              if (parent.scopeKey !== item.scopeKey)
                fail(
            ...
            if (
              item.coverImageKey &&
              !(await d.getFolderCoverImage(item.scopeKey, item.coverImageKey))
            )
              fail(
                "CONTENT_NOT_FOUND",
                "Folder cover image was not found in this scope.",
                tool,
                "read",
                item.coverImageKey,
              );
          ...
          run: async () => {
            ...
            return {
              folder: {
                ...
                ...(input.includeChildrenCount
                  ? {
                      childrenCount: allFolders.filter(
                        (item) => item.parentFolderKey === key,
                      ).length,
                    }
                  : {}),
                ...(input.includeDocumentCount
                  ? {
                      documentCount: allDocuments.filter(
                        (item) => item.folderKey === key,
                      ).length,
                    }
                  : {}),
              },
      ...
      if (input.parentFolderKey) {
        ...
        if (parent.scopeKey !== input.scopeKey)
          fail(
            "CONTENT_NOT_FOUND",
            "Folder was not found in this scope.",
            tool,
            "read",
            input.parentFolderKey,
          );
      }
      ...
      const isVisible = (item: Folder) => {
        ...
        while (current) {
          ...
          visited.add(current.key);
          if (
            current.parentFolderKey &&
            !folderByKey.has(current.parentFolderKey)
          )
            return false;
          current = current.parentFolderKey
            ? folderByKey.get(current.parentFolderKey)
            : undefined;
        }
        return true;
      };
      const values = allFolders.filter((item) => {
        if (!withinCreationDateRange(item, input.createdFrom, input.createdTo))
          return false;
        if (!isVisible(item)) return false;
        if (!input.includeDescendants)
          return item.parentFolderKey === input.parentFolderKey;
      ...
      values.sort(
        (left: any, right: any) =>
          String(left[sort.field]).localeCompare(String(right[sort.field])) *
          (sort.direction === "asc" ? 1 : -1),
      );
      const offset = input.cursor
        ? Number(Buffer.from(input.cursor, "base64url").toString()) || 0
        : 0;
      const limit = input.limit ?? 50;
      const documents =
        input.includeDocuments && input.parentFolderKey
          ? (await repo.listDocuments(input.scopeKey))
              .filter(
                (item) =>
                  archiveVisible(item) &&
                  item.folderKey === input.parentFolderKey &&
      ...
      if (input.atomic && requiresEmbedding)
        fail(
          "CONTENT_CONFLICT",
          "Atomic folder metadata updates are unavailable because embedding is an external side effect.",
          tool,
          "embed",
        );
      result = await batch(
        ...
        items.map((item: any) => ({
          key: item.folderKey,
          preflight: async () => {
            await folder(item.folderKey, "moderator", false);
          },
          run: async (mutationRepository: ContentRepository) => {
            ...
            if (
              ...
              (tool === "folder.rename" ||
                ...
                item.coverImageKey !== undefined)
            )
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folder metadata is read-only.",
                tool,
                "update",
                current.key,
              );
            ...
            const embedding = changesEmbedding
              ? await embed(
            ...
            const patch = {
              ...
              ...(item.description !== undefined
                ...
                : {}),
              ...(item.coverImageKey !== undefined
                ? { coverImageKey: item.coverImageKey ?? undefined }
                : {}),
            ...
            return {
              folder: await folderView(
                await mutationRepository.updateFolder(current.key, patch),
                d,
              ),
            };
          },
      ...
      result = await batch(
        tool,
        input.moves.map((item: any) => ({
          key: item.folderKey,
          preflight: async () => {
            const source = await folder(item.folderKey, "admin", false);
            if (managedFolder(source))
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folders cannot be moved.",
                tool,
                "update",
                source.key,
              );
            const all = await foldersIn(source.scopeKey);
            await forbidManagedInFolders(
              source.scopeKey,
              new Set([
                source.key,
                ...descendants(all, source.key).map((child) => child.key),
              ]),
              "move",
            );
            if (!item.targetParentFolderKey) return;
            const target = await folder(
              item.targetParentFolderKey,
              "admin",
              false,
            );
            if (managedFolder(target))
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folders cannot contain moved folders.",
                tool,
                "update",
                target.key,
              );
            if (source.scopeKey !== target.scopeKey)
              fail(
                "FOLDER_MOVE_FORBIDDEN",
                "Cross-scope folder moves are forbidden.",
                tool,
                "update",
                source.key,
              );
            if (
              ...
              descendants(all, source.key).some(
                (child) => child.key === target.key,
              )
            )
              fail(
                "FOLDER_CYCLE_DETECTED",
          ...
          run: async (mutationRepository: ContentRepository) => {
            ...
            const all = await mutationRepository.listFolders(
              source.scopeKey,
              true,
            );
            await forbidManagedInFolders(
      ...
      if (input.atomic)
        fail(
          "CONTENT_CONFLICT",
          "Atomic folder copy is unavailable because storage copy cannot be rolled back transactionally.",
          tool,
          "storage",
        );
      result = await batch(
        tool,
        input.copies.map((item: any) => ({
          key: item.folderKey,
          run: async () => {
            ...
            if (managedFolder(source))
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folders cannot be copied.",
                tool,
                "copy",
                source.key,
              );
            ...
            const sourceFolders = [
              source,
              ...descendants(sourceScopeFolders, source.key),
            ];
            await forbidManagedInFolders(
              source.scopeKey,
              new Set(sourceFolders.map((current) => current.key)),
              "copy",
            );
            ...
            if (target && managedFolder(target))
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folders cannot contain copied folders.",
                tool,
                "copy",
                target.key,
              );
            ...
            const sourceFolderKeys = new Set(
              sourceFolders.map((candidate) => candidate.key),
            );
            const sourceDocuments = (
              await repo.listDocuments(source.scopeKey)
            ).filter(
              (candidate) =>
                candidate.folderKey &&
                sourceFolderKeys.has(candidate.folderKey),
            );
            const folderKeys = new Map(
              sourceFolders.map((candidate) => [candidate.key, d.id()]),
            );
            ...
            const copyObject = async (
              ...
              const copied = await storageOperation(
                "copy",
                source.key,
                item.targetScopeKey,
            ...
            try {
              for (const current of sourceFolders) {
                ...
                const parentFolderKey =
                  current.key === source.key
                    ? target?.key
                    : folderKeys.get(current.parentFolderKey!);
                const name =
                  current.key === source.key ? rootName : current.name;
                const embedding =
                  current.key === source.key && name !== current.name
                    ? await embed(
                ...
                const created = await repo.insertFolder({
                  ...
                  name,
                  embedding,
                  ...(source.scopeKey === item.targetScopeKey
                    ? {}
              ...
              for (const current of sourceDocuments) {
                const key = d.id();
                const base = `content/${context.teamKey}/${item.targetScopeKey}/${key}`;
                const storageKey =
                  current.storageKey && current.extension
                    ? await copyObject(
                        current.storageKey,
                        `${base}/original.${current.extension}`,
                        current.mimeType,
                      )
                    : undefined;
                ...
                for (
                  let index = 0;
                  index < (current.sourceStorageKeys?.length ?? 0);
                  index += 1
                ) {
                  const sourceKey = current.sourceStorageKeys![index]!;
                  const suffix = sourceKey.match(/\.[a-z0-9]+$/i)?.[0] ?? "";
                  sourceStorageKeys.push(
                    await copyObject(
                      sourceKey,
                      `${base}/sources/${index + 1}${suffix}`,
                    ),
                  );
                }
                ...
                for (
                  let index = 0;
                  index < (current.speechStorageKeys?.length ?? 0);
                  index += 1
                ) {
                  const sourceKey = current.speechStorageKeys![index]!;
                  const suffix = sourceKey.match(/\.[a-z0-9]+$/i)?.[0] ?? "";
                  speechStorageKeys.push(
                    await copyObject(
                      sourceKey,
                      `${base}/speech/${index + 1}${suffix}`,
                    ),
                  );
                }
                const created = await repo.insertDocument({
                  ...
                  ...(sourceStorageKeys.length
                    ? { sourceStorageKeys }
                    : { sourceStorageKeys: undefined }),
                  ...(speechStorageKeys.length
                    ? { speechStorageKeys }
                    : { speechStorageKeys: undefined }),
                  isFavorite: false,
                  _internalDeletion: undefined,
                  createdAt: timestamp,
                  updatedAt: timestamp,
              ...
              const copiedRoot = await repo.getFolder(
                folderKeys.get(source.key)!,
              );
              if (!copiedRoot)
                fail(
                  "CONTENT_CONFLICT",
                  "Copied folder root could not be read.",
                  tool,
                  "read",
                  source.key,
                );
              return {
                folder: await folderView(copiedRoot, d),
                folderCount: sourceFolders.length,
                documentCount: sourceDocuments.length,
              };
            } catch (error) {
              console.error("folder copy failed before compensation", {
                sourceFolderKey: source.key,
                targetScopeKey: item.targetScopeKey,
                targetParentFolderKey: item.targetParentFolderKey,
                error,
              });
              ...
              const folderMarker = {
                ...
                owner: invocationKey,
                startedAt: timestamp,
                folderKeys: insertedFolderKeys,
                documentKeys: insertedDocumentKeys,
                objectKeys: copiedStorageKeys,
              };
              try {
                if (!repo.transaction)
                  throw new Error(
                    "Transaction-bound copy compensation is unavailable.",
                  );
                await repo.transaction(async (bound) => {
                  for (const key of insertedDocumentKeys) {
                    const marker = {
                      ...
                      owner: invocationKey,
                      startedAt: timestamp,
                      objectKeys: documentStorageKeys.get(key) ?? [],
                    };
                    if (!(await bound.setDocumentDeletion(key, marker)))
                      throw new Error(
                        `Copied document ${key} could not be frozen for compensation.`,
                      );
                  }
                  for (const key of [...insertedFolderKeys].reverse())
                    if (!(await bound.setFolderDeletion(key, folderMarker)))
                      throw new Error(
                        `Copied folder ${key} could not be frozen for compensation.`,
                      );
                });
              } catch (cleanupError) {
              ...
              if (cleanupErrors.length === 0) {
                ...
                for (const key of [...insertedDocumentKeys].reverse())
                  await repo.deleteDocument(key).catch((cleanupError) => {
                    failedDocumentKeys.add(key);
                    cleanupErrors.push(cleanupError);
                  });
                const retainedStorageKeys = new Set(
                  [...failedDocumentKeys].flatMap(
                    (key) => documentStorageKeys.get(key) ?? [],
                  ),
                );
                const failedStorageKeys = new Set(retainedStorageKeys);
                for (const key of [...copiedStorageKeys].reverse())
                  if (!retainedStorageKeys.has(key)) {
                ...
                if (cleanupErrors.length === 0)
                  for (const key of [...insertedFolderKeys].reverse()) {
                    await repo
                      .deleteFolder(key)
                      .then(() => remainingFolderKeys.delete(key))
                      .catch((cleanupError) =>
                        cleanupErrors.push(cleanupError),
                      );
                  }
                if (cleanupErrors.length > 0 && remainingFolderKeys.size > 0) {
                  const recoveryMarker = {
                    ...folderMarker,
                    folderKeys: [...remainingFolderKeys],
                    documentKeys: [...failedDocumentKeys],
                    objectKeys: [...failedStorageKeys],
                  };
                  try {
                    await repo.transaction!(async (bound) => {
                      for (const key of remainingFolderKeys)
                        if (
                          !(await bound.setFolderDeletion(
                            key,
                            recoveryMarker,
                            invocationKey,
                          ))
                          ...
                          throw new Error(
                            `Copied folder ${key} compensation manifest could not be updated.`,
                          );
                    });
                  } catch (cleanupError) {
                    cleanupErrors.push(cleanupError);
              ...
              if (cleanupErrors.length)
                throw new ContentError(
                  "CONTENT_CONFLICT",
                  "Folder copy failed and compensation requires retry.",
                  tool,
                  {
                    action: "cleanup",
                    resourceKey: insertedFolderKeys[0] ?? source.key,
      ...
      result = await batch(
        ...
        input.folderKeys.map((key: string) => ({
          ...
          preflight: async () => {
            ...
            if (managedFolder(root))
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folders cannot be deleted.",
                tool,
                "delete",
                key,
              );
            ...
            await forbidManagedInFolders(
              ...
              new Set([
                root.key,
                ...descendants(all, root.key).map((child) => child.key),
              ]),
              "delete",
            );
            if (root._internalDeletion) {
              ...
              if (input.atomic)
                fail(
                  "CONTENT_CONFLICT",
                  "Atomic folder deletion cannot resume an existing deletion manifest.",
                  tool,
                  "transaction",
                  key,
                );
              if (
                ...
                !manifest.folderKeys ||
                !manifest.documentKeys ||
                !manifest.folderKeys.includes(key)
              )
                fail(
                  "CONTENT_CONFLICT",
                  "Folder deletion manifest is incomplete.",
                  tool,
                  "delete",
                  key,
                );
            ...
            if (root.isFavorite)
              fail(
                "CONTENT_CONFLICT",
                "Unfavorite the folder before deleting it.",
                tool,
                "delete",
                key,
              );
          ...
          run: async (mutationRepository: ContentRepository) => {
            if (input.atomic) {
              ...
              const affected = input.recursive
                ? [candidate, ...children]
                : [candidate];
              const affectedKeys = new Set(affected.map((item) => item.key));
              const documents = (
                await mutationRepository.listDocuments(candidate.scopeKey, true)
              ).filter(
                (item) =>
                  item.folderKey !== undefined &&
                  affectedKeys.has(item.folderKey),
              );
              const managed = documents.find(
                (item) =>
                  managedDocument(item) || isSystemManagedMailDocument(item),
              );
              if (managed) forbidManagedDocumentMutation(managed, "delete");
              if (!input.recursive && children.length > 0)
                fail(
                  "FOLDER_NOT_EMPTY",
                  "Folder is not empty.",
                  tool,
              ...
              if (documents.length > 0)
                fail(
                  "CONTENT_CONFLICT",
                  "Atomic folder deletion is unavailable when storage objects are involved.",
                  tool,
                  "storage",
                  key,
                );
              for (const item of affected) {
                if (item._internalDeletion)
                  fail(
                    "CONTENT_CONFLICT",
                    "A folder deletion is already pending.",
                    tool,
                    "delete",
                    item.key,
                  );
                if (item.isFavorite)
                  fail(
                    "CONTENT_CONFLICT",
                    "Unfavorite every recursively deleted folder before deleting it.",
                    tool,
                    "delete",
                    item.key,
                  );
              }
              if (documents.some((item) => item.isFavorite))
                fail(
                  "CONTENT_CONFLICT",
                  "Unfavorite every document in the folder before deleting it.",
                  tool,
                  "delete",
                  key,
                );
              const marker = {
                ...
                owner: invocationKey,
                startedAt: now(),
                folderKeys: affected.map((item) => item.key),
                documentKeys: [],
                objectKeys: [],
              };
              for (const item of [...affected].reverse())
                if (
                  !(await mutationRepository.setFolderDeletion(
                    item.key,
                    marker,
                  ))
                )
                  fail(
                    "CONTENT_CONFLICT",
                    "Folder could not be frozen for deletion.",
                    tool,
                    "transaction",
                    item.key,
                  );
              for (const item of [...affected].reverse())
                await mutationRepository.deleteFolder(item.key);
            ...
            if (!repo.transaction)
              fail(
                "CONTENT_CONFLICT",
                "Transaction-bound deletion marking is unavailable.",
                tool,
                "transaction",
                key,
              );
            ...
            try {
              ({ root, affected, documents } = await repo.transaction(
                async (bound) => {
                  ...
                  if (candidate._internalDeletion) {
                    if (candidate._internalDeletion.kind !== "folder")
                      fail(
                        "CONTENT_CONFLICT",
                        "A different deletion is already pending.",
                        tool,
                        "delete",
                        key,
                      );
                    if (
                      !candidate._internalDeletion.folderKeys ||
                      !candidate._internalDeletion.documentKeys
                    )
                      fail(
                        "CONTENT_CONFLICT",
                        "Folder deletion manifest is incomplete.",
                    ...
                    const all = await bound.listFolders(
                      candidate.scopeKey,
                      true,
                    );
                    const intendedFolderKeys =
                      candidate._internalDeletion.folderKeys;
                    const frozen = all.filter((item) =>
                      intendedFolderKeys.includes(item.key),
                    );
                    if (
                      frozen.length !== intendedFolderKeys.length ||
                      frozen.some(
                        (item) =>
                          item._internalDeletion?.kind !== "folder" ||
                          item._internalDeletion.owner !==
                            candidate._internalDeletion!.owner,
                      )
                    ) {
                      fail(
                        "CONTENT_CONFLICT",
                        "Folder deletion manifest ownership changed.",
                        tool,
                        "delete",
                        key,
                      );
                    ...
                    const intendedDocumentKeys =
                      candidate._internalDeletion.documentKeys;
                    const frozenDocuments = (
                      await bound.listDocuments(candidate.scopeKey, true)
                    ).filter((item) => intendedDocumentKeys.includes(item.key));
                    if (
                      frozenDocuments.length !== intendedDocumentKeys.length ||
                      frozenDocuments.some(
                        (item) =>
                          item._internalDeletion?.kind !== "document" ||
                          item._internalDeletion.owner !==
                            candidate._internalDeletion!.owner ||
                          item.folderKey === undefined ||
                          !frozenKeys.has(item.folderKey),
                      )
                    ) {
                      fail(
                        "CONTENT_CONFLICT",
                        "Document deletion manifest ownership changed.",
                        tool,
                        "delete",
                        key,
                      );
                  ...
                  const managed = ownedDocuments.find(
                    (item) =>
                      managedDocument(item) ||
                      isSystemManagedMailDocument(item),
                  );
                  if (managed) forbidManagedDocumentMutation(managed, "delete");
                  if (
                    !input.recursive &&
                    (children.length > 0 || ownedDocuments.length > 0)
                  )
                    fail(
                      "FOLDER_NOT_EMPTY",
                      "Folder is not empty.",
                  ...
                  if (ownedDocuments.length > 0 && input.atomic)
                    fail(
                      "CONTENT_CONFLICT",
                      "Atomic folder deletion is unavailable when storage objects are involved.",
                      tool,
                  ...
                  for (const item of frozen) {
                    if (item._internalDeletion)
                      fail(
                        "CONTENT_CONFLICT",
                        "A descendant folder deletion is already pending.",
                        tool,
                        "delete",
                        item.key,
                      );
                  ...
                  for (const item of ownedDocuments) {
                    if (item._internalDeletion)
                      fail(
                        "CONTENT_CONFLICT",
                        "A descendant document deletion is already pending.",
                        tool,
                        "delete",
                        item.key,
                      );
                  ...
                  const folderMarker = {
                    ...
                    owner: invocationKey,
                    startedAt,
                    folderKeys: frozen.map((item) => item.key),
                    documentKeys: ownedDocuments.map((item) => item.key),
                  };
                  for (const item of ownedDocuments)
                    if (
                      !(await bound.setDocumentDeletion(
                        item.key,
                        documentMarker,
                      ))
                    )
                      fail(
                        "CONTENT_CONFLICT",
                        "Document could not be frozen for deletion.",
                        tool,
                        "transaction",
                        item.key,
                      );
                  for (const item of [...frozen].reverse())
                    if (
                      !(await bound.setFolderDeletion(item.key, folderMarker))
                    )
                      fail(
                        "CONTENT_CONFLICT",
                        "Folder could not be frozen for deletion.",
                  ...
                  ownsFreeze = true;
                  return {
                    root: { ...candidate, _internalDeletion: folderMarker },
                    affected: frozen,
              ...
              const related = await Promise.all(
                documents.map(async (item) => {
                  const summaries = repo.listSummaries
                    ? await repo.listSummaries(item.scopeKey, [item.key])
                    : [];
                  return {
                    document: item,
                    versions: await repo.listVersions(item.scopeKey, [
                      item.key,
                    ]),
                    audioVersions: repo.listAudioVersions
                      ? await repo.listAudioVersions(item.scopeKey, [item.key])
                      : [],
                    summaries,
                    summaryAudio: repo.listSummaryAudio
                      ? await repo.listSummaryAudio(
                          item.scopeKey,
              ...
              if (
                related.some((item) => item.audioVersions.length > 0) &&
                !repo.deleteAudioVersion
              )
                fail(
                  "CONTENT_CONFLICT",
                  "Document audio deletion is unavailable.",
                  tool,
                  "delete",
                  key,
                );
              if (
                related.some((item) => item.summaries.length > 0) &&
                !repo.deleteSummary
              )
                fail(
                  "CONTENT_CONFLICT",
                  "Document summary deletion is unavailable.",
                  tool,
                  "delete",
                  key,
                );
              if (
                related.some((item) => item.summaryAudio.length > 0) &&
                !repo.deleteSummaryAudio
              )
                fail(
                  "CONTENT_CONFLICT",
                  "Document summary audio deletion is unavailable.",
                  tool,
                  "delete",
                  key,
                );
              const inventoriedKeys = [
                ...new Set(
                  related
                    .flatMap((item) => [
                      item.document.storageKey,
                      ...(item.document.speechStorageKeys ?? []),
                      ...(item.document.sourceStorageKeys ?? []),
                      ...item.audioVersions.map((audio) => audio.storageKey),
                      ...item.summaryAudio.map((audio) => audio.storageKey),
                    ])
                    .filter((item): item is string => Boolean(item)),
                ),
              ];
              const manifest = root._internalDeletion?.objectKeys
                ? root._internalDeletion
                : { ...root._internalDeletion!, objectKeys: inventoriedKeys };
              if (!root._internalDeletion?.objectKeys) {
                const persisted = await repo.transaction((bound) =>
                  bound.setFolderDeletion(
                    root.key,
                    manifest,
                    root._internalDeletion!.owner,
                  ),
                );
                if (!persisted)
                  fail(
              ...
              const removeMetadata = async (bound: ContentRepository) => {
                for (const item of related) {
                  for (const version of item.versions)
                    await bound.deleteVersion(version.key);
                  for (const audio of item.audioVersions) {
                    if (!bound.deleteAudioVersion)
                      fail(
                        "CONTENT_CONFLICT",
                        "Transaction-bound audio deletion is unavailable.",
                        tool,
                        "transaction",
                        audio.key,
                      );
                  ...
                  for (const audio of item.summaryAudio) {
                    if (!bound.deleteSummaryAudio)
                      fail(
                        "CONTENT_CONFLICT",
                        "Transaction-bound summary audio deletion is unavailable.",
                        tool,
                        "transaction",
                        audio.key,
                      );
                  ...
                  for (const summary of item.summaries) {
                    if (!bound.deleteSummary)
                      fail(
                        "CONTENT_CONFLICT",
                        "Transaction-bound summary deletion is unavailable.",
                        tool,
                        "transaction",
                        summary.key,
                      );
                ...
                for (const item of affected.reverse())
                  await bound.deleteFolder(item.key);
              };
              await repo.transaction(removeMetadata);
              changedTripScopeKeys.add(root.scopeKey);
            } catch (error) {
              if (ownsFreeze && !storageStarted)
                await repo
                  .transaction(async (bound) => {
      ...
      const processed = await d.parseDocument(processingInput, {
        ...
        !dependencies.ingestion?.embedBatch
          ? {
              embedBatch: ({ texts, purpose = "document" }) =>
                d.embedBatch(texts, purpose),
            }
          : {}),
        storage: d.storage,
        logger(processingEvent) {
          ...
          const status =
            processingEvent.status === "started"
              ? "started"
              : processingEvent.status === "completed"
                ? "succeeded"
                : "failed";
          void event(
            "action",
            status,
            typeof processingEvent.action === "string"
              ? processingEvent.action
              : "document.parse",
            typeof processingEvent.documentKey === "string"
              ? processingEvent.documentKey
              : undefined,
            input.scopeKey,
            typeof processingEvent.durationMs === "number"
              ? processingEvent.durationMs
              : undefined,
          );
        },
      });
      ...
      result = await batch(
        tool,
        input.documentKeys.map((key: string) => ({
          key,
          run: async () => {
            const current = await document(key);
            const include: string[] = input.include ?? [];
            const latest = include.includes("latestVersion")
              ? (await repo.listVersions(current.scopeKey, [current.key]))[0]
              : undefined;
            const parent =
              include.includes("folder") && current.folderKey
                ? await repo.getFolder(current.folderKey)
                : undefined;
            return {
              document: {
                ...documentView(current),
                ...(include.includes("content")
                  ? { content: publicDocumentContent(current) }
                  : {}),
                ...(include.includes("embedding")
                  ? { embedding: current.embedding }
                  : {}),
                ...(parent ? { folder: await folderView(parent, d) } : {}),
                ...(latest ? { latestVersion: versionView(latest) } : {}),
                ...(include.includes("sourceImages")
                  ? {
                      sourceImages: await Promise.all(
                        (current.sourceStorageKeys ?? []).map(
                          async (storageKey, index) => ({
                            page: index + 1,
                            url: await d.signDocumentSourceUrl(storageKey),
                          }),
                        ),
                      ),
                    }
      ...
      const folderKeys =
        input.includeDescendants && parent
          ? new Set([
              parent.key,
              ...descendants(await foldersIn(input.scopeKey), parent.key).map(
                (item) => item.key,
              ),
            ])
          : undefined;
      const values = (await repo.listDocuments(input.scopeKey)).filter(
        (item) =>
          ...
          (folderKeys
            ? item.folderKey !== undefined && folderKeys.has(item.folderKey)
            : item.folderKey === parent?.key) &&
          (!input.extensions ||
            (item.extension !== undefined &&
              input.extensions.includes(item.extension))) &&
          withinCreationDateRange(item, input.createdFrom, input.createdTo),
      );
      const sort = input.sort ?? { field: "name", direction: "asc" };
      values.sort(
        (left: any, right: any) =>
      ...
      const hasRepresentationUpdates = input.updates.some(
        (item: any) => item.content !== undefined,
      );
      if (input.atomic && hasRepresentationUpdates)
        fail(
          "CONTENT_CONFLICT",
          "Atomic document updates are unavailable because transformation and embedding actions are external side effects.",
          tool,
          "document-embed",
        );
      const updates = input.updates.map((item: any) => ({
        ...
        run: async (mutationRepository: ContentRepository) => {
          ...
          if (
            managedDocument(current) &&
            (item.content !== undefined || item.createVersion)
          )
            forbidManagedDocumentMutation(current, "update");
          if (!managedDocument(current))
            forbidManagedDocumentMutation(current, "update");
          if (
          ...
          const transformed = hasRepresentation
          ...
          try {
            const updated = await mutationRepository.updateDocument(
              current.key,
              {
                ...(transformed ?? {}),
            ...
            if (backup) {
              try {
                await mutationRepository.deleteVersion(backup.key);
              } catch (cleanupError) {
                throw new ContentError(
                  "CONTENT_CONFLICT",
                  "Document update failed and version compensation requires retry.",
                  tool,
                  {
                    action: "cleanup",
                    resourceKey: current.key,
      ...
      if (input.atomic)
        fail(
          "CONTENT_CONFLICT",
          "Atomic document rename is unavailable because embedding is an external side effect.",
          tool,
          "document-embed",
        );
      result = await batch(
        tool,
        input.renames.map((item: any) => ({
          key: item.documentKey,
          preflight: async () => {
            await document(item.documentKey, "moderator", false);
          },
          run: async (mutationRepository: ContentRepository) => {
            ...
            const semantics = await generatedSemantics(
              item.name,
              current.content,
              current.key,
              current.scopeKey,
            );
            const renamed = await mutationRepository.updateDocument(
              current.key,
              {
                name: item.name,
                ...semantics,
      ...
      result = await batch(
        ...
        input.moves.map((item: any) => ({
          ...
          preflight: async () => {
            ...
            forbidStructuralChange(source, "document", "move");
            const target = await location(
              item.targetScopeKey,
              item.targetFolderKey,
              "admin",
            );
            if (target && managedFolder(target))
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folders cannot contain moved documents.",
                tool,
                "update",
                target.key,
              );
            if (source.scopeKey !== item.targetScopeKey)
              fail(
                "FOLDER_MOVE_FORBIDDEN",
                "Cross-scope document moves are not supported.",
                tool,
                "update",
                source.key,
              );
          ...
          run: async (mutationRepository: ContentRepository) => {
            ...
            forbidStructuralChange(current, "document", "move");
            const moved = await mutationRepository.updateDocument(current.key, {
              folderKey: item.targetFolderKey,
              updatedAt: now(),
            });
      ...
      if (input.atomic)
        fail(
          "CONTENT_CONFLICT",
          "Atomic copy is unavailable because storage copy cannot be rolled back transactionally.",
          tool,
          "storage",
        );
      result = await batch(
        ...
        input.copies.map((item: any) => ({
          ...
          run: async () => {
            ...
            forbidStructuralChange(source, "document", "copy");
            const target = await location(
              item.targetScopeKey,
              item.targetFolderKey,
              "moderator",
            ...
            if (target && managedFolder(target))
              fail(
                "CONTENT_FORBIDDEN",
                "Managed folders cannot contain copied documents.",
                tool,
                "copy",
                target.key,
              );
            ...
            try {
              if (source.storageKey && source.extension)
                storageKey = await copyObject(
                  source.storageKey,
                  `${base}/original.${source.extension}`,
                  source.mimeType,
                );
              for (
                let index = 0;
                index < (source.sourceStorageKeys?.length ?? 0);
                index += 1
              ) {
              ...
              for (
                let index = 0;
                index < (source.speechStorageKeys?.length ?? 0);
                index += 1
              ) {
              ...
              if (item.includeVersions) {
                const versions = (
                  await repo.listVersions(source.scopeKey, [source.key])
              ...
              return { document: documentView(copy) };
            } catch (error) {
              try {
                for (const versionKey of insertedVersionKeys.reverse())
                  await repo.deleteVersion(versionKey);
                if (insertedDocument) await repo.deleteDocument(key);
                for (const copiedStorageKey of copiedStorageKeys.reverse())
                  await storageOperation(
                    "delete",
                    key,
                    item.targetScopeKey,
                    () => d.storage.delete(copiedStorageKey),
                  );
              } catch (cleanupError) {
                throw new ContentError(
                  "CONTENT_CONFLICT",
                  "Document copy failed and compensation requires retry.",
                  tool,
                  {
                    action: "cleanup",
                    resourceKey: key,
      ...
      if (input.atomic)
        fail(
          "CONTENT_CONFLICT",
          "Atomic deletion is unavailable because storage deletion cannot be rolled back.",
          tool,
          "storage",
        );
      result = await batch(
        ...
        input.documentKeys.map((key: string) => ({
          key,
          preflight: async () => {
            const generatedByPrincipal = linkedBindings.some(
              (binding) =>
                binding.documentKey === key &&
                binding.createdByKey === member.user.key,
            );
            const current = await document(
              key,
              generatedByPrincipal ? "moderator" : "owner",
              true,
            );
            forbidManagedDocumentMutation(current, "delete");
            forbidStructuralChange(current, "document", "delete");
            if (current.isFavorite)
              fail(
                "CONTENT_CONFLICT",
                "Unfavorite the document before deleting it.",
                tool,
                "delete",
                key,
              );
          ...
          run: async () => {
            if (!repo.transaction)
              fail(
                "CONTENT_CONFLICT",
                "Transaction-bound metadata deletion is unavailable.",
                tool,
                "transaction",
                key,
              );
            ...
            let current = await repo.transaction(async (bound) => {
              ...
              forbidStructuralChange(candidate, "document", "delete");
              if (candidate._internalDeletion) {
                if (candidate._internalDeletion.kind !== "document")
                  fail(
                    "CONTENT_CONFLICT",
                    "A different deletion is already pending.",
                    tool,
            ...
            try {
              ...
              const audioVersions = repo.listAudioVersions
                ? await repo.listAudioVersions(current.scopeKey, [key])
                : [];
              const summaries = repo.listSummaries
                ? await repo.listSummaries(current.scopeKey, [key])
                : [];
              const summaryAudio = repo.listSummaryAudio
                ? await repo.listSummaryAudio(
                    current.scopeKey,
                    summaries.map((summary) => summary.key),
                  )
                : [];
              if (audioVersions.length > 0 && !repo.deleteAudioVersion)
                fail(
                  "CONTENT_CONFLICT",
                  "Document audio deletion is unavailable.",
                  tool,
              ...
              if (summaries.length > 0 && !repo.deleteSummary)
                fail(
                  "CONTENT_CONFLICT",
                  "Document summary deletion is unavailable.",
                  tool,
              ...
              if (summaryAudio.length > 0 && !repo.deleteSummaryAudio)
                fail(
                  "CONTENT_CONFLICT",
                  "Document summary audio deletion is unavailable.",
                  tool,
              ...
              const inventoriedKeys = [
                ...new Set(
                  [
                    current.storageKey,
                    ...(current.speechStorageKeys ?? []),
                    ...(current.sourceStorageKeys ?? []),
                    ...audioVersions.map((audio) => audio.storageKey),
                    ...summaryAudio.map((audio) => audio.storageKey),
                  ].filter((item): item is string => Boolean(item)),
                ),
              ];
              const deletion = current._internalDeletion?.objectKeys
                ? current._internalDeletion
                : {
                    ...current._internalDeletion!,
                    objectKeys: inventoriedKeys,
                  };
              if (!current._internalDeletion?.objectKeys) {
                const persisted = await repo.transaction((bound) =>
                  bound.setDocumentDeletion(
                    key,
                    deletion,
                    current._internalDeletion!.owner,
                  ),
                );
                if (!persisted)
                  fail(
              ...
              if (ownsFreeze && !storageStarted)
                await repo
                  .transaction((bound) =>
                    bound.setDocumentDeletion(key, undefined, invocationKey),
                  )
                  .catch(() => undefined);
              throw error;
            }
      ...
      const byteBudget = Math.min(
        Math.max(dependencies.maxDownloadBytes ?? 25_000_000, 1),
        100_000_000,
      );
        ...
        run: async () => {
          ...
          if (item.format === "original") {
            if (!current.storageKey || !current.extension || !current.mimeType)
              fail(
                "CONTENT_NOT_FOUND",
                "This document has no imported original to download.",
                tool,
                "storage",
                current.key,
              );
            const object = await storageOperation(
              ...
              current.key,
              current.scopeKey,
              () => d.storage.download(current.storageKey!),
            );
            downloadedBytes += object.bytes.byteLength;
            if (downloadedBytes > byteBudget)
              fail(
                "DOCUMENT_TOO_LARGE",
                "Combined download byte budget exceeded.",
                tool,
                "storage",
                current.key,
              );
          ...
          if (item.format === "html") {
            if (
              !current.storageKey ||
              !current.extension ||
              current.extension === "pdf"
            )
              fail(
                "CONTENT_NOT_FOUND",
                "This document has no supported original preview.",
                tool,
                "storage",
                current.key,
              );
            ...
            if (object.bytes.byteLength > byteBudget - downloadedBytes)
              fail(
                "DOCUMENT_TOO_LARGE",
                "Combined download byte budget exceeded.",
                tool,
            ...
            const preview = await (
              dependencies.generatePreview ??
              (await import("@/lib/ai/document-processing/preview"))
                .generateDocumentPreview
            )({ extension: current.extension, bytes: object.bytes });
            downloadedBytes += preview.bytes.byteLength;
            if (downloadedBytes > byteBudget)
              fail(
                "DOCUMENT_TOO_LARGE",
                "Combined preview byte budget exceeded.",
                tool,
                "storage",
                current.key,
              );
          ...
          const exported = await d.generateExport({
            format: item.format,
            content: publicDocumentContent(current),
          });
          ...
          if (downloadedBytes > byteBudget)
            fail(
              "DOCUMENT_TOO_LARGE",
              "Combined export byte budget exceeded.",
              tool,
              "export",
              current.key,
            );
        ...
        for (const operation of fileOperations) {
          try {
            results.push({
              key: operation.key,
              success: true,
              data: await operation.run(),
            });
          } catch (error) {
            throw mappedError(error, tool, "export", operation.key);
          }
        }
        result = {
          results,
      ...
      result = await batch(
        ...
        input.documentKeys.map((key: string) => ({
          key,
          preflight: async () => {
            forbidManagedDocumentMutation(
              await document(key, "moderator", false),
              "create-version",
            );
          },
          run: async (mutationRepository: ContentRepository) => {
            ...
            const version = await mutationRepository.createVersion({
              ...
              content,
              ...(await currentVersionSemantics(
                { ...current, content },
                current.name,
              )),
            });
            if (content === current.content)
              await mutationRepository.updateDocument(current.key, {
      ...
      result = await batch(
        tool,
        input.versionKeys.map((key: string) => ({
          key,
          run: async () => {
      ...
      if (!repo.getAudioVersion || !repo.updateAudioPlayback)
        fail(
          "CONTENT_CONFLICT",
          "Document audio playback state is unavailable.",
          tool,
          "update",
          input.audioVersionKey,
        );
      const audio = await repo.getAudioVersion(input.audioVersionKey);
      if (!audio)
        fail(
          "CONTENT_NOT_FOUND",
          "Audio version was not found.",
          tool,
          "update",
          input.audioVersionKey,
        );
      ...
      if (input.playbackPositionMs > audio.durationMs)
        fail(
          "CONTENT_INVALID_INPUT",
          "Playback position cannot exceed audio duration.",
          tool,
          "update",
          input.audioVersionKey,
        );
      const updated = await repo.updateAudioPlayback(
        audio.scopeKey,
        audio.key,
      ...
      if (!updated)
        fail(
          "CONTENT_CONFLICT",
          "Audio playback state could not be updated.",
          tool,
          "update",
          input.audioVersionKey,
        );
      result = {
        audioVersionKey: updated.key,
        documentKey: updated.documentKey,
        playbackPositionMs: updated.playbackPositionMs,
      };
      ...
      if (!repo.clearCurrentAudioVersion)
        fail(
          "CONTENT_CONFLICT",
          "Document audio playback state is unavailable.",
          tool,
      ...
      result = await batch(
        ...
        input.documentKeys.map((key: string) => ({
          ...
          run: async () => {
            ...
            if (!repo.listAudioVersions)
              fail(
                "CONTENT_CONFLICT",
                "Document audio history is unavailable.",
                tool,
                "read",
                key,
              );
            ...
            try {
              ...
              throw new ContentError(
                ...
                "Audio version history could not be loaded.",
                tool,
                {
                  action: "audio-history",
                  resourceKey: key,
                  cause: error,
                  retryable: true,
                },
            ...
            return {
              ...
              audioVersions: await Promise.all(
                versions
                  .slice(offset, offset + limit)
                  .map((version) =>
                    audioVersionView(version, current, d.signAudioUrl),
                  ),
              ),
              ...(offset + limit < versions.length
                ? {
                    cursor: Buffer.from(String(offset + limit)).toString(
                      "base64url",
                    ),
                  }
                : {}),
        ...
        input.documentKeys.map((key: string) => ({
          ...
          run: async () => {
            ...
            return {
              ...
              versions: versions
                .slice(offset, offset + limit)
                .map((version) => versionView(version)),
              ...(offset + limit < versions.length
                ? {
                    cursor: Buffer.from(String(offset + limit)).toString(
                      "base64url",
        ...
        input.restores.map((item: any) => ({
          ...
          preflight: async () => {
            ...
            const version = rawVersion
              ? documentVersionSchema.safeParse(rawVersion)
              : null;
            if (
              !version?.success ||
              version.data.documentKey !== current.key ||
              version.data.scopeKey !== current.scopeKey
            ) {
              fail(
                "DOCUMENT_VERSION_CONFLICT",
                "A complete version belonging to the document is required.",
                tool,
                "read",
                item.versionKey,
              );
          ...
          run: async (mutationRepository: ContentRepository) => {
            ...
            const version = documentVersionSchema.parse(
              await mutationRepository.getVersion(item.versionKey),
            );
            let backup: DocumentVersion | undefined;
            ...
            try {
              const restored = await mutationRepository.updateDocument(
                current.key,
                {
                  content: version.content,
                  currentVersionKey: version.key,
              ...
              if (backup && !input.atomic)
                await mutationRepository
                  .deleteVersion(backup.key)
                  .catch(() => undefined);
              throw error;
            }
          },
        ...
        input.versionKeys.map((key: string) => ({
          ...
          preflight: async () => {
            ...
            if (current._internalDeletion)
              fail(
                "CONTENT_CONFLICT",
                "Document deletion is already pending.",
                tool,
                "delete",
                key,
              );
          ...
          run: async (mutationRepository: ContentRepository) => {
            ...
            if (input.atomic)
              await mutationRepository.deleteVersion(selected.key);
            else {
              if (!repo.transaction)
                fail(
                  "CONTENT_CONFLICT",
                  "Transaction-bound version deletion is unavailable.",
                  tool,
                  "transaction",
                  key,
                );
              await repo.transaction((bound) =>
                bound.deleteVersion(selected.key),
              );
            }
            return {};
          },
        ...
        input.documentKeys.map((key: string) => ({
          ...
          run: async () => {
            ...
            if (!repo.listSummaries)
              fail(
                "CONTENT_CONFLICT",
                "Document summary history is unavailable.",
                tool,
                "read",
                key,
              );
            ...
            const audio = repo.listSummaryAudio
              ? await repo.listSummaryAudio(
                  current.scopeKey,
                  page.map((summary) => summary.key),
                )
              : [];
            const audioBySummary = new Map(
              audio.map((item) => [item.summaryKey, item]),
            );
            return {
              documentKey: key,
              summaries: await Promise.all(
                page.map((summary) =>
                  projectedSummaryView(
                    summary,
                    audioBySummary.get(summary.key),
                    d.signAudioUrl,
                  ),
                ),
              ),
      ...
      result = await batch(
        tool,
        input.summaryKeys.map((key: string) => ({
          key,
          run: async () => {
            if (!repo.getSummary)
              fail(
            ...
            if (current.scopeKey !== summary.scopeKey)
              fail(
                "CONTENT_NOT_FOUND",
                "Summary was not found.",
                tool,
            ...
            const audio = repo.getSummaryAudio
              ? await repo.getSummaryAudio(summary.key)
              : null;
            return {
              summary: await projectedSummaryView(
                summary,
                audio ?? undefined,
                d.signAudioUrl,
              ),
      ...
      const current = await document(input.documentKey, "viewer", false);
      if (managedDocument(current))
        forbidManagedDocumentMutation(current, "generate-topics");
      const generated = await action(
        "text.topics",
        {
          systemPrompt:
            'Identify the document\'s distinct primary topics. Return strict JSON only in the form {"topics":["topic"]}. Include no more than 10 concise topic strings, with no duplicates or commentary.',
          messages: [
            {
              role: "user",
              content: [
      ...
      try {
        ...
        parsed = z
          .object({
            topics: z.array(z.string().trim().min(1).max(200)).max(10),
          })
          .strict()
          .parse(JSON.parse(fenced?.[1]?.trim() ?? text));
      } catch (error) {
        fail(
          "CONTENT_INVALID_INPUT",
          "The topic model returned invalid JSON.",
          tool,
          "text.topics",
          current.key,
          error,
      ...
      result = {
        documentKey: current.key,
        topics: [...new Set(parsed.topics)].slice(0, 10),
      };
    } else if (tool === "document.summarize") {
      if (input.atomic && input.persist)
        fail(
          "CONTENT_CONFLICT",
          "Atomic persisted summaries are unavailable because generation is an external side effect.",
          tool,
          "text.summarize",
        );
      ...
      for (const key of input.documentKeys)
        sources.push(
          await document(key, input.persist ? "moderator" : "viewer", false),
        );
      for (const source of sources)
        if (managedDocument(source))
          forbidManagedDocumentMutation(source, "generate-summary");
              ...
              .parse(
                (
                  await action(
                    ...
                    sourceDocuments[0]!.key,
                    sourceDocuments[0]!.scopeKey,
                  )
                ).text,
              ),
        );
      if (input.combine) {
        ...
        result = {
          results: sources.map((source) => ({
            key: source.key,
            success: true,
            data: { documentKey: source.key, text },
          })),
          summary: {
        ...
        result = await batch(
          ...
          sources.map((current) => ({
            ...
            run: async () => {
              ...
              if (!repo.createSummary)
                fail(
                  "CONTENT_CONFLICT",
                  "Document summary persistence is unavailable.",
                  tool,
                  "create",
                  current.key,
                );
              const summary = await repo.createSummary({
                key: d.id(),
                scopeKey: current.scopeKey,
                documentKey: current.key,
                summary: text,
                ...
                createdByKey: member.userTeam.key,
                createdAt: now(),
              });
              const audio = repo.getSummaryAudio
                ? await repo.getSummaryAudio(summary.key)
      ...
      tool === "document.enhance" ||
      tool === "document.translate" ||
      tool === "document.rewrite"
    ) {
      const items =
        tool === "document.rewrite"
          ? input.rewrites
      ...
      if (input.atomic && items.some((item: any) => item.mode !== "preview"))
        fail(
          "CONTENT_CONFLICT",
          "Atomic persisted AI transformations are unavailable because generation and storage cannot be rolled back.",
          tool,
          "text.generate",
        );
      ...
      result = await batch(
        ...
        items.map((item: any) => ({
          ...
          run: async () => {
            const current = await document(
              item.documentKey,
              item.mode === "preview" ? "viewer" : "moderator",
              false,
            );
            ...
            const text =
              tool === "document.enhance"
                ...
                : tool === "document.translate"
                  ? await generateDocumentTranslation(
                      {
                        content: current.content,
      ...
      if (
        !(await activeFolderHierarchy(
          input.folderKey ? source.key : (source as Document).folderKey,
          source.scopeKey,
        ))
      ) {
        fail(
          "CONTENT_NOT_FOUND",
          "The source folder hierarchy was not found.",
          tool,
          "read",
          source.key,
        );
      }
      const parsedEmbedding = z
        .array(z.number().finite())
        .length(EMBEDDING_DIMENSIONS)
        .safeParse(source.embedding);
      if (!parsedEmbedding.success)
        fail(
          "CONTENT_CONFLICT",
          "Semantic similarity is not ready for this resource.",
          tool,
          "semantic-neighbors",
          source.key,
        );
      if (!repo.semanticNeighbors)
        fail(
          "CONTENT_CONFLICT",
          "Semantic similarity is unavailable.",
          tool,
          "semantic-neighbors",
          source.key,
        );
      ...
      const foldersByKey = new Map(
        allFolders.map((current) => [current.key, current]),
      );
      ...
      const activeFolderKeys = allFolders
        .filter((current) => hierarchyIsActive(current.key))
      ...
      const matches = await repo.semanticNeighbors({
        ...
        activeFolderKeys,
        ...(input.folderKey
          ? { sourceFolderKey: input.folderKey }
      ...
      const folders = matches.folders
        .filter(
          ({ folder: current }) =>
            ...
            current.key !== input.folderKey &&
            activeFolderKeys.includes(current.key) &&
            !current._internalDeletion,
        )
        .slice(0, 10);
      const activeDocument = (current: Document) =>
        archiveVisible(current) &&
        current.scopeKey === source.scopeKey &&
        current.key !== input.documentKey &&
        !current._internalDeletion &&
        (!current.folderKey || activeFolderKeys.includes(current.folderKey));
      const documents = matches.documents
        .filter(
          ({ document: current }) =>
            activeDocument(current) && !current.extension,
        )
        .slice(0, 10);
      const files = matches.files
        .filter(
          ({ document: current }) =>
            activeDocument(current) && Boolean(current.extension),
        )
        .slice(0, 10);
      result = {
        folders: await Promise.all(
          folders.map(({ folder: current }) => folderView(current, d)),
        ),
        documents: documents.map(({ document: current }) =>
          documentView(current),
        ),
      ...
      await roleFor(input.scopeKey, "viewer");
      const history =
        dependencies.userSearches ??
        (
          await import("@/lib/user-searches/service")
        ).getDefaultUserSearchService();
      ...
      await roleFor(input.scopeKey, "viewer");
      const allowed = await repo.allowedScopeKeys(
        context.teamKey,
        member.userTeam.key,
      );
      if (!allowed.includes(input.scopeKey))
        fail(
          "CONTENT_FORBIDDEN",
          "The principal lacks access to the requested scope.",
          tool,
          "authorization",
          input.scopeKey,
        );
      ...
      const includeDescendants =
        folderKey !== null && (input.includeDescendants ?? true);
      const store =
        dependencies.searchQueries ??
        (await import("@/lib/db/content-search-queries.node"))
      ...
      const recordSearch = async (output: unknown) => {
        await Promise.allSettled([
          ...
          ...(input.recordHistory
            ? [history.record(member.user.key, input.query)]
            : []),
        ]);
      };
      ...
      const folderDescriptors = new Map(
        allFolders.map((current) => [
          current.key,
          { key: current.key, name: current.name },
        ]),
      );
      ...
      if (folderKey) {
        ...
        folderKeys = [
          folderKey,
          ...(includeDescendants
            ? descendants(allFolders, folderKey).map((item) => item.key)
            : []),
        ];
        const relevant = new Set(folderKeys);
        let ancestorKey = current.parentFolderKey;
        while (ancestorKey && !relevant.has(ancestorKey)) {
          relevant.add(ancestorKey);
          ancestorKey = allFolders.find(
            (item) => item.key === ancestorKey,
          )?.parentFolderKey;
        }
        revisionFolders = allFolders.filter((item) => relevant.has(item.key));
      }
      const revisionDocuments = folderKeys
        ? allDocuments.filter(
            (item) => item.folderKey && folderKeys!.includes(item.folderKey),
          )
        : allDocuments;
      const folderRevision = revisionFolders.map(
        (item) =>
          `${item.key}:${item.name}:${item.parentFolderKey ?? ""}:${item.updatedAt}:${item.isFavorite ? "favorite" : ""}:${item._internalDeletion ? "pending" : ""}`,
      );
      const documentRevision = revisionDocuments.map(
        (item) =>
          `${item.key}:${item.name}:${item.folderKey ?? ""}:${item.semanticContentHash ?? ""}:${item.isFavorite ? "favorite" : ""}:${item._internalDeletion ? "pending" : ""}`,
      );
      const sourceRevision = createHash("sha256")
        .update([...folderRevision, ...documentRevision].sort().join("\n"))
        .digest("hex");
      const cached = await store
        ...
        .catch(() => null);
      const cachedValue = cached?.output as
        | {
            result?: unknown;
      ...
      if (reusable) {
        const parsed = contentToolOutputSchemas[tool].parse({
          ...(cachedValue!.result as object),
          query: input.query,
          cached: true,
        });
        ...
        const queryTokens = new Set<string>(
          queryText.match(/[\p{L}\p{N}]+/gu) ?? [],
        );
        const queryTokenText = [...queryText.matchAll(/[\p{L}\p{N}]+/gu)]
          .map(([token]) => token)
          .join(" ");
        ...
        const scoreText = (value: string) => {
          ...
          return matched > 0
            ? 0.4 + 0.3 * (matched / Math.max(queryTokens.size, 1))
            : 0;
        };
        const activeFolders = [];
        for (const current of allFolders) {
          ...
          if (
            !archiveVisible(current) ||
            current._internalDeletion ||
            !(await activeFolderHierarchy(current.key, current.scopeKey))
          )
            continue;
          const exactNameMatch = nameMentioned(current.name);
          const score = Math.max(
            scoreText(`${current.name}\n${current.description ?? ""}`),
            exactNameMatch ? 0.75 : 0,
          );
          if (score >= input.minimumScore)
            activeFolders.push({ folder: current, score, exactNameMatch });
        }
        ...
        for (const current of allDocuments) {
          ...
          if (
            !archiveVisible(current) ||
            current._internalDeletion ||
            !(await activeFolderHierarchy(current.folderKey, current.scopeKey))
          )
            continue;
          const exactNameMatch = nameMentioned(current.name);
          const score = Math.max(
          ...
          if (score >= input.minimumScore)
            activeDocuments.push({ document: current, score, exactNameMatch });
        }
        const hasExactMatch =
          activeFolders.some(({ score }) => score >= 0.75) ||
          activeDocuments.some(({ score }) => score >= 0.75);
        if (!hasExactMatch || input.minimumScore < 0) {
          let embeddingTimer: number | undefined;
          const queryEmbedding =
            dependencies.queryEmbedding ??
            (await Promise.race<number[] | undefined>([
              embed(input.query, undefined, input.scopeKey, "query").catch(
                () => undefined,
              ),
              new Promise<undefined>((resolve) => {
                embeddingTimer = setTimeout(
                  resolve,
                  dependencies.searchEmbeddingTimeoutMs ?? 500,
                );
              }),
            ]));
          ...
          if (queryEmbedding) {
            ...
            for (const match of folderMatches) {
              if (
                ...
                match.folder.scopeKey !== input.scopeKey ||
                (folderKeys && !folderKeys.includes(match.folder.key)) ||
                match.folder._internalDeletion ||
                !(await activeFolderHierarchy(
                  match.folder.key,
                  match.folder.scopeKey,
                ))
              )
                continue;
              const previous = activeFolders.find(
                ({ folder: current }) => current.key === match.folder.key,
              );
              if (previous)
                previous.score = Math.max(previous.score, match.score);
              else
                activeFolders.push({
                  ...match,
                  exactNameMatch: nameMentioned(match.folder.name),
                });
            }
            for (const match of documentMatches) {
              if (
                ...
                match.document.scopeKey !== input.scopeKey ||
                (folderKeys &&
                  (!match.document.folderKey ||
                    !folderKeys.includes(match.document.folderKey))) ||
                match.document._internalDeletion ||
                !(await activeFolderHierarchy(
                  match.document.folderKey,
                  match.document.scopeKey,
                ))
              )
                continue;
              const previous = activeDocuments.find(
                ({ document: current }) => current.key === match.document.key,
              );
              if (previous)
                previous.score = Math.max(previous.score, match.score);
              else
                activeDocuments.push({
                  ...match,
                  exactNameMatch: nameMentioned(match.document.name),
                });
            }
          }
        }
        const folders = activeFolders
          .sort(
            (left, right) =>
              ...
              left.folder.key.localeCompare(right.folder.key),
          )
          .slice(
            0,
            input.minimumScore < 0 ? input.limit : Math.min(4, input.limit),
          )
          .map(({ folder: current, score }) => ({
            key: current.key,
            scopeKey: current.scopeKey,
            ...(current.parentFolderKey
              ? { parentFolderKey: current.parentFolderKey }
              : {}),
            name: current.name,
            ...(current.description
        ...
        const documents = activeDocuments
          ...
          .slice(
            0,
            input.minimumScore < 0 ? input.limit : Math.min(10, input.limit),
          )
          .map(({ document: current, score }) => ({
            documentKey: current.key,
            scopeKey: current.scopeKey,
            ...(current.folderKey
              ? {
                  folderKey: current.folderKey,
                  folder: folderDescriptors.get(current.folderKey),
                }
              : {}),
            name: current.name,
        ...
        try {
          queryEmbedding =
            dependencies.queryEmbedding ??
            (await embed(input.query, undefined, input.scopeKey, "query"));
        } catch (error) {
          fail(
        ...
        for (const match of folderMatches) {
          if (
            ...
            continue;
          folders.push({
            key: match.folder.key,
            scopeKey: match.folder.scopeKey,
            ...(match.folder.parentFolderKey
              ? { parentFolderKey: match.folder.parentFolderKey }
              : {}),
            name: match.folder.name,
            ...(match.folder.description
              ? { description: match.folder.description }
              : {}),
        ...
        for (const match of documentMatches) {
          if (
            ...
            match.document.scopeKey === input.scopeKey &&
            (!folderKeys ||
              (match.document.folderKey !== undefined &&
                folderKeys.includes(match.document.folderKey))) &&
            !match.document._internalDeletion &&
            (await activeFolderHierarchy(
              match.document.folderKey,
        ...
        for (let start = 0; start < selectedDocuments.length; start += 3) {
          const batch = await Promise.all(
            selectedDocuments
              .slice(start, start + 3)
              .map(async ({ document: current, matchedContent, score }) => {
                let summary: string;
                let complete = true;
                try {
                  const generatedSummary = await action(
                    "text.search-summary",
                    {
                      systemPrompt:
                        "Summarize only how the supplied document relates to the search query. Use only the supplied text and return only the concise summary without commentary.",
                      messages: [
                        {
                          role: "user",
                          content: [
        ...
        await recordSearch({
          ...
          createdFrom: input.createdFrom,
          createdTo: input.createdTo,
          replayable: summariesComplete,
        });
        result = freshResult;
      }
    } else {
      const teamSearch = tool === "document.search-all";
      if (
        teamSearch &&
        input.teamKey !== context.teamKey
      )
        fail(
          "CONTENT_FORBIDDEN",
          "Team key does not match the execution context.",
          tool,
          "authorization",
        );
      const allowed = await repo.allowedScopeKeys(
      ...
      if (!teamSearch) await roleFor(input.scopeKey, "viewer");

      const filterScopeKeys = new Set<string>(
        input.filters?.scopeKeys ?? allowed,
      );
      ...
      const sourceInputs = input.sources ?? [
        {
          type: "scope",
          scopeKeys: teamSearch ? allowed : [input.scopeKey],
        },
      ];
      ...
      for (const source of sourceInputs) {
        if (source.type === "scope") {
          for (const key of source.scopeKeys)
            resolvedSources.push({ type: "scope", key, scopeKeys: [key] });
        } else {
          for (const key of source.folderKeys) {
            const current = await folder(key, "viewer");
            if (!(await activeFolderHierarchy(key, current.scopeKey)))
              fail(
                "CONTENT_NOT_FOUND",
                "Search folder hierarchy was not found.",
                tool,
                "resolution",
                key,
              );
            const children = source.includeDescendants
              ? descendants(await foldersIn(current.scopeKey), key)
              : [];
            const folderKeys = [key, ...children.map((item) => item.key)];
            resolvedSources.push({
              type: "folder",
              key,
              scopeKeys: [current.scopeKey],
              folderKeys,
            });
          }
        }
      ...
      const queryTokens = [
        ...new Set<string>(queryText.match(/[\p{L}\p{N}]+/gu) ?? []),
      ];
      ...
      const lexicalMatches = async (
        ...
        return documents.flatMap((document) => {
          if (!archiveVisible(document) || document._internalDeletion)
            return [];
          if (
            folderKeys &&
            (!document.folderKey || !folderKeys.includes(document.folderKey))
          )
            return [];
          if (
            input.filters?.documentKeys &&
            !input.filters.documentKeys.includes(document.key)
          )
            return [];
          if (
            input.filters?.extensions &&
            (!document.extension ||
              !input.filters.extensions.includes(document.extension))
          )
            return [];
          if (
            input.filters?.createdAfter &&
            document.createdAt < input.filters.createdAfter
          )
            return [];
          if (
            input.filters?.createdBefore &&
            document.createdAt > input.filters.createdBefore
          )
            return [];
          if (
            input.filters?.updatedAfter &&
            document.updatedAt < input.filters.updatedAfter
          )
            return [];
          if (
            input.filters?.updatedBefore &&
            document.updatedAt > input.filters.updatedBefore
          )
            return [];
          const searchable =
            `${document.name}\n${document.content}`.toLocaleLowerCase();
          const documentTokens = new Set(
            searchable.match(/[\p{L}\p{N}]+/gu) ?? [],
          );
          ...
          const score = exactMatch
            ? 0.75
            : 0.4 +
              0.3 * (matchedTokens.length / Math.max(queryTokens.length, 1));
          if (input.minimumScore !== undefined && score < input.minimumScore)
            return [];
          const contentText = document.content.toLocaleLowerCase();
          const contentMatchAt = contentText.indexOf(
            exactMatch ? queryText : matchedTokens[0]!,
          );
          ...
          return [
            {
              score,
              document,
              matchedContent: document.content.slice(matchAt, matchAt + 300),
            },
          ];
        });
      };
      const candidates = new Map<
        string,
        {
          ...
          source: { type: "scope" | "folder"; key: string };
        }
      >();
      const searchableSources: Array<{
        source: (typeof resolvedSources)[number];
        scopeKeys: string[];
        folderKeys?: string[];
      }> = [];
      for (const source of resolvedSources) {
        const scopeKeys = source.scopeKeys.filter(
          (key) => allowed.includes(key) && filterScopeKeys.has(key),
        );
        if (scopeKeys.length === 0) continue;
        let folderKeys = source.folderKeys;
        if (filterFolderKeys)
          folderKeys = folderKeys
            ? folderKeys.filter((key) => filterFolderKeys.includes(key))
            : filterFolderKeys;
      ...
      const collectMatches = async (
        ...
        for (const match of matches) {
          ...
          if (!previous || match.score > previous.score)
            candidates.set(match.document.key, {
              ...match,
              source: { type: source.type, key: source.key },
            });
        }
      };
      ...
      if (!exactFolderMatches) {
        ...
        const embedding = await Promise.race<number[] | undefined>([
          embed(input.query, undefined, undefined, "query").catch(
            () => undefined,
          ),
          new Promise<undefined>((resolve) => {
            embeddingTimer = setTimeout(
      ...
      if (
        ...
        (candidates.size === 0 &&
          !resolvedSources.some((source) =>
            source.scopeKeys.some((key) => allowed.includes(key)),
          ))
      )
        fail(
          "CONTENT_SEARCH_NO_ACCESSIBLE_SOURCES",
          "No accessible Content search source remains.",
          tool,
          "authorization",
        );
      ...
      result = {
        ...
        results: await Promise.all(
          selected.map(
            async ({ score, document: current, matchedContent, source }) => {
              const normalizedScore = Math.max(0, Math.min(1, score));
              const parent =
                input.include?.includes("folder") && current.folderKey
                  ? await repo.getFolder(current.folderKey)
                  : undefined;
              return {
                documentKey: current.key,
                ...
                ...(input.include?.includes("snippet")
                  ? {
                      snippet: (matchedContent ?? current.content).slice(
                        0,
                        300,
                      ),
                    }
                  : {}),
                ...(input.include?.includes("content")
                  ? { content: current.content }
                  : {}),
                ...(parent ? { folder: await folderView(parent, d) } : {}),
                ...(input.include?.includes("scope")
                  ? { scope: { key: current.scopeKey } }
                  : {}),
                ...(input.include?.includes("scoreBreakdown")
                  ? {
                      scoreBreakdown: {
    ...
    if (idempotencyIdentity && requestHash && ownsIdempotencyClaim)
      await d.idempotency.complete(
        idempotencyIdentity,
        requestHash,
        invocationKey,
        parsed,
    ...
    if (tool === "folder.delete" && changedTripScopeKeys.size > 0) {
      const publishTripChange =
        dependencies.publishTripChange ??
        (!dependencies.repository
          ? async (scopeKey: string) =>
              (await import("@/api/events")).publishScopeEvent(
                scopeKey,
                "trip.changed",
              )
          : undefined);
      if (publishTripChange)
        await Promise.all(
          [...changedTripScopeKeys].map((scopeKey) =>
            publishTripChange(scopeKey).catch(() => undefined),
          ),
        );
    }
    if (
      ...
      "summary" in parsed &&
      parsed.summary.succeeded > 0
    ) {
      const contentScopes = new Set(
        linkedBindings.map(({ scopeKey }) => scopeKey),
      );
      const tripScopes = new Set(
        linkedBindings
          .filter(({ subjectType }) => subjectType === "trip")
          .map(({ scopeKey }) => scopeKey),
      );
      const placeScopes = new Set(
        linkedBindings
          .filter(({ subjectType }) => subjectType === "place")
          .map(({ scopeKey }) => scopeKey),
      );
      const publishContentChange =
        dependencies.publishContentChange ??
        (!dependencies.repository
          ? async (scopeKey: string) =>
              (await import("@/api/events")).publishScopeEvent(
                scopeKey,
      ...
      const publishTripChange =
        ...
        (!dependencies.repository
          ...
          : undefined);
      const publishPlaceReferenceChange =
        dependencies.publishPlaceReferenceChange ??
        (!dependencies.repository
          ? async (scopeKey: string) =>
              (await import("@/api/events")).publishScopeEvent(
                scopeKey,
          ...
          : undefined);
      await Promise.all([
        ...[...contentScopes].map((scopeKey) =>
          publishContentChange?.(scopeKey).catch(() => undefined),
        ),
        ...[...tripScopes].map((scopeKey) =>
          publishTripChange?.(scopeKey).catch(() => undefined),
        ),
        ...[...placeScopes].map((scopeKey) =>
          publishPlaceReferenceChange?.(scopeKey).catch(() => undefined),
        ),
      ]);
    }
    await event(
    ...
    if (
      idempotencyIdentity &&
      requestHash &&
      idempotencyExecutionStarted &&
      !executionSucceeded
    ) {
      const terminalized = await d.idempotency
        .fail(
          idempotencyIdentity,
          requestHash,
          invocationKey,
      ...
      if (terminalized)
        terminalError = new ContentError(
          "CONTENT_IDEMPOTENCY_FAILED",
          mapped.message,
          tool,
          { action: "idempotency", retryable: mapped.retryable },
        );
    }
    await event(
      "action",
