...
export const contentToolContracts = {
  ...
  'scope.content.search-history': { description: 'List the current user\'s search history in a scope.', input: z.object({ scopeKey: keySchema, folderKey: keySchema.optional(), includeDescendants: z.boolean().optional(), allLocations: z.boolean().default(false), limit: z.number().int().min(1).max(100).default(20) }).strict().superRefine((value, context) => { if (!value.folderKey && value.includeDescendants !== undefined) context.addIssue({ code: z.ZodIssueCode.custom, path: ['includeDescendants'], message: 'includeDescendants requires folderKey' }); if (value.allLocations && value.folderKey) context.addIssue({ code: z.ZodIssueCode.custom, path: ['allLocations'], message: 'allLocations cannot be combined with folderKey' }); }), output: z.object({ history: z.array(contentSearchHistoryItemSchema) }).strict() },
  'scope.content.search-history.delete': { description: 'Delete one current-user Content search-history entry.', input: z.object({ scopeKey: keySchema, normalizedQuery: textSchema.max(8_000), folderKey: keySchema.optional(), includeDescendants: z.boolean().optional(), allLocations: z.boolean().default(false), ...idempotencyShape }).strict().superRefine((value, context) => { if (!value.folderKey && value.includeDescendants !== undefined) context.addIssue({ code: z.ZodIssueCode.custom, path: ['includeDescendants'], message: 'includeDescendants requires folderKey' }); if (value.allLocations && value.folderKey) context.addIssue({ code: z.ZodIssueCode.custom, path: ['allLocations'], message: 'allLocations cannot be combined with folderKey' }); }), output: z.object({ normalizedQuery: textSchema, deleted: z.boolean() }).strict() },
