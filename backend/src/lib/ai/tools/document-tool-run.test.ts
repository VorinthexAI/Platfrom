...
import { runDocumentAgentTool } from './document-tool-run';

const now = '2026-07-22T00:00:00.000Z';
function fixture() {
  const organization = organizationSchema.parse({ key: newId(), name: 'Acme', createdAt: now, updatedAt: now });
  const scope = scopeSchema.parse({ key: newId(), organizationKey: organization.key, slug: 'document', name: 'Document', summary: 'Document', description: 'Document', position: 1 });
  ...
  const agent = agentSchema.parse({ key: newId(), slug: 'document-agent', name: 'Document Agent', title: 'Archivist', scopeKey: scope.key });
  const skill = skillSchema.parse({ key: newId(), slug: 'document', name: 'Document', title: 'Document', definition: 'Use Document tools.' });
...
describe('runDocumentAgentTool', () => {
  test('derives the principal from the authenticated user and dispatches with safe events', async () => {
    ...
    const output = await runDocumentAgentTool({ organizationKey: f.organization.key, agentKey: f.agent.key, tool: 'folder.list', input: { scopeKey: f.scope.key } }, {
  ...
  test('rejects organization-agent mismatch before membership resolution', async () => {
    ...
    await expect(runDocumentAgentTool({ organizationKey: newId(), agentKey: f.agent.key, tool: 'folder.list', input: {} }, {
      authenticatedUserKey: f.user.key, runtimeData: f.runtimeData, accessData: f.accessData,
      resolveMembership: async () => { resolved = true; return f.membership; }, events: async () => {},
    })).rejects.toMatchObject({ code: 'DOCUMENT_FORBIDDEN' });
  ...
  test('emits a safe failed event without tool input or error content', async () => {
    ...
    await expect(runDocumentAgentTool({ organizationKey: f.organization.key, agentKey: f.agent.key, tool: 'folder.list', input: { secret: 'do-not-log' } }, {
      authenticatedUserKey: f.user.key, runtimeData: f.runtimeData, accessData: f.accessData,
  ...
  test('cannot use a membership belonging to another authenticated user', async () => {
    const f = fixture();
    await expect(runDocumentAgentTool({ organizationKey: f.organization.key, agentKey: f.agent.key, tool: 'folder.list', input: {} }, {
  ...
  test('strictly rejects unknown tools and top-level fields', async () => {
    ...
    await expect(runDocumentAgentTool({ organizationKey: f.organization.key, agentKey: f.agent.key, tool: 'unknown', input: {} } as any, options)).rejects.toThrow();
    await expect(runDocumentAgentTool({ organizationKey: f.organization.key, agentKey: f.agent.key, tool: 'folder.list', input: {}, principal: { kind: 'system' } } as any, options)).rejects.toThrow();
  });
});
