...
export const imageCreateVisualIdentityTool = {
  ...
  async execute(rawInput: unknown, dependencies: ImageCreateVisualIdentityToolDependencies = {}): Promise<VisualIdentityDescriptionOutput> {
    ...
    const response = dependencies.executeDescription
      ...
      : await executeAction<VisualIdentityDescriptionInput, VisualIdentityDescriptionOutput>({ mode: 'fixed', organizationKey, actionSlug: 'describe-visual-identity', modelSlug: IMAGE_CAPTION_MODEL, providerSlug: 'openrouter' }, input, dependencies);
