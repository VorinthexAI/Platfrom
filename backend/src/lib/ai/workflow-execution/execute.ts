...
import { getDefaultWorkflowExecutionRepository, type WorkflowExecutionRepository } from './repository';
import { workflowExecutionToolInputSchemas, type WorkflowExecutionActionSlug } from './tool-schemas';
...
export type WorkflowExecutionRole = (typeof READ_ROLES)[number];
export interface WorkflowExecutionReasonInput {
...
export interface WorkflowExecutionAuthorizationDependency {
  authorize(scopeKey: string, allowedRoles: readonly WorkflowExecutionRole[]): void | Promise<void>;
}

export interface WorkflowExecutionEmbeddingDependency {
  generateEmbedding?: (text: string) => Promise<readonly number[]>;
}

export interface WorkflowExecutionReasonDependency {
  reason?: (input: WorkflowExecutionReasonInput) => Promise<string>;
}

export interface WorkflowExecutionExecutionDependencies extends WorkflowExecutionAuthorizationDependency, WorkflowExecutionEmbeddingDependency, WorkflowExecutionReasonDependency {
...
export interface WorkflowExecutionItemSuccess<T = unknown> {
  index: number;
  success: true;
  value: T;
}

export interface WorkflowExecutionItemFailure {
  index: number;
  success: false;
  error: { code: string; message: string };
}

export type WorkflowExecutionItemResult<T = unknown> = WorkflowExecutionItemSuccess<T> | WorkflowExecutionItemFailure;

export interface WorkflowExecutionToolResult {
  action: WorkflowExecutionActionSlug;
  results?: WorkflowExecutionItemResult[];
  items?: unknown[];
}

export class WorkflowExecutionExecutionError extends Error {
  constructor(public readonly code: string, message: string) {
    super(message);
    this.name = 'WorkflowExecutionExecutionError';
  }
}

type WorkflowExecutionResourceKeys = Partial<Record<'scopeKey' | 'projectKey' | 'milestoneKey' | 'taskKey' | 'contentFolderKey', string>>;

type Prepared<T> = {
  keys: WorkflowExecutionResourceKeys;
  scopeKeys?: string[];
  run(repository: WorkflowExecutionRepository): Promise<T>;
};

type BatchInput = { items: unknown[]; atomic: boolean };

function failure(index: number, error: unknown): WorkflowExecutionItemFailure {
  return {
    index,
    success: false,
    error: {
      code: error instanceof WorkflowExecutionExecutionError ? error.code : error instanceof z.ZodError ? 'invalid_input' : 'operation_failed',
      message: error instanceof Error ? error.message : 'WorkflowExecution operation failed.',
    },
  };
}

function requireNode<T>(node: T | null, kind: string, key: string): T {
  if (!node) throw new WorkflowExecutionExecutionError(`${kind}_not_found`, `${kind} ${key} was not found.`);
  return node;
}

function requireActive(node: { deletedAt: string | null }, kind: string): void {
  if (node.deletedAt !== null) throw new WorkflowExecutionExecutionError(`${kind}_archived`, `${kind} is archived.`);
}

function requireArchived(node: { deletedAt: string | null }, kind: string): void {
  if (node.deletedAt === null) throw new WorkflowExecutionExecutionError(`${kind}_not_archived`, `${kind} must be archived before permanent deletion.`);
}
...
function validEmbedding(value: readonly number[]): number[] {
  ...
  if (embedding.length === 0 || embedding.some((entry) => !Number.isFinite(entry))) {
    throw new WorkflowExecutionExecutionError('invalid_embedding', 'Embedding must contain non-empty finite numbers.');
  }
  return embedding;
}
...
function safeKeys(item: Record<string, unknown>): WorkflowExecutionResourceKeys {
  const output: WorkflowExecutionResourceKeys = {};
  for (const key of ['scopeKey', 'projectKey', 'milestoneKey', 'taskKey', 'contentFolderKey'] as const) {
    if (typeof item[key] === 'string') output[key] = item[key];
  }
  return output;
}

async function assertTransactionState(action: WorkflowExecutionActionSlug, prepared: Prepared<unknown>, repository: WorkflowExecutionRepository): Promise<void> {
  ...
  if (action === 'milestone.create') {
    ...
    if (project.scopeKey !== prepared.keys.scopeKey) throw new WorkflowExecutionExecutionError('invalid_ownership', 'Project scope changed during mutation preparation.');
  ...
  if (action === 'task.create') {
    ...
    const project = requireNode(await repository.getProject(projectKey!), 'project', projectKey!);
    if (milestone.projectKey !== project.key || milestone.scopeKey !== project.scopeKey || project.scopeKey !== prepared.keys.scopeKey) throw new WorkflowExecutionExecutionError('invalid_ownership', 'Milestone ownership changed during mutation preparation.');
  ...
  if (action.startsWith('milestone.')) {
    ...
    if (action !== 'milestone.archive' && action !== 'milestone.delete') {
      ...
      if (milestone.scopeKey !== project.scopeKey) throw new WorkflowExecutionExecutionError('invalid_ownership', 'Milestone ownership changed during mutation preparation.');
      requireActive(project, 'project');
    }
  ...
  if (action.startsWith('task.')) {
    ...
    if (action !== 'task.archive' && action !== 'task.delete') {
      ...
      if (task.scopeKey !== project.scopeKey || milestone.projectKey !== project.key || milestone.scopeKey !== project.scopeKey) throw new WorkflowExecutionExecutionError('invalid_ownership', 'Task ownership changed during mutation preparation.');
      requireActive(project, 'project'); requireActive(milestone, 'milestone');
    }
    if (action === 'task.move') {
      ...
      if (destinationMilestone.projectKey !== destinationProject.key || destinationMilestone.scopeKey !== destinationProject.scopeKey) throw new WorkflowExecutionExecutionError('invalid_ownership', 'Destination ownership changed during mutation preparation.');
...
async function executeBatch<T>(
...
): Promise<WorkflowExecutionToolResult> {
  if (input.atomic) {
    ...
    if (new Set(targets).size !== targets.length) {
      const error = new WorkflowExecutionExecutionError('atomic_duplicate_target', 'An atomic batch may target each existing resource only once.');
      const results = input.items.map((_, index) => failure(index, error));
      return { action, results };
    }
    const prepared: Prepared<T>[] = [];
    const preparationFailures: WorkflowExecutionItemFailure[] = [];
    ...
    if (preparationFailures.length) {
      ...
      const failed = input.items.map((_, index) => preparationFailures.find((item) => item.index === index) ?? failure(index, new WorkflowExecutionExecutionError('atomic_prevalidation_failed', `Atomic batch was not written because another item failed prevalidation: ${first.message}`)));
    ...
    try {
      const values = await repository.withAtomic(async (transaction) => {
        ...
        if (!await transaction.areScopesActive(scopeKeys)) throw new WorkflowExecutionExecutionError('scope_archived', 'A scope was archived before the transaction committed.');
        const output: T[] = [];
        for (const item of prepared) { await assertTransactionState(action, item, transaction); output.push(await item.run(transaction)); }
        return output;
      });
      const results = values.map((value, index): WorkflowExecutionItemSuccess<T> => ({ index, success: true, value }));
      return { action, results };
    } catch (error) {
      const results = input.items.map((_, index) => failure(index, error));
      return { action, results };
  ...
  const results: WorkflowExecutionItemResult<T>[] = [];
  for (let index = 0; index < input.items.length; index += 1) {
    let outcome: WorkflowExecutionItemResult<T>;
    try {
      ...
      const value = await repository.withAtomic(async (transaction) => {
        ...
        if (!await transaction.areScopesActive(scopeKeys)) throw new WorkflowExecutionExecutionError('scope_archived', 'The scope was archived before the transaction committed.');
...
/** Executes one validated WorkflowExecution action using only repository-level persistence APIs. */
export async function executeWorkflowExecutionTool(
  action: WorkflowExecutionActionSlug,
  rawInput: unknown,
  dependencies: WorkflowExecutionExecutionDependencies,
): Promise<WorkflowExecutionToolResult> {
  const input = workflowExecutionToolInputSchemas[action].parse(rawInput) as any;
  const repository = dependencies.repository ?? getDefaultWorkflowExecutionRepository();
  ...
  const projectContext = async (projectKey: string, roles: readonly WorkflowExecutionRole[] = WRITE_ROLES, active = true) => {
    const project = requireNode(await repository.getProject(projectKey), 'project', projectKey);
    await authorize(project.scopeKey, roles);
    if (active) requireActive(project, 'project');
    return project;
  };
  const milestoneContext = async (milestoneKey: string, roles: readonly WorkflowExecutionRole[] = WRITE_ROLES, active = true) => {
    const milestone = requireNode(await repository.getMilestone(milestoneKey), 'milestone', milestoneKey);
    const project = requireNode(await repository.getProject(milestone.projectKey), 'project', milestone.projectKey);
    if (milestone.scopeKey !== project.scopeKey) throw new WorkflowExecutionExecutionError('invalid_ownership', 'Milestone ownership is inconsistent.');
    await authorize(project.scopeKey, roles);
    if (active) { requireActive(project, 'project'); requireActive(milestone, 'milestone'); }
    return { milestone, project };
  };
  const taskContext = async (taskKey: string, roles: readonly WorkflowExecutionRole[] = WRITE_ROLES, active = true) => {
    const task = requireNode(await repository.getTask(taskKey), 'task', taskKey);
    const milestone = requireNode(await repository.getMilestone(task.milestoneKey), 'milestone', task.milestoneKey);
    const project = requireNode(await repository.getProject(task.projectKey), 'project', task.projectKey);
    if (task.projectKey !== milestone.projectKey || task.scopeKey !== milestone.scopeKey || project.scopeKey !== task.scopeKey) {
      throw new WorkflowExecutionExecutionError('invalid_ownership', 'Task ownership is inconsistent.');
    }
    await authorize(project.scopeKey, roles);
  ...
  const projectChildren = async (projectKey: string, source: WorkflowExecutionRepository = repository) => ({
  ...
  const updateProject = async (source: WorkflowExecutionRepository, project: Project, patch: Partial<Project>) => {
  ...
  const updateMilestone = async (source: WorkflowExecutionRepository, milestone: Milestone, patch: Partial<Milestone>) => {
  ...
  const updateTask = async (source: WorkflowExecutionRepository, task: Task, patch: Partial<Task>) => {
  ...
  const updateFolder = async (source: WorkflowExecutionRepository, project: Project, patch: Partial<Folder>) => source.updateContentFolder(project.contentFolderKey, {
    ...patch,
  ...
  if (action === 'project.create') return executeBatch(action, input, repository, async (item) => {
    ...
    const contentFolderKey = key();
    const timestamp = now();
    const project: Project = { key: projectKey, scopeKey: item.scopeKey, contentFolderKey, name: item.name, ...(item.description ? { description: item.description } : {}), embedding: await embeddingFor(item), deletedAt: null, createdAt: timestamp, updatedAt: timestamp };
    const folder: Folder = { key: contentFolderKey, scopeKey: item.scopeKey, name: item.name, ...(item.description ? { description: item.description } : {}), isFavorite: false, embedding: await embeddingFor(item), deletedAt: null, createdAt: timestamp, updatedAt: timestamp };
    return { keys: { scopeKey: item.scopeKey, projectKey, contentFolderKey }, run: async (source) => { const created = await source.createProject(project); await source.createContentFolder(folder); return created; } };
  });
  ...
  if (action === 'project.update' || action === 'project.rename') return executeBatch(action, input, repository, async (item) => {
    ...
    return { keys: { scopeKey: project.scopeKey, projectKey: project.key, contentFolderKey: project.contentFolderKey }, run: async (source) => { const updated = await updateProject(source, project, patch); await updateFolder(source, project, patch); return updated; } };
  });
  if (action === 'project.move') return executeBatch(action, input, repository, async (item) => {
    const project = await projectContext(item.projectKey);
    await authorize(item.scopeKey, WRITE_ROLES);
    return { keys: { scopeKey: item.scopeKey, projectKey: project.key, contentFolderKey: project.contentFolderKey }, scopeKeys: [project.scopeKey, item.scopeKey], run: async (source) => {
  ...
  if (action === 'project.archive' || action === 'project.restore') return executeBatch(action, input, repository, async (item) => {
    ...
    return { keys: { scopeKey: project.scopeKey, projectKey: project.key, contentFolderKey: project.contentFolderKey }, run: async (source) => {
  ...
  if (action === 'project.delete') return executeBatch(action, input, repository, async (item) => {
    ...
    return { keys: { scopeKey: project.scopeKey, projectKey: project.key, contentFolderKey: project.contentFolderKey }, run: async (source) => {
      ...
      await source.deleteContentFolder(project.contentFolderKey);
  ...
  if (action === 'task.list') {
    const project = await projectContext(input.projectKey, READ_ROLES, false);
    if (input.milestoneKey) { const context = await milestoneContext(input.milestoneKey, READ_ROLES, false); if (context.project.key !== project.key) throw new WorkflowExecutionExecutionError('invalid_ownership', 'Milestone belongs to another project.'); }
  ...
  if (action === 'task.summarize' || action === 'task.translate' || action === 'task.rewrite') return executeBatch(action, input, repository, async (item) => {
    ...
    if (!dependencies.reason) throw new WorkflowExecutionExecutionError('reason_unavailable', 'A reason(input) dependency is required because WorkflowExecution input does not contain an organization key for route selection.');
    const operation = action.slice('task.'.length) as WorkflowExecutionReasonInput['action'];
    ...
    if (!text) throw new WorkflowExecutionExecutionError('empty_reason_result', 'Reasoning returned empty text.');
  ...
  throw new WorkflowExecutionExecutionError('unsupported_action', `Unsupported WorkflowExecution action: ${exhaustive}`);
}
