...
const WORKFLOW_EXECUTION_COLLECTIONS = [PROJECTS_COLLECTION, MILESTONES_COLLECTION, TASKS_COLLECTION, FOLDERS_COLLECTION, SCOPES_COLLECTION] as const;
const listOptionsSchema = z.object({ includeDeleted: z.boolean().optional() }).strict();

export interface WorkflowExecutionDatabase {
  query(query: string, bindVars?: Record<string, unknown>): Promise<{ all(): Promise<unknown[]> }>;
}

export type WorkflowExecutionTransactionRunner = <T>(
  collections: readonly string[],
  operation: (database: WorkflowExecutionDatabase) => Promise<T>,
) => Promise<T>;
...
export interface WorkflowExecutionRepository {
  ...
  updateProject(key: string, patch: ProjectUpdate, expectedUpdatedAt?: string): Promise<Project>;
  deleteProject(key: string): Promise<void>;
  listProjects(scopeKey: string, options?: WorkflowExecutionListOptions): Promise<Project[]>;
  listAllProjects(options?: WorkflowExecutionListOptions): Promise<Project[]>;
  listProjectsByScopes(scopeKeys: string[], options?: WorkflowExecutionListOptions): Promise<Project[]>;
  createMilestone(milestone: Milestone): Promise<Milestone>;
  getMilestone(key: string): Promise<Milestone | null>;
  updateMilestone(key: string, patch: MilestoneUpdate, expectedUpdatedAt?: string): Promise<Milestone>;
  deleteMilestone(key: string): Promise<void>;
  listMilestones(projectKey: string, options?: WorkflowExecutionListOptions): Promise<Milestone[]>;
  createTask(task: Task): Promise<Task>;
  getTask(key: string): Promise<Task | null>;
  updateTask(key: string, patch: TaskUpdate, expectedUpdatedAt?: string): Promise<Task>;
  deleteTask(key: string): Promise<void>;
  listTasks(projectKey: string, options?: WorkflowExecutionListOptions & { milestoneKey?: string }): Promise<Task[]>;
  createContentFolder(folder: Folder): Promise<Folder>;
  updateContentFolder(key: string, patch: Partial<Folder> & Pick<Folder, 'embedding' | 'updatedAt'>, expectedUpdatedAt?: string): Promise<Folder>;
  deleteContentFolder(key: string): Promise<void>;
  areScopesActive(scopeKeys: string[]): Promise<boolean>;
  withAtomic<T>(operation: (repository: WorkflowExecutionRepository) => Promise<T>): Promise<T>;
}

const defaultTransactionRunner: WorkflowExecutionTransactionRunner = (collections, operation) =>
  withTransaction([...collections], (transaction) => operation(transaction));

export function createWorkflowExecutionRepository(
  database: WorkflowExecutionDatabase = db,
  runTransaction: WorkflowExecutionTransactionRunner = defaultTransactionRunner,
  ...
  async function update<Schema extends z.ZodTypeAny>(
    ...
    if (!current) throw new WorkflowExecutionDocumentNotFoundError(collection, validKey);
    if (expectedUpdatedAt !== undefined && (current as { updatedAt?: string }).updatedAt !== expectedUpdatedAt) {
      throw new WorkflowExecutionConcurrentUpdateError(collection, validKey);
    }
    ...
    const [saved] = await cursor.all();
    if (!saved) throw new WorkflowExecutionDocumentNotFoundError(collection, validKey);
    return schema.parse(withArangoKey(saved as Record<string, unknown>));
  }
  ...
  async function listAll<Schema extends z.ZodTypeAny>(collection: string, schema: Schema, options?: WorkflowExecutionListOptions): Promise<z.output<Schema>[]> {
    const validOptions = listOptionsSchema.parse(options ?? {});
    const cursor = await database.query(
  ...
  async function listProjectsByScopes(scopeKeys: string[], options?: WorkflowExecutionListOptions): Promise<Project[]> {
    const validOptions = listOptionsSchema.parse(options ?? {});
  ...
  const repository: WorkflowExecutionRepository = {
    ...
    updateProject: (key, patch, expectedUpdatedAt) => update(PROJECTS_COLLECTION, projectSchema, projectSchema.shape.key, key, patch, expectedUpdatedAt),
    ...
    updateMilestone: (key, patch, expectedUpdatedAt) => update(MILESTONES_COLLECTION, milestoneSchema, milestoneSchema.shape.key, key, patch, expectedUpdatedAt),
    ...
    updateTask: (key, patch, expectedUpdatedAt) => update(TASKS_COLLECTION, taskSchema, taskSchema.shape.key, key, patch, expectedUpdatedAt),
    ...
    createContentFolder: (folder) => insert(FOLDERS_COLLECTION, folderSchema, folder),
    updateContentFolder: (key, patch, expectedUpdatedAt) => update(FOLDERS_COLLECTION, folderSchema, folderSchema.shape.key, key, patch, expectedUpdatedAt),
    deleteContentFolder: (key) => remove(FOLDERS_COLLECTION, folderSchema.shape.key, key),
    areScopesActive: async (scopeKeys) => {
      const keys = z.array(projectSchema.shape.scopeKey).max(100).parse([...new Set(scopeKeys)]);
      if (keys.length === 0) return true;
      const cursor = await database.query(
        'FOR scope IN scopes FILTER scope._key IN @scopeKeys && scope.deletedAt == null RETURN scope._key',
        { scopeKeys: keys },
      );
      return (await cursor.all()).length === keys.length;
    },
    withAtomic: (operation) => runTransaction(WORKFLOW_EXECUTION_COLLECTIONS, (transaction) =>
      operation(createWorkflowExecutionRepository(transaction, runTransaction))),
  };
...
function requireSuppliedEmbedding(input: unknown): void {
  if (typeof input !== 'object' || input === null || !Object.prototype.hasOwnProperty.call(input, 'embedding')) {
    throw new Error('WorkflowExecution repository writes require a precomputed embedding.');
  }
}

export class WorkflowExecutionDocumentNotFoundError extends Error {
  constructor(collection: string, key: string) {
    super(`${collection} document not found: ${key}`);
    this.name = 'WorkflowExecutionDocumentNotFoundError';
  }
}

export class WorkflowExecutionConcurrentUpdateError extends Error {
  constructor(collection: string, key: string) {
    super(`${collection} document changed during mutation preparation: ${key}`);
    this.name = 'WorkflowExecutionConcurrentUpdateError';
  }
}

let defaultRepository: WorkflowExecutionRepository | null = null;

export function getDefaultWorkflowExecutionRepository(): WorkflowExecutionRepository {
  defaultRepository ??= createWorkflowExecutionRepository();
