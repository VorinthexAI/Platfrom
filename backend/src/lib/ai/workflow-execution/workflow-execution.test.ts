...
import { workflowExecutionToolJsonSchemas } from './tool-schemas';
import { executeWorkflowExecutionTool } from './execute';
import type { MilestoneUpdate, WorkflowExecutionRepository, ProjectUpdate, TaskUpdate } from './repository';
...
function memoryRepository() {
  ...
  const repository: WorkflowExecutionRepository = {
    ...
    async createContentFolder(value) { if (folders.has(value.key)) throw new Error('duplicate folder'); folders.set(value.key, structuredClone(value)); return value; },
    async updateContentFolder(key, patch) { return structuredClone(update(folders, key, patch)); },
    async deleteContentFolder(key) { folders.delete(key); },
...
function harness(deniedScope?: string) {
  ...
  let counter = 0;
  const execute = (action: WorkflowExecutionActionSlug, input: unknown) => executeWorkflowExecutionTool(action, input, {
...
describe('WorkflowExecution schemas and registration', () => {
  test('registers every specified tool with strict runtime and JSON schemas', () => {
    expect(Object.keys(workflowExecutionToolInputSchemas)).toHaveLength(40);
    for (const [action, schema] of Object.entries(workflowExecutionToolInputSchemas)) {
  ...
  test('enforces exact statuses, priorities, hierarchy, and batch bounds', () => {
    expect(() => workflowExecutionToolInputSchemas['milestone.create'].parse({ items: [{ projectKey: scopeKey, name: 'M', status: 'pending' }] })).toThrow();
    expect(() => workflowExecutionToolInputSchemas['task.create'].parse({ items: [{ milestoneKey: scopeKey, title: 'T', priority: 'urgent' }] })).toThrow();
    expect(() => workflowExecutionToolInputSchemas['project.create'].parse({ items: [] })).toThrow();
    expect(workflowExecutionToolInputSchemas['task.create'].parse({ items: [{ milestoneKey: scopeKey, title: 'T' }] }).atomic).toBe(true);
  });

  test('keeps model JSON requirements aligned with strict runtime schemas', () => {
    const projectCreate = workflowExecutionToolJsonSchemas['project.create'] as { properties: { items: { items: { required: string[] } } } };
    const taskTranslate = workflowExecutionToolJsonSchemas['task.translate'] as { properties: { items: { items: { required: string[] } } } };
    const scopeSearch = workflowExecutionToolJsonSchemas['scope.project.search'] as { required: string[]; properties: { sources: { items: { oneOf: unknown[] } } } };
...
describe('Project tools', () => {
  test('creates a project and canonical Content folder atomically', async () => {
    ...
    expect(project.contentFolderKey).toBeDefined();
    expect(context.folders.get(project.contentFolderKey)).toMatchObject({ scopeKey, name: 'Apollo', deletedAt: null });
    expect(project.embedding.length).toBeGreaterThan(0);
  });

  test('rolls back project insertion when Content folder creation fails', async () => {
    const context = harness();
    const original = context.repository.createContentFolder;
    context.repository.createContentFolder = async () => { throw new Error('folder failed'); };
    ...
    context.repository.createContentFolder = original;
  });
  ...
  test('clears optional descriptions and preserves committed writes', async () => {
    ...
    const result = await executeWorkflowExecutionTool('project.rename', { items: [{ projectKey: project.key, name: 'Audit-safe' }], atomic: true }, {
...
describe('Batch, authorization, search, and domain envelope', () => {
  ...
  test('executes a registered WorkflowExecution tool through the real domain result envelope', async () => {
    const context = harness();
    ...
    const result = await executeDomainTool('project.create', { items: [{ scopeKey, name: 'Envelope' }], atomic: true }, { organizationKey: 'org', runtimeScopeKey: scopeKey, principal }, {
      workflowExecution: { repository: context.repository, createKey: () => `cmrnlzf64${String(context.projects.size).padStart(4, '0')}qc7k4p5zem5w`, now: () => timestamp, generateEmbedding: async () => [1], reason: async () => 'reason' },
