...
export const appAudioVoiceSchema = z.enum(['calm', 'clear', 'warm']);
export const appAudioInputSchema = z.object({
  documentKey: key,
  voice: appAudioVoiceSchema.default('clear'),
...
export type AppAudioVoice = z.infer<typeof appAudioVoiceSchema>;
export type AppAudioDocumentVersion = z.infer<typeof contentDocumentAudioVersionSchema>;

export interface AppAudioTargetOptions<T> extends ExecuteActionOptions {
  afterSpeech?: () => Promise<void>;
  persist: (audio: { storageKey: string; mimeType: 'audio/mpeg'; sizeBytes: number; durationMs: number; durationSeconds: number; voice: AppAudioVoice; speakingRate: number }) => Promise<T>;
  compensate?: (storageKey: string) => Promise<void>;
}

export interface AppAudioService {
  generateDocument(input: z.input<typeof appAudioInputSchema>, context: ToolContext, options?: ExecuteActionOptions): Promise<AppAudioDocumentVersion>;
  generateForTarget<T>(input: z.input<typeof trustedTargetInputSchema>, options: AppAudioTargetOptions<T>): Promise<{ target: T; storageKey: string; durationSeconds: number }>;
}

interface AppAudioDependencies {
...
function member(context: ToolContext) {
  ...
  if (principal.userOrganization.organizationId !== context.organizationKey || principal.userOrganization.userId !== principal.user.key || principal.userOrganization.status !== 'active') {
    throw new ContentError('CONTENT_FORBIDDEN', 'Active matching organization membership is required.', 'app.audio', { action: 'authorization' });
  }
  return principal;
}

function narrationText(document: Pick<Document, 'name' | 'content'>, input: z.output<typeof appAudioInputSchema>) {
...
export function createAppAudioService(dependencies: AppAudioDependencies = {}): AppAudioService {
  const storage = dependencies.storage ?? documentStorage;
  const speech = dependencies.speech ?? (async (input, organizationKey, options) => {
    const output = speechOutputSchema.parse((await executeAction({ mode: 'auto', organizationKey, actionSlug: 'generate-speech' }, input, options)).output);
  ...
  return {
    ...
    async generateDocument(rawInput, context, options) {
      const input = appAudioInputSchema.parse(rawInput);
      ...
      if (!projected?.success || !projected.data?.document) throw new ContentError('CONTENT_NOT_FOUND', 'Document was not found.', 'app.audio', { action: 'read', resourceKey: input.documentKey });
      ...
      if (!document || document.scopeKey !== context.runtimeScopeKey || document.scopeKey !== projected.data.document.scopeKey || document._internalDeletion || document.archiveVisibility === 'domain-only') {
        throw new ContentError('CONTENT_NOT_FOUND', 'Document was not found in the active Archive scope.', 'app.audio', { action: 'resolution', resourceKey: input.documentKey });
      }
      if (document.mutationPolicy === 'system-only' || document.managedPurpose) throw new ContentError('CONTENT_FORBIDDEN', 'Managed documents are read-only.', 'app.audio', { action: 'persist', resourceKey: input.documentKey });
      if (!repository.createAudioVersion) throw new ContentError('CONTENT_CONFLICT', 'Document audio persistence is unavailable.', 'app.audio', { action: 'persist', resourceKey: input.documentKey });
      const text = narrationText(document, input);
      if (!text) throw new ContentError('CONTENT_INVALID_INPUT', 'Document narration is empty.', 'app.audio', { action: 'speech', resourceKey: input.documentKey });
