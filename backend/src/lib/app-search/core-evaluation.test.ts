...
import { createAppSearchService, projectAppSearchRetrieval, type AppSearchInput, type AppSearchResult, type AppSearchRetrieval } from './service';
...
function searchScenario(input: {
  ...
  prompts: Prompts;
  query: string;
  collectionSlug: NonNullable<AppSearchInput['collectionSlugs']>[number];
  limit: number;
  keys: string[];
  facts: string[];
  answers?: Prompts;
...
function providerQueue(responses: ProviderStreamChunk[][], inputs: CoreChatInput[]) {
  return async function* (_team: string, input: CoreChatInput) {
    if (input.responseFormat?.name === 'conversation_name') {
      yield { type: 'text-delta' as const, text: JSON.stringify({ name: 'Search evaluation' }) };
...
describe('Core App Search deterministic evaluation', () => {
  ...
  for (const evaluation of evaluationCases) test(evaluation.name, async () => {
    ...
    const projectedRetrieval = projectAppSearchRetrieval(evaluation.input, rawResults[0]);
    expect(saved?.retrievals).toEqual(evaluation.retrievals?.length ? evaluation.retrievals : projectedRetrieval ? [projectedRetrieval] : []);
    expect(inputs).toHaveLength(2);
    expect(inputs[1]!.messages.at(-2)?.content[0]).toMatchObject({ type: 'tool-result', result: { slug: 'app.search', status: 'succeeded' } });
    expect(inputs[1]!.messages.at(-1)).toMatchObject({ role: 'user', content: [{ type: 'text', text: evaluation.prompt }] });
