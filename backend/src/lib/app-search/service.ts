...
type AppSearchFilterName = 'folderKey' | 'rootOnly' | 'includeDescendants' | 'collectionKey' | 'connectorKey' | 'readState' | 'emailFacets' | 'status' | 'isFavorite' | 'createdFrom' | 'createdTo' | 'tagNames' | 'tagKeys' | 'tagMatch' | 'targetTypes';
...
export const APP_SEARCH_COLLECTION_ADAPTERS = Object.freeze({
  folders: { description: 'Containers that organize saved documents and files; searched by folder name and description.', operations: ['search', 'list', 'count', 'get'], filters: ['folderKey', 'rootOnly', 'includeDescendants', 'createdFrom', 'createdTo', 'tagNames', 'tagKeys', 'tagMatch'], fields: ['key', 'scopeKey', 'name', 'description', 'parentFolderKey', 'presentation', 'managed', 'structuralProtection', 'isFavorite', 'createdAt', 'updatedAt', 'tags'] },
...
const appSearchFiltersShape = {
  ...
  rootOnly: z.boolean().optional().describe('Restrict folders, documents, and files to the scope root, excluding items inside folders. Cannot be combined with folderKey or includeDescendants.'),
...
export function validateAppSearchInput(input: z.infer<z.ZodObject<typeof appSearchInputShape>>, context: z.RefinementCtx) {
  ...
  if (input.filters?.rootOnly && (input.filters.folderKey || input.filters.includeDescendants !== undefined)) context.addIssue({ code: 'custom', path: ['filters', 'rootOnly'], message: 'rootOnly cannot be combined with folderKey or includeDescendants.' });
...
const notificationResultSchema = z.object({ key: z.string().cuid(), title: z.string(), message: z.string(), isRead: z.boolean(), createdAt: z.string().datetime(), kind: z.literal('email-thread').optional(), connectorKey: z.string().min(1).max(160).optional(), threadKey: z.string().min(1).max(160).optional(), messageKey: z.string().min(1).max(160).optional(), score: z.number().optional() }).strict();
    ...
    try {
      ...
      const contentPromise = [...requested].some((slug) => slug === 'folders' || slug === 'documents' || slug === 'files')
        ? (dependencies.executeContent ?? runContentTool)('content.search', {
            ...
            limit: contentLimit,
            recordHistory: false,
            ...(input.filters?.rootOnly ? { rootOnly: true } : {}),
      ...
      const groupPromises = collectionSlugs.map(async (collectionSlug) => {
        ...
        if (collectionSlug === 'folders') {
          ...
          return { collectionSlug, results: output.folders.filter((item) => isCandidate('folders', item)).slice(0, input.limit).map((item) => ({ key: item.key, scopeKey: item.scopeKey, name: item.name, ...(item.description ? { description: item.description } : {}), ...(item.parentFolderKey ? { parentFolderKey: item.parentFolderKey } : {}), ...(item.presentation ? { presentation: item.presentation } : {}), ...(item.managed !== undefined ? { managed: item.managed } : {}), ...(item.structuralProtection ? { structuralProtection: true as const } : {}), isFavorite: item.isFavorite, ...(item.createdAt ? { createdAt: item.createdAt } : {}), ...(item.updatedAt ? { updatedAt: item.updatedAt } : {}), score: item.score })) };
        }
        if (collectionSlug === 'documents' || collectionSlug === 'files') {
          ...
          try { if (documents.length) {
           ...
           return { collectionSlug, results: documents.map((item) => ({ key: item.documentKey, scopeKey: item.scopeKey, name: item.name, ...(item.folderKey ? { folderKey: item.folderKey } : {}), ...(item.folder ? { folder: item.folder } : {}), ...(item.extension ? { extension: item.extension } : {}), ...(item.mimeType ? { mimeType: item.mimeType } : {}), ...(item.sizeBytes !== undefined ? { sizeBytes: item.sizeBytes } : {}), ...(item.managed !== undefined ? { managed: item.managed } : {}), ...(item.structuralProtection ? { structuralProtection: true as const } : {}), isFavorite: item.isFavorite, ...(item.createdAt ? { createdAt: item.createdAt } : {}), ...(item.updatedAt ? { updatedAt: item.updatedAt } : {}), score: item.score, ...(contentByKey.has(item.documentKey as string) ? { content: contentByKey.get(item.documentKey as string) } : {}) })) };
        }
