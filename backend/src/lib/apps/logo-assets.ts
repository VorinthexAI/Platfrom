...
export async function seedInitialGalleryAssets({
  client,
  bucket,
  repositoryRoot,
  forceUpload = false,
  ...
  if (!bucket.trim()) throw new Error('Initial Gallery asset seeding requires a non-empty S3 bucket.');
  const assets: readonly SystemAsset[] = INITIAL_GALLERY_ASSET_MANIFEST.map(({ storageKey, sourcePath }) => ({ storageKey, sourcePath, contentType: 'image/png' }));
  const results: Array<{ storageKey: string; status: 'uploaded' | 'unchanged'; checksum: string }> = [];
  for (const asset of assets) {
    const body = new Uint8Array(await Bun.file(resolve(repositoryRoot, asset.sourcePath)).arrayBuffer());
    if (body.byteLength === 0) throw new Error(`Initial Gallery asset source is empty: ${asset.sourcePath}`);
    const checksum = createHash('sha256').update(body).digest('hex');
    let unchanged = false;
    if (!forceUpload) {
      try {
