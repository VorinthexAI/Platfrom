...
import { bookGenerationJobOptions, bookGenerationJobSchema, enqueueBookGenerationIn, processBookGenerationJob } from './generation-queue';
...
describe('book generation queue', () => {
  ...
  test('keeps real runtime failures recoverable until worker exhaustion', async () => {
    ...
    let book: any = { key: bookKey, scopeKey, title: 'Decisions', description: 'Description', goal: 'Decide well', audience: 'Basics', outcome: 'Decide well', language: 'English', generationInput, generationOwnerKey: userKey, generationStage: 'accepted', generationCompletedUnits: 0, generationTotalUnits: 34, generationAttempt: 0, estimatedMinutes: 0, chapterCount: 10, status: 'queued', embedding: Array(EMBEDDING_DIMENSIONS).fill(0), createdAt: timestamp, updatedAt: timestamp };
    ...
    const runtime = createBookRuntime({ repository, research: async () => { throw new Error('provider secret must not persist'); }, publishChanged: async () => {} });
    ...
    expect(book.status).toBe('researching'); expect(book.generationError).toBeUndefined(); expect(terminalFailures).toBe(0);
