...
export function startBookGenerationWorker() { const worker = new Worker<BookGenerationJob>(QUEUE_NAME, async (job) => processBookGenerationJob(job, (await import('./default-service')).defaultBookService), { connection: connection(), concurrency: 2 }); worker.on('error', (error) => console.error('book generation worker error', { error })); worker.on('failed', (job, error) => console.error('book generation attempt failed', { bookKey: job?.data.bookKey, attemptsMade: job?.attemptsMade, attempts: error instanceof Error && 'attempts' in error ? error.attempts : undefined, cause: error.cause })); return { close: () => worker.close() }; }
