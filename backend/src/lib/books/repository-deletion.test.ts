...
describe('book hard deletion', () => {
  test('removes only canonical Ascend records and enqueues canonical media', async () => {
    ...
    await expect(createBookRepository(database, transact).deleteBook({ teamKey: 'team', scopeKey, userKey: newId() }, bookKey, '2026-08-25T12:00:00.000Z')).resolves.toEqual({ deleted: true, bookKey });
    ...
    for (const collection of ['books', 'bookContexts', 'bookThemes', 'bookSources', 'bookParts', 'bookChapters', 'chapterContexts', 'bookProgress', 'generatedDocumentBindings', 'storageDeletionJobs']) expect(writes).toContain(collection);
  ...
  test('blocks favorite deletion before enqueuing media or removing records', async () => {
    ...
    await expect(repository.deleteBook({ teamKey: 'team', scopeKey, userKey: newId() }, bookKey, '2026-08-25T12:00:00.000Z')).rejects.toMatchObject({ reason: 'favorite', message: 'Unfavorite the audio book before deleting it.' });
    expect(queries.some((query) => query.includes('IN storageDeletionJobs') || query.includes('REMOVE'))).toBe(false);
  });

  test('deletes audio books without media records', async () => {
    ...
    const repository = createBookRepository(database, async (_collections, run) => run(database));
    await expect(repository.deleteBook({ teamKey: 'team', scopeKey, userKey: newId() }, bookKey, '2026-08-25T12:00:00.000Z')).resolves.toEqual({ deleted: true, bookKey });
  ...
  test('persists favorite state only on an authorized scoped book', async () => {
    ...
    await expect(createBookRepository(database).setFavorite({ teamKey: 'team', scopeKey, userKey: newId() }, bookKey, true, persisted.updatedAt)).resolves.toMatchObject({ key: bookKey, isFavorite: true });
