...
describe('book extension repository', () => {
  test('atomically fences readiness, title uniqueness, receipt insertion, and activation', async () => {
    ...
    const database: any = { query: async (query: string, bind: Record<string, any> = {}) => { queries.push({ query, bind }); return { all: async () => query.includes('userTeams') ? [1] : query.includes('FOR item IN bookExtensions') ? [] : query.includes('INSERT @extension INTO bookExtensions') ? [updatedBook] : [] }; } };
    ...
    await expect(repository.acceptExtension({ teamKey: 'team', scopeKey, userKey }, extension, timestamp)).resolves.toMatchObject({ replayed: false, extension, book: { key: bookKey, status: 'queued', chapterCount: 11 } });
  ...
  test('appends one contiguous chapter only while the book lease is owned', async () => {
    ...
    const database: any = { query: async (query: string) => { queries.push(query); return { all: async () => query.includes('userTeams') || query.includes('INSERT @chapter INTO bookChapters') ? [1] : [] }; } };
    ...
    await repository.appendChapter({ teamKey: 'team', scopeKey, userKey: newId(), generationLeaseToken: 'lease' }, bookKey, chapter, chapterContext);
