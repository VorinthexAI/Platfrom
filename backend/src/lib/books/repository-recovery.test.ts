...
describe('book generation failure persistence', () => {
  test('terminalizes and enqueues the exact linked refund only for the active lease owner', async () => {
    ...
    await expect(repository.terminalizeGeneration({ bookKey, teamKey: 'team', scopeKey, userKey }, 'lease-token', 'safe failure', intentKey, now)).resolves.toBe(true);
    ...
    expect(bindings).toEqual({ bookKey, teamKey: 'team', scopeKey, userKey, leaseToken: 'lease-token', message: 'safe failure', intentKey, now });
  });
  ...
  test('lists a bounded set of unleased accepted generations for process recovery', async () => {
    ...
    const database: BookDatabase = { async query(query, bind = {}) { queryText = query; bindings = bind; return { all: async () => [{ bookKey, teamKey: 'team', scopeKey, userKey }] }; } };
    const repository = createBookRepository(database);
    await expect(repository.listRecoverableGenerations('2026-08-25T12:00:00.000Z', 100)).resolves.toEqual([{ bookKey, teamKey: 'team', scopeKey, userKey }]);
