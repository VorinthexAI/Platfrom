  ...
  return {
    ...
    async ensureGalleryExportCollection(context, bookKey, bookTitle, embedding, now) {
      return transact({ read: ['userOrganizations', 'scopes', 'scopeMembers', 'books'], write: ['collections', 'collectionMembers'] }, async (transaction) => {
        ...
        await transaction.query('UPSERT { _key: @collectionKey } INSERT { _key: @collectionKey, scopeKey: @scopeKey, name: @bookTitle, description: "Artwork generated for this audio book", presentation: "learning", mutationPolicy: "user", embedding: @embedding, isFavorite: false, createdAt: @now, updatedAt: @now } UPDATE { name: @bookTitle, description: "Artwork generated for this audio book", presentation: "learning", embedding: @embedding, updatedAt: @now } IN collections', { collectionKey, scopeKey: context.scopeKey, bookTitle, embedding, now });
