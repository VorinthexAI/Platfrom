...
const BOOK_ART_DIRECTION = 'Subject fidelity is the highest priority: the dominant scene, objects, environment, action, and visual metaphor must clearly and specifically represent the supplied subject rather than generic technology or decoration. Prefer concrete subject-specific imagery over vague abstraction. Use an editorial cinematic style with obsidian and void foundations #030507, #080B0F, #0D1117, #141922, and #000000; silver and chrome highlights #F5F7F8, #DDE2E5, #AEB6BC, #7B858C, #3C434A, and #FFFFFF; and restrained cold metallic accents #9FB4C7, #1B232C, and #C9CED2. Black should dominate, with controlled silver focus, sparse cold steel-blue tint, atmospheric depth, polished metal or obsidian glass where appropriate, precise lighting, and low visual clutter. Do not force futuristic motifs when they do not communicate the subject. Avoid generic circuitry, glowing spheres, anonymous tunnels, random geometric decoration, stock sci-fi scenes, and unrelated visual spectacle.';
const ARTWORK_NO_TEXT = 'The output is a wordless image built entirely from subject-specific visual forms. Every surface is blank and unmarked. Absolute restriction: zero words, letters, initials, numbers, typography, captions, titles, labels, signage, logos, product names, company names, brands, watermarks, signatures, interface elements, writing, pseudo-writing, glyphs, runes, or readable symbols.';
const ARTWORK_NO_PEOPLE = 'The scene contains only subject-specific objects, materials, structures, physical effects, or abstract forms directly required to communicate the topic. Absolute restriction: zero humans, faces, heads, eyes, mouths, skin, bodies, hands, limbs, silhouettes, crowds, portraits, characters, mannequins, statues, avatars, humanoid robots, or anthropomorphic figures, including abstract, reflected, shadowed, partial, or distant forms.';
...
const truncateProse = (value: string, maximum: number) => {
  ...
  return sentence && words(sentence) >= BOOK_CHAPTER_WORD_MIN ? sentence : limited;
};
...
export function createBookRuntime(options: BookRuntimeDependencies = {}): BookGenerator {
  ...
  const inspectArtwork = options.inspectArtwork ?? (async (bytes, mimeType, organizationKey, signal) => {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        const output = imageCaptionOutputSchema.parse((await executeAction<ImageCaptionInput, ImageCaptionOutput>({ mode: 'fixed', organizationKey, actionSlug: 'caption-image', modelSlug: IMAGE_CAPTION_MODEL, providerSlug: 'openai' }, input, { signal, timeoutMs: 180_000 })).output);
    ...
    async write(bookKey, input, context) {
      ...
      const compliantArtwork = async (artworkPrompt: string, label: string) => {
        ...
        for (let attempt = 1; attempt <= 4; attempt += 1) {
          const artwork = await pngMedia(await provider(() => image(`${artworkPrompt} Compliance is mandatory. Render only subject-specific objects, materials, structures, physical effects, or abstract forms, with every surface completely blank and unmarked.`, context.organizationKey, signal)));
          if (!artwork) continue;
        ...
        throw new Error(`${label} violated the no-people or no-text artwork policy after 4 attempts: ${reason}`);
      };
      const boundedProse = async (instruction: string, data: Record<string, unknown>, label: string) => {
        let content = '';
        for (let attempt = 0; attempt < 6; attempt += 1) {
          const count = words(content);
          if (count >= BOOK_CHAPTER_WORD_MIN && count <= BOOK_CHAPTER_WORD_MAX) return formatChapterParagraphs(content);
          if (count > BOOK_CHAPTER_WORD_MAX) return formatChapterParagraphs(truncateProse(content, BOOK_CHAPTER_WORD_MAX));
          if (count > 0 && count < BOOK_CHAPTER_WORD_MIN) {
            const continuation = (await provider(() => ask(prompt(`Continue the existing ${label} with ${BOOK_CHAPTER_WORD_MIN - count}-${BOOK_CHAPTER_WORD_MAX - count} additional words. Return only new prose that follows directly from the existing ending; do not repeat, restart, add headings, or add meta commentary.`, JSON.stringify({ ...data, existingProse: content })), context.organizationKey, signal))).trim();
            content = `${content}\n\n${continuation}`.trim();
          } else content = (await provider(() => ask(prompt(instruction, JSON.stringify(data)), context.organizationKey, signal))).trim();
        }
        const count = words(content);
        if (count < BOOK_CHAPTER_WORD_MIN || count > BOOK_CHAPTER_WORD_MAX) throw new Error(`${label} contained ${count} words after 6 attempts; expected ${BOOK_CHAPTER_WORD_MIN}-${BOOK_CHAPTER_WORD_MAX}.`);
      ...
      try {
        ...
        const coverInputHash = hash({ generation: currentBook.generationBriefFingerprint, promptVersion: 11 });
        const coverPrompt = (metadata: Record<string, unknown>) => `Generate one vertical 9:16 full-bleed editorial book-cover illustration for this nonfiction subject. Make the central topic immediately legible through concrete subject-specific objects, materials, structures, physical effects, or a precise inanimate visual metaphor derived from the topic and goal. The complete focal subject must communicate the idea without writing or characters. ${ARTWORK_NO_PEOPLE} ${ARTWORK_NO_TEXT} ${BOOK_ART_DIRECTION} Fill the entire frame with one cohesive composition: render meaningful subject-specific detail continuously to all four edges, make the focal subject large and dominant while keeping it fully visible, and use supporting forms to occupy the perimeter without cropping the focal subject. There must be no empty area, blank band, border, frame, margin, matte, title area, letterboxing, inset panel, floating card, page, poster, interface, advertisement, or diagram. Produce a full-frame cover illustration rather than an isolated object on a background. Semantic subject reference only: ${JSON.stringify(metadata)}`;
        coverTask = (extension || (currentBook.coverStorageKey && currentBook.coverInputHash === coverInputHash) ? Promise.resolve() : (async () => {
          const cover = await compliantArtwork(coverPrompt({ topic: input.topic, goal: input.goal, currentKnowledge: input.currentKnowledge }), 'Audio book cover');
        ...
        if (extension) {
          ...
          for (let index = Math.max(chapters.length, suffixStart); index < extension.targetChapterCount; index += 1) {
            ...
            const plan = { title, description: title, objective: `Continue the book through ${title}.`, visualPlan: `A concrete, subject-specific scene or object metaphor that uniquely communicates ${title}, with no text, lettering, logos, or branding.`, evidenceKeyPoints: ['Build on the complete existing book context.'], topics: [title], priorTransition: index === 0 ? 'Open the book.' : `Continue from chapter ${index}.`, nextTransition: index + 1 === extension.targetChapterCount ? 'Complete the extended arc.' : `Prepare chapter ${index + 2}.`, repetitionBoundaries: ['Do not repeat existing chapters.'], targetWordMin: BOOK_CHAPTER_WORD_MIN, targetWordMax: BOOK_CHAPTER_WORD_MAX };
          ...
          const planResponseFormat = { name: 'book_generation_plan', schema: { type: 'object', additionalProperties: false, required: ['title', 'description', 'outcome', 'summary', 'chapters'], properties: { title: { type: 'string', minLength: 1 }, description: { type: 'string', minLength: 1 }, outcome: { type: 'string', minLength: 1 }, summary: { type: 'string', minLength: 1 }, chapters: { type: 'array', minItems: input.chapterCount, maxItems: input.chapterCount, items: { type: 'object', additionalProperties: false, required: ['title', 'objective', 'evidenceKeyPoints', 'topics', 'priorTransition', 'nextTransition', 'repetitionBoundaries', 'visualPlan', 'targetWordMin', 'targetWordMax'], properties: { title: { type: 'string', minLength: 1 }, objective: { type: 'string', minLength: 1 }, evidenceKeyPoints: { type: 'array', minItems: 2, maxItems: 12, items: { type: 'string', minLength: 1 } }, topics: { type: 'array', minItems: 1, maxItems: 20, items: { type: 'string', minLength: 1 } }, priorTransition: { type: 'string', minLength: 1 }, nextTransition: { type: 'string', minLength: 1 }, repetitionBoundaries: { type: 'array', minItems: 1, maxItems: 12, items: { type: 'string', minLength: 1 } }, visualPlan: { type: 'string', minLength: 1 }, targetWordMin: { type: 'integer', const: BOOK_CHAPTER_WORD_MIN }, targetWordMax: { type: 'integer', const: BOOK_CHAPTER_WORD_MAX } } } } } } };
          for (let attempt = 0; attempt < 3 && !plan; attempt += 1) {
            const correction = planError ? ` Previous output was invalid: ${planError}. Correct it.` : '';
            const response = await provider(() => ask(prompt(`Design one coherent nonfiction audiobook and its complete ordered internal chapter plan in one request. Return metadata title, description, outcome, summary and exactly ${input.chapterCount} chapters. Each chapter needs a unique title, objective, evidenceKeyPoints, topics, priorTransition, nextTransition, repetitionBoundaries, and visualPlan, with targetWordMin ${BOOK_CHAPTER_WORD_MIN} and targetWordMax ${BOOK_CHAPTER_WORD_MAX}. Every visualPlan must be one descriptive string containing concrete, subject-specific objects, materials, structures, physical effects, or abstract forms that uniquely communicate that chapter rather than generic decoration. A visualPlan must never request botanical, fungal, vegetation, forest, wetland, or organic scenery, and must not request text, lettering, numbers, logos, products, brands, signage, interfaces, or diagrams. Do not write chapter summaries yet.${correction}`, JSON.stringify({ generationBrief: input, evidence }), Math.max(8_000, input.chapterCount * 640), planResponseFormat), context.organizationKey, signal));
        ...
        artTask = (!input.chapterImages ? Promise.resolve() : boundedMap(suffix, 3, async (chapter) => {
          const imageInputHash = hash({ title: chapter.title, visualPlan: chapter.visualPlan, objective: chapter.objective, promptVersion: 10 });
          if (chapter.imageStorageKey && chapter.imageInputHash === imageInputHash) return;
          const art = await compliantArtwork(`Generate one vertical 9:16 full-bleed editorial chapter-cover illustration specifically about this chapter. Make the chapter's unique objective unmistakable through subject-specific objects, materials, structures, physical effects, or a precise inanimate visual metaphor. Distinguish it from adjacent chapters and include only details that support this chapter's meaning. ${ARTWORK_NO_PEOPLE} ${ARTWORK_NO_TEXT} ${BOOK_ART_DIRECTION} Follow the internal visual plan only where it accurately communicates the chapter objective. Fill the entire frame with one cohesive composition: render meaningful subject-specific detail continuously to all four edges, make the focal subject large and dominant while keeping it fully visible, and use supporting forms to occupy the perimeter without cropping the focal subject. There must be no empty area, blank band, border, frame, margin, matte, title area, letterboxing, inset panel, floating card, page, poster, interface, advertisement, or diagram. Produce a full-frame cover illustration rather than an isolated object on a background. Semantic reference only: ${JSON.stringify({ bookSubject: currentBook.description, chapterObjective: chapter.objective, visualPlan: chapter.visualPlan })}`, `Chapter ${chapter.position} artwork`);
