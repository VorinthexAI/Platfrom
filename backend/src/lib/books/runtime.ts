...
const BOOK_ART_DIRECTION = 'Subject fidelity is the highest priority. Distill the subject into a bold near-future visual metaphor built from recognizable subject-derived forms, then transform those forms through controlled abstraction, unexpected scale, translucent layers, precise geometric rhythm, and editorial material contrast. Favor tactile printmaking, layered paper, refined collage, screen-print texture, brushed metal, dark glass, sculptural forms, and speculative editorial abstraction over photorealism or a literal scene. The futuristic treatment must reveal the subject from a fresh angle rather than replace it with generic science-fiction decoration. Avoid ordinary rooms, straightforward product arrangements, and documentary depictions. Follow the product palette exactly: obsidian #030507 and graphite #262D36 foundations; chrome #F5F7F8, silver #DDE2E5, and muted steel #7B858C for contrast; restrained white #FFFFFF highlights only. Use high-contrast directional light and visible material detail so the composition is never an unreadable black void. Do not introduce arbitrary saturated hues, colorful or rainbow gradients, neon or cyberpunk colors, gold, or blue. A real-world subject color may appear only as a restrained, recognizable detail when essential to identify the subject. Do not use empty black space, black fog, silhouettes, isolated glowing objects, anonymous tunnels, random circuitry, generic glowing spheres, or stock sci-fi spectacle.';
...
type Ask = (input: Record<string, unknown>, teamKey: string, signal?: AbortSignal) => Promise<string>;
type Media = (prompt: string, teamKey: string, signal?: AbortSignal) => Promise<{ bytes: Uint8Array; mimeType: string } | null>;
...
export function createBookRuntime(options: BookRuntimeDependencies = {}): BookGenerator {
  ...
  const ask = options.ask ?? (async (input, teamKey, signal) => (await executeAsk<ChatOutput>(teamKey, input as never, { signal, timeoutMs: 180_000 })).output.text);
  const embed = options.embed ?? (async (text, teamKey, signal) => (await executeAction<{ text: string }, { embedding: number[] }>({ mode: 'auto', teamKey, actionSlug: 'embed' }, { text }, { signal })).output.embedding);
  ...
  const dumpArchiveCopies = async (detail: Awaited<ReturnType<BookRepository['detail']>>, context: Parameters<BookRepository['detail']>[0]) => {
    ...
    await boundedMap(detail.chapters, 4, async ({ chapter }, index) => {
      ...
      const chunkEmbeddings = await Promise.all(documentEmbeddingTexts(chapter.title, contentChunks).map((text) => embed(text, context.teamKey, context.signal)));
  ...
  return {
    ...
    async write(bookKey, input, context) {
      ...
      const generateArtwork = async (artworkPrompt: string, label: string) => {
        const artwork = await pngMedia(await provider(() => image(`${artworkPrompt} Render only recognizable subject-specific objects, materials, tools, environments, and physical effects, with every surface completely blank and unmarked.`, context.teamKey, signal)));
      ...
      try {
        ...
        if (unembeddedSources.length) { const embedded: BookSource[] = []; await boundedMap(unembeddedSources, 4, async (source) => { embedded.push({ ...source, embedding: await vector(bookSourcesEmbeddingFields, source, context.teamKey, signal) }); }); await repository.addSources(context, bookKey, embedded); sources = sources.map((source) => embedded.find(({ key }) => key === source.key) ?? source); }
        ...
        if (extension) {
          ...
          for (let index = Math.max(chapters.length, suffixStart); index < extension.targetChapterCount; index += 1) {
            ...
            const chapter = bookChapterSchema.parse({ ...draft, embedding: await vector(bookChaptersEmbeddingFields, draft, context.teamKey, signal) });
            ...
            await repository.appendChapter(context, bookKey, chapter, chapterContextSchema.parse({ ...contextDraft, embedding: await vector(chapterContextsEmbeddingFields, contextDraft, context.teamKey, signal) })); chapters.push(chapter); await notify(input.scopeKey).catch(() => undefined);
          }
          ...
          for (let attempt = 0; attempt < 3 && !plan; attempt += 1) {
            ...
            const response = await provider(() => ask(prompt(`Design one coherent nonfiction audiobook and its complete production plan in one request. Return the final title, subtitle, description, outcome, overall summary, one coverVisualPlan, and exactly ${input.chapterCount} ordered chapters. Each chapter needs a unique title, a distinct 30-40 word description, distinct promptGuidance for writing the finished prose, objective, evidenceKeyPoints, topics, priorTransition, nextTransition, and repetitionBoundaries, with targetWordMin ${BOOK_CHAPTER_WORD_MIN} and targetWordMax ${BOOK_CHAPTER_WORD_MAX}. The coverVisualPlan must be one descriptive string for an abstract-futurist visual metaphor rooted in concrete subject-specific forms, materials, tools, places, or physical effects. Transform those recognizable references through unusual scale, translucent layering, chrome-on-obsidian contrast, geometric rhythm, and imaginative spatial relationships. Keep the palette to obsidian #030507, graphite #262D36, chrome #F5F7F8, silver #DDE2E5, muted steel #7B858C, and restrained white #FFFFFF highlights. Avoid arbitrary saturated hues, colorful gradients, neon, gold, blue, literal room scenes, generic decoration, unreadable dark backgrounds, text, lettering, numbers, logos, products, brands, signage, interfaces, or diagrams.${correction}`, JSON.stringify({ generationBrief: input, evidence }), Math.max(12_000, input.chapterCount * 900), planResponseFormat), context.teamKey, signal));
          ...
          await boundedMap(plan.chapters, 4, async (item, index) => { const draft = { ...item, key: id(), scopeKey: input.scopeKey, bookKey, position: index + 1, status: 'planned' as const, createdAt: timestamp, updatedAt: timestamp }; const chapter = bookChapterSchema.parse({ ...draft, embedding: await vector(bookChaptersEmbeddingFields, draft, context.teamKey, signal) }); chapters[index] = chapter; const contextDraft = { key: id(), scopeKey: input.scopeKey, chapterKey: chapter.key, previousContext: item.priorTransition, objectiveContext: item.objective, sourceContext: item.evidenceKeyPoints.join('\n'), personalizationContext: input.currentKnowledge || currentBook.audience, noveltyContext: item.repetitionBoundaries.join('\n'), nextContext: item.nextTransition, generationBrief: item.promptGuidance, createdAt: timestamp, updatedAt: timestamp }; contexts[index] = chapterContextSchema.parse({ ...contextDraft, embedding: await vector(chapterContextsEmbeddingFields, contextDraft, context.teamKey, signal) }); });
          await repository.replaceChapters(context, bookKey, chapters, contexts, { title: plan.title, subtitle: plan.subtitle, description: plan.description, summary: plan.summary, coverVisualPlan: plan.coverVisualPlan, outcome: plan.outcome, embedding: await vector(booksEmbeddingFields, { ...currentBook, ...plan }, context.teamKey, signal), chapterCount: chapters.length, generationStage: 'draft', status: 'writing', updatedAt: now() }); await notify(input.scopeKey).catch(() => undefined);
        }
        ...
        const coverInputHash = hash({ ...coverMetadata, promptVersion: 17 });
        artTask = (async () => {
