  ...
  return {
    ...
    async write(bookKey, input, context) {
      ...
      try {
        ...
        const coverInputHash = hash({ generation: currentBook.generationBriefFingerprint, promptVersion: 13 });
        const coverPrompt = (metadata: Record<string, unknown>) => `Generate one vertical 9:16 full-bleed editorial book-cover illustration for this nonfiction subject. Depict the topic literally through a recognizable real-world scene and concrete subject-specific objects, materials, tools, places, and physical effects derived from the topic and goal. Do not substitute symbolism or abstraction when literal imagery is possible. The focal subject must communicate the idea without writing or characters. ${ARTWORK_NO_PEOPLE} ${ARTWORK_NO_TEXT} ${BOOK_ART_DIRECTION} Use an immersive close or near-field composition. Overscale the focal subject and its connected environment so major forms continue beyond and are deliberately cropped by the frame on multiple sides. Subject-specific detail must occupy the full canvas continuously to all four edges. Keep every region illuminated and visually informative. Do not render a small or fully visible object in the center. No distant view, wide establishing shot, centered product shot, pedestal display, icon, vignette, sparse backdrop, empty negative space, dark void, or featureless shadow. There must be no blank band, border, frame, margin, matte, title area, letterboxing, inset panel, floating card, page, poster, interface, advertisement, or diagram. Produce one bright edge-to-edge scene, not an isolated object on a background. Semantic subject reference only: ${JSON.stringify(metadata)}`;
        ...
        artTask = (!input.chapterImages ? Promise.resolve() : boundedMap(suffix, 3, async (chapter) => {
          const imageInputHash = hash({ title: chapter.title, visualPlan: chapter.visualPlan, objective: chapter.objective, promptVersion: 12 });
          if (chapter.imageStorageKey && chapter.imageInputHash === imageInputHash) return;
          const art = await compliantArtwork(`Generate one vertical 9:16 full-bleed editorial chapter-cover illustration specifically about this chapter. Depict the chapter objective literally through a recognizable real-world scene and concrete subject-specific objects, materials, tools, places, and physical effects. Do not substitute symbolism or abstraction when literal imagery is possible. Distinguish it from adjacent chapters and include only details that support this chapter's meaning. ${ARTWORK_NO_PEOPLE} ${ARTWORK_NO_TEXT} ${BOOK_ART_DIRECTION} Follow the internal visual plan only when it is concrete, literal, bright, and accurate; ignore any abstract, symbolic, futuristic, or dark direction in that plan. Use an immersive close or near-field composition. Overscale the focal subject and its connected environment so major forms continue beyond and are deliberately cropped by the frame on multiple sides. Subject-specific detail must occupy the full canvas continuously to all four edges. Keep every region illuminated and visually informative. Do not render a small or fully visible object in the center. No distant view, wide establishing shot, centered product shot, pedestal display, icon, vignette, sparse backdrop, empty negative space, dark void, or featureless shadow. There must be no blank band, border, frame, margin, matte, title area, letterboxing, inset panel, floating card, page, poster, interface, advertisement, or diagram. Produce one bright edge-to-edge scene, not an isolated object on a background. Semantic reference only: ${JSON.stringify({ bookSubject: currentBook.description, chapterObjective: chapter.objective, visualPlan: chapter.visualPlan })}`, `Chapter ${chapter.position} artwork`);
