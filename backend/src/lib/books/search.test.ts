...
describe('book semantic search creation dates', () => {
  test('rejects reversed ranges and forwards inclusive boundaries to the repository', async () => {
    const reversed = bookSearchInputSchema.safeParse({ teamKey, scopeKey, query: 'decisions', createdFrom: '2026-08-26T00:00:00.000Z', createdTo: timestamp });
    ...
    const result = await createBookService({ repository, signUrl: async () => 'signed' }).search({ teamKey, scopeKey, query: 'decisions', createdFrom: timestamp, createdTo: timestamp }, userKey, { queryEmbedding: embedding });
  ...
  test('binds inclusive creation-date predicates before semantic ranking and limit', async () => {
    ...
    const rows = await createBookRepository(database).search({ teamKey, scopeKey, userKey }, 'decisions', embedding, -1, 10, { createdFrom: timestamp, createdTo: timestamp });
