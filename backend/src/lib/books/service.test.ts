...
describe('book service asynchronous lifecycle', () => {
  ...
  test('authorizes and generates ten creative topic suggestions through the AI action boundary', async () => {
    ...
    await expect(service.suggestTopics({ teamKey, scopeKey, excludeTopics: ['Old idea'] }, userKey)).resolves.toEqual({ topics });
    expect(calls[0]).toEqual(['authorize', { teamKey, scopeKey, userKey }, false]);
  ...
  test('strictly rejects invalid topic requests and malformed model output', async () => {
    ...
    await expect(service.suggestTopics({ teamKey, scopeKey, unknown: true }, userKey)).rejects.toBeDefined();
    expect(generated).toBe(0);
    await expect(service.suggestTopics({ teamKey, scopeKey }, userKey)).rejects.toThrow('Topics must be unique');
  });
  ...
  test('generates ten unique goals for the selected topic and excludes the previous batch', async () => {
    ...
    await expect(service.suggestGoals({ teamKey, scopeKey, topic: 'Decision making', excludeGoals: ['Old goal'] }, userKey)).resolves.toEqual({ goals });
  ...
  test('previews one exact continuation batch and durably accepts generation', async () => {
    ...
    await expect(service.extend(bookKey, { teamKey, scopeKey, mode: 'preview', chapterCount: 1 }, userKey)).resolves.toEqual({ titles: ['The Next Decision'] }); expect(suggestions).toBe(1);
    await expect(service.extend(bookKey, { teamKey, scopeKey, mode: 'generate', chapterCount: 1, titles: ['The Next Decision'], requestKey: 'extension-1' }, userKey)).resolves.toMatchObject({ key: bookKey, status: 'queued', chapterCount: 11, isExtending: true });
  ...
  test('detaches a queued retry replay', async () => {
    ...
    const result = await createBookService({ repository, generator: { create: async () => bookKey, write: async () => {} }, detach: (run) => { detached.push(run); }, signUrl: async () => 'signed', publishChanged: async () => {} }).retry(bookKey, { teamKey, scopeKey }, userKey);
    expect(result).toMatchObject({ key: bookKey, status: 'queued' });
    expect(detached).toHaveLength(1);
  });
  ...
  test('recovers a bounded batch of durable generations through the lease-claiming path', async () => {
    ...
    const repository: any = { listRecoverableGenerations: async (_now: string, limit: number) => { expect(limit).toBe(25); return [{ bookKey, teamKey, scopeKey, userKey }]; } };
  ...
  test('retries in-process while cancellation and hard deletion remain durable-first', async () => {
    ...
    const repository: any = { retryGeneration: async () => { calls.push('retry'); current = row(); return current.book; }, cancelGeneration: async () => { calls.push('cancel'); current = row('cancelled'); }, deleteBook: async () => { calls.push('delete'); return { deleted: true, bookKey }; }, detail: async () => current };
    const service = createBookService({ repository, generator: { create: async () => bookKey, write: async () => {} }, detach: () => { calls.push('detach'); }, signUrl: async () => 'signed', publishChanged: async () => {} });
    await service.retry(bookKey, { teamKey, scopeKey }, userKey); await service.cancel(bookKey, { teamKey, scopeKey }, userKey); expect(await service.delete(bookKey, { teamKey, scopeKey }, userKey)).toEqual({ key: bookKey });
  ...
  test('strictly sets favorite state through the repository and publishes the updated summary', async () => {
    ...
    await expect(service.setFavorite(bookKey, { teamKey, scopeKey, isFavorite: true }, userKey)).resolves.toMatchObject({ key: bookKey, isFavorite: true });
    expect(calls).toEqual([[{ teamKey, scopeKey, userKey }, bookKey, true, now], ['publish', scopeKey]]);
    await expect(service.setFavorite(bookKey, { teamKey, scopeKey, isFavorite: false, forged: true }, userKey)).rejects.toThrow('Unrecognized key');
  });
  ...
  test('does not publish when authorized hard deletion fails', async () => {
    ...
    await expect(service.delete(bookKey, { teamKey, scopeKey }, userKey)).rejects.toThrow('forbidden');
  ...
  test('computes listening progress from audio duration rather than chapter count', async () => {
    ...
    expect((await service.detail(bookKey, { teamKey, scopeKey }, userKey)).book).toMatchObject({ progressPercent: 50, currentChapterKey: second });
  });
  ...
  test('does not presign chapter media for overview summaries', async () => {
    ...
    await service.overview({ teamKey, scopeKey }, userKey);
    expect(signed).toEqual(['cover']);
    signed.length = 0;
    await service.detail(bookKey, { teamKey, scopeKey }, userKey);
