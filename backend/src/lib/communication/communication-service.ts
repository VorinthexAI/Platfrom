...
export type CommunicationErrorCode = 'forbidden' | 'not_found' | 'conflict';
...
export class CommunicationService {
  ...
  async listMessages(actor: CommunicationActor, channelKey: string, limit = 100) {
  ...
  async persistUserMessage(actor: CommunicationActor, channelKey: string, content: string, threadKey?: string, replyToMessageKey?: string) {
    const access = await this.requireChannel(actor, channelKey);
  ...
  async resolveOrchestrators(actor: CommunicationActor, channelKey: string, content: string) {
    const access = await this.requireChannel(actor, channelKey);
  ...
  async react(actor: CommunicationActor, channelKey: string, messageKey: string, reaction: string, mode: 'add' | 'remove' | 'toggle') {
    const access = await this.requireChannel(actor, channelKey);
    await this.requireMessage(channelKey, messageKey);
    ...
    if (!result) throw new CommunicationError('not_found', 'message not found');
    if (result.active && result.changed) {
      try {
        await this.repository.recordUserReaction(access.viewerUserKey, reaction, this.now());
      } catch (error) {
        console.error('communication reaction usage recording failed', { userKey: access.viewerUserKey, reaction, error });
      }
    }
    return { active: result.active };
  }

  async frequentReactions(actor: CommunicationActor, limit = 10) {
    const access = await this.generalChannel(actor);
    return this.repository.listUserReactions(access.viewerUserKey, limit);
  }

  async readReplies(actor: CommunicationActor, channelKey: string, parentMessageKey: string) {
    const access = await this.requireChannel(actor, channelKey);
  ...
  async createPoll(actor: CommunicationActor, channelKey: string, messageKey: string, question: string, optionTexts: string[], allowMultiple: boolean) {
    const access = await this.requireChannel(actor, channelKey);
    await this.requireMessage(channelKey, messageKey);
    const now = this.now();
    ...
    try {
      ...
      if (error instanceof CommunicationConflictError) throw new CommunicationError('conflict', error.message);
      throw error;
    }
  }

  async readPoll(actor: CommunicationActor, channelKey: string, pollKey: string) {
    const access = await this.requireChannel(actor, channelKey);
    const poll = await this.repository.getPollProjection(pollKey, channelKey, access.humanParticipant.key);
    if (!poll) throw new CommunicationError('not_found', 'poll not found');
    return poll;
  }

  async votePoll(actor: CommunicationActor, channelKey: string, pollKey: string, optionKey: string) {
    const access = await this.requireChannel(actor, channelKey);
    const poll = await this.repository.getPollProjection(pollKey, channelKey, access.humanParticipant.key);
    if (!poll) throw new CommunicationError('not_found', 'poll not found');
    if (poll.status !== 'open') throw new CommunicationError('conflict', 'poll is closed');
    if (!poll.options.some((option) => option.key === optionKey)) throw new CommunicationError('not_found', 'poll option not found');
    const now = this.now();
    ...
    if (result.outcome === 'not_found') throw new CommunicationError('not_found', 'poll or option not found');
    if (result.outcome === 'conflict') throw new CommunicationError('conflict', 'poll is closed');
    return result.poll;
  }

  async closePoll(actor: CommunicationActor, channelKey: string, pollKey: string) {
    const access = await this.requireChannel(actor, channelKey);
    const poll = await this.repository.closePoll(pollKey, channelKey, access.humanParticipant.key, this.now());
    if (!poll) throw new CommunicationError('forbidden', 'only the poll creator may close this poll');
    return poll;
  }
  ...
  private async requireMessage(channelKey: string, messageKey: string) {
    ...
    if (!message || message.channelKey !== channelKey) throw new CommunicationError('not_found', 'message not found');
    return message;
  }
  ...
  private async resolveReplyContext(channelKey: string, requestedThreadKey?: string, replyToMessageKey?: string) {
    ...
    if (replyToMessageKey) {
      ...
      if (threadKey && threadKey !== derivedThreadKey) throw new CommunicationError('conflict', 'reply target must remain in the same reply group');
      threadKey = derivedThreadKey;
    }
    if (threadKey) {
      const legacyReplyGroup = await this.repository.getThread(threadKey);
      if (!legacyReplyGroup || legacyReplyGroup.channelKey !== channelKey) throw new CommunicationError('not_found', 'reply group not found');
      if (legacyReplyGroup.status !== 'open') throw new CommunicationError('conflict', 'replies are closed');
    }
    return threadKey;
  }
