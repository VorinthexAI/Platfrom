  ...
  async ensureGeneralChannel(organizationKey, membershipKey) {
    return withTransaction({
      ...
      const candidates = await trx.query<Record<string, any>>(`
        LET people = (FOR membership IN userOrganizations FILTER membership.organizationId == @organizationKey && membership.status == "active" COLLECT userKey = membership.userId INTO memberships = membership LET member = FIRST(memberships) LET user = DOCUMENT(users, userKey) FILTER user != null LET participant = FIRST(FOR item IN channelParticipants FILTER item.channelKey == @channelKey && item.userOrganizationKey == member._key LIMIT 1 RETURN item) RETURN { membershipKey: member._key, participantKey: participant == null ? null : participant._key, type: "user", key: user._key, name: NOT_NULL(user.name, user.alias, user.email, "Member"), mentionCount: 0 })
