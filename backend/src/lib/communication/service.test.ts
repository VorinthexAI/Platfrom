...
describe('communication referential validation', () => {
  ...
  test('rejects a message authored by a participant from another channel', async () => {
    ...
    const channel = channelSchema.parse({ key: newId(), teamKey: newId(), scopeKey, name: 'engineering', position: 0, createdAt: now, updatedAt: now });
    const participant = channelParticipantSchema.parse({ key: newId(), scopeKey, channelKey: newId(), userTeamKey: newId(), joinedAt: now, createdAt: now, updatedAt: now });
