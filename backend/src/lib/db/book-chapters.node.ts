...
export const bookChapterSchema = z.object({
  ...
  title: z.string().trim().min(1), description: z.string().trim().min(1), objective: z.string().trim().min(1), visualPlan: z.string().trim().min(1).optional(), promptGuidance: z.string().trim().min(1).optional(), topics: z.array(z.string().trim().min(1)).max(20).default([]),
  evidenceKeyPoints: z.array(z.string().trim().min(1)).min(1).max(12), priorTransition: z.string().trim().min(1), nextTransition: z.string().trim().min(1), repetitionBoundaries: z.array(z.string().trim().min(1)).min(1).max(12), targetWordMin: z.literal(BOOK_CHAPTER_WORD_MIN), targetWordMax: z.literal(BOOK_CHAPTER_WORD_MAX),
  ...
  summaryInputHash: z.string().regex(/^[a-f0-9]{64}$/).optional(), draftInputHash: z.string().regex(/^[a-f0-9]{64}$/).optional(), finalizationInputHash: z.string().regex(/^[a-f0-9]{64}$/).optional(), audioInputHash: z.string().regex(/^[a-f0-9]{64}$/).optional(), imageInputHash: z.string().regex(/^[a-f0-9]{64}$/).optional(),
