...
export const fixedChargeReceiptSchema = z.object({
  userKey: z.string().cuid(), toolSlug: z.enum(['book.create', 'book.extend']), transactionKey: z.string().cuid(), executionIdentity: z.string().regex(/^[a-f0-9]{64}$/), microSparks: z.number().int().positive(), replayed: z.boolean(),
}).strict();
...
export const bookGenerationInputSchema = z.object({
  ...
  archiveDocumentKeys: z.array(z.string().cuid()).max(50), narratorVoiceKey: bookNarratorVoiceSchema, narrationPace: bookNarrationPaceSchema, additionalInstructions: z.string().trim().max(12_000).optional(),
}).strict();
export const bookSchema = z.object({
  key: z.string().cuid(), scopeKey: z.string().cuid(), title: z.string().trim().min(1), subtitle: z.string().trim().min(1).optional(),
  description: z.string().trim().min(1), summary: z.string().trim().min(1).optional(), coverVisualPlan: z.string().trim().min(1).optional(), goal: z.string().trim().min(1), audience: z.string().trim().min(1), outcome: z.string().trim().min(1), narratorVoiceKey: bookNarratorVoiceSchema.optional(), narrationPace: bookNarrationPaceSchema.optional(),
  ...
  archiveFolderKey: z.string().cuid().optional(), activeExtensionKey: z.string().cuid().optional(), fixedChargeReceipt: fixedChargeReceiptSchema.optional(), createdAt: z.string().datetime(), updatedAt: z.string().datetime(),
});
