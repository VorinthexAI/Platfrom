...
export async function provisionPersonalAuthContext(user: { key: string; name: string | null; email: string }): Promise<PersonalAuthContext> {
  ...
  const result = await withTransaction(
    ['organizations', 'userOrganizations', 'scopes', 'scopeMembers', 'agents', 'skills', 'agentSkills', 'scopeAgents', 'agentMembers'],
    async (transaction) => {
