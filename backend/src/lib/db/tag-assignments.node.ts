...
export const gallerySourceTypeSchema = z.enum(['document', 'image', 'collection']);
export const tagAssignmentSourceSchema = z.enum(['user', 'ai']);
export const tagAssignmentSchema = z.object({ key: z.string().cuid(), scopeKey: z.string().cuid(), tagKey: z.string().cuid(), sourceType: gallerySourceTypeSchema, sourceKey: z.string().cuid(), source: tagAssignmentSourceSchema.default('user'), createdAt: z.string().datetime() });
...
export async function listTagAssignmentsByScope(scopeKey: string, sourceType?: z.infer<typeof gallerySourceTypeSchema>, sourceKey?: string): Promise<TagAssignment[]> {
