...
export const tripSchema = z.object({
  ...
  embedding: rolloutEmbeddingSchema, createdAt: z.string().datetime(), updatedAt: z.string().datetime(),
...
export async function listTripsByScope(scopeKey: string): Promise<Trip[]> {
  const cursor = await db.query(aql`FOR trip IN ${db.collection(TRIPS_COLLECTION)} FILTER trip.scopeKey == ${scopeKey} SORT trip.startDate ASC, trip.name ASC, trip._key ASC RETURN trip`);
