...
describe('visual identity records', () => {
  test('validates an active identity and an image confidence relation', () => {
    ...
    expect(visualIdentitySchema.parse({ key: identityKey, scopeKey, createdByKey: newId(), name: 'Viggo', description: 'A black dog with a white chest blaze.', referenceImageKey: imageKey, embedding: Array(EMBEDDING_DIMENSIONS).fill(0.1), createdAt: now, updatedAt: now })).toMatchObject({ name: 'Viggo' });
