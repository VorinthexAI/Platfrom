...
export async function executeEmailAsk<TOutput = ChatOutput>(teamKey: string, rawInput: CoreChatInput, options: ExecuteActionOptions = {}) {
...
export async function executeEmailEmbedding(teamKey: string, rawInput: EmbedTextInput, options: ExecuteActionOptions = {}) {
  const input = emailEmbeddingInputSchema.parse({ text: rawInput.text, purpose: rawInput.purpose });
  const response = await executeAction<{ text: string }, { embedding: number[] }>({ mode: 'auto', teamKey, actionSlug: 'embed' }, { text: prepareEmbeddingText(input.text, input.purpose) }, { ...options, signal: rawInput.signal ?? options.signal, timeoutMs: rawInput.timeoutMs ?? options.timeoutMs });
