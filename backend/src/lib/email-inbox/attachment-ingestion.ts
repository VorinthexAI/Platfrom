...
import { ensureMailInboxFilesFolder, mailInboxFilesFolderKey } from './folders';
...
export interface EmailAttachmentRepository {
  ...
  ensureDocumentFolder(scopeKey: string, connectorKey: string, now: string): Promise<string>;
...
export function createEmailAttachmentRepository(database = db): EmailAttachmentRepository {
  return {
    ...
    async ensureDocumentFolder(scopeKey, connectorKey, now) {
      try { return await ensureMailInboxFilesFolder(database, scopeKey, connectorKey, now); }
      catch { throw new EmailAttachmentIngestionError('ATTACHMENT_PERSIST_FAILED', 'The managed inbox Files folder could not be created.', true); }
    },
...
function canonicalImageName(filename: string) { return `${filename.replace(/\.[^.]+$/, '').slice(0, 250) || 'attachment'}.png`; }
...
export function createEmailAttachmentIngestionService(dependencies: EmailAttachmentIngestionDependencies = {}) {
  ...
  return {
    async ingest(input: { organizationKey: string; scopeKey: string; membershipKey: string; connectorKey: string; providerMessageId: string; part: GmailAttachmentPart; bytes: Uint8Array; connectorLeaseToken?: string; heartbeat?: () => Promise<void>; deferCompletion?: boolean }): Promise<any> {
      ...
      const documentFolderKey = mailInboxFilesFolderKey(input.scopeKey, input.connectorKey);
      ...
      try {
        ...
        if (input.part.type === 'document') {
          const folderKey = await repository.ensureDocumentFolder(input.scopeKey, input.connectorKey, timestamp);
          ...
          await process({ scopeKey: input.scopeKey, ownerKey: input.membershipKey, imageKey: targetKey, idempotencyKey: bindingKey, file: { filename: canonicalImageName(input.part.filename), mimeType: 'image/png', sizeBytes: sanitized.bytes.byteLength, bytes: sanitized.bytes }, mutationPolicy: 'system-only' }, { ...dependencies.imageDependencies, persistImage: dependencies.imageDependencies?.persistImage ?? insertPreparedImageWithCaption });
          targetPersisted = true;
        }
        await heartbeat();
        if (heartbeatFailure) throw heartbeatFailure;
