...
type AttachmentSource = { key: string; teamKey: string; scopeKey: string; connectorKey: string; providerMessageId: string; partPath: string; contentHash: string; sourceMimeType: string; sourceFilename: string; sourceSize: number; targetType: 'document' | 'image'; targetKey: string };
...
export interface EmailAttachmentRepository {
  activeMembership(input: { teamKey: string; scopeKey: string; preferredTeamMembershipKey: string }): Promise<string>;
  completed(input: Omit<AttachmentSource, 'contentHash' | 'targetKey'>, teamMembershipKey: string): Promise<AttachmentView | null>;
  claim(input: AttachmentSource, teamMembershipKey: string, leaseToken: string, now: string, leaseExpiresAt: string): Promise<{ status: 'claimed' | 'replay'; binding: AttachmentView }>;
  ...
  complete(bindingKey: string, leaseToken: string, targetType: 'document' | 'image', targetKey: string, collectionKey: string | undefined, teamMembershipKey: string, now: string): Promise<boolean>;
...
export interface StagedEmailAttachment { bindingKey: string; leaseToken: string; targetType: 'document' | 'image'; targetKey: string; collectionKey?: string; teamMembershipKey: string; storageKey?: string }
...
export function createEmailAttachmentRepository(database = db): EmailAttachmentRepository {
  const authorize = `LET member = DOCUMENT(userTeams, @teamMembershipKey) LET scope = DOCUMENT(scopes, @scopeKey) LET scopeMember = FIRST(FOR item IN scopeMembers FILTER item.scopeKey == @scopeKey && item.userTeamKey == @teamMembershipKey && item.status == "active" LIMIT 1 RETURN item) FILTER member != null && member.status == "active" && member.teamKey == @teamKey && scope != null && scope.teamKey == @teamKey FILTER member.teamRole IN ["owner", "admin"] || scopeMember != null`;
  return {
...
export function createEmailAttachmentIngestionService(dependencies: EmailAttachmentIngestionDependencies = {}) {
  ...
  const bestEffortExport = async (input: { scopeKey: string; connectorKey: string; teamMembershipKey: string; billingUserKey?: string; bindingKey: string; part: GmailAttachmentPart; bytes: Uint8Array }) => {
    try {
      const containers = await ensureExportContainers(exportDatabase, input.scopeKey, input.connectorKey, input.teamMembershipKey, now().toISOString());
      if (input.part.type === 'document') {
        ...
        await (dependencies.processImage ?? processImage)({ scopeKey: input.scopeKey, ownerKey: input.teamMembershipKey, ...(input.billingUserKey ? { billingUserKey: input.billingUserKey } : {}), origin: 'uploaded', imageKey: exportKey, idempotencyKey: `export:${input.bindingKey}`, file: { filename: `${safeSegment(input.part.filename.replace(/\.[^.]+$/, ''))}.png`, mimeType: 'image/png', sizeBytes: sanitized.bytes.byteLength, bytes: sanitized.bytes }, mutationPolicy: 'user' }, { ...dependencies.imageDependencies, persistImage: async ({ image, caption }) => {
          const bindVars = { imageKey: exportKey, image: toArangoDoc(imageSchema.parse({ ...image, key: exportKey, mutationPolicy: 'user' })), scopeKey: input.scopeKey, collectionKey: containers.collectionKey, relationKey: stableKey('email-gallery-export-relation', input.bindingKey), teamMembershipKey: input.teamMembershipKey, now: now().toISOString() };
  ...
  const ingest = async (input: { teamKey: string; scopeKey: string; teamMembershipKey: string; billingUserKey?: string; connectorKey: string; providerMessageId: string; part: GmailAttachmentPart; bytes: Uint8Array; connectorLeaseToken?: string; heartbeat?: () => Promise<void>; deferCompletion?: boolean }): Promise<any> => {
    ...
    const claim = await repository.claim({ key: bindingKey, teamKey: input.teamKey, scopeKey: input.scopeKey, connectorKey: input.connectorKey, providerMessageId: input.providerMessageId, partPath: input.part.path, contentHash, sourceMimeType: input.part.mimeType, sourceFilename: input.part.filename, sourceSize: input.part.size, targetType: input.part.type, targetKey: bindingKey }, input.teamMembershipKey, leaseToken, timestamp, new Date(now().getTime() + PROCESSING_LEASE_MS).toISOString());
    ...
    try {
      ...
      await bestEffortExport({ ...input, bindingKey });
      const staged = { bindingKey, leaseToken, targetType: input.part.type, targetKey: bindingKey, teamMembershipKey: input.teamMembershipKey, storageKey };
      if (input.deferCompletion) return { ref: { type: input.part.type, key: bindingKey }, staged };
      if (!await repository.complete(bindingKey, leaseToken, input.part.type, bindingKey, undefined, input.teamMembershipKey, now().toISOString())) throw new EmailAttachmentIngestionError('ATTACHMENT_ACCESS_REVOKED', 'Attachment ingestion lost its lease before completion.', true);
  ...
  return {
    ingest,
    async ingestMessage(input: { teamKey: string; scopeKey: string; teamMembershipKey: string; billingUserKey?: string; connectorKey: string; gmail: GmailClient; message: GmailMessageResource }) { const refs: EmailAttachmentRef[] = [], parts = gmailAttachmentParts(input.message.payload); if (!parts.length) return refs; const teamMembershipKey = await repository.activeMembership({ teamKey: input.teamKey, scopeKey: input.scopeKey, preferredTeamMembershipKey: input.teamMembershipKey }); for (const part of parts) { const key = stableKey('email-attachment-binding', input.scopeKey, input.connectorKey, input.message.id, part.path), completed = await repository.completed({ key, teamKey: input.teamKey, scopeKey: input.scopeKey, connectorKey: input.connectorKey, providerMessageId: input.message.id, partPath: part.path, sourceMimeType: part.mimeType, sourceFilename: part.filename, sourceSize: part.size, targetType: part.type }, teamMembershipKey); if (completed) refs.push({ type: completed.targetType, key }); else try { refs.push(await ingest({ ...input, teamMembershipKey, providerMessageId: input.message.id, part, bytes: await input.gmail.attachment(input.message.id, part) })); } catch (caught) { if (!(caught instanceof GmailPermanentAttachmentError)) throw caught; } } return refs; },
    async stageMessage(input: { teamKey: string; scopeKey: string; teamMembershipKey: string; billingUserKey?: string; connectorKey: string; connectorLeaseToken: string; heartbeat: () => Promise<void>; gmail: GmailClient; message: GmailMessageResource }): Promise<StagedEmailAttachments> { const discovery = discoverGmailAttachmentParts(input.message.payload), hasAttachments = messageBodies(input.message.payload).hasAttachments, result: StagedEmailAttachments = { refs: [], staged: [], availability: hasAttachments ? discovery.truncated ? 'truncated' : 'complete' : 'none', ...(discovery.unavailableCount ? { unavailableCount: discovery.unavailableCount } : {}) }; if (!discovery.parts.length) return result; const teamMembershipKey = await repository.activeMembership({ teamKey: input.teamKey, scopeKey: input.scopeKey, preferredTeamMembershipKey: input.teamMembershipKey }); try { for (const part of discovery.parts) { const value = await ingest({ ...input, teamMembershipKey, providerMessageId: input.message.id, part, bytes: await input.gmail.attachment(input.message.id, part), deferCompletion: true }); result.refs.push(value.ref); if (value.staged) result.staged.push(value.staged); } return result; } catch (caught) { await compensate(result.staged, input.scopeKey); throw caught; } },
