...
} from "@/lib/ai/document-processing/storage";
import {
  parseDocument,
...
} from "@/lib/db/email-attachments.node";
import { EMBEDDING_DIMENSIONS } from "@/lib/embeddings";
import {
  discoverGmailAttachmentParts,
  GmailPermanentAttachmentError,
...
export {
  emailArchiveInboxFolderKey,
  emailArchiveRootFolderKey,
  emailMediaCollectionKey,
} from "./export-container-keys";
...
type AttachmentView = AttachmentSource & {
...
const view = (attachment: EmailAttachment): AttachmentView => ({
  key: attachment.key,
  userKey: attachment.userKey,
  teamKey: attachment.teamKey,
  scopeKey: attachment.scopeKey,
  connectorKey: attachment.connectorKey,
  providerMessageId: attachment.providerMessageId,
  partPath: attachment.partPath,
  contentHash: attachment.contentHash,
  sourceMimeType: attachment.mimeType,
  sourceFilename: attachment.filename,
  sourceSize: attachment.sizeBytes,
  targetType: attachment.kind,
  targetKey: attachment.key,
  status: attachment.status,
  ...(attachment.leaseToken ? { leaseToken: attachment.leaseToken } : {}),
...
export interface EmailAttachmentRepository {
  ...
  activeMembership(input: {
    teamKey: string;
    scopeKey: string;
    preferredTeamMembershipKey: string;
  }): Promise<string>;
  completed(
    input: Omit<AttachmentSource, "contentHash" | "targetKey">,
  ...
  claim(
    ...
    now: string,
    leaseExpiresAt: string,
  ): Promise<{ status: "claimed" | "replay"; binding: AttachmentView }>;
  persistStorage(
    bindingKey: string,
    leaseToken: string,
    storageKey: string,
...
export function createEmailAttachmentRepository(
  database = db,
  ...
  return {
    ...
    async activeMembership(input) {
      ...
      const key = (await cursor.next()) as string | undefined;
      if (!key)
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_ACCESS_REVOKED",
          "No active destination membership is available for attachment export.",
          false,
        );
      return key;
    },
    async completed(input, _teamMembershipKey) {
      ...
      if (raw === undefined)
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_ACCESS_REVOKED",
          "The authenticated user no longer owns this connector.",
          false,
        );
      if (!raw) return null;
      const value = parse(raw);
      if (
        value.userKey !== input.userKey ||
        value.connectorKey !== input.connectorKey ||
        value.providerMessageId !== input.providerMessageId ||
        value.partPath !== input.partPath ||
        value.kind !== input.targetType ||
        value.mimeType !== input.sourceMimeType ||
        value.filename !== input.sourceFilename ||
        value.sizeBytes !== input.sourceSize
      )
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_CONFLICT",
          "The Gmail attachment source identity resolved to different metadata.",
          false,
        );
      return view(value);
    },
    async claim(input, _teamMembershipKey, leaseToken, now, leaseExpiresAt) {
      ...
      const existing = existingRaw ? parse(existingRaw) : null;
      if (
        existing &&
        (existing.userKey !== input.userKey ||
          existing.connectorKey !== input.connectorKey ||
          existing.providerMessageId !== input.providerMessageId ||
          existing.partPath !== input.partPath ||
          existing.kind !== input.targetType ||
          existing.contentHash !== input.contentHash)
      )
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_CONFLICT",
          "The Gmail attachment source identity resolved to different bytes or metadata.",
          false,
        );
      if (existing?.status === "completed")
        return { status: "replay", binding: view(existing) };
      if (existing?.leaseExpiresAt && existing.leaseExpiresAt > now)
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_BUSY",
          "The Gmail attachment is already processing.",
          true,
        );
      const value = emailAttachmentSchema.parse({
        key: input.key,
        userKey: input.userKey,
      ...
      const cursor = await database.query(
        'UPSERT { _key: @key } INSERT @value UPDATE (OLD.status == "processing" && OLD.leaseExpiresAt <= @now ? MERGE(@value, { createdAt: OLD.createdAt, storageKey: OLD.storageKey }) : {}) IN @@attachments OPTIONS { keepNull: false } RETURN NEW',
        {
          "@attachments": EMAIL_ATTACHMENTS_COLLECTION,
          key: input.key,
      ...
      const claimed = parse(await cursor.next());
      ...
      return { status: "claimed", binding: view(claimed) };
    },
    async persistStorage(bindingKey, leaseToken, storageKey, now) {
      const cursor = await database.query(
        'FOR value IN @@attachments FILTER value._key == @key && value.status == "processing" && value.leaseToken == @token && value.leaseExpiresAt > @now UPDATE value WITH { storageKey: @storageKey, updatedAt: @now } IN @@attachments RETURN true',
        {
          "@attachments": EMAIL_ATTACHMENTS_COLLECTION,
          key: bindingKey,
          token: leaseToken,
    ...
    async renew(
      ...
      const cursor = await database.query(
        'LET value = DOCUMENT(@@attachments, @key) LET connector = value == null ? null : DOCUMENT(userConnectors, value.connectorKey) FILTER value != null && value.status == "processing" && value.leaseToken == @token && value.leaseExpiresAt > @now && connector != null && connector.userKey == value.userKey && connector.syncLeaseToken == @connectorToken && connector.syncLeaseExpiresAt > @now UPDATE value WITH { leaseExpiresAt: @leaseExpiresAt, updatedAt: @now } IN @@attachments RETURN true',
        {
          "@attachments": EMAIL_ATTACHMENTS_COLLECTION,
          key: bindingKey,
          token: leaseToken,
          connectorToken: connectorLeaseToken,
          now,
          leaseExpiresAt,
        },
      );
    ...
    async complete(
      ...
      if (bindingKey !== targetKey) return false;
      const cursor = await database.query(
        'FOR value IN @@attachments FILTER value._key == @key && value.kind == @kind && value.status == "processing" && value.leaseToken == @token && value.leaseExpiresAt > @now && IS_STRING(value.storageKey) UPDATE value WITH { status: "completed", leaseToken: null, leaseExpiresAt: null, updatedAt: @now } IN @@attachments OPTIONS { keepNull: false } RETURN true',
        {
          "@attachments": EMAIL_ATTACHMENTS_COLLECTION,
          key: bindingKey,
          kind: targetType,
    ...
    async compensateTarget(
      ...
      if (bindingKey !== targetKey) return;
      await database.query(
        'FOR value IN @@attachments FILTER value._key == @key && value.scopeKey == @scopeKey && value.status == "processing" && value.leaseToken == @token REMOVE value IN @@attachments',
        {
          "@attachments": EMAIL_ATTACHMENTS_COLLECTION,
          key: bindingKey,
          scopeKey,
    ...
    async release(bindingKey, leaseToken) {
      await database.query(
        'FOR value IN @@attachments FILTER value._key == @key && value.status == "processing" && value.leaseToken == @token REMOVE value IN @@attachments',
        {
          "@attachments": EMAIL_ATTACHMENTS_COLLECTION,
          key: bindingKey,
          token: leaseToken,
...
export interface EmailAttachmentIngestionDependencies {
  ...
  publishScopeEvent?: (
    scopeKey: string,
    event: "content.changed" | "image.changed",
  ) => Promise<unknown>;
  publishCollectionEvent?: (
    collectionKey: string,
    event: "collection.content.changed" | "collection.index.changed",
  ) => Promise<unknown>;
}

async function ensureExportContainers(
  ...
  await database.query('UPSERT { _key: @rootKey } INSERT { _key: @rootKey, scopeKey: @scopeKey, parentFolderKey: @platformKey, name: "Signal", presentation: "communication", mutationPolicy: "user", embedding: @embedding, isFavorite: false, createdAt: @now, updatedAt: @now } UPDATE { presentation: "communication" } IN folders', { rootKey, scopeKey, platformKey: initialWorkspaceFolderKey(scopeKey, 'platform'), embedding, now });
...
export function createEmailAttachmentIngestionService(
  dependencies: EmailAttachmentIngestionDependencies = {},
) {
  const repository =
    dependencies.repository ?? createEmailAttachmentRepository();
  const storage =
  ...
  const bestEffortExport = async (input: {
    ...
    try {
      ...
      if (input.part.type === "document") {
        ...
        const exportStorage = input.billingUserKey
          ? {
              ...storage,
              upload: (value: Parameters<DocumentObjectStorage["upload"]>[0]) =>
                storage.upload({
                  ...value,
          ...
          : storage;
        await (dependencies.parse ?? parseDocument)(
          {
            file: {
          ...
          },
          {
            ...
            insert: async (document: Document) => {
              ...
              const cursor = await exportDatabase.query(
                "UPSERT { _key: @key } INSERT @value UPDATE {} IN documents RETURN NEW",
                { key: exportKey, value: toArangoDoc(value) },
              );
              return documentSchema.parse(withArangoKey(await cursor.next()));
            },
          },
        );
      } else {
  ...
  const compensate = async (
    ...
    for (const item of [...staged].reverse())
      try {
        await repository.compensateTarget(
          item.bindingKey,
          item.leaseToken,
          item.targetType,
          item.targetKey,
    ...
    if (failures.length)
      throw new AggregateError(
        failures,
        "One or more staged email attachments could not be compensated",
      );
  };
  const ingest = async (input: {
    ...
    const contentHash = createHash("sha256").update(input.bytes).digest("hex"),
      bindingKey = stableKey(
        "email-attachment-binding",
        input.userKey,
        input.connectorKey,
    ...
    const claim = await repository.claim(
      {
        ...
        connectorKey: input.connectorKey,
        providerMessageId: input.providerMessageId,
        partPath: input.part.path,
        contentHash,
        sourceMimeType: input.part.mimeType,
        sourceFilename: input.part.filename,
        sourceSize: input.part.size,
        targetType: input.part.type,
        targetKey: bindingKey,
      },
      input.teamMembershipKey,
      leaseToken,
      timestamp,
    ...
    try {
      if (!storageKey)
        storageKey = (
          await storage.upload({
            key: `email/${input.scopeKey}/${input.connectorKey}/${bindingKey}/${contentHash}/${safeSegment(input.part.filename)}`,
            bytes: input.bytes,
            mimeType: input.part.mimeType,
            ...(input.billingUserKey
              ? { billingUserKey: input.billingUserKey }
      ...
      if (
        !(await repository.persistStorage(
          bindingKey,
          leaseToken,
          storageKey,
          now().toISOString(),
        ...
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_ACCESS_REVOKED",
          "Attachment ingestion lost its lease before canonical persistence.",
          true,
        );
      await input.heartbeat?.();
      if (
        input.connectorLeaseToken &&
        ...
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_ACCESS_REVOKED",
          "Attachment ingestion lost its connector lease fence.",
          true,
        );
      ...
      if (input.deferCompletion)
        return { ref: { type: input.part.type, key: bindingKey }, staged };
      if (
        ...
        throw new EmailAttachmentIngestionError(
          "ATTACHMENT_ACCESS_REVOKED",
          "Attachment ingestion lost its lease before completion.",
          true,
        );
      ...
      await repository.release(bindingKey, leaseToken).catch(() => undefined);
      throw caught;
    }
  };
  return {
    ...
    async ingestMessage(input: {
      ...
      const refs: EmailAttachmentRef[] = [],
        parts = gmailAttachmentParts(input.message.payload);
      if (!parts.length) return refs;
      const teamMembershipKey = await repository.activeMembership({
        teamKey: input.teamKey,
        scopeKey: input.scopeKey,
        preferredTeamMembershipKey: input.teamMembershipKey,
      });
      for (const part of parts) {
        const key = stableKey(
          ...
          completed = await repository.completed(
            {
              key,
              userKey: input.userKey,
              teamKey: input.teamKey,
              ...
              targetType: part.type,
            },
            teamMembershipKey,
          );
        if (completed) refs.push({ type: completed.targetType, key });
        else
          try {
            refs.push(
              await ingest({
            ...
            if (!(caught instanceof GmailPermanentAttachmentError))
              throw caught;
          }
      }
      return refs;
    ...
    async stageMessage(input: {
      ...
      gmail: GmailClient;
      message: GmailMessageResource;
    }): Promise<StagedEmailAttachments> {
      const discovery = discoverGmailAttachmentParts(input.message.payload),
        hasAttachments = messageBodies(input.message.payload).hasAttachments,
        result: StagedEmailAttachments = {
          refs: [],
          staged: [],
          availability: hasAttachments
            ? discovery.truncated
              ? "truncated"
              : "complete"
            : "none",
          ...(discovery.unavailableCount
            ? { unavailableCount: discovery.unavailableCount }
            : {}),
        };
      if (!discovery.parts.length) return result;
      const teamMembershipKey = await repository.activeMembership({
        teamKey: input.teamKey,
        scopeKey: input.scopeKey,
        preferredTeamMembershipKey: input.teamMembershipKey,
    ...
    renew: async (
      ...
      for (const item of staged)
        if (
          ...
          throw new EmailAttachmentIngestionError(
            "ATTACHMENT_ACCESS_REVOKED",
            "Attachment processing lease was lost before mail persistence.",
            true,
          );
    },
    compensate,
...
export type EmailAttachmentIngestionService = ReturnType<
  typeof createEmailAttachmentIngestionService
>;
