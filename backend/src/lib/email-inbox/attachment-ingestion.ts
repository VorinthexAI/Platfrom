...
export function createEmailAttachmentIngestionService(dependencies: EmailAttachmentIngestionDependencies = {}) {
  ...
  const bestEffortExport = async (input: { scopeKey: string; connectorKey: string; membershipKey: string; bindingKey: string; part: GmailAttachmentPart; bytes: Uint8Array }) => {
    try {
  ...
  const ingest = async (input: { organizationKey: string; scopeKey: string; membershipKey: string; connectorKey: string; providerMessageId: string; part: GmailAttachmentPart; bytes: Uint8Array; connectorLeaseToken?: string; heartbeat?: () => Promise<void>; deferCompletion?: boolean }): Promise<any> => {
    ...
    try {
      if (!storageKey) storageKey = (await storage.upload({ key: `email/${input.scopeKey}/${input.connectorKey}/${bindingKey}/${contentHash}/${safeSegment(input.part.filename)}`, bytes: input.bytes, mimeType: input.part.mimeType })).storageKey;
