...
import { emailAttachmentRefsSchema, emailDraftPayloadSchema, emailToneDataSchema, type EmailDraft, type EmailDraftCreate, type EmailAttachmentRef, type EmailMessage, type EmailReplyContext, type EmailThread, type EmailTone } from './archive-payloads';
...
import type { EmailCreatedAtRange, EmailOverviewRepositoryQuery } from './repository';
import { INBOX_NEW_EMAIL_SPARKS, MICRO_SPARKS_PER_SPARK } from '@/lib/costs';
...
export function createCanonicalEmailRepository(database: Database, error: RepositoryError, stableKey: StableKey, publishBalance: (userKey: string) => Promise<void> = async () => {}) {
  ...
  const transaction = <T>(write: string[], operation: (executor: Pick<typeof db, 'query'>) => Promise<T>) => database.beginTransaction
    ? withDatabaseTransaction(database as typeof db, { read: [TEAM_CONNECTORS_COLLECTION], write }, operation)
    : operation(database);
  ...
  const draftRecord = (input: EmailDraftCreate & { userKey: string }, key: string, timestamp: string) => {
    const kind = input.variant === 'new' ? 'mail-new-draft' : 'mail-reply-draft';
    const { userKey, scopeKey, embedding, ...draft } = input;
    const data = emailDraftPayloadSchema.parse({ version: 1, kind, data: draft }).data;
    return emailDraftRecordSchema.parse({ ...data, key, userKey, scopeKey, embedding, createdAt: timestamp, updatedAt: timestamp });
  };
  const exportThread = async (thread: z.infer<typeof emailThreadRecordSchema>, messages: Array<z.infer<typeof emailMessageRecordSchema>>, timestamp: string, representations?: { thread?: PreparedDocumentRepresentation; messages?: Array<PreparedDocumentRepresentation | undefined> }) => {
    try {
      const destination = await database.query('LET connector = DOCUMENT(userConnectors, @connectorKey) FILTER connector != null && connector.userKey == @userKey RETURN connector.scopeKey', { connectorKey: thread.accountKey, userKey: thread.userKey });
      const destinationScopeKey = await destination.next() as string | undefined;
      if (!destinationScopeKey) return;
      const { rootKey, inboxKey } = emailExportContainerKeys(destinationScopeKey, thread.accountKey);
      const embedding = Array(EMBEDDING_DIMENSIONS).fill(0);
      const root = await database.query('UPSERT { _key: @key } INSERT { _key: @key, scopeKey: @scopeKey, parentFolderKey: @platformKey, name: "Signal", presentation: "communication", mutationPolicy: "system-container", embedding: @embedding, isFavorite: false, createdAt: @now, updatedAt: @now } UPDATE { parentFolderKey: @platformKey, presentation: "communication", mutationPolicy: "system-container" } IN folders RETURN NEW', { key: rootKey, platformKey: initialWorkspaceFolderKey(destinationScopeKey, 'platform'), scopeKey: destinationScopeKey, embedding, now: timestamp });
      const rootValue = await root.next() as { scopeKey?: unknown } | undefined;
      if (rootValue?.scopeKey !== destinationScopeKey) return;
      const inbox = await database.query('LET source = FIRST(FOR value IN emailInboxes FILTER value.userKey == @userKey && value.connectorKey == @connectorKey LIMIT 1 RETURN value) FILTER source != null UPSERT { _key: @key } INSERT { _key: @key, scopeKey: @scopeKey, parentFolderKey: @rootKey, name: source.name, mutationPolicy: "user", embedding: source.embedding, isFavorite: false, createdAt: @now, updatedAt: @now } UPDATE { presentation: null } IN folders OPTIONS { keepNull: false } RETURN NEW', { key: inboxKey, rootKey, userKey: thread.userKey, scopeKey: destinationScopeKey, connectorKey: thread.accountKey, now: timestamp });
      const inboxValue = await inbox.next() as { scopeKey?: unknown } | undefined;
      if (inboxValue?.scopeKey !== destinationScopeKey) return;
      const exports = [
        exportEmailThreadToArchive(thread, { scopeKey: destinationScopeKey, exportKey: stableKey('email-archive-thread-export', thread.key), folderKey: inboxKey, exportedAt: timestamp }, representations?.thread),
        ...messages.map((message, index) => exportEmailMessageToArchive(message, { scopeKey: destinationScopeKey, exportKey: stableKey('email-archive-message-export', message.key), folderKey: inboxKey, exportedAt: timestamp }, representations?.messages?.[index])),
      ];
      await database.query('FOR value IN @values UPSERT { _key: value._key } INSERT value UPDATE MERGE(value, { createdAt: OLD.createdAt }) IN documents', { values: exports.map(toArangoDoc) });
  ...
  return {
    ...
    async syncThread(input: {
      thread: Omit<EmailThread, 'key' | 'createdAt' | 'updatedAt'> & { userKey: string; archiveRepresentation?: PreparedDocumentRepresentation };
      messages: Array<Omit<EmailMessage, 'key' | 'threadKey' | 'createdAt' | 'updatedAt' | 'attachmentAvailability'> & Partial<Pick<EmailMessage, 'attachmentAvailability'>> & { userKey: string; archiveRepresentation?: PreparedDocumentRepresentation }>;
      reconcileMessages?: boolean;
      lease?: { kind: 'sync' | 'send'; connectorKey: string; token: string };
      attachmentCommits?: StagedEmailAttachment[];
      subscriptionBilling?: { userKey: string; providerMessageIds: string[] };
    }) {
      let chargedEmailCount = 0;
      const timestamp = new Date().toISOString();
      const archiveRepresentations = { thread: input.thread.archiveRepresentation, messages: input.messages.map(({ archiveRepresentation }) => archiveRepresentation) };
      ...
      const stored = await transaction([EMAIL_THREADS_COLLECTION, EMAIL_MESSAGES_COLLECTION, EMAIL_DRAFTS_COLLECTION, EMAIL_ATTACHMENTS_COLLECTION, 'tagAssignments', ...(input.lease ? [TEAM_CONNECTORS_COLLECTION] : []), ...(input.subscriptionBilling ? ['users', 'sparkTransactions'] : [])], async (trx) => {
        if (input.lease) {
          ...
          const fence = await trx.query('LET connector = DOCUMENT(@@connectors, @key) FILTER connector != null && connector.status != "revoked" && connector.syncEnabled != false && connector[@tokenField] == @token && connector[@expiryField] > @now RETURN true', { '@connectors': TEAM_CONNECTORS_COLLECTION, key: input.lease.connectorKey, token: input.lease.token, tokenField, expiryField, now: timestamp });
          if (await fence.next() !== true) throw error('conflict', `Email ${input.lease.kind} lease was lost before persistence`);
        }
        if (input.subscriptionBilling) {
          const providerMessageIds = [...new Set(input.subscriptionBilling.providerMessageIds)].sort();
          const chargeKey = stableKey('inbox-subscription-charge', input.subscriptionBilling.userKey, input.thread.accountKey, ...providerMessageIds);
          const charge = await trx.query(`
            LET billableIds = (FOR message IN @messages FILTER message.providerMessageId IN @providerMessageIds && DOCUMENT(emailMessages, message._key) == null SORT message.providerMessageId RETURN message.providerMessageId)
            LET amount = LENGTH(billableIds) * @microSparksPerEmail
            LET user = DOCUMENT(users, @userKey)
            FILTER amount == 0 || (user != null && (!IS_NUMBER(user.microSparkDebt) || user.microSparkDebt == 0))
            LET previousBalance = user == null ? 0 : (IS_NUMBER(user.microSparkBalance) ? user.microSparkBalance : 0)
            FILTER amount == 0 || previousBalance >= amount
            LET nextBalance = previousBalance - amount
            LET updated = FIRST(FOR ignored IN amount > 0 ? [1] : [] UPDATE user WITH { microSparkBalance: nextBalance } IN users RETURN NEW)
            LET ledger = FIRST(FOR ignored IN amount > 0 ? [1] : [] INSERT { _key: @chargeKey, userKey: @userKey, kind: "tool", deltaMicroSparks: -amount, idempotencyKey: @idempotencyKey, requestHash: @requestHash, toolSlug: "inbox.subscribe", metadata: { category: "new-email", connectorKey: @connectorKey, scopeKey: @scopeKey, newEmailCount: LENGTH(billableIds) }, balanceAfterMicroSparks: nextBalance, createdAt: @now } IN sparkTransactions RETURN NEW)
            RETURN { chargedEmailCount: LENGTH(billableIds), transactionKey: ledger == null ? null : ledger._key }
          `, {
            messages: messages.map(toArangoDoc), providerMessageIds, userKey: input.subscriptionBilling.userKey,
            microSparksPerEmail: INBOX_NEW_EMAIL_SPARKS * MICRO_SPARKS_PER_SPARK,
            chargeKey, idempotencyKey: `inbox.subscribe:${chargeKey}`,
            requestHash: createHash('sha256').update(JSON.stringify({ connectorKey: input.thread.accountKey, providerMessageIds, microSparksPerEmail: INBOX_NEW_EMAIL_SPARKS * MICRO_SPARKS_PER_SPARK })).digest('hex'),
            connectorKey: input.thread.accountKey, scopeKey: input.thread.scopeKey, now: timestamp,
          });
          const chargeResult = await charge.next() as { chargedEmailCount?: number } | undefined;
          if (!chargeResult) {
            const state = await trx.query('LET user = DOCUMENT(users, @userKey) RETURN user == null ? null : { balance: IS_NUMBER(user.microSparkBalance) ? user.microSparkBalance : 0, debt: IS_NUMBER(user.microSparkDebt) ? user.microSparkDebt : 0 }', { userKey: input.subscriptionBilling.userKey });
            const current = await state.next() as { balance: number; debt: number } | null | undefined;
            if (!current) throw new SparkRepositoryError('USER_NOT_FOUND', 'Connected inbox billing user was not found.');
            if (current.debt > 0) throw new SparkRepositoryError('OUTSTANDING_DEBT', 'Spark spending is blocked until refund debt is resolved.');
            throw new SparkRepositoryError('INSUFFICIENT_BALANCE', 'Spark balance is insufficient to receive these new emails.');
          }
          chargedEmailCount = chargeResult.chargedEmailCount ?? 0;
        }
        ...
        if (input.reconcileMessages !== false) await trx.query('LET removedKeys = (FOR message IN emailMessages FILTER message.scopeKey == @scopeKey && message.threadKey == @threadKey && message._key NOT IN @keep RETURN message._key) LET cleanupTags = (FOR assignment IN tagAssignments FILTER assignment.scopeKey == @scopeKey && assignment.sourceType == "email-message" && assignment.sourceKey IN removedKeys REMOVE assignment IN tagAssignments RETURN 1) FOR message IN emailMessages FILTER message._key IN removedKeys REMOVE message IN emailMessages', { scopeKey: thread.scopeKey, threadKey, keep });
      ...
      if (input.subscriptionBilling && chargedEmailCount > 0) await publishBalance(input.subscriptionBilling.userKey).catch(() => undefined);
      await exportThread(thread, messages, timestamp, archiveRepresentations);
      return stored;
    },
    ...
    async createDraft(input: EmailDraftCreate & { userKey: string }) {
    ...
    async createSubscriptionDraft(input: EmailDraftCreate & { userKey: string; variant: 'reply'; creationSource: 'subscription' }) {
    ...
    async listDraftPage(scopeKey: string, connectorKey: string, input: EmailCreatedAtRange & { offset: number; limit: number }) {
      const cursor = await database.query(`LET eligible = (FOR draft IN emailDrafts FILTER draft.scopeKey == @scopeKey && draft.variant == "reply" && draft.creationSource == "subscription" && draft.status IN ["generated", "edited"] FILTER @createdFrom == null || draft.createdAt >= @createdFrom FILTER @createdTo == null || draft.createdAt <= @createdTo LET thread = DOCUMENT(emailThreads, draft.threadKey) FILTER thread != null && thread.scopeKey == @scopeKey && thread.accountKey == @connectorKey SORT draft.updatedAt DESC, draft._key ASC RETURN draft) RETURN { drafts: SLICE(eligible, @offset, @limit), total: LENGTH(eligible) }`, { scopeKey, connectorKey, offset: input.offset, limit: input.limit, createdFrom: input.createdFrom ?? null, createdTo: input.createdTo ?? null });
      const row = await cursor.next() as { drafts: unknown[]; total: number } | undefined;
      return { drafts: (row?.drafts ?? []).map(parseDraft), total: row?.total ?? 0 };
    },
    ...
    async outboundDraftAttachments(scopeKey: string, connectorKey: string, draftKey: string) {
      const draft = await get(EMAIL_DRAFTS_COLLECTION, parseDraft, scopeKey, draftKey);
      if (!draft || !['sending', 'sent'].includes(draft.status)) return null;
      const accountKey = draft.variant === 'new' ? draft.accountKey : (await get(EMAIL_THREADS_COLLECTION, parseThread, scopeKey, draft.threadKey))?.accountKey;
      if (accountKey !== connectorKey) return null;
      const refs = emailAttachmentRefsSchema.parse(draft.attachments ?? []);
      // Workspace selections must be ingested from the sent MIME message so the
      // email owns its own bytes even if the selected asset is later deleted.
      const cursor = await database.query('FOR ref IN @refs LET item = DOCUMENT(emailAttachments, ref.key) FILTER item != null && item.userKey == @userKey && item.status == "completed" && item.kind == ref.type RETURN ref', { refs, userKey: scopeKey });
      return (await cursor.all()).length === refs.length ? refs : null;
    },
    async searchDrafts(scopeKey: string, connectorKey: string, embedding: number[], query: string, minimumScore: number, limit: number, range: EmailCreatedAtRange = {}) { const threads = new Map((await list(EMAIL_THREADS_COLLECTION, parseThread, scopeKey)).map((thread) => [thread.key, thread])); const normalizedQuery = query.trim().toLowerCase(); return (await list(EMAIL_DRAFTS_COLLECTION, parseDraft, scopeKey)).filter((draft) => (!range.createdFrom || draft.createdAt >= range.createdFrom) && (!range.createdTo || draft.createdAt <= range.createdTo) && ['generated', 'edited'].includes(draft.status) && (draft.variant === 'new' ? draft.accountKey === connectorKey : draft.creationSource === 'subscription' && threads.get(draft.threadKey)?.accountKey === connectorKey)).map((draft) => ({ draft, score: `${draft.variant === 'new' ? draft.subject : ''} ${draft.generatedContent} ${draft.finalContent ?? ''}`.toLowerCase().includes(normalizedQuery) ? 1 : cosineSimilarity(draft.embedding, embedding) })).filter(({ score }) => score >= minimumScore).sort((left, right) => right.score - left.score || right.draft.updatedAt.localeCompare(left.draft.updatedAt) || left.draft.key.localeCompare(right.draft.key)).slice(0, limit); },
    ...
    async deleteDraft(scopeKey: string, draftKey: string) { const cursor = await database.query('LET draft = DOCUMENT(emailDrafts, @key) FILTER draft != null && draft.scopeKey == @scopeKey && draft.status IN ["generated", "edited", "discarded"] LET cleanupTags = (FOR assignment IN tagAssignments FILTER assignment.scopeKey == @scopeKey && assignment.sourceType == "email-draft" && assignment.sourceKey == @key REMOVE assignment IN tagAssignments RETURN 1) REMOVE draft IN emailDrafts RETURN OLD._key', { key: draftKey, scopeKey }); const key = await cursor.next(); if (!key) throw error('conflict', 'Only generated, edited, or discarded drafts can be deleted'); return { deletedKey: key as string, storageKeys: [] as string[] }; },
    ...
    async attachmentResources(userKey: string, refs: EmailAttachmentRef[], destinationScopeKey?: string) {
      const parsed = emailAttachmentRefsSchema.parse(refs);
      const cursor = await database.query(`FOR ref IN @refs
        LET item = DOCUMENT(emailAttachments, ref.key)
        LET asset = item == null && @scopeKey != null ? (ref.type == "document" ? DOCUMENT(documents, ref.key) : DOCUMENT(images, ref.key)) : null
        LET owned = item != null && item.userKey == @userKey && item.status == "completed" && item.kind == ref.type
        LET accessible = asset != null && asset.scopeKey == @scopeKey && asset._internalDeletion == null && asset.archiveVisibility != "domain-only" && (asset.privateOwnerUserKey == null || asset.privateOwnerUserKey == @userKey)
        FILTER owned || accessible
        RETURN owned ? { type: item.kind, key: item._key, name: item.filename, mimeType: item.mimeType, sizeBytes: item.sizeBytes, storageKey: item.storageKey }
          : { type: ref.type, key: ref.key, name: ref.type == "image" ? asset.filename : asset.name, extension: asset.extension, mimeType: asset.mimeType, sizeBytes: asset.sizeBytes, storageKey: asset.storageKey, content: asset.content }`, { userKey, scopeKey: destinationScopeKey ?? null, refs: parsed });
      const values = await cursor.all() as Array<{ type: 'document' | 'image'; key: string; name: string; extension?: string; mimeType?: string; sizeBytes?: number; storageKey?: string; content?: string }>;
      if (values.length !== parsed.length) throw error('forbidden', 'Every attachment must belong to the authenticated user or authorized workspace');
      return values.map((value) => ({ ...value, name: value.type === 'document' && value.extension && !value.name.toLowerCase().endsWith(`.${value.extension}`) ? `${value.name}.${value.extension}` : value.name }));
    },
    async resolveAttachments(userKey: string, refs: EmailAttachmentRef[], destinationScopeKey?: string) { await this.attachmentResources(userKey, refs, destinationScopeKey); return emailAttachmentRefsSchema.parse(refs); },
    async attachmentReferencesForRead(userKey: string, refs: EmailAttachmentRef[]) {
      const cursor = await database.query('FOR ref IN @refs LET item = DOCUMENT(emailAttachments, ref.key) FILTER item == null || (item.userKey == @userKey && item.kind == ref.type && item.status == "completed") RETURN { ref, attachment: item }', { userKey, refs: emailAttachmentRefsSchema.parse(refs) });
      return (await cursor.all() as Array<{ ref: EmailAttachmentRef; attachment: unknown }>).map(({ ref, attachment }) => {
        if (!attachment) return ref;
        const item = emailAttachmentSchema.parse(raw(attachment));
        return { type: ref.type, key: ref.type === 'document' ? item.archiveDocumentKey ?? stableKey('email-archive-export', item.key) : item.galleryImageKey ?? stableKey('email-gallery-export', item.key) };
      });
    },
    async listReplyContext(userKey: string) { const cursor = await database.query('FOR value IN emailReplyContext FILTER value.userKey == @userKey RETURN value', { userKey }); const values = (await cursor.all()).map(parseNote).sort((a, b) => a.createdAt.localeCompare(b.createdAt) || a.key.localeCompare(b.key)); if (values.length > 20 || values.reduce((sum, value) => sum + value.text.length, 0) > 24_000) throw error('conflict', 'Stored reply context exceeds its defensive limits'); return values; },
    async getReplyContext(userKey: string, noteKey: string) { const cursor = await database.query('LET value = DOCUMENT(emailReplyContext, @key) FILTER value != null && value.userKey == @userKey RETURN { value, revision: value._rev }', { key: noteKey, userKey }); const row = await cursor.next() as any; return row ? { note: parseNote(row.value), revision: z.string().min(1).parse(row.revision) } : null; },
    async createReplyContext(userKey: string, scopeKey: string, input: { name: string; text: string; embedding: number[] }) {
      const data = emailReplyContextRecordSchema.pick({ name: true, text: true, embedding: true }).parse(input);
      const now = new Date().toISOString(), value = emailReplyContextRecordSchema.parse({ ...data, key: newId(), userKey, scopeKey, createdAt: now, updatedAt: now });
      const cursor = await database.query('LET notes = (FOR note IN emailReplyContext FILTER note.userKey == @userKey RETURN note) FILTER LENGTH(notes) < 20 && SUM(notes[* RETURN LENGTH(CURRENT.text)]) + LENGTH(@text) <= 24000 INSERT @value IN emailReplyContext RETURN NEW', { userKey, text: value.text, value: toArangoDoc(value) });
      const stored = await cursor.next(); if (!stored) throw error('conflict', 'Reply context is limited to 20 notes and 24,000 text characters'); return parseNote(stored);
    },
    async updateReplyContext(userKey: string, noteKey: string, expectedUpdatedAt: string, expectedRevision: string, input: { name: string; text: string; embedding: number[] }) {
      const data = emailReplyContextRecordSchema.pick({ name: true, text: true, embedding: true }).parse(input), now = new Date().toISOString();
      const cursor = await database.query('LET current = DOCUMENT(emailReplyContext, @key) FILTER current != null && current.userKey == @userKey && current.updatedAt == @expectedUpdatedAt && current._rev == @expectedRevision LET otherLength = SUM(FOR note IN emailReplyContext FILTER note.userKey == @userKey && note._key != @key RETURN LENGTH(note.text)) FILTER otherLength + LENGTH(@text) <= 24000 UPDATE current WITH { name: @name, text: @text, embedding: @embedding, updatedAt: @now } IN emailReplyContext RETURN NEW', { key: noteKey, userKey, expectedUpdatedAt, expectedRevision, name: data.name, text: data.text, embedding: data.embedding, now });
      const value = await cursor.next(); return value ? parseNote(value) : null;
    },
    async semanticReplyContext(userKey: string, embedding: number[], currentThreadKey: string, currentMessageKeys: string[]) { const cursor = await database.query(`FOR message IN emailMessages FILTER message.userKey == @userKey && message.threadKey != @currentThreadKey && message._key NOT IN @currentMessageKeys && message.embeddingContentVersion == 4 LET similarity = COSINE_SIMILARITY(message.embedding, @embedding) FILTER IS_NUMBER(similarity) && similarity >= 0.70 SORT similarity DESC, message._key ASC LIMIT 60 RETURN { message, similarity }`, { userKey, embedding, currentThreadKey, currentMessageKeys }); const rows = await cursor.all() as Array<{ message: unknown; similarity: number }>; const owners = new Set<string>(); const items: any[] = []; for (const row of rows) { const message = parseMessage(row.message); if (owners.has(message.threadKey)) continue; owners.add(message.threadKey); items.push({ kind: 'message', key: message.key, similarity: row.similarity, providerMessageId: message.providerMessageId, threadKey: message.threadKey, subject: message.subject, body: message.body, from: message.from, to: message.to, direction: message.direction, sentAt: message.sentAt, trueOutboundReply: message.direction === 'outbound' && (message.replyDepth > 0 || message.inReplyTo != null || message.parentMessageId != null) }); if (items.length >= 20 || JSON.stringify(items).length >= 24_000) break; } return items; },
    async deleteReplyContext(userKey: string, noteKeys: string[]) { const cursor = await database.query('LET matches = (FOR note IN emailReplyContext FILTER note.userKey == @userKey && note._key IN @keys RETURN note) FILTER LENGTH(matches) == LENGTH(@keys) FOR note IN matches REMOVE note IN emailReplyContext RETURN OLD._key', { userKey, keys: noteKeys }); const removed = await cursor.all(); if (removed.length !== noteKeys.length) throw error('not_found', 'Every reply-context note must belong to the authenticated user'); return { deletedKeys: noteKeys }; },
    async listTones(userKey: string) { const cursor = await database.query('FOR value IN emailTones FILTER value.userKey == @userKey RETURN value', { userKey }); return (await cursor.all()).map(parseTone).sort((a, b) => a.createdAt.localeCompare(b.createdAt) || a.key.localeCompare(b.key)); },
    async searchTones(userKey: string, embedding: number[], query: string, minimumScore: number, limit: number, range: EmailCreatedAtRange = {}) { const cursor = await database.query('FOR tone IN emailTones FILTER tone.userKey == @userKey FILTER @createdFrom == null || tone.createdAt >= @createdFrom FILTER @createdTo == null || tone.createdAt <= @createdTo LET direct = CONTAINS(LOWER(CONCAT_SEPARATOR(" ", tone.name, tone.instruction)), @query) LET score = COSINE_SIMILARITY(tone.embedding, @embedding) FILTER direct || IS_NUMBER(score) && score >= @minimumScore SORT direct DESC, score DESC LIMIT @limit RETURN { tone, score: direct ? 1 : score }', { userKey, embedding, query: query.trim().toLowerCase(), minimumScore, limit, createdFrom: range.createdFrom ?? null, createdTo: range.createdTo ?? null }); return (await cursor.all() as any[]).map((row) => ({ tone: parseTone(row.tone), score: row.score })); },
    async writingProfile(userKey: string, profileKey?: string, toneSlug?: string) { const tones = await this.listTones(userKey); const tone = profileKey ? tones.find((value) => value.key === profileKey) : toneSlug ? tones.find((value) => value.key === toneSlug || value.identifier === toneSlug || value.slug === toneSlug) : tones[0]; return tone ? { ...tone, tone: tone.instruction, style: '', structure: '', vocabulary: tone.instruction, conventions: tone.instruction } : null; },
    async getTone(userKey: string, toneKey: string) { const cursor = await database.query('LET value = DOCUMENT(emailTones, @key) FILTER value != null && value.userKey == @userKey RETURN value', { key: toneKey, userKey }); const value = await cursor.next(); return value ? parseTone(value) : null; },
    async createTone(userKey: string, scopeKey: string, input: { name: string; instruction: string; isFavorite: boolean; embedding: number[] }) { const now = new Date().toISOString(), key = newId(), data = emailToneDataSchema.parse({ identifier: key, name: input.name, instruction: input.instruction }), value = emailToneRecordSchema.parse({ ...data, key, userKey, scopeKey, embedding: input.embedding, isFavorite: input.isFavorite, createdAt: now, updatedAt: now }); const cursor = await database.query('INSERT @value IN emailTones RETURN NEW', { value: toArangoDoc(value) }); return parseTone(await cursor.next()); },
    async updateTone(userKey: string, toneKey: string, expectedUpdatedAt: string, patch: { name?: string; instruction?: string; isFavorite?: boolean; embedding?: number[] }) { const cursor = await database.query('FOR tone IN emailTones FILTER tone._key == @key && tone.userKey == @userKey && tone.updatedAt == @expected UPDATE tone WITH MERGE(@patch, { updatedAt: @now }) IN emailTones RETURN NEW', { key: toneKey, userKey, expected: expectedUpdatedAt, patch, now: new Date().toISOString() }); const value = await cursor.next(); return value ? parseTone(value) : null; },
    async deleteTone(userKey: string, toneKey: string) { const cursor = await database.query('LET tone = DOCUMENT(emailTones, @key) FILTER tone != null && tone.userKey == @userKey && tone.slug == null LET cleanupTags = (FOR assignment IN tagAssignments FILTER assignment.scopeKey == tone.scopeKey && assignment.sourceType == "email-tone" && assignment.sourceKey == @key REMOVE assignment IN tagAssignments RETURN 1) REMOVE tone IN emailTones RETURN OLD._key', { key: toneKey, userKey }); const key = await cursor.next(); if (!key) throw error('forbidden', 'Built-in email tones cannot be deleted'); return { deletedKey: key as string, storageKeys: [] as string[] }; },
    async initializeTones(userKey: string, scopeKey: string, embed?: (text: string) => Promise<number[]>) { for (const tone of [{ slug: 'casual', name: 'Casual', instruction: 'Use conversational language, natural contractions, and an approachable tone.' }, { slug: 'formal', name: 'Formal', instruction: 'Use professional language, complete sentences, and a clear conventional structure.' }, { slug: 'direct', name: 'Direct', instruction: 'Lead with the answer or action and avoid hedging.' }] as const) { const key = stableKey('mail-tone', userKey, tone.slug), now = new Date().toISOString(), embedding = embed ? await embed(tone.name) : Array(EMBEDDING_DIMENSIONS).fill(0), value = emailToneRecordSchema.parse({ ...tone, key, userKey, scopeKey, embedding, isFavorite: false, createdAt: now, updatedAt: now }); await database.query('UPSERT { scopeKey: @scopeKey, slug: @slug } INSERT @value UPDATE {} IN emailTones', { scopeKey, slug: tone.slug, value: toArangoDoc(value) }); } return this.listTones(userKey); },
  };
}
