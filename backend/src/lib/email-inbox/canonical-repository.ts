...
export function createCanonicalEmailRepository(database: Database, error: RepositoryError, stableKey: StableKey) {
  ...
  const exportThread = async (thread: z.infer<typeof emailThreadRecordSchema>, messages: Array<z.infer<typeof emailMessageRecordSchema>>, timestamp: string, representations?: { thread?: PreparedDocumentRepresentation; messages?: Array<PreparedDocumentRepresentation | undefined> }) => {
    try {
      ...
      const exports = [
        exportEmailThreadToArchive(thread, { scopeKey: thread.scopeKey, exportKey: stableKey('email-archive-thread-export', thread.key), folderKey: inboxKey, exportedAt: timestamp }, representations?.thread),
        ...messages.map((message, index) => exportEmailMessageToArchive(message, { scopeKey: thread.scopeKey, exportKey: stableKey('email-archive-message-export', message.key), folderKey: inboxKey, exportedAt: timestamp }, representations?.messages?.[index])),
      ];
  ...
  return {
    ...
    async deleteTone(scopeKey: string, toneKey: string) { const cursor = await database.query('LET tone = DOCUMENT(emailTones, @key) FILTER tone != null && tone.scopeKey == @scopeKey && tone.slug == null LET cleanupTags = (FOR assignment IN tagAssignments FILTER assignment.scopeKey == @scopeKey && assignment.sourceType == "email-tone" && assignment.sourceKey == @key REMOVE assignment IN tagAssignments RETURN 1) REMOVE tone IN emailTones RETURN OLD._key', { key: toneKey, scopeKey }); const key = await cursor.next(); if (!key) throw error('forbidden', 'Built-in email tones cannot be deleted'); return { deletedKey: key as string, storageKeys: [] as string[] }; },
