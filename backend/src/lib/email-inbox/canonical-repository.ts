...
export function createCanonicalEmailRepository(database: Database, error: RepositoryError, stableKey: StableKey, publishBalance: (userKey: string) => Promise<void> = async () => {}) {
  ...
  return {
    ...
    async createReplyContext(userKey: string, scopeKey: string, input: { name: string; text: string; embedding: number[] }) { const data = emailReplyContextDataSchema.parse(input), now = new Date().toISOString(), value = emailReplyContextRecordSchema.parse({ ...data, key: newId(), userKey, scopeKey, embedding: input.embedding, createdAt: now, updatedAt: now }); const cursor = await database.query('LET notes = (FOR note IN emailReplyContext FILTER note.userKey == @userKey RETURN note) FILTER LENGTH(notes) < 20 && SUM(notes[* RETURN LENGTH(CURRENT.text)]) + LENGTH(@text) <= 24000 INSERT @value IN emailReplyContext RETURN NEW', { userKey, text: value.text, value: toArangoDoc(value) }); const stored = await cursor.next(); if (!stored) throw error('conflict', 'Reply context is limited to 20 notes and 24,000 text characters'); return parseNote(stored); },
    async updateReplyContext(userKey: string, noteKey: string, expectedUpdatedAt: string, expectedRevision: string, input: { name: string; text: string; embedding: number[] }) { const data = emailReplyContextDataSchema.parse(input), now = new Date().toISOString(); const cursor = await database.query('LET current = DOCUMENT(emailReplyContext, @key) FILTER current != null && current.userKey == @userKey && current.updatedAt == @expectedUpdatedAt && current._rev == @expectedRevision LET otherLength = SUM(FOR note IN emailReplyContext FILTER note.userKey == @userKey && note._key != @key RETURN LENGTH(note.text)) FILTER otherLength + LENGTH(@text) <= 24000 UPDATE current WITH { name: @name, text: @text, embedding: @embedding, updatedAt: @now } IN emailReplyContext RETURN NEW', { key: noteKey, userKey, expectedUpdatedAt, expectedRevision, name: data.name, text: data.text, embedding: input.embedding, now }); const value = await cursor.next(); return value ? parseNote(value) : null; },
