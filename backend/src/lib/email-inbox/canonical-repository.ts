  ...
  return {
    ...
    async overview(scopeKey: string, connectorKey: string, query: EmailOverviewRepositoryQuery) {
      ...
      const filtered = threads.filter((thread) => {
        ...
        if (legacyFilter) return legacyFilter === 'trash' ? trash : !trash && (legacyFilter === 'all' || legacyFilter === 'important' && thread.inboxCategory === 'Important' || legacyFilter === 'urgent' && thread.inboxCategory === 'Urgent' || legacyFilter === 'needs_action' && thread.state === 'needs_action' || legacyFilter === 'filtered' && thread.inboxCategory === 'Filtered' || legacyFilter === 'unread' && thread.unread || legacyFilter === 'favorite' && thread.isFavorite);
        const facets: string[] = queryFacets;
        return !trash && (readState == null || thread.unread === (readState === 'unread')) && (!facets.includes('favorite') || thread.isFavorite) && (!facets.some((value) => ['urgent', 'important', 'filtered'].includes(value)) || facets.includes((thread.inboxCategory ?? 'Important').toLowerCase()));
    ...
    async searchThreads(scopeKey: string, connectorKey: string, embedding: number[], query: string, minimumScore: number, limit: number, filters?: { readState?: 'read' | 'unread'; facets?: Array<'urgent' | 'important' | 'filtered' | 'favorite'> } & EmailCreatedAtRange) {
