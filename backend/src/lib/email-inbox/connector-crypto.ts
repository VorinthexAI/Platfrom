...
function aad(teamKey: string, scopeKey: string, providerAccountId: string, provider: EmailProvider = 'gmail') {
  return Buffer.from(`${teamKey}\0${scopeKey}\0${provider}\0${providerAccountId}`, 'utf8');
}

export function encryptEmailConnectorCredentials(
  credentials: EmailConnectorCredentials,
  binding: { teamKey: string; scopeKey: string; providerAccountId: string; provider?: EmailProvider },
  ...
  cipher.setAAD(aad(binding.teamKey, binding.scopeKey, binding.providerAccountId, binding.provider));
...
export function decryptEmailConnectorCredentials(
  ...
  decipher.setAAD(aad(binding.teamKey, binding.scopeKey, binding.providerAccountId, binding.provider));
