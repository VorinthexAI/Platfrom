...
const teamMembershipKey = 'cmrnlzf650002qc7k4p5zem5w';
...
describe('team connector repository', () => {
  test('upserts exact provider accounts and preserves only the reconnected account key', async () => {
    ...
    expect(accountA.initialSyncChargeKey).toBeString();
    expect(reconnectedA.initialSyncChargeKey).not.toBe(accountA.initialSyncChargeKey);
  ...
  test('requires initial synchronization completion in the public connector projection', () => {
    const connector = teamConnectorSchema.parse({ key: teamMembershipKey, teamKey: 'team-1', scopeKey, provider: 'gmail', providerAccountId: 'google-public', email: 'public@example.com', encryptedCredentials: 'cipher', encryptionKeyId: 'test', accessTokenFingerprint: 'a'.repeat(64), scopes: ['email'], createdByTeamMembershipKey: teamMembershipKey, status: 'active', initialSyncCompleted: true, createdAt: '2026-08-11T12:00:00.000Z', updatedAt: '2026-08-11T12:00:00.000Z' });
  ...
  test('credential refresh is fenced by observed update time and active status', async () => {
    ...
    const connector = teamConnectorSchema.parse({
      key: teamMembershipKey, teamKey: 'team-1', scopeKey, provider: 'gmail', providerAccountId: 'google-a', email: 'a@example.com',
      encryptedCredentials: 'cipher', encryptionKeyId: 'test', accessTokenFingerprint: 'a'.repeat(64), scopes: ['email'], createdByTeamMembershipKey: teamMembershipKey,
  ...
  test('fences disconnect against active connector sync and send leases', async () => {
    let call: { query: string; bindVars: Record<string, unknown> } | undefined;
    const database = { collection: () => ({}), query: async (query: string, bindVars: Record<string, unknown>) => { call = { query, bindVars }; return { next: async () => null }; } };
    expect(await createConnectorRepository(database as never).revoke(teamMembershipKey, '2026-08-11T12:00:00.000Z')).toBe(false);
    ...
    expect(call?.bindVars).toMatchObject({ key: teamMembershipKey, expectedUpdatedAt: '2026-08-11T12:00:00.000Z' });
  });

  test('preserves immutable billing attribution without recurring billing state', async () => {
    const calls: Array<{ query: string; bindVars: Record<string, any> }> = [];
  ...
  test('rolls back a reconnect without recurring billing-period cleanup', async () => {
    const previous = teamConnectorSchema.parse({ key: teamMembershipKey, teamKey: 'team-1', scopeKey, provider: 'gmail', providerAccountId: 'google-rollback', email: 'billing@example.com', encryptedCredentials: 'cipher', encryptionKeyId: 'test', accessTokenFingerprint: 'a'.repeat(64), scopes: ['email'], createdByTeamMembershipKey: teamMembershipKey, billingUserKey: teamMembershipKey, status: 'active', createdAt: '2026-08-10T12:00:00.000Z', updatedAt: '2026-08-11T12:00:00.000Z' });
    ...
    expect(call?.query).not.toContain('inboxBillingPeriods');
  });
  ...
  test('makes connector sync and send claims mutually exclusive', async () => {
    ...
    const repository = createConnectorRepository(database as never);
    await repository.claimSync(teamMembershipKey, 'sync-token', '2026-08-11T13:00:00.000Z');
    await repository.claimSend(teamMembershipKey, 'send-token', '2026-08-11T13:00:00.000Z');
  ...
  test('atomically reactivates an errored connector when its sync lease starts work', async () => {
    ...
    expect(await createConnectorRepository(database as never).setSyncState(teamMembershipKey, 'syncing', { leaseToken: '11111111-1111-4111-8111-111111111111' })).toBe('next');
  ...
  test('atomically completes initial sync only with a final idle state and no continuation', async () => {
    ...
    const repository = createConnectorRepository(database as never);
    await expect(repository.setSyncState(teamMembershipKey, 'syncing', { completeInitialSync: true })).rejects.toThrow('final idle state');
    await expect(repository.setSyncState(teamMembershipKey, 'idle', { pendingHistoryId: 'pending', completeInitialSync: true })).rejects.toThrow('final idle state');
    await expect(repository.setSyncState(teamMembershipKey, 'idle', { pendingSubscriptionMessages: [{ id: 'message', threadId: 'thread' }], completeInitialSync: true })).rejects.toThrow('final idle state');
    expect(await repository.setSyncState(teamMembershipKey, 'idle', { pendingHistoryId: null, pendingThreadIds: null, completeInitialSync: true, leaseToken: '11111111-1111-4111-8111-111111111111' })).toBe('complete');
  ...
  test('rejects renewal after lease expiry or connector disablement while allowing error recovery', async () => {
    ...
    const repository = createConnectorRepository(database as never);
    expect(await repository.renewSync(teamMembershipKey, '11111111-1111-4111-8111-111111111111', '2026-08-11T13:00:00.000Z')).toBe(false);
    expect(await repository.renewSend(teamMembershipKey, '22222222-2222-4222-8222-222222222222', '2026-08-11T13:00:00.000Z')).toBe(false);
  ...
  test('fences watch persistence against disconnects and connector revisions', async () => {
    let call: { query: string; bindVars: Record<string, unknown> } | undefined;
    const database = { collection: () => ({}), query: async (query: string, bindVars: Record<string, unknown>) => { call = { query, bindVars }; return { next: async () => null }; } };
    await createConnectorRepository(database as never).updateWatch(teamMembershipKey, { historyId: '1', expiration: String(Date.now() + 60_000) }, undefined, '2026-08-11T12:00:00.000Z');
  ...
  test('persists notification high-water marks and exposes durable recovery targets', async () => {
    const calls: Array<{ query: string; bindVars: Record<string, unknown> }> = [];
    const database = { collection: () => ({}), query: async (query: string, bindVars: Record<string, unknown>) => { calls.push({ query, bindVars }); return { next: async () => true, all: async () => [{ teamKey: 'team-1', scopeKey, connectorKey: teamMembershipKey, initialSyncCompleted: true, pendingNotificationHistoryId: '123' }] }; } };
    const repository = createConnectorRepository(database as never);
    expect(await repository.markNotificationPending(teamMembershipKey, '123')).toBe(true);
    expect(await repository.clearPendingNotification(teamMembershipKey, '123')).toBe(true);
    expect(await repository.listSyncRecoveryTargets()).toEqual([{ teamKey: 'team-1', scopeKey, connectorKey: teamMembershipKey, initialSyncCompleted: true, pendingNotificationHistoryId: '123' }]);
  ...
  test('renews watch metadata without changing the persisted History or pending continuation cursor', async () => {
    const record: Record<string, any> = {
      ...teamConnectorSchema.parse({
        key: teamMembershipKey, teamKey: 'team-1', scopeKey, provider: 'gmail', providerAccountId: 'google-watch', email: 'watch@example.com',
        encryptedCredentials: 'cipher', encryptionKeyId: 'test', accessTokenFingerprint: 'a'.repeat(64), scopes: ['email'], createdByTeamMembershipKey: teamMembershipKey,
    ...
    expect(await createConnectorRepository(database as never).updateWatch(teamMembershipKey, { historyId: 'watch-response-history-must-not-win', expiration: String(Date.parse('2026-08-12T12:00:00.000Z')) }, 'before-watch', '2026-08-11T12:00:00.000Z')).toBe('after-watch');
