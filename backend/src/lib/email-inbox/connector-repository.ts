...
export function createConnectorRepository(database: Database = db) {
  async function findExact(organizationKey: string, scopeKey: string, providerAccountId: string): Promise<RevisionedConnector | null> {
  ...
  return {
    ...
    async upsert(input: {
      ...
      try {
        const cursor = await database.query(`
          LET existing = FIRST(FOR connector IN @@collection FILTER connector.organizationKey == @organizationKey && connector.scopeKey == @scopeKey && connector.provider == "gmail" && connector.providerAccountId == @providerAccountId LIMIT 1 RETURN connector)
    ...
    async updateWatch(key: string, input: { historyId: string; expiration: string }, expectedRevision?: string) {
      const updatedAt = new Date().toISOString();
      const cursor = await database.query('FOR connector IN @@collection FILTER connector._key == @key && (@expectedRevision == null || connector._rev == @expectedRevision) UPDATE connector WITH { watchRegisteredAt: @updatedAt, watchExpiresAt: @watchExpiresAt, updatedAt: @updatedAt } IN @@collection RETURN NEW', { '@collection': ORGANIZATION_CONNECTORS_COLLECTION, key, expectedRevision: expectedRevision ?? null, updatedAt, watchExpiresAt: new Date(Number(input.expiration)).toISOString() });
      const raw = await cursor.next();
      return raw ? revision(raw) : null;
    },
