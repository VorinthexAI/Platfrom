  ...
  return {
    ...
    async rollbackReconnect(input: { connectorKey: string; connectorRevision: string; previousConnector: UserConnector | null; inboxKey?: string; inboxRevision?: string; previousInbox: Inbox | null }) {
      ...
      const cursor = await database.query(`
        ...
        FILTER @inboxRevision == null || (inbox != null && inbox.teamKey == connector.teamKey && inbox.scopeKey == connector.scopeKey && inbox.connectorKey == connector._key && inbox._rev == @inboxRevision)
        ${connectorMutation}
        ${inboxMutation}
        RETURN true
      `, {
