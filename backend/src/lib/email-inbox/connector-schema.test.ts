...
describe('team connector mail projection', () => {
  ...
  test('accepts only strict Gmail OAuth credentials', () => {
    ...
    expect(() => teamConnectorSchema.parse({ key: 'cmrnlzf650002qc7k4p5zem5w', teamKey: 'team-1', scopeKey: 'cmrnlzf640001qc7kazsr96k5', provider: 'unsupported', providerAccountId: 'unsupported-1', email: 'person@example.com', encryptedCredentials: 'cipher', encryptionKeyId: 'v1', accessTokenFingerprint: 'a'.repeat(64), scopes: ['email'], createdByTeamMembershipKey: 'cmrnlzf640001qc7kazsr96k5', status: 'active', createdAt: now, updatedAt: now })).toThrow();
  });
});
