...
export function buildMailDevSeedManifest(input: {
  ...
  const documents = fixtures.threads.flatMap(({ thread, messages }) => {
    ...
    const messageDocuments = messages.map((message) => {
      ...
      emailMessages.push(emailMessageRecordSchema.parse({ ...message, key, threadKey, userKey: input.userKey, scopeKey: input.userKey, developmentFixtureIdentifier: MAIL_DEV_FIXTURE_PREFIX, createdAt: MAIL_DEV_FIXTURE_AT, updatedAt: MAIL_DEV_FIXTURE_AT }));
