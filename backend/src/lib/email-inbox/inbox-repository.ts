...
import type { TeamConnector } from './connector-schema';
...
export function createInboxRepository(database: Database = db) {
  return {
    async getByConnector(teamKey: string, scopeKey: string, connectorKey: string): Promise<(EmailInbox & { revision: string }) | null> {
      const cursor = await database.query(`LET connector = DOCUMENT(teamConnectors, @connectorKey) FILTER connector != null && connector.teamKey == @teamKey && connector.scopeKey == @scopeKey && connector.provider == "gmail" && connector.status != "revoked" LET inbox = DOCUMENT(@@inboxes, @inboxKey) FILTER inbox != null && inbox.teamKey == @teamKey && inbox.scopeKey == @scopeKey && inbox.connectorKey == connector._key RETURN inbox`, { '@inboxes': EMAIL_INBOXES_COLLECTION, teamKey, scopeKey, connectorKey, inboxKey: emailInboxKey(scopeKey, connectorKey) });
      const value = await cursor.next();
      return value ? { ...parse(value), revision: revision(value) } : null;
    },
    async search(teamKey: string, scopeKey: string, connectorKeys: string[], embedding: number[], query: string, minimumScore: number, limit: number, range: EmailCreatedAtRange = {}) {
      if (!connectorKeys.length) return [];
      const normalized = query.normalize('NFKC').trim().replace(/\s+/g, ' ').toLocaleLowerCase('en-US');
      const cursor = await database.query(`FOR inbox IN @@inboxes FILTER inbox.teamKey == @teamKey && inbox.scopeKey == @scopeKey && inbox.connectorKey IN @connectorKeys FILTER @createdFrom == null || inbox.createdAt >= @createdFrom FILTER @createdTo == null || inbox.createdAt <= @createdTo LET connector = DOCUMENT(teamConnectors, inbox.connectorKey) FILTER connector != null && connector.provider == "gmail" && connector.status != "revoked" FILTER IS_ARRAY(inbox.embedding) && LENGTH(inbox.embedding) == LENGTH(@embedding) LET direct = CONTAINS(LOWER(CONCAT_SEPARATOR(" ", inbox.name, inbox.description)), @query) LET score = COSINE_SIMILARITY(inbox.embedding, @embedding) FILTER direct || IS_NUMBER(score) && score >= @minimumScore SORT direct DESC, score DESC, inbox.updatedAt DESC, inbox._key ASC LIMIT @limit RETURN { inbox, score: direct ? 1 : score }`, { '@inboxes': EMAIL_INBOXES_COLLECTION, teamKey, scopeKey, connectorKeys, embedding, query: normalized, minimumScore, limit, createdFrom: range.createdFrom ?? null, createdTo: range.createdTo ?? null });
      return (await cursor.all() as Array<{ inbox: unknown; score: number }>).map(({ inbox, score }) => ({ inbox: parse(inbox), score }));
    },
    async ensure(connector: TeamConnector, metadata: { name: string; description?: string }, embedding: number[], overwrite: boolean, expectedRevision?: string | null): Promise<(EmailInbox & { revision: string }) | null> {
      const timestamp = new Date().toISOString();
      const key = emailInboxKey(connector.scopeKey, connector.key);
      const value = emailInboxSchema.parse({ key, teamKey: connector.teamKey, scopeKey: connector.scopeKey, connectorKey: connector.key, name: metadata.name, ...(metadata.description ? { description: metadata.description } : {}), isFavorite: false, embedding, createdAt: timestamp, updatedAt: timestamp });
      try {
        const cursor = await database.query(`LET connector = DOCUMENT(teamConnectors, @connectorKey) FILTER connector != null && connector.teamKey == @teamKey && connector.scopeKey == @scopeKey && connector.provider == "gmail" LET existing = DOCUMENT(@@inboxes, @key) FILTER existing == null ? @expectedRevision == null : (!@overwrite || existing._rev == @expectedRevision) UPSERT { _key: @key } INSERT @value UPDATE (@overwrite && OLD.teamKey == @teamKey && OLD.scopeKey == @scopeKey && OLD.connectorKey == @connectorKey ? MERGE(@value, { _key: OLD._key, createdAt: OLD.createdAt, isFavorite: OLD.isFavorite, coverImageKey: OLD.coverImageKey }) : {}) IN @@inboxes OPTIONS { keepNull: false } RETURN NEW`, { '@inboxes': EMAIL_INBOXES_COLLECTION, key, connectorKey: connector.key, teamKey: connector.teamKey, scopeKey: connector.scopeKey, overwrite, expectedRevision: expectedRevision ?? null, value: toArangoDoc(value) });
