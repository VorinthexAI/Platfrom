...
import { ensureTeamConnectorsCollection } from './indexes';

describe('team connector indexes', () => {
  test('replaces single-provider scope uniqueness with provider-account uniqueness', async () => {
    ...
    const collection = {
      exists: async () => true,
      indexes: async () => [
        { id: 'legacy', fields: ['teamKey', 'scopeKey', 'provider'], unique: true },
  ...
  test('refuses to create the unique index over duplicate account bindings', async () => {
    ...
    await expect(ensureTeamConnectorsCollection({ collection: () => collection, query: async () => ({ next: async () => 1 }) } as never)).rejects.toThrow('duplicate provider-account bindings');
