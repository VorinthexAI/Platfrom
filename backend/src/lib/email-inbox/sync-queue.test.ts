...
describe('email synchronization jobs', () => {
  ...
  test('synchronizes every active scope resolved from persisted accounts', async () => {
    ...
    const repository = { listSyncTargetsByEmail: async () => [{ organizationKey: 'org-1', scopeKey: 'scope-1' }, { organizationKey: 'org-2', scopeKey: 'scope-2' }] };
    const service = { sync: async (actor: unknown) => { actors.push(actor); return {}; } };
    expect(await processEmailSyncJob({ schemaVersion: 1, kind: 'notification', emailAddress: 'person@example.com', historyId: '123', messageId: 'message', subscription: 'subscription' }, { repository: repository as never, service: service as never })).toEqual({ synchronized: 2 });
  ...
  test('attempts every watch renewal before retrying failed accounts', async () => {
    ...
    const repository = { listWatchRenewalTargets: async () => [{ organizationKey: 'broken', scopeKey: 'scope-1' }, { organizationKey: 'healthy', scopeKey: 'scope-2' }] };
    ...
    await expect(processEmailSyncJob({ schemaVersion: 1, kind: 'renew-watches', day: '2026-08-12' }, { repository: repository as never, service: service as never })).rejects.toThrow('1 account');
    expect(attempted).toEqual(['broken', 'healthy']);
  });

  test('attempts every matching scope before retrying notification failures', async () => {
    const attempted: string[] = [];
    const repository = { listSyncTargetsByEmail: async () => [{ organizationKey: 'broken', scopeKey: 'scope-1' }, { organizationKey: 'healthy', scopeKey: 'scope-2' }] };
    ...
    await expect(processEmailSyncJob({ schemaVersion: 1, kind: 'notification', emailAddress: 'person@example.com', historyId: '123', messageId: 'message', subscription: 'subscription' }, { repository: repository as never, service: service as never })).rejects.toThrow('1 account');
    expect(attempted).toEqual(['broken', 'healthy']);
  });
});
