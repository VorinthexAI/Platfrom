...
import type { getUserTeamByTeamAndUser } from "@/lib/db/user-team.node";
import {
  imageSearchInputSchema,
  imageSearchTool,
} from "@/lib/ai/tools/image-search";
...
import {
  mutationEventTargets,
  publishGalleryEvents,
  type GalleryMutationEventName,
...
const validCreationDateRange = (value: {
  createdFrom?: string;
  createdTo?: string;
}) =>
...
const overviewSchema = strictObject({
  collectionKey: z.string().cuid().optional(),
  origin: z.enum(["uploaded", "generated"]).optional(),
  maxCaptionScore: z.number().int().min(1).max(100).optional(),
  ...creationDateRangeShape,
  ...cursorPaginationInputShape,
}).refine(validCreationDateRange, {
  message: "createdFrom must be before or equal to createdTo.",
  path: ["createdTo"],
});
const collectionCreateSchema = strictObject({
  name: z.string().trim().min(1).max(120),
  isFavorite: z.boolean().default(false),
});
const collectionUpdateSchema = strictObject({
  collectionKey: z.string().cuid(),
  name: z.string().trim().min(1).max(120),
  isFavorite: z.boolean(),
  coverImageKey: z.string().cuid().nullable().optional(),
});
const collectionDeleteSchema = strictObject({
  collectionKey: z.string().cuid(),
});
const imageUpdateSchema = strictObject({
  ...
  name: z
    ...
    .refine(
      ...
      "Image name cannot contain path separators.",
    ),
  isFavorite: z.boolean(),
});
const uploadFileSchema = strictObject({
...
const presignSchema = strictObject({
  ...
  ({ files }) =>
    new Set(files.map(({ clientKey }) => clientKey)).size === files.length,
  "Upload client keys must be unique.",
);
const completeSchema = strictObject({
  uploadKeys: z.array(z.string().cuid()).min(1).max(20),
}).refine(
  ({ uploadKeys }) => new Set(uploadKeys).size === uploadKeys.length,
  "Upload keys must be unique.",
);
const searchSchema = imageSearchInputSchema;
export const galleryCollectionSearchInputSchema = strictObject({
...
const favoriteSchema = strictObject({
  imageKey: z.string().cuid(),
  isFavorite: z.boolean(),
});
const deleteImagesSchema = strictObject({
  imageKeys: z.array(z.string().cuid()).min(1).max(100),
}).refine(
  ({ imageKeys }) => new Set(imageKeys).size === imageKeys.length,
  "Image keys must be unique",
);
const deleteDuplicatesSchema = strictObject({
  collectionKey: z.string().cuid(),
  imageKeys: z.array(z.string().cuid()).min(1).max(500),
}).refine(
  ({ imageKeys }) => new Set(imageKeys).size === imageKeys.length,
  "Image keys must be unique",
);
const subjectListSchema = strictObject({});
const subjectCreateSchema = strictObject({
  name: z.string().trim().min(1).max(120),
  imageKeys: z.array(z.string().cuid()).min(1).max(8),
}).refine(
  ({ imageKeys }) => new Set(imageKeys).size === imageKeys.length,
  "Reference image keys must be unique",
);
...
const collectionTransferSchema = strictObject({
  ...
  if (new Set(value.imageKeys).size !== value.imageKeys.length)
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Image keys must be unique.",
      path: ["imageKeys"],
    });
  if (value.destinationCollectionKeys.includes(value.sourceCollectionKey))
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message: "The source collection cannot be a destination.",
      path: ["destinationCollectionKeys"],
    });
});
const highlightCreateSchema = strictObject({
  collectionKey: z.string().cuid(),
  imageKeys: z.array(z.string().cuid()).min(2).max(10).optional(),
}).refine(
  ({ imageKeys }) =>
    imageKeys === undefined || new Set(imageKeys).size === imageKeys.length,
  "Highlight image keys must be unique",
);
const highlightListSchema = strictObject({
  collectionKey: z.string().cuid().optional(),
});
const highlightKeySchema = strictObject({ highlightKey: z.string().cuid() });
const memoryCreateSchema = strictObject({
  collectionKey: z.string().cuid(),
...
const memoryDeleteSchema = strictObject({
  memoryKey: z.string().cuid(),
  collectionKey: z.string().cuid(),
});

type GalleryMembership = NonNullable<
  Awaited<ReturnType<typeof getUserTeamByTeamAndUser>>
>;
...
export interface GalleryOperationContext {
  ...
  verifyUploadObject?: (
    upload: z.infer<typeof galleryUploadSchema>,
  ) => Promise<boolean>;
...
async function authorize(context: GalleryOperationContext) {
  if (
    context.membership.teamKey !== context.teamKey ||
    context.membership.status !== "active"
  )
    throw new GalleryOperationError(
      403,
      "GALLERY_FORBIDDEN",
      "Gallery scope access denied.",
    );
  return context.membership;
}

async function collectionRole(
  context: GalleryOperationContext,
  collectionKey: string,
) {
  const membership = await authorize(context);
  const role = await (
    context.getCollectionRole ?? repository.getCollectionRole
  )(context.scopeKey, collectionKey, membership.key);
  if (!role)
    throw new GalleryOperationError(
      404,
      "GALLERY_COLLECTION_NOT_FOUND",
...
function requestIdentity(operation: string, context: GalleryOperationContext) {
  return context.idempotencyKey
    ? `c${createHash("sha256").update(`${operation}\0${context.scopeKey}\0${context.membership.key}\0${context.idempotencyKey}`).digest("hex").slice(0, 24)}`
    : newId();
}
...
export function normalizeGalleryOperationError(error: unknown) {
  ...
  if (error instanceof ZodError || error instanceof SyntaxError)
    return new GalleryOperationError(
      400,
      "GALLERY_INVALID_INPUT",
      "Gallery request input was invalid.",
    );
  return new GalleryOperationError(
    500,
    "GALLERY_FAILED",
...
export async function safeImage(
  ...
  return {
    ...
    countryCode: image.countryCode ?? null,
    latitude: image.latitude ?? null,
    longitude: image.longitude ?? null,
    locationSource: image.locationSource ?? null,
    origin: image.origin,
    mutationPolicy: image.mutationPolicy,
    isFavorite: image.isFavorite,
    createdByKey: image.createdByKey,
    createdAt: image.createdAt,
    updatedAt: image.updatedAt,
    url: await imageUrl(image.storageKey),
    ...(score === undefined ? {} : { score }),
  };
}

export function projectGalleryCollection(
  ...
  return {
    ...
    mutationPolicy: collection.mutationPolicy,
...
async function reconcileVisualIdentity(
  identity: z.infer<typeof visualIdentitySchema>,
  teamKey: string,
  actorKey: string,
  context: GalleryOperationContext,
) {
  const matches = await repository.searchAccessibleImages({
    teamKey,
  ...
  if (
    await repository.persistIdentityMatches(
      identity.scopeKey,
      identity.key,
    ...
    await publish(context, "reconcileSubject", {
      users: await repository.listScopeManagerUserKeys(identity.scopeKey),
    });
  }
}

async function safeSubject(row: {
  identity: z.infer<typeof visualIdentitySchema>;
  reference: z.infer<typeof imageSchema>;
  imageCount: number;
}) {
  ...
  return {
    ...
    referenceImageKey: identity.referenceImageKey,
    referenceUrl: await imageUrl(reference.storageKey),
...
async function safeMemory(
  ...
  return {
    ...
    createdByKey: memory.createdByKey,
    createdAt: memory.createdAt,
    updatedAt: memory.updatedAt,
  };
}

export function selectMemoryCandidate<T extends { captionScore: number }>(
  candidates: T[],
  random: () => number = Math.random,
): T | undefined {
  const weights = candidates.map(
    ({ captionScore }) => Math.max(1, Math.min(100, captionScore)) ** 2,
  );
...
export function normalizeMemoryText(value: string) {
  ...
  if (!sections.length)
    throw new GalleryOperationError(
      500,
      "GALLERY_MEMORY_GENERATION_FAILED",
      "A readable memory could not be generated.",
    );
...
function memoryPrompt(candidate: {
  ...
  const data = JSON.stringify({
    canonicalCaption: candidate.caption,
    captionScore: candidate.captionScore,
    visualIdentityNames: candidate.identityNames,
  });
...
async function overview(rawInput: unknown, context: GalleryOperationContext) {
  ...
  if (
    input.collectionKey &&
    !(await repository.getCollectionRole(
      input.scopeKey,
      input.collectionKey,
      membership.key,
    ))
  ...
  const { collections, images } = await (
    context.listOverview ?? repository.listOverview
  )({
    scopeKey: input.scopeKey,
    actorKey: membership.key,
    collectionKey: input.collectionKey,
    origin: input.origin,
    maxCaptionScore: input.maxCaptionScore,
    createdFrom: input.createdFrom,
    createdTo: input.createdTo,
    cursor: input.cursor,
    limit: input.limit,
  });
  const canCreateCollections = await (
    context.canManageScope ?? repository.canManageScope
  )(input.scopeKey, membership.key);
  return {
    collections: await Promise.all(
      collections.map(async ({ collection, count, cover, role, isOwned }) =>
        projectGalleryCollection(
          collection,
          count,
          cover ? await imageUrl(cover.storageKey) : null,
          membership.key,
          role,
          isOwned,
        ),
...
async function createCollection(
  ...
  const membership = await authorize(context);
  if (!(await repository.canManageScope(input.scopeKey, membership.key)))
    throw new GalleryOperationError(
      403,
      "GALLERY_FORBIDDEN",
      "Gallery collection creation denied.",
    );
  const now = new Date().toISOString();
  const collection = collectionSchema.parse({
    key: newId(),
    scopeKey: input.scopeKey,
    ownerKey: membership.key,
    name: input.name,
    embedding: currentEmbeddingSchema.parse(
      await embedText({ text: input.name }),
    ),
    isFavorite: input.isFavorite,
    createdAt: now,
    updatedAt: now,
  });
  if (!(await repository.createCollection(collection)))
    throw new GalleryOperationError(
      403,
      "GALLERY_FORBIDDEN",
      "Gallery collection creation denied.",
...
async function updateCollection(
  ...
  const previous = await repository.getCollection(
    input.scopeKey,
    input.collectionKey,
  );
  if (!previous)
  ...
  const collection = await repository.updateCollectionDetails(
    input.scopeKey,
    input.collectionKey,
  ...
  const overview = await repository.listOverview({
    scopeKey: input.scopeKey,
    actorKey: context.membership.key,
    collectionKey: collection.key,
    limit: 1,
  });
  const row = overview.collections.find(
    ({ collection: candidate }) => candidate.key === collection.key,
  );
  return {
    collection: projectGalleryCollection(
      collection,
      row?.count ?? 0,
      row?.cover ? await imageUrl(row.cover.storageKey) : null,
      context.membership.key,
    ),
  };
}
...
async function deleteCollection(
  ...
  const deletion = await (
    context.deleteCollection ?? repository.deleteCollection
  )(
    input.scopeKey,
    input.collectionKey,
    context.membership.key,
  ...
  if (deletion.status === "favorite")
    throw new GalleryOperationError(
      409,
      "GALLERY_COLLECTION_FAVORITE",
      "Unfavorite the collection before deleting it.",
    );
  await publish(context, "deleteCollection", {
    users: deletion.formerUserKeys,
  });
...
async function updateImageDetails(
  ...
  if (!previous || previous.scopeKey !== input.scopeKey)
    throw new GalleryOperationError(
      404,
      "GALLERY_IMAGE_NOT_FOUND",
      "Image not found.",
    );
  const embeddingText = buildImageEmbeddingText({
    filename: input.name,
  ...
  const updated = await repository.updateImageDetails(
    input.scopeKey,
    input.imageKey,
    membership.key,
    input.name,
    input.isFavorite,
    currentEmbeddingSchema.parse(await embedText({ text: embeddingText })),
    new Date().toISOString(),
  );
  if (!updated)
    throw new GalleryOperationError(
  ...
  await publish(context, "updateImage", {
    collections: updated.collectionKeys,
  });
  if (updated.collectionKeys.length === 0)
    await publish(context, "unfiledImageChanged", {
      users: [membership.userId],
    });
  return { image: await safeImage(updated.image) };
}

async function reserveUploads(
  rawInput: unknown,
  context: GalleryOperationContext,
) {
  ...
  const uploads = records.map((record, index) => ({
    clientKey: input.files[index]!.clientKey,
...
async function completeUploads(
  ...
  const records = await Promise.all(
    input.uploadKeys.map((key) =>
      (context.getUpload ?? repository.getUpload)(key),
    ),
  );
  const uploads = records.map((upload) => {
    if (
      !upload ||
      upload.scopeKey !== input.scopeKey ||
      upload.teamKey !== input.teamKey ||
      upload.actorKey !== membership.key
    )
      throw new GalleryOperationError(
        404,
        "GALLERY_UPLOAD_NOT_FOUND",
    ...
    if (upload.status !== "reserved")
      throw new GalleryOperationError(
        ...
        "Upload reservation is no longer pending.",
      );
    if (Date.parse(upload.expiresAt) <= Date.now())
      throw new GalleryOperationError(
        409,
        "GALLERY_UPLOAD_EXPIRED",
  ...
  await Promise.all(
    uploads.map(async (upload) => {
      const matches = context.verifyUploadObject
        ? await context.verifyUploadObject(upload)
        : await s3
            .send(
            ...
            .then(
              (head) =>
                head.ContentLength === upload.sizeBytes &&
  ...
  try {
    ...
    throw new GalleryOperationError(
      500,
      "GALLERY_UPLOAD_QUEUE_UNAVAILABLE",
      "Uploads are durably queued and will be recovered automatically.",
    );
  }
  return {
    jobs: queued.map(({ key, imageKey, status }) => ({
      key,
      imageKey,
      status,
    })),
...
async function uploadStatus(
  ...
  const uploads = await Promise.all(
    input.uploadKeys.map((key) => repository.getUpload(key)),
  );
  if (
    uploads.some(
      (upload) =>
...
async function search(rawInput: unknown, context: GalleryOperationContext) {
  ...
  const membership = await authorize(context);
  const collection = input.collectionKey
    ? await repository.getCollection(input.scopeKey, input.collectionKey)
    : undefined;
  if (input.collectionKey && !collection)
    throw new GalleryOperationError(
      404,
      "GALLERY_COLLECTION_NOT_FOUND",
      "Collection not found.",
  ...
  if ("imageKey" in input) {
    ...
    if (
      !sourceImage ||
      sourceImage.scopeKey !== input.scopeKey ||
      !(await repository.canAccessImage(
        input.scopeKey,
        sourceImage.key,
        membership.key,
      ))
  ...
  if ("identityKey" in input) {
    sourceIdentity =
      (await repository.getVisualIdentity(
        input.scopeKey,
        input.identityKey,
        membership.key,
      )) ?? undefined;
    if (!sourceIdentity)
      throw new GalleryOperationError(
        404,
        "GALLERY_SUBJECT_NOT_FOUND",
        "Visual identity not found.",
      );
    await reconcileVisualIdentity(
      sourceIdentity,
      input.teamKey,
      membership.key,
      context,
  ...
  const collectionsByImageKey = new Map<
    string,
  ...
  const output = await imageSearchTool.execute(toolInput, {
    ...
    searchImages: async (searchInput) => {
      ...
      for (const result of results) {
        ...
        if (result.collections.length)
          collectionsByImageKey.set(result.image.key, result.collections);
      }
      return results;
    },
    listMatchingVisualIdentities: (scopeKey, query) =>
      repository.listMatchingIdentityNames(scopeKey, query, membership.key),
    getImage: async (key) =>
      resolvedImages.get(key) ?? repository.getImage(key),
    ...
    findDuplicateImages: async (scopeKey, collectionKey) => {
      const images = await repository.listRedundantCollectionImages(
        scopeKey,
        collectionKey,
      );
    ...
    listVisualIdentityImages: async (scopeKey, identityKey, collectionKey) => {
      const rows = await repository.listSubjectImages(
        scopeKey,
        identityKey,
        membership.key,
        collectionKey,
  ...
  let matches: Array<{ image: z.infer<typeof imageSchema>; score?: number }> =
    await Promise.all(
      output.images.map(async ({ key, score }) => {
        const image =
          resolvedImages.get(key) ?? (await repository.getImage(key));
        if (!image)
          throw new GalleryOperationError(
            404,
            "GALLERY_IMAGE_NOT_FOUND",
  ...
  if (sourceImage)
    matches = [{ image: sourceImage }, ...matches].slice(
      0,
      "limit" in input ? input.limit : 50,
    );
  const images = await Promise.all(
    matches.map(async ({ image, score }) => {
      const collections = collectionsByImageKey.get(image.key);
      return {
        ...(await safeImage(image, score)),
        ...(collections ? { collections } : {}),
      };
    }),
  );
  if ("query" in input && input.recordHistory)
    await (context.recordUserSearch ?? getDefaultUserSearchService().record)(
      membership.userId,
      input.query,
    );
  return { images };
...
export async function searchGalleryCollections(
  rawInput: unknown,
  context: GalleryOperationContext,
) {
  const input = {
    ...galleryCollectionSearchInputSchema.parse(rawInput),
    ...context,
  };
  const membership = await authorize(context);
  const queryEmbedding = currentEmbeddingSchema.parse(
    context.queryEmbedding ?? (await embedText({ text: input.query })),
  );
  const rows = await (
    context.searchAccessibleCollections ??
    repository.searchAccessibleCollections
  )({
    scopeKey: input.scopeKey,
    actorKey: membership.key,
  ...
  return {
    collections: await Promise.all(
      rows.map(async ({ collection, count, cover, role, isOwned, score }) => ({
        ...projectGalleryCollection(
          collection,
          count,
          cover ? await imageUrl(cover.storageKey) : null,
...
async function setFavorite(
  ...
  await publish(context, "setFavorite", {
    collections: updated.collectionKeys,
  });
  if (updated.collectionKeys.length === 0)
    await publish(context, "unfiledImageChanged", {
...
async function deleteImages(
  ...
  if (
    (
      await Promise.all(
        input.imageKeys.map((imageKey) =>
          (context.canMutateImage ?? repository.canMutateImage)(
            input.scopeKey,
            imageKey,
            membership.key,
          ),
    ...
    throw new GalleryOperationError(
      403,
      "GALLERY_IMAGE_READ_ONLY",
      "One or more images are read-only.",
    );
  const deletion = await (context.deleteImages ?? repository.deleteImages)(
    input.scopeKey,
    input.imageKeys,
    membership.key,
    new Date().toISOString(),
  ...
  await Promise.all(
    deletion.storageKeys.map(async (storageKey) => {
      try {
        await (context.deleteStorageObject ?? documentStorage.delete)(
          storageKey,
        );
        await (
          context.acknowledgeStorageDeletion ?? acknowledgeStorageDeletionKey
        )(storageKey);
      } catch {
        /* The durable outbox is retried at startup. */
      }
    }),
  );
  if (deletion.deletedImageKeys.length > 0) {
    await publish(context, "deleteImages", {
      collections: deletion.collectionKeys,
    });
    if (deletion.memoryCollectionKeys.length > 0)
      await publish(context, "memoryDeleted", {
        collections: deletion.memoryCollectionKeys,
      });
    if (deletion.hadUnfiledImages)
      await publish(context, "unfiledImageChanged", {
        users: [membership.userId],
      });
    if (deletion.subjectChanged)
      await publish(context, "reconcileSubject", {
        users: await (
          context.listScopeManagerUserKeys ??
          repository.listScopeManagerUserKeys
        )(input.scopeKey),
      });
  }
  const {
    storageKeys: _storageKeys,
    memoryCollectionKeys: _memoryCollectionKeys,
    ...result
  } = deletion;
  return result;
}
...
async function deleteDuplicates(
  ...
  const now = new Date().toISOString();
  const deletion = await (
    context.deleteDuplicateImages ?? repository.deleteDuplicateImages
  )(input.scopeKey, input.collectionKey, input.imageKeys, membership.key, now);
  if (!deletion)
    throw new GalleryOperationError(
      409,
      "GALLERY_DUPLICATES_CHANGED",
      "The duplicate set changed. Find duplicates again before deleting.",
    );
  await Promise.all(
    deletion.storageKeys.map(async (storageKey) => {
      try {
  ...
  if (deletion.removedImageKeys.length > 0)
    await publish(context, "deleteDuplicates", {
      collections: deletion.collectionKeys,
    });
  if (deletion.memoryCollectionKeys.length > 0)
    await publish(context, "memoryDeleted", {
...
async function transferCollectionImages(
  rawInput: unknown,
  context: GalleryOperationContext,
) {
    ...
    (
      await Promise.all(
        input.imageKeys.map((key) =>
          repository.canMutateImage(input.scopeKey, key, membership.key),
        ),
      )
  ...
  const now = new Date().toISOString();
  const transfer = await repository.transferCollectionImages({
    scopeKey: input.scopeKey,
    actorKey: membership.key,
    sourceCollectionKey: input.sourceCollectionKey,
    destinationCollectionKeys: input.destinationCollectionKeys,
    imageKeys: input.imageKeys,
    mode: input.mode,
    now,
  });
  if (transfer.status === "selection-changed")
    throw new GalleryOperationError(
      409,
      "GALLERY_SELECTION_CHANGED",
      "One or more selected images are no longer in the source collection.",
    );
  if (transfer.status === "destination-forbidden")
    throw new GalleryOperationError(
      403,
      "GALLERY_DESTINATION_FORBIDDEN",
      "Destination collection membership is required.",
    );
  ...
  await publish(context, "transferCollectionImages", {
    collections: transfer.collectionKeys,
  });
  return {
    mode: input.mode,
    imageKeys: input.imageKeys,
    destinationCollectionKeys: input.destinationCollectionKeys,
    createdRelationCount: transfer.createdRelationCount,
  };
}
...
async function listSubjects(
  ...
  if (!(await repository.canManageScope(input.scopeKey, membership.key)))
    throw new GalleryOperationError(
      403,
      "GALLERY_FORBIDDEN",
      "Gallery subjects are unavailable.",
    );
...
async function createSubject(
  ...
  const references = await Promise.all(
    input.imageKeys.map(async (key) => {
      ...
      if (!image || image.scopeKey !== input.scopeKey)
        throw new GalleryOperationError(
          404,
          "GALLERY_IMAGE_NOT_FOUND",
  ...
  const profile = await imageCreateVisualIdentityTool.execute(
    {
      imageUrls: await Promise.all(
        references.map(({ storageKey }) =>
          storedImageAnalysisDataUrl(storageKey, 1024),
        ),
      ),
    },
    { teamKey: input.teamKey, signal: context.signal },
  );
  const now = new Date().toISOString();
  const embedding = currentEmbeddingSchema.parse(
    await embedText({ text: `${input.name}\n\n${profile.description}` }),
  );
  const identity = visualIdentitySchema.parse({
    key: identityKey,
    scopeKey: input.scopeKey,
    createdByKey: membership.key,
    name: input.name,
    description: profile.description,
    referenceImageKey: references[0]!.key,
    embedding,
    createdAt: now,
    updatedAt: now,
  });
  ...
  const confidence = new Map(
    matches.map(({ image, score }) => [image.key, score]),
  );
  for (const reference of references) confidence.set(reference.key, 1);
  const referenceKeys = new Set(references.map(({ key }) => key));
  const relations = [...confidence].map(([imageKey, score]) =>
    imageIdentitySchema.parse({
      key: context.idempotencyKey
        ? `c${createHash("sha256").update(`subject-image\0${identity.key}\0${imageKey}`).digest("hex").slice(0, 24)}`
        : newId(),
      scopeKey: input.scopeKey,
      imageKey,
      identityKey: identity.key,
      confidence: score,
      isReference: referenceKeys.has(imageKey),
      createdAt: now,
    }),
  );
  if (
    ...
    throw new GalleryOperationError(
      409,
      "GALLERY_REFERENCES_CHANGED",
      "A reference image or current access changed before the Subject was created.",
    );
  const row = await repository.getSubject(
    input.scopeKey,
    identity.key,
    membership.key,
  );
  if (!row)
    throw new GalleryOperationError(
      500,
      "GALLERY_SUBJECT_FAILED",
      "Subject could not be read after creation.",
    );
  await publish(context, "createSubject", {
    users: await repository.listScopeManagerUserKeys(input.scopeKey),
  });
  return { subject: await safeSubject(row) };
}

async function listSubjectImages(
  rawInput: unknown,
  context: GalleryOperationContext,
) {
  ...
  return {
    images: await Promise.all(
      rows.map(({ image, confidence }) => safeImage(image, confidence)),
    ),
  };
}
...
async function deleteSubject(
  ...
  const value = await repository.deleteSubject(
    input.scopeKey,
    input.identityKey,
    membership.key,
  );
  ...
  await publish(context, "deleteSubject", {
    users: await repository.listScopeManagerUserKeys(input.scopeKey),
  });
...
async function createHighlight(
  ...
  if (imageKeys === undefined) {
    const candidates = await (
      context.listHighlightCandidates ?? repository.listHighlightCandidates
    )(input.scopeKey, input.collectionKey, membership.key);
    if (!candidates)
      throw new GalleryOperationError(
        404,
        "GALLERY_COLLECTION_NOT_FOUND",
    ...
    imageKeys = selectHighlightCandidates(candidates, context.random).map(
      ({ image }) => image.key,
    );
  }
  const now = new Date().toISOString();
  const highlight = imageCollectionHighlightSchema.parse({
    key: requestIdentity("highlight-create", context),
    scopeKey: input.scopeKey,
    collectionKey: input.collectionKey,
    imageKeys,
    createdByKey: membership.key,
  ...
  const saved =
    input.imageKeys === undefined
      ...
      : await persist(highlight, membership.key, {
          requireExactImageKeys: true,
        });
  if (!saved) {
    if (input.imageKeys !== undefined)
      throw new GalleryOperationError(
        409,
        "GALLERY_HIGHLIGHT_IMAGES_UNAVAILABLE",
        "Every selected image must still belong to this collection.",
      );
    throw new GalleryOperationError(
      404,
      "GALLERY_COLLECTION_NOT_FOUND",
  ...
  const row = await (context.getHighlight ?? repository.getHighlight)(
    input.scopeKey,
    saved.key,
    membership.key,
  );
  await publish(context, "highlightChanged", {
    collections: [input.collectionKey],
  });
  return {
    highlight: await safeHighlight(row?.highlight ?? saved, row?.images ?? []),
  };
}
...
async function listHighlights(
  ...
  const membership = await authorize(context);
  const rows = await (context.listHighlights ?? repository.listHighlights)(
    input.scopeKey,
    input.collectionKey,
    membership.key,
  );
  return {
    highlights: await Promise.all(
      rows.map((row) => safeHighlight(row.highlight, row.images)),
    ),
  };
}
...
async function deleteHighlight(
  ...
  await publish(context, "highlightChanged", {
    collections: [highlight.collectionKey],
  });
...
async function createMemory(
  ...
  const memoryKey = requestIdentity(
  ...
  if (context.idempotencyKey) {
    ...
    if (replay)
      return { memory: await safeMemory(replay.memory, replay.image) };
  }
  const candidates = await (
    context.listMemoryCandidates ?? repository.listMemoryCandidates
  )(input.scopeKey, input.collectionKey, membership.key);
  if (!candidates)
    throw new GalleryOperationError(
      404,
  ...
  if (!candidate)
    throw new GalleryOperationError(
      409,
      "GALLERY_MEMORY_IMAGES_EXHAUSTED",
      "Add more unique images to this collection to create another memory.",
    );
  const generationStartedAt = performance.now();
  const generated = context.generateMemory
    ? await context.generateMemory(memoryPrompt(candidate))
    : (
        await executeAsk<ChatOutput>(
          input.teamKey,
          {
            systemPrompt:
            ...
            messages: [
              {
                role: "user",
                content: [{ type: "text", text: memoryPrompt(candidate) }],
              },
            ],
  ...
  const now = new Date().toISOString();
  const memory = imageCollectionMemorySchema.parse({
    key: memoryKey,
    scopeKey: input.scopeKey,
    imageKey: candidate.image.key,
    text,
    createdByKey: membership.key,
    createdAt: now,
    updatedAt: now,
  ...
  const result = await (context.createMemory ?? repository.createMemory)(
    memory,
    input.collectionKey,
    membership.key,
  );
  ...
  console.info("gallery memory created", {
    collectionKey: input.collectionKey,
    ...metrics,
  });
  await publish(context, "memoryCreated", {
    collections: result.collectionKeys,
  });
...
async function listMemories(
  ...
  const membership = await collectionRole(context, input.collectionKey).then(
    ({ membership }) => membership,
  );
  const rows = await (context.listMemories ?? repository.listMemories)(
    input.scopeKey,
    input.collectionKey,
    membership.key,
  );
  return {
    memories: await Promise.all(
      rows.map(({ memory, image }) => safeMemory(memory, image)),
    ),
  };
}
...
async function readMemory(rawInput: unknown, context: GalleryOperationContext) {
  ...
  const membership = await authorize(context);
  const row = await (context.getMemory ?? repository.getAccessibleMemory)(
    input.scopeKey,
    input.memoryKey,
    membership.key,
  );
...
async function deleteMemory(
  ...
  const deleted = await (
    context.deleteMemory ?? repository.deleteAccessibleMemory
  )(input.scopeKey, input.memoryKey, input.collectionKey, membership.key);
  if (!deleted)
    throw new GalleryOperationError(
      403,
      "GALLERY_OWNER_REQUIRED",
