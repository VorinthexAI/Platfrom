...
import { createGalleryService, galleryRequestFingerprint, hashGallerySharePassword, verifyGallerySharePassword, type GalleryRepository } from './index';

const now = '2026-08-07T12:00:00.000Z'; const later = '2026-08-08T12:00:00.000Z';
function repository(overrides: Partial<GalleryRepository> = {}): GalleryRepository {
...
const requestFingerprint = (value: string) => galleryRequestFingerprint(value, Buffer.alloc(32, 9));
describe('Gallery service boundaries', () => {
  test('strictly rejects unknown fields', async () => { await expect(createGalleryService({ repository: repository() }).leaveCollection({ scopeKey: newId(), collectionKey: newId(), actorKey: newId(), forged: true })).rejects.toThrow(); });
  test('returns one-time tokens while removing persisted hashes', async () => {
    ...
    const service = createGalleryService({ repository: repository(), token: () => 'one-time-gallery-token-that-is-long-enough', hashPassword: async () => 'scrypt:long-enough-password-hash', ...replay });
  ...
  test('enforces ownership and future expiry', async () => {
    const service = createGalleryService({ repository: repository({ ownsCollection: async () => false, createGlobalShare: async () => null }) });
  ...
  test('verifies protected active shares without exposing hashes', async () => {
    const passwordHash = await hashGallerySharePassword('correct password');
    expect(await verifyGallerySharePassword('correct password', passwordHash)).toBe(true);
    expect(await verifyGallerySharePassword('wrong password', passwordHash)).toBe(false);
    ...
    const service = createGalleryService({ repository: repository({ getActiveGlobalShareByTokenHash: async () => active }) });
  ...
  test('replays token creates by idempotency key and rejects changed payloads', async () => {
    ...
    let sequence = 0;
    const service = createGalleryService({ repository: repo, token: () => `token-${String(++sequence).padStart(32, '0')}`, encryptReplay: JSON.stringify, decryptReplay: JSON.parse, hashPassword: async () => 'scrypt:long-enough-password-hash', requestFingerprint });
  ...
  test('returns an existing accepted membership without accepting twice', async () => {
    ...
    const service = createGalleryService({ repository: repository({ getAcceptedCollectionInviteMembership: async () => membership, acceptCollectionInvite: async () => { acceptanceCalls += 1; return null; } }) });
  ...
  test('requires source image access before add, copy, or move', async () => {
    let mutations = 0;
    const service = createGalleryService({ repository: repository({ canAccessImage: async () => false, addImageToCollection: async (value) => { mutations += 1; return value; }, copyImageToCollection: async (value) => { mutations += 1; return value; }, moveImageBetweenCollections: async (_source, value) => { mutations += 1; return value; } }) });
  ...
  test('revalidates current ownership before replaying invite or share secrets', async () => {
    ...
    const service = createGalleryService({ repository: repo, token: () => 'replay-token-that-is-at-least-thirty-two-characters', encryptReplay: JSON.stringify, decryptReplay: (value) => { decryptions += 1; return JSON.parse(value); }, requestFingerprint });
