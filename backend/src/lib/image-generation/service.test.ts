...
describe('image generation service', () => {
  ...
  test('pins idea and raw image actions to their required model routes', async () => {
    ...
    const service = createImageGenerationService({
      ...
      execute: (async (...args: unknown[]) => {
        ...
        return request.actionSlug === 'ask'
          ? { output: { text: '{"concepts":[{"title":"Orbit","prompt":"A complete orbital image"}]}', toolCalls: [], stopReason: 'completed' }, usage: {}, providerId: 'openrouter', modelId: 'google.gemini-2.5-flash-lite', externalModelId: 'google/gemini-2.5-flash-lite' }
          : { output: { images: [{ base64: png, mimeType: 'image/png' }] }, usage: {}, costUsd: 0.12, providerId: 'openrouter', modelId: 'openai.gpt-image-2', externalModelId: 'openai/gpt-image-2' };
      }) as any,
    ...
    expect(calls[0]?.[0]).toEqual({ mode: 'model', organizationKey, actionSlug: 'ask', modelSlug: 'google.gemini-2.5-flash-lite' });
    expect(calls[1]?.[0]).toEqual({ mode: 'fixed', organizationKey, actionSlug: 'generate-image', modelSlug: 'openai.gpt-image-2', providerSlug: 'openrouter' });
  ...
  test('persists generated bytes with deterministic Gallery idempotency and returns only safe projections', async () => {
    ...
    const service = createImageGenerationService({
      getImage: async () => null,
      execute: (async () => { providerCalls += 1; await Bun.sleep(5); return { output: { images: [{ base64: png, mimeType: 'image/png' }] }, usage: {}, costUsd: 0.2, providerId: 'openrouter', modelId: 'openai.gpt-image-2', externalModelId: 'openai/gpt-image-2' }; }) as any,
