...
describe('MediaLibrary repository transactions', () => {
  test('requires existing source-image access in the add query', async () => {
    ...
    expect(query).toContain('collection.ownerKey == @addedByKey && sourceAccess');
  });

  test('requires write-level scope access to own standalone images', async () => {
    let query = '';
    const database: MediaLibraryDatabase = { async query(value) { query = value; return { async all() { return []; } }; } };
    const repository = createMediaLibraryRepository(database, async (operation) => operation(database));
...
describe('MediaLibrary image similarity search', () => {
  test('grants semantic search access to live scope members without a managed collection membership', async () => {
    const teamKey = newId(), scopeKey = newId(), actorKey = newId(), collectionKey = newId(), imageKey = newId();
    ...
    const results = await searchAccessibleImages({ teamKey, scopeKey, actorKey, collectionKey, embedding, limit: 50 }, database);
    expect(query).toContain('actorMembership.status == "active"');
    expect(query).toContain('actorMembership.teamKey == @teamKey');
    ...
    expect(bindVars).toMatchObject({ teamKey, scopeKey, actorKey, collectionKey, dimensions: EMBEDDING_DIMENSIONS, threshold: null, limit: 50, createdFrom: null, createdTo: null });
  ...
  test('binds an explicit threshold without interpolating it into AQL', async () => {
    ...
    await searchAccessibleImages({ teamKey: newId(), scopeKey: newId(), actorKey: newId(), embedding: Array.from({ length: EMBEDDING_DIMENSIONS }, () => 0), threshold: 0.97, limit: 10 }, database);
