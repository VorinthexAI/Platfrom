...
export async function searchAccessibleImages(
    ...
    FOR image IN images
      ...
      LET collectionAccess = LENGTH(
        FOR relation IN collectionImages
          ...
          LET managedViewer = collection.purpose IN ["email-media", "generated-media"] && collection.mutationPolicy == "system-only" && scoped
  ...
  return (await cursor.all()).map((value) => {
    const row = value as { image?: unknown; score?: unknown; collections?: unknown };
    if (typeof row.score !== 'number' || !Number.isFinite(row.score)) throw new Error('Image similarity query returned an invalid score.');
    const collections = Array.isArray(row.collections) ? row.collections as Array<{ key: string; name: string }> : [];
    return { image: parse(imageSchema, row.image), score: row.score, collections };
  });
}
...
export function createMediaLibraryRepository(database: MediaLibraryDatabase = db, runTransaction: MediaLibraryTransactionRunner = defaultTransactionRunner): MediaLibraryRepository {
  ...
  const owns = async (kind: 'image' | 'collection', scopeKey: string, resourceKey: string, ownerKey: string) => Boolean(await one(database, kind === 'collection'
    ? `LET target = DOCUMENT(collections, @resourceKey) ${activeActor} FILTER target != null && target.scopeKey == @scopeKey && target.mutationPolicy != "system-only" FOR member IN collectionMembers FILTER member.scopeKey == @scopeKey && member.collectionKey == @resourceKey && member.memberKey == @ownerKey && member.role == "owner" LIMIT 1 RETURN true`
    : `LET target = DOCUMENT(images, @resourceKey) ${activeActor} FILTER target != null && target.scopeKey == @scopeKey && target.mutationPolicy != "system-only" FILTER writable || elevated || LENGTH(FOR relation IN collectionImages FILTER relation.scopeKey == @scopeKey && relation.imageKey == @resourceKey LET collection = DOCUMENT(collections, relation.collectionKey) FILTER collection != null && collection.mutationPolicy != "system-only" FOR member IN collectionMembers FILTER member.scopeKey == @scopeKey && member.collectionKey == relation.collectionKey && member.memberKey == @ownerKey && member.role == "owner" LIMIT 1 RETURN 1) > 0 RETURN true`, { scopeKey, resourceKey, ownerKey, actorKey: ownerKey }));
  return {
