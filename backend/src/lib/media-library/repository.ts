...
const transactionCollections = ['images', 'collections', 'collectionImages', 'collectionMembers', 'collectionInvites', 'tags', 'tagAssignments', 'shares', 'documents', 'places', 'trips', 'scopes', 'scopeMembers', 'userOrganizations', 'users'] as const;
...
export function createMediaLibraryRepository(database: MediaLibraryDatabase = db, runTransaction: MediaLibraryTransactionRunner = defaultTransactionRunner): MediaLibraryRepository {
  ...
  const owns = async (kind: 'image' | 'collection', scopeKey: string, resourceKey: string, ownerKey: string) => Boolean(await one(database, kind === 'collection'
    ? `LET target = DOCUMENT(collections, @resourceKey) ${activeActor} FILTER target != null && target.scopeKey == @scopeKey && target.deletedAt == null FOR member IN collectionMembers FILTER member.scopeKey == @scopeKey && member.collectionKey == @resourceKey && member.memberKey == @ownerKey && member.role == "owner" LIMIT 1 RETURN true`
    : `LET target = DOCUMENT(images, @resourceKey) ${activeActor} FILTER target != null && target.scopeKey == @scopeKey && target.deletedAt == null FILTER writable || elevated || LENGTH(FOR relation IN collectionImages FILTER relation.scopeKey == @scopeKey && relation.imageKey == @resourceKey LET collection = DOCUMENT(collections, relation.collectionKey) FILTER collection != null && collection.deletedAt == null FOR member IN collectionMembers FILTER member.scopeKey == @scopeKey && member.collectionKey == relation.collectionKey && member.memberKey == @ownerKey && member.role == "owner" LIMIT 1 RETURN 1) > 0 RETURN true`, { scopeKey, resourceKey, ownerKey, actorKey: ownerKey }));
  return {
    async getImage(scopeKey, imageKey) { const value = await one(database, 'FOR image IN images FILTER image._key == @imageKey && image.scopeKey == @scopeKey && image.deletedAt == null LIMIT 1 RETURN image', { scopeKey, imageKey }); return value ? parse(imageSchema, value) : null; },
