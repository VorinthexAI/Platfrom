...
const shareInputSchema = z.preprocess((raw) => raw && typeof raw === 'object' && !Array.isArray(raw) && !('permission' in raw) ? { ...raw, permission: (raw as { sourceType?: unknown }).sourceType === 'collection' ? 'viewer' : 'read' } : raw, z.object({ scopeKey: key, sourceType: z.enum(['image', 'collection', 'place', 'trip']), sourceKey: key, ownerKey: key, permission: z.enum(['read', 'comment', 'viewer', 'collaborator']), expiresAt: timestamp.optional(), password: z.string().min(1).max(256).optional(), now: timestamp, idempotencyKey }).strict().superRefine((value, context) => {
