...
const inviteInputSchema = z.object({ scopeKey: key, collectionKey: key, invitedByKey: key, inviteeKey: key.optional(), email: z.string().trim().toLowerCase().email().optional(), expiresAt: timestamp.optional(), now: timestamp, idempotencyKey }).strict().refine((value) => (value.inviteeKey === undefined) !== (value.email === undefined), 'Exactly one recipient is required');
const acceptInviteInputSchema = z.object({ token: z.string().min(32).max(512), recipientKey: key, now: timestamp }).strict();
const assignmentInputSchema = z.object({ scopeKey: key, tagKey: key, sourceType: z.enum(['document', 'image', 'collection', 'place', 'trip']), sourceKey: key, actorKey: key, source: z.enum(['user', 'ai']), now: timestamp }).strict();
const coverInputSchema = z.object({ scopeKey: key, collectionKey: key, imageKey: key, ownerKey: key, now: timestamp }).strict();
const shareInputSchema = z.object({ scopeKey: key, sourceType: z.enum(['image', 'collection', 'place', 'trip']), sourceKey: key, ownerKey: key, permission: z.enum(['read', 'comment']).default('read'), expiresAt: timestamp.optional(), password: z.string().min(1).max(256).optional(), now: timestamp, idempotencyKey }).strict();
