...
export function resolveS3ClientConfig(env: NodeJS.ProcessEnv = process.env): S3ClientConfig {
  ...
  const accessKeyId = env.S3_AWS_ACCESS_KEY_ID ?? env.AWS_ACCESS_KEY_ID;
  const secretAccessKey = env.S3_AWS_SECRET_ACCESS_KEY ?? env.AWS_SECRET_ACCESS_KEY;
  const sessionToken = env.S3_AWS_SESSION_TOKEN ?? env.AWS_SESSION_TOKEN;
  return {
    region: env.S3_REGION ?? env.AWS_REGION ?? 'eu-north-1',
    endpoint,
