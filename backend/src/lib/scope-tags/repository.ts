...
export type ScopeTagOwner = { teamKey: string; scopeKey: string; userKey: string; teamMembershipKey: string };
...
const transactionCollections = {
  read: ['scopes', 'userTeams', 'scopeMembers', 'folders', 'documents', 'collections', 'collectionImages', 'images', 'imageCollecitionHightlights', 'imageCollectionMemories', 'places', 'trips', 'emailInboxes', 'emailTones', 'emailThreads', 'emailMessages', 'emailDrafts', 'books'],
...
const targetAccessPreludeQuery = `
  ...
  LET scopeRole = membership == null ? null : FIRST(FOR member IN scopeMembers FILTER member.scopeKey == @scopeKey && member.userTeamKey == @teamMembershipKey && member.status == "active" LIMIT 1 RETURN member.role)
  LET active = membership != null && membership.status == "active" && membership.userId == @userKey && membership.teamKey == @teamKey && scope != null && scope.teamKey == @teamKey
  LET scoped = scopeRole IN ["owner", "admin", "moderator", "viewer"]
  LET elevated = membership != null && membership.teamRole IN ["owner", "admin"]
`;
...
export function createScopeTagRepository(database: ScopeTagDatabase = db as unknown as ScopeTagDatabase, transact: ScopeTagTransactionRunner = defaultTransaction): ScopeTagRepository {
  return {
    async list(owner, input) {
      const authorization = input.target ? `${targetAccessQuery} FILTER readable` : 'LET membership = DOCUMENT(userTeams, @teamMembershipKey) LET scope = DOCUMENT(scopes, @scopeKey) LET role = FIRST(FOR member IN scopeMembers FILTER member.scopeKey == @scopeKey && member.userTeamKey == @teamMembershipKey && member.status == "active" LIMIT 1 RETURN member.role) FILTER membership != null && membership.status == "active" && membership.userId == @userKey && membership.teamKey == @teamKey && scope != null && scope.teamKey == @teamKey && (membership.teamRole IN ["owner", "admin"] || role IN ["owner", "admin", "moderator", "viewer"])';
