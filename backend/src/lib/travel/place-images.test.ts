...
const generated = (costUsd: number | null = 0.125, base64 = 'AQ==') => ({ output: { images: [{ base64, mimeType: 'image/png' as const }] }, durationMs: 25, costUsd });
...
describe('transient place image generation', () => {
  ...
  test('strictly accepts only trusted context and an opaque token, then authorizes before token use', async () => {
    ...
    const generate = createPlaceImageGenerator({
      ...
      images: { generateRaw: async () => { providerCalls += 1; return generated(); } },
    });
  ...
  test('runs exactly four provider calls in parallel and returns ordered inline WebP images without persistence', async () => {
    let active = 0;
    const calls: Array<{ value: any; organization: string; execution: any }> = [];
    ...
    const generate = createPlaceImageGenerator({
      repository,
      ...token,
      images: { generateRaw: async (value, organization, execution) => { calls.push({ value, organization, execution }); active += 1; await gate; active -= 1; return generated(); } },
    ...
    expect(calls.every(({ value, organization, execution }) => value.count === 1 && value.size === '1024x1536' && value.quality === 'low' && organization === organizationKey && execution.timeoutMs === 12_345 && execution.signal instanceof AbortSignal)).toBe(true);
    ...
    expect(metrics[0]).toMatchObject({ state: 'ready', countryCode: 'JP', conceptTitles: assetConcepts.map(({ title }) => title), providerDurationMs: [25, 25, 25, 25], costUsd: 0.5 });
  ...
  test('uses Sharp to crop every generated image to exact 864x1536 WebP output', async () => {
    ...
    const generate = createPlaceImageGenerator({ repository, ...token, images: { generateRaw: async () => generated(0.1, source.toString('base64')) }, log: () => {} });
  ...
  test('throws provider and processing failures and reports all-or-null aggregate cost', async () => {
    let calls = 0;
    const providerFailure = createPlaceImageGenerator({ repository, ...token, images: { generateRaw: async () => { calls += 1; if (calls === 2) throw new Error('provider failed'); return generated(); } }, transform: async () => new Uint8Array([1]), log: () => {} });
    ...
    const unknownCost = createPlaceImageGenerator({ repository, ...token, images: { generateRaw: async () => generated(costs.shift()!) }, transform: async () => new Uint8Array([1]), log: () => {} });
    ...
    const processingFailure = createPlaceImageGenerator({ repository, ...token, images: { generateRaw: async () => generated() }, transform: async () => { throw new Error('sharp failed'); }, log: () => {} });
  ...
  test('coalesces concurrent token use and rejects completed replay while retaining no sensitive state', async () => {
    ...
    const dependencies = {
      repository,
      ...token,
      images: { generateRaw: async () => { calls += 1; active += 1; maximum = Math.max(maximum, active); if (calls <= 4) await firstGate; active -= 1; return generated(); } },
  ...
  test('propagates cancellation while a set is waiting for the process semaphore', async () => {
    ...
    let release!: () => void;
    const gate = new Promise<void>((resolve) => { release = resolve; });
    const dependencies = { repository, ...token, images: { generateRaw: async () => { firstCalls += 1; await gate; return generated(); } }, transform: async () => new Uint8Array([1]), log: () => {} };
    const first = createPlaceImageGenerator(dependencies)(input, userKey);
  ...
  test('rejects transformed WebP output larger than four MiB', async () => {
    const generate = createPlaceImageGenerator({ repository, ...token, images: { generateRaw: async () => generated() }, transform: async () => new Uint8Array(PLACE_IMAGE_WEBP_MAX_BYTES + 1), log: () => {} });
