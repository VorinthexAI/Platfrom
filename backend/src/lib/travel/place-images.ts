      ...
      try {
        ...
        const generated = await Promise.all(concepts.map(async (concept) => {
          ...
          const response = await execute<Record<string, unknown>, ImageOutput>(
            ...
            { prompt: `Authoritative country: ${country.name} (${country.countryCode}), ${country.continent}. ${concept.prompt}`, count: 1, size: '1024x1536', quality: 'low' },
            { signal, timeoutMs: execution.timeoutMs } satisfies Pick<ExecuteActionOptions, 'signal' | 'timeoutMs'>,
          );
          const output = imageOutputSchema.parse(response.output);
          if (output.images.length !== 1) throw new Error(`Image provider returned ${output.images.length} images; expected 1.`);
          return { output, durationMs: elapsed(now, providerStarted), costUsd: response.costUsd ?? null };
        }));
        if (signal?.aborted) throw signal.reason ?? new Error('Place image generation aborted.');
