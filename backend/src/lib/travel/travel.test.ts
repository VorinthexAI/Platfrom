...
describe('travel contracts and service', () => {
  test('searches through the ask action after authorization without reading or mutating place persistence', async () => {
    ...
    await expect(service.findPlaces({ teamKey: 'team', scopeKey, query: ' culture and food ' }, key)).resolves.toEqual({ results });
    expect(calls).toEqual(['authorize', 'execute']);
    expect(travelPlaceSearchInputSchema.safeParse({ teamKey: 'team', scopeKey, query: 'x' }).success).toBe(false);
    expect(travelPlaceSearchInputSchema.safeParse({ teamKey: 'team', scopeKey, query: 'x'.repeat(501) }).success).toBe(false);
  });
  ...
  test('retries malformed or duplicate mixed search output once and fails safely', async () => {
    ...
    await expect(service.findPlaces({ teamKey: 'team', scopeKey, query: 'Japan' }, key)).rejects.toMatchObject({ guideKind: 'search', message: 'Place search provider returned an invalid response.' });
  ...
  test('semantically searches only repository-authorized saved places and signs covers', async () => {
    ...
    await expect(service.searchPlaces({ teamKey: 'team', scopeKey, query: ' neighborhoods ', recordHistory: false, createdFrom: timestamp, createdTo: timestamp }, key)).resolves.toEqual({ places: [placeDto(place, 'https://signed.test/media/tokyo.png')] });
    ...
    calls.length = 0;
    expect(JSON.stringify(await service.searchPlaces({ teamKey: 'team', scopeKey, query: 'Tokyo' }, key))).not.toContain('embedding');
  ...
  test('searches trips with the same complete aggregate projection as list', async () => {
    ...
    const result = await service.searchTrips({ teamKey: 'team', scopeKey, query: 'Japan spring', recordHistory: false, createdFrom: timestamp, createdTo: timestamp }, key);
  ...
  test('rejects reversed creation-date ranges for place and trip searches', () => {
    const reversed = { teamKey: 'team', scopeKey, query: 'Tokyo', createdFrom: '2026-08-12T00:00:00.000Z', createdTo: timestamp };
  ...
  test('keeps committed trip mutations successful when place and cover signing fail', async () => {
    ...
    await expect(service.createTrip({ teamKey: 'team', scopeKey, name: 'Tokyo route', placeKeys: [key], idempotencyKey: 'request-1' }, key)).resolves.toEqual({ trip: { key, name: 'Tokyo route', status: 'planned', isFavorite: false, createdAt: timestamp, updatedAt: timestamp, places: [placeDto(place)], attachments: [] } });
  });
  ...
  test('generates, embeds, persists, replays, and lists strict formatted trip guides', async () => {
    ...
    const service = createTravelService({
      ...
      execute: (async (_team: string, input: any) => {
        attempts += 1;
    ...
    const input = { teamKey: 'team', scopeKey, tripKey: key, idempotencyKey: 'guide-request-1' };
    ...
    await expect(service.listTripGuides({ teamKey: 'team', scopeKey, tripKey: key }, key)).resolves.toEqual({ guides: [generated.guide] });
    expect(travelTripGuideGenerateInputSchema.safeParse({ ...input, userKey: key }).success).toBe(false);
    expect(travelTripGuideListInputSchema.safeParse({ teamKey: 'team', scopeKey, limit: 1 }).success).toBe(false);
  ...
  test('generates parameterized place references, replays, and publishes best effort', async () => {
    for (const [referenceKind, expected] of [['brief', ['Overview', 'stable general knowledge']], ['accommodations', ['five specific', 'budget tradeoffs']], ['restaurants', ['five specific', 'live hours']], ['activities', ['five specific', 'pacing']]] as const) {
      ...
      const service = createTravelService({
        ...
        execute: (async (_team: string, input: any) => { calls.push(chatPrompt(input)); return chatResponse(JSON.stringify({ summary: generatedSummary })); }) as any,
        embed: async ({ text }) => { calls.push(`embed:${text}`); return embedding; },
        publishPlaceReferenceChanged: async (changedScopeKey: string) => { calls.push(`publish:${changedScopeKey}`); throw new Error('best effort'); },
        publishContentChanged: async () => {},
      });
      const input = { teamKey: 'team', scopeKey, placeKey: key, kind: referenceKind, idempotencyKey: `${referenceKind}-reference` };
      ...
      await expect(service.listPlaceReferences({ teamKey: 'team', scopeKey, placeKey: key, kind: referenceKind }, key)).resolves.toEqual({ references: [generated.reference] });
      expect(travelPlaceReferenceGenerateInputSchema.safeParse({ ...input, kind: 'country' }).success).toBe(false);
      expect(travelPlaceReferenceListInputSchema.safeParse({ teamKey: 'team', scopeKey, placeKey: key, kind: referenceKind, userKey: key }).success).toBe(false);
    }
  });
  ...
  test('varies repeated recommendation references and rejects malformed five-item output', async () => {
    ...
    const base = { teamKey: 'team', scopeKey, placeKey: key, kind: 'activities' as const };
  ...
  test('creates and lists strict trips with server identity, ordered places, and first-place cover', async () => {
    ...
    const input = { teamKey: 'team', scopeKey, name: ' Tokyo route ', description: ' Spring trip ', placeKeys: [key], idempotencyKey: 'request-1' };
    ...
    expect(context).toEqual({ teamKey: 'team', scopeKey, userKey: key });
    ...
    await expect(service.listTrips({ teamKey: 'team', scopeKey }, key)).resolves.toEqual({ trips: [created.trip] });
    ...
    expect(travelTripListInputSchema.safeParse({ teamKey: 'team', scopeKey, userKey: key }).success).toBe(false);
  });
  test('replaces trip attachments with server-owned relation fields and reference-only output', async () => {
    ...
    const result = await service.setTripAttachments({ teamKey: 'team', scopeKey, tripKey: key, attachments }, key);
    expect(result.trip.attachments).toEqual(attachments);
    expect(call[0]).toEqual({ teamKey: 'team', scopeKey, userKey: key });
    ...
    await expect(service.setTripAttachments({ teamKey: 'team', scopeKey, tripKey: key, attachments: [] }, key)).resolves.toMatchObject({ trip: { attachments: [] } });
    expect(travelTripAttachmentSetInputSchema.safeParse({ teamKey: 'team', scopeKey, tripKey: key, attachments: [attachments[0], attachments[0]] }).success).toBe(false);
    expect(travelTripAttachmentSetInputSchema.safeParse({ teamKey: 'team', scopeKey, tripKey: key, attachments: [{ type: 'file', key }] }).success).toBe(false);
    expect(travelTripAttachmentSetInputSchema.safeParse({ teamKey: 'team', scopeKey, tripKey: key, attachments: [{ type: 'folder', key, position: 0 }] }).success).toBe(false);
  });
  test('updates aggregate trip fields with server-owned relation metadata and deletes by strict key', async () => {
    ...
    const input = { teamKey: 'team', scopeKey, tripKey: key, name: ' Spring route ', description: null, coverImageKey: key, status: 'completed' as const, isFavorite: true, placeKeys: [key] };
    ...
    await expect(service.deleteTrip({ teamKey: 'team', scopeKey, tripKey: key }, key)).resolves.toEqual({ tripKey: key });
    await expect(service.deletePlace({ teamKey: 'team', scopeKey, placeKey: key }, key)).resolves.toEqual({ placeKey: key });
    expect(travelTripUpdateInputSchema.safeParse({ teamKey: 'team', scopeKey, tripKey: key }).success).toBe(false);
    expect(travelTripUpdateInputSchema.safeParse({ teamKey: 'team', scopeKey, tripKey: key, placeKeys: [key, key] }).success).toBe(false);
    expect(travelTripUpdateInputSchema.safeParse({ teamKey: 'team', scopeKey, tripKey: key, isFavorite: true, position: 0 }).success).toBe(false);
    expect(travelTripDeleteInputSchema.safeParse({ teamKey: 'team', scopeKey, tripKey: key, userKey: key }).success).toBe(false);
    expect(travelPlaceDeleteInputSchema.safeParse({ teamKey: 'team', scopeKey, placeKey: key, userKey: key }).success).toBe(false);
  });
  test('regenerates trip embeddings only when semantic fields actually change', async () => {
    ...
    await service.updateTrip({ teamKey: 'team', scopeKey, tripKey: key, status: 'completed' }, key);
    await service.updateTrip({ teamKey: 'team', scopeKey, tripKey: key, name: 'Tokyo route', description: 'Spring' }, key);
    await service.updateTrip({ teamKey: 'team', scopeKey, tripKey: key, description: 'Autumn' }, key);
  ...
  test('updates a saved place through the canonical repository and signs its cover', async () => {
    ...
    const input = { teamKey: 'team', scopeKey, placeKey: key, status: 'visited' as const, isFavorite: true };
    await expect(service.updatePlace(input, key)).resolves.toEqual({ place: placeDto(updated, 'https://signed.test/media/tokyo.png') });
    expect(calls).toEqual([[{ teamKey: 'team', scopeKey, userKey: key }, key, { status: 'visited', isFavorite: true }]]);
    expect(travelPlaceUpdateInputSchema.safeParse({ teamKey: 'team', scopeKey, placeKey: key }).success).toBe(false);
  ...
  test('keeps saved-place metadata updates successful when cover signing fails', async () => {
    ...
    await expect(service.updatePlace({ teamKey: 'team', scopeKey, placeKey: key, status: 'visited' }, key)).resolves.toEqual({ place: placeDto(updated) });
  });
  test('creates canonical hero storage and an independent best-effort Gallery copy from staged bytes', async () => {
    ...
    const service = createTravelService({ repository, storage, decryptImageRequest: () => token, embed: async () => embedding, now: () => timestamp, signImageUrl: async (storageKey) => `https://signed.test/${storageKey}`,
      ...
      ingestGalleryUpload: async (input) => { exportOrder.push('dump'); processed.push(input.bytes); expect(input).toMatchObject({ collectionKey: key, mimeType: 'image/png', filename: 'japan.png' }); return { key } as never; } });
  ...
  test('keeps inputs and the focused recommendation response strict', () => {
    const context = { teamKey: 'team', scopeKey };
  ...
  test('preserves the simplified place overview projection', async () => {
    ...
    await expect(service.overview({ teamKey: 'team', scopeKey }, 'user')).resolves.toEqual({ places: [placeDto(place, 'https://signed.test/media/tokyo.png')], recentPlaces: [recentPlaceDto(recent)] });
  });
  ...
  test('opens only an existing authorized generated place with a server timestamp', async () => {
    ...
    const service = createTravelService({ repository, now: () => timestamp });
    await expect(service.openPlace({ teamKey: 'team', scopeKey, name: ' Tokyo ', countryCode: 'jp' }, key)).resolves.toEqual({ place: recentPlaceDto(opened) });
    expect(calls).toEqual([[{ teamKey: 'team', scopeKey, userKey: key }, 'JP', 'Tokyo', timestamp]]);
  });
  ...
  test('authorizes writes before embedding and saves countries and cities through one canonical method', async () => {
    ...
    const service = createTravelService({ repository, ingestGalleryUpload: async (value) => { calls.push(['ingest-gallery', value]); return { key } as never; }, storage: { delete: async () => {}, upload: async ({ key: storageKey }: any) => ({ storageKey }), download: async () => ({ bytes: placePngBytes }), copy: async () => ({ storageKey: '' }) }, decryptImageRequest: () => ({ version: 5, issuedAt: Date.parse(timestamp), nonce: 'A'.repeat(43), teamKey: 'team', scopeKey, country: { name: 'Japan', countryCode: 'JP', continent: 'Asia', latitude: 36.2, longitude: 138.2 }, place: { kind: 'country', name: 'Japan', summary: 'Island country.', countryCode: 'JP', latitude: 36.2048, longitude: 138.2529 }, hero: { title: 'Japan', prompt: 'prompt' } }), embed: async (value) => { calls.push(['embed', value]); return embedding; }, now: () => timestamp, signImageUrl: async (storageKey) => `https://signed.test/${storageKey}` });
  ...
  test('saves the explicit city kind when the city and country have the same name', async () => {
    ...
    const service = createTravelService({
      repository, ingestGalleryUpload: async () => ({ key } as never), embed: async () => embedding, now: () => timestamp,
  ...
  test('does not call the embedding provider when place creation is unauthorized', async () => {
    ...
    await expect(service.createPlace({ teamKey: 'team', scopeKey, name: 'Japan', summary: 'Island country.', countryCode: 'JP', latitude: 36.2, longitude: 138.2, imageRequestToken: 'token' }, 'user')).rejects.toMatchObject({ reason: 'forbidden' });
  ...
  test('authorizes before one guide and image-brief request and seals the destination-specific hero prompt', async () => {
    ...
    const result = await service.findPlaceGuide({ teamKey: 'team', scopeKey, query: 'Portugal', country }, key, { signal: controller.signal, timeoutMs: 2_000 });
    ...
    expect(calls[0]).toEqual(['authorize', { teamKey: 'team', scopeKey, userKey: key }]);
    ...
    expect(sealed).toMatchObject({ version: 5, issuedAt: Date.parse('2026-08-19T12:00:00.000Z'), nonce: 'A'.repeat(43), teamKey: 'team', scopeKey, country: { name: 'Portugal', countryCode: 'PT' }, place: { kind: 'country', name: 'Portugal', summary, countryCode: 'PT', latitude: 39.4, longitude: -8.2 }, hero: { title: 'Portugal travel interpretation', prompt: expect.stringContaining(imageBriefFor('Portugal')) } });
  ...
  test('retries one malformed country guide response and returns the valid retry', async () => {
    ...
    const service = createTravelService({
      ...
      execute: (async (_teamKey: string, input: any) => isBrief(input) ? (briefs += 1, chatResponse(imageBriefFor('Kyoto'))) : chatResponse(++attempts === 1 ? '{"incomplete":' : JSON.stringify(retriedDetail))) as any,
      embed: async () => embedding,
      issueImageNonce: () => 'A'.repeat(43),
      encryptImageRequest: () => 'image-token',
    });
    await expect(service.findPlaceGuide({ teamKey: 'team', scopeKey, query: 'Kyoto' }, key)).resolves.toMatchObject({ place: { title: 'Kyoto' } });
    expect(attempts).toBe(2);
  ...
  test('accepts an exact fenced JSON guide returned by the destination model', async () => {
    const service = createTravelService({
      repository: { ...generatedPersistence, authorizeWrite: async () => key } as unknown as TravelRepository,
      execute: (async (_teamKey: string, input: any) => isBrief(input)
    ...
    const result = await service.findPlaceGuide({ teamKey: 'team', scopeKey, query: 'Japan', country: { name: 'Japan', code: 'JP', continent: 'Asia', lat: 36.2, lon: 138.2 } }, key);
  ...
  test('normalizes wrapped country JSON and falls back from invalid image briefs', async () => {
    ...
    const service = createTravelService({
      repository: { ...generatedPersistence, authorizeWrite: async () => key } as unknown as TravelRepository,
      execute: (async (_teamKey: string, input: any) => isBrief(input) ? (briefAttempts += 1, chatResponse('invalid')) : chatResponse(`Country guide follows:\n${JSON.stringify(wrapped)}\nEnd of guide.`)) as any,
      embed: async () => embedding,
      issueImageNonce: () => 'A'.repeat(43),
      encryptChildrenRequest: () => 'children-token',
  ...
  test('retries a generic or human-focused image brief before sealing it', async () => {
    ...
    await expect(service.findPlaceGuide({ teamKey: 'team', scopeKey, query: 'Japan', country: { name: 'Japan', code: 'JP', continent: 'Asia', lat: 36.2, lon: 138.2 } }, key)).resolves.toMatchObject({ place: { title: 'Japan' } });
    expect(briefAttempts).toBe(2);
  ...
  test('JSON-quotes an authoritative destination title in the image brief and retry', async () => {
    ...
    const service = createTravelService({
      repository: { ...generatedPersistence, authorizeWrite: async () => key } as unknown as TravelRepository,
      execute: (async (_teamKey: string, input: any) => {
    ...
    await service.findPlaceGuide({ teamKey: 'team', scopeKey, query: destination, country: { name: destination, code: 'PT', continent: 'Europe', lat: 39.4, lon: -8.2 } }, key);
  ...
  test('returns durable generated detail with fresh tokens without model or embedding work', async () => {
    ...
    const input = { teamKey: 'team', scopeKey, query: 'Japan', country: { name: 'Japan', code: 'JP', continent: 'Asia', lat: 36.2048, lon: 138.2529 } };
  ...
  test('reuses schema-valid persisted country detail without requiring a version marker', async () => {
    ...
    const service = createTravelService({
      ...
      execute: (async (_teamKey: string, input: any) => { actionCalls += 1; return chatResponse(isBrief(input) ? imageBriefFor('Japan') : JSON.stringify(modelGuideDetail)); }) as any,
      embed: async () => { embeds += 1; return embedding; },
      issueImageNonce: () => 'A'.repeat(43),
      encryptImageRequest: () => 'image-token',
      encryptChildrenRequest: () => 'children-token',
  ...
  test('reuses schema-valid persisted city detail without requiring a version marker', async () => {
    ...
    const result = await service.findCity({ teamKey: 'team', scopeKey, city: 'Tokyo', country: { name: 'Japan', code: 'JP', continent: 'Asia', lat: 36.2, lon: 138.2 } }, key);
  ...
  test('returns ten cities and focused recommendations across representative countries', async () => {
    for (const country of [{ name: 'Japan', code: 'JP', continent: 'Asia' }, { name: 'Brazil', code: 'BR', continent: 'South America' }, { name: 'Kenya', code: 'KE', continent: 'Africa' }, { name: 'Norway', code: 'NO', continent: 'Europe' }]) {
      ...
      const service = createTravelService({ repository: { ...generatedPersistence, authorizeWrite: async () => key } as unknown as TravelRepository, execute: (async (_teamKey: string, input: any) => chatResponse(isBrief(input) ? imageBriefFor(country.name) : JSON.stringify(researched))) as any, embed: async () => embedding, issueImageNonce: () => 'A'.repeat(43), encryptImageRequest: () => 'token' });
      const result = await service.findPlaceGuide({ teamKey: 'team', scopeKey, query: country.name, country: { ...country, lat: 1, lon: 1 } }, key);
      expect(result.place.popularCities).toHaveLength(10);
  ...
  test('grounds a city guide in its authoritative country and seals a city-specific hero', async () => {
    ...
    const service = createTravelService({ repository: { ...generatedPersistence, authorizeWrite: async () => key } as unknown as TravelRepository, execute: (async (_teamKey: string, input: any) => { if (isBrief(input)) return chatResponse(imageBriefFor('Tokyo')); cityPrompt = chatPrompt(input); return chatResponse(JSON.stringify(cityDetail)); }) as any, embed: async () => embedding, issueImageNonce: () => 'A'.repeat(43), encryptImageRequest: (value) => { sealed = value; return 'token'; } });
    const result = await service.findCity({ teamKey: 'team', scopeKey, city: 'Tokyo', country: { name: 'Japan', code: 'JP', continent: 'Asia', lat: 36.2, lon: 138.2 } }, key);
  ...
  test('seals the exact country children contract and expands all ten cities concurrently in order', async () => {
    ...
    expect(childrenRequestTokenSchema.parse(sealedChildren)).toEqual({
      version: 1, teamKey: 'team', scopeKey, issuedAt, expiresAt: issuedAt + CHILDREN_REQUEST_TOKEN_VALIDITY_MS, nonce: 'Z'.repeat(43),
      country: { name: 'Japan', code: 'JP', continent: 'Asia', lat: 36.2, lon: 138.2 }, cities,
    });

    const pending = service.findChildren({ teamKey: 'team', scopeKey, childrenRequestToken: 'children-token' }, key);
    ...
    await Promise.all(result.cities.map(({ imageRequestToken }) => service.generatePlaceHeroImage({ teamKey: 'team', scopeKey, imageRequestToken }, key)));
  ...
  test('authorizes children tokens before decryption, expiry checks, or provider work', async () => {
    ...
    await expect(service.findChildren({ teamKey: 'team', scopeKey, childrenRequestToken: 'token' }, 'user')).rejects.toMatchObject({ reason: 'forbidden' });
    ...
    const expired = createTravelService({
      ...
      decryptChildrenRequest: () => ({ version: 1, teamKey: 'team', scopeKey, issuedAt, expiresAt: currentTime, nonce: 'E'.repeat(43), country: { name: 'Japan', code: 'JP', continent: 'Asia', lat: 36.2, lon: 138.2 }, cities }),
      execute: (async () => { providerCalls += 1; }) as any,
    });
    await expect(expired.findChildren({ teamKey: 'team', scopeKey, childrenRequestToken: 'token' }, 'user')).rejects.toThrow('expired');
  ...
  test('waits for the final destination brief before allowing hero generation', async () => {
    ...
    await service.generatePlaceHeroImage({ teamKey: 'team', scopeKey, imageRequestToken: found.imageRequestToken }, key);
  ...
  test('rejects wrong-country results and denies model work without access', async () => {
    ...
    await expect(denied.findPlaceGuide({ teamKey: 'team', scopeKey, query: 'Japan' }, 'user')).rejects.toMatchObject({ reason: 'forbidden' });
    expect(calls).toBe(0);
    let mismatchCalls = 0;
    const mismatch = createTravelService({ repository: { ...generatedPersistence, authorizeWrite: async () => key } as unknown as TravelRepository, execute: (async (_teamKey: string, input: any) => { if (!isBrief(input)) mismatchCalls += 1; return chatResponse(isBrief(input) ? imageBriefFor('Portugal') : JSON.stringify(modelGuideDetail)); }) as any, embed: async () => embedding, encryptImageRequest: () => 'token', decryptImageRequest: () => ({}) });
    await expect(mismatch.findPlaceGuide({ teamKey: 'team', scopeKey, query: 'Portugal', country: { name: 'Portugal', code: 'PT', continent: 'Europe', lat: 39.4, lon: -8.2 } }, key)).rejects.toThrow('Country provider returned an invalid guide.');
...
describe('travel repository', () => {
  test('converges a place and its canonical hero without Gallery collections or image links', async () => {
    ...
    await expect(createTravelRepository(database, async (value, operation) => { collections = value; return operation(database); }).convergePlace({ context: { teamKey: 'team', scopeKey, userKey: key }, place, hero })).resolves.toEqual({ place, heroStorageKey: hero.storageKey });
    expect(collections).toEqual({ read: ['userTeams', 'scopes', 'scopeMembers'], write: ['places', 'placeHeroMedia'] });
    expect(calls).toHaveLength(3);
    expect(calls[2]!.query).toContain('IN placeHeroMedia');
    expect(calls[1]!.bindVars).not.toHaveProperty('teamKey');
  ...
  test('lists authorized places and exposes the same read authorization', async () => {
    ...
    await expect(repository.authorizeRead({ teamKey: 'team', scopeKey, userKey: 'user' })).resolves.toBeUndefined();
    await expect(repository.authorizeWrite({ teamKey: 'team', scopeKey, userKey: 'user' })).resolves.toBe('membership');
    await expect(repository.overview({ teamKey: 'team', scopeKey, userKey: 'user' })).resolves.toEqual({ places: [{ place, heroStorageKey: 'media/tokyo.png' }], recentPlaces: [{ place: placeSchema.parse(recent) }] });
  ...
  test('inclusively filters saved-place creation dates before deterministic cosine ordering', async () => {
    ...
    await expect(createTravelRepository(database).searchPlaces({ teamKey: 'team', scopeKey, userKey: key }, embedding, { createdFrom: timestamp, createdTo: timestamp })).resolves.toEqual([]);
    ...
    expect(queries[0]!.bindVars).toMatchObject({ teamKey: 'team', scopeKey, userKey: key, dimensions: EMBEDDING_DIMENSIONS, createdFrom: timestamp, createdTo: timestamp });
  });
  ...
  test('atomically authorizes and opens only the trusted user and scope place', async () => {
    ...
    await expect(createTravelRepository(database).open({ teamKey: 'team', scopeKey, userKey: key }, 'JP', 'Tokyo', timestamp)).resolves.toEqual(opened);
    ...
    expect(calls[0]?.bindVars).toEqual({ teamKey: 'team', scopeKey, userKey: key, countryCode: 'JP', name: 'Tokyo', openedAt: timestamp });
    const denied: TravelDatabase = { async query() { return { async all() { return []; } }; } };
    await expect(createTravelRepository(denied).open({ teamKey: 'team', scopeKey, userKey: key }, 'JP', 'Tokyo', timestamp)).rejects.toMatchObject({ reason: 'forbidden' });
  });
  ...
  test('upserts generated detail as unsaved without allowing regeneration to downgrade saved rows', async () => {
    ...
    await expect(createTravelRepository(database).upsertGenerated({ teamKey: 'team', scopeKey, userKey: key }, generated)).resolves.toEqual(generated);
  ...
  test('upserts duplicate places under the write policy and returns persisted data', async () => {
    ...
    const saved = await createTravelRepository(database).create({ teamKey: 'team', scopeKey, userKey: 'user' }, place);
    ...
    expect(queries[0]?.bindVars).toMatchObject({ teamKey: 'team', scopeKey, userKey: 'user', countryCode: 'JP', name: 'Tokyo' });
  });
  ...
  test('authorizes saved-place updates, preserves no-ops, and returns hero storage', async () => {
    ...
    let collections: unknown;
    await expect(createTravelRepository(database, async (value, operation) => { collections = value; return operation(database); }).updatePlace({ teamKey: 'team', scopeKey, userKey: key }, key, { status: 'wishlist', isFavorite: false })).resolves.toEqual({ place, heroStorageKey: 'compass/tokyo.png' });
    ...
    expect(collections).toEqual({ read: ['userTeams', 'scopes', 'scopeMembers', 'placeHeroMedia'], write: ['places'] });
  });
  ...
  test('atomically creates a trip only from distinct saved places owned by the trusted user and scope', async () => {
    ...
    await expect(repository.createTrip({ teamKey: 'team', scopeKey, userKey: key }, trip, [relation], receipt)).resolves.toEqual({ trip, places: [place], placeHeroStorageKeys: ['media/tokyo.png'], attachments: [], coverStorageKey: 'media/tokyo.png' });
    expect(calls).toHaveLength(4);
    expect(collections).toEqual({ read: ['userTeams', 'scopes', 'scopeMembers', 'places', 'images', 'placeHeroMedia', 'tripAttachments', 'folders', 'collections', 'collectionImages'], write: ['tripCreationReceipts', 'trips', 'tripPlaces'] });
    ...
    expect(calls[0]!.bindVars).toMatchObject({ teamKey: 'team', scopeKey, userKey: key, placeKeys: [key] });
  });
  ...
  test('lists only trusted trips with ordered saved places and first-place cover storage', async () => {
    ...
    await expect(createTravelRepository(database).listTrips({ teamKey: 'team', scopeKey, userKey: key })).resolves.toEqual([{ trip, places: [place], placeHeroStorageKeys: [null], attachments: [], coverStorageKey: 'media/tokyo.png' }]);
  ...
  test('inclusively filters trip creation dates before ranking then reuses list aggregate loading', async () => {
    ...
    await expect(createTravelRepository(database).searchTrips({ teamKey: 'team', scopeKey, userKey: key }, embedding, { createdFrom: timestamp, createdTo: timestamp })).resolves.toMatchObject([{ trip: { key }, places: [{ key }] }]);
  ...
  test('replays the current persisted aggregate without writes and never resurrects a deleted receipted trip', async () => {
    ...
    await expect(repository.createTrip({ teamKey: 'team', scopeKey, userKey: key }, trip, [relation], receipt)).resolves.toMatchObject({ trip, places: [place], placeHeroStorageKeys: ['media/current.png'] });
    expect(calls).toHaveLength(1);
    deleted = true;
    await expect(repository.createTrip({ teamKey: 'team', scopeKey, userKey: key }, trip, [relation], receipt)).rejects.toMatchObject({ reason: 'gone' });
  ...
  test('atomically validates and replaces all trip attachments before returning the preloaded presentation', async () => {
    ...
    await expect(createTravelRepository(database, runTransaction).setTripAttachments({ teamKey: 'team', scopeKey, userKey: key }, key, [attachment], timestamp)).resolves.toMatchObject({ attachments: [attachment] });
    ...
    expect(query).toContain('collection.ownerKey == membership._key');
    ...
    expect(calls[0]!.bindVars).toMatchObject({ teamKey: 'team', scopeKey, userKey: key, tripKey: key });
  });
  ...
  test('atomically authorizes aggregate trip updates, cover access, and full place replacement', async () => {
    ...
    let collections: unknown;
    const repository = createTravelRepository(database, async (value, operation) => { collections = value; return operation(database); });
    await expect(repository.updateTrip({ teamKey: 'team', scopeKey, userKey: key }, key, { description: null, coverImageKey: key, isFavorite: true, status: 'completed' }, [relation], timestamp)).resolves.toMatchObject({ trip, places: [visitedPlace], accessibleCoverImageKey: key, coverStorageKey: 'media/custom.png' });
  ...
  test('normalizes a missing trip description from Arango for semantic updates', async () => {
    ...
    const repository = createTravelRepository(database);
    await expect(repository.tripSemanticSourceForUpdate({ teamKey: 'team', scopeKey, userKey: key }, key)).resolves.toEqual({ name: 'Route' });
  });
  ...
  test('keeps identical aggregate updates and attachment sets as true no-ops', async () => {
    ...
    await expect(updateRepository.updateTrip({ teamKey: 'team', scopeKey, userKey: key }, key, { name: trip.name, isFavorite: false }, undefined, '2026-08-21T13:00:00.000Z')).resolves.toMatchObject({ trip: { updatedAt: timestamp } });
    ...
    await expect(attachmentRepository.setTripAttachments({ teamKey: 'team', scopeKey, userKey: key }, key, [attachment], '2026-08-21T13:00:00.000Z')).resolves.toMatchObject({ trip: { updatedAt: timestamp }, attachments: [attachment] });
  ...
  test('blocks favorite trip deletion and hard-deletes all trip relations otherwise', async () => {
    ...
    const repository = createTravelRepository(database, async (_value, operation) => operation(database));
    await expect(repository.deleteTrip({ teamKey: 'team', scopeKey, userKey: key }, key)).rejects.toMatchObject({ reason: 'favorite' });
    await expect(repository.deleteTrip({ teamKey: 'team', scopeKey, userKey: key }, key)).resolves.toEqual({ tripKey: key });
  ...
  test('hard-deletes an owned saved place and dependents, updates affected trips, and replays deletion', async () => {
    ...
    expect(collections).toEqual({ read: ['userTeams', 'scopes', 'scopeMembers', 'places'], write: ['places', 'placeHeroMedia', 'placeReferences', 'tripPlaces', 'trips', 'storageDeletionJobs', 'tagAssignments'] });
    expect(calls[0]!.query).toContain('place.scopeKey == @scopeKey && place.userKey == @userKey && place.saved == true');
  ...
  test('uses member-but-not-viewer policy for private Compass writes without broadening Gallery elevation', async () => {
    ...
    const writeFilters = source.match(/FILTER membership\.teamRole IN \["owner", "admin"\] \|\| scopeRole IN \[[^\n]+/g) ?? [];
    expect(writeFilters.filter((line) => line.includes('"member"') && !line.includes('"viewer"')).length).toBeGreaterThanOrEqual(13);
    expect(source).not.toContain('FILTER membership.teamRole IN ["owner", "admin"] || scopeRole IN ["owner", "admin", "moderator"]\n');
    expect(source).toContain('LET elevated = membership.teamRole IN ["owner", "admin"] || scopeRole IN ["owner", "admin", "moderator"]');
  ...
  test('rejects reuse of a trip request key for different data', async () => {
    ...
    await expect(createTravelRepository(database, async (_value, operation) => operation(database)).createTrip({ teamKey: 'team', scopeKey, userKey: key }, trip, [relation], receipt)).rejects.toMatchObject({ reason: 'conflict' });
  });

  test('denies absent membership', async () => {
    const database: TravelDatabase = { async query() { return { async all() { return []; } }; } };
    await expect(createTravelRepository(database).authorizeRead({ teamKey: 'team', scopeKey, userKey: 'user' })).rejects.toMatchObject({ reason: 'forbidden' });
  });
});
