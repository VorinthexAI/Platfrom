...
describe('user settings service', () => {
  test('reads and updates settings through one canonical repository boundary', async () => {
    const calls: unknown[] = [];
    const repository = {
      read: async (userKey: string) => { calls.push(['read', userKey]); return { archive: { showOnlyFavorites: false }, gallery: { showOnlyFavorites: false } }; },
    ...
    expect(await service.read('user-1')).toEqual({ archive: { showOnlyFavorites: false }, gallery: { showOnlyFavorites: false } });
    expect(await service.update('user-1', { archive: { showOnlyFavorites: true }, gallery: { showOnlyFavorites: true } })).toEqual({ archive: { showOnlyFavorites: true }, gallery: { showOnlyFavorites: true } });
    expect(calls).toEqual([
      ['read', 'user-1'],
      ['update', 'user-1', { archive: { showOnlyFavorites: true }, gallery: { showOnlyFavorites: true } }, '2026-08-17T00:00:00.000Z'],
    ]);
  });
  ...
  test('rejects unknown settings and missing users', async () => {
    ...
    await expect(service.update('user-1', { archive: { showOnlyFavorites: false } })).rejects.toThrow();
