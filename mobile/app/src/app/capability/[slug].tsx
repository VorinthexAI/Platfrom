...
export default function CapabilityRoute() {
  ...
  if (parsed.data === "archive") return <KnowledgeWorkspace initialCollectionKind={params.collectionKind} initialDocumentKey={params.documentKey} initialFolderKey={params.assetKey} initialSearchQuery={params.initialQuery} key={`${scopeKey}:${params.assetKey ?? "root"}:${params.documentKey ?? ""}:${params.collectionKind ?? ""}:${params.initialQuery ?? ""}`} returnSignalConnectorKey={params.returnSignalConnectorKey} returnSignalMessageKey={params.returnSignalMessageKey} returnSignalThreadKey={params.returnSignalThreadKey} returnTripKey={params.returnTripKey} returnTripName={params.returnTripName} />;
  if (parsed.data === "gallery") return <GalleryWorkspace initialCollectionKey={params.assetKey} initialImageKey={params.imageKey} initialSearchQuery={params.initialQuery} key={`${scopeKey}:${params.assetKey ?? "root"}:${params.imageKey ?? ""}:${params.initialQuery ?? ""}`} returnSignalConnectorKey={params.returnSignalConnectorKey} returnSignalMessageKey={params.returnSignalMessageKey} returnSignalThreadKey={params.returnSignalThreadKey} returnTripKey={params.returnTripKey} returnTripName={params.returnTripName} />;
  if (parsed.data === "compass") return <TravelWorkspace initialCollectionKind={params.collectionKind} initialCountryCode={params.countryCode} initialPlaceKey={params.placeKey} initialSearchQuery={params.initialQuery} initialTripKey={params.tripKey} key={`${scopeKey}:${params.placeKey ?? ""}:${params.tripKey ?? ""}:${params.countryCode ?? ""}:${params.collectionKind ?? ""}:${params.initialQuery ?? ""}:${params.openTripAssets ?? ""}`} openTripAssets={params.openTripAssets === "1"} />;
  if (parsed.data === "signal") return <EmailWorkspace initialConnectorKey={params.connectorKey} initialDraftKey={params.draftKey} initialMessageKey={params.signalMessageKey} initialThreadKey={params.signalThreadKey} initialToneKey={params.toneKey} navigatedFromRoot={params.signalReturn === "root"} openAttachments={params.openSignalAttachments === "1"} initialCollectionKind={params.collectionKind} initialSearchQuery={params.initialQuery} key={`${scopeKey}:${params.connectorKey ?? "root"}:${params.signalThreadKey ?? ""}:${params.signalMessageKey ?? ""}:${params.draftKey ?? ""}:${params.toneKey ?? ""}:${params.collectionKind ?? ""}:${params.initialQuery ?? ""}`} />;
  return <AscendWorkspace initialBookKey={params.bookKey} initialSearchQuery={params.initialQuery} key={`${scopeKey}:${params.bookKey ?? "root"}:${params.initialQuery ?? ""}`} />;
}
