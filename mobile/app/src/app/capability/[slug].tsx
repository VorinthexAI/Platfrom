...
export default function CapabilityRoute() {
  ...
  if (parsed.data === "signal") return <EmailWorkspace initialCommunicationThreadKey={params.thread} initialCompose={params.compose === "issue" || params.compose === "feedback" ? params.compose : undefined} initialConnectorKey={params.connectorKey} initialDraftKey={params.draftKey} initialInbox={params.inbox} initialMessageKey={params.signalMessageKey} initialTab={communicationTabSchema.catch("unread").parse(params.tab)} initialThreadKey={params.signalThreadKey} initialToneKey={params.toneKey} navigatedFromRoot={params.signalReturn === "root"} openAttachments={params.openSignalAttachments === "1"} initialCollectionKind={params.collectionKind} initialSearchQuery={params.initialQuery} key={`${scopeKey}:${params.connectorKey ?? "root"}:${params.signalThreadKey ?? params.thread ?? ""}:${params.signalMessageKey ?? ""}:${params.draftKey ?? ""}:${params.toneKey ?? ""}:${params.collectionKind ?? ""}:${params.initialQuery ?? ""}:${params.compose ?? ""}:${params.inbox ?? ""}:${params.tab ?? ""}`} />;
