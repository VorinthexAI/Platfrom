...
export function ConversationRetrievalSheet({ contextIdentity, fresh, onClose, onNavigate, open, retrievals }: ConversationRetrievalSheetProps) {
  const queryClient = useQueryClient();
  const inputs = retrievals.map((retrieval) => ({ query: retrieval.query, collectionSlugs: [...new Set(retrieval.groups.map(({ collectionSlug }) => collectionSlug))], recordHistory: false as const, limit: retrieval.limit, minimumScore: retrieval.minimumScore, ...(retrieval.filters ? { filters: retrieval.filters } : {}) }));
  const queries = useQueries({
    queries: inputs.map((input) => ({ queryKey: appSearchQueryKey(contextIdentity, input), queryFn: ({ signal }: { signal: AbortSignal }) => searchApp(input, signal), enabled: open, staleTime: 0 })),
  });
  useEffect(() => {
    if (!open) return;
    const keys = retrievals.map((retrieval) => appSearchQueryKey(contextIdentity, { query: retrieval.query, collectionSlugs: [...new Set(retrieval.groups.map(({ collectionSlug }) => collectionSlug))], recordHistory: false, limit: retrieval.limit, minimumScore: retrieval.minimumScore, ...(retrieval.filters ? { filters: retrieval.filters } : {}) }));
    return () => { keys.forEach((queryKey) => queryClient.removeQueries({ queryKey, exact: true })); };
  }, [contextIdentity, open, queryClient, retrievals]);
