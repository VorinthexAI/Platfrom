...
export function NotificationsSheet({ onClose, open }: { onClose: () => void; open: boolean }) {
  ...
  const query = useQuery({ queryKey: notificationQueryKey(userKey, teamKey), queryFn: () => listNotifications({ teamKey, scopeKey }), enabled: open && Boolean(userKey && teamKey && scopeKey) });

  useEffect(() => {
    if (!open || !userKey || !teamKey || !scopeKey) return;
  ...
  }, [open, teamKey, queryClient, scopeKey, userKey]);

  return <BottomSheet focusKey="notifications" footer={<Button onPress={onClose} size="md" variant="secondary">Close</Button>} height="full" onDismissRequest={onClose} onOpenChange={(next) => { if (!next) onClose(); }} open={open} title="Notifications">
