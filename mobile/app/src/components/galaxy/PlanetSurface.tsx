...
/**
 * Ported verbatim from the web galaxy's PlanetSurface: shader-driven biome
 * worlds lit from the origin (there the Nexus sun, here the brain core).
 * Segments default lower for mobile GPUs; hover states dropped (no cursor).
 */
...
const planetFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // The brain core sits at the origin — planets are lit from within the system.
    ...
    // Chrome/water specular from the core light.
    ...
    // Cool silver dusk band: the core's light grazing the terminator.
...
export function PlanetSurface({
  ...
  const style = useMemo(() => biomeStyleFor(entityId, biome), [entityId, biome]);

  const geometry = useMemo(
    () => createBiomePlanetGeometry(hashString(entityId), biome, radius, segments),
    [entityId, biome, radius, segments],
  );
  useEffect(() => {
    return () => {
      geometry.dispose();
  ...
  useFrame((_, delta) => {
    ...
    const rim = materialRef.current.uniforms.uRimStrength;
    if (!paused) {
    ...
    if (rim) {
      rim.value = THREE.MathUtils.damp(
        rim.value,
        focused ? 1.05 : 0.65,
        4,
        delta,
