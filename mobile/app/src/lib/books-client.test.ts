...
beforeEach(() => { calls.splice(0); lifecycleFailure = false; favoriteDeleteFailure = false; authState.team = { key: "team-key" }; authState.scope = { key: "scope-key" }; });
...
test("sends strictly scoped overview, creation, detail, and progress requests", async () => {
  ...
  expect(calls[1]?.body).toEqual({ teamKey: "team-key", scopeKey: "scope-key", generationRequestKey: "request-key", topic: "Useful habits", goal: "Build a durable practice", currentKnowledge: "A curious beginner", writingTone: "Warm and direct", language: "English", narratorVoiceKey: "clear", narrationPace: 1, archiveDocumentKeys: ["document-key"], additionalInstructions: "Use concrete examples." });
  expect(calls[1]?.config).toEqual({ timeout: 15 * 60_000 });
  expect(calls[3]?.body).toEqual({ teamKey: "team-key", scopeKey: "scope-key", progressSeconds: 480, isCompleted: true });
});
...
test("searches books through app.search without recording history for result refreshes", async () => {
  ...
  expect(calls[0]).toEqual({ method: "POST", path: "/app/search", body: { teamKey: "team-key", scopeKey: "scope-key", query: "practice", collectionSlugs: ["books"], recordHistory: false, limit: 50 }, config: { signal: controller.signal, timeout: 15_000 } });
});
...
test("filters book searches and empty-query lists by every normalized tag", async () => {
  ...
  expect(calls.map(({ body }) => body)).toEqual([
    { teamKey: "team-key", scopeKey: "scope-key", query: "practice", collectionSlugs: ["books"], recordHistory: true, limit: 50, filters: { tagKeys: ["tag-a", "tag-b"], tagMatch: "all" } },
    { teamKey: "team-key", scopeKey: "scope-key", operation: "list", collectionSlugs: ["books"], recordHistory: false, limit: 50, filters: { tagKeys: ["tag-a", "tag-b"], tagMatch: "all" } },
  ]);
});
...
test("requests ten fresh topic suggestions with exclusions", async () => {
  ...
  expect(calls[0]).toEqual({ method: "POST", path: "/books/topic-suggestions", body: { teamKey: "team-key", scopeKey: "scope-key", excludeTopics: ["Old idea"] }, config: { timeout: 50_000 } });
});
...
test("requests ten fresh goal suggestions for the selected topic", async () => {
  ...
  expect(calls[0]).toEqual({ method: "POST", path: "/books/goal-suggestions", body: { teamKey: "team-key", scopeKey: "scope-key", topic: "Decision making", excludeGoals: ["Old goal"] }, config: { timeout: 50_000 } });
});
...
test("previews and generates strict scoped book extensions", async () => {
  ...
  expect(calls).toEqual([
    { method: "POST", path: "/books/book-key/extension/preview", body: { teamKey: "team-key", scopeKey: "scope-key", chapterCount: 3 }, config: { timeout: 30_000 } },
    { method: "POST", path: "/books/book-key/extension", body: { teamKey: "team-key", scopeKey: "scope-key", chapterCount: 3, titles: ["Continue the Practice", "Make It Durable", "Teach the System"], requestKey: "extension-key" }, config: { timeout: 15 * 60_000 } },
  ]);
  expect(client.bookExtensionPreviewRequestSchema.safeParse({ teamKey: "team", scopeKey: "scope", chapterCount: 2 }).success).toBe(false);
  expect(client.bookExtensionRequestSchema.safeParse({ teamKey: "team", scopeKey: "scope", chapterCount: 3, titles: ["One", "Two", "Three"], requestKey: "key", extra: true }).success).toBe(false);
  expect(client.bookExtensionRequestSchema.safeParse({ teamKey: "team", scopeKey: "scope", chapterCount: 3, titles: ["Title"], requestKey: "key" }).success).toBe(false);
...
test("rejects invalid requests and unsafe response fields", async () => {
  expect(client.createBookRequestSchema.safeParse({ teamKey: "team", scopeKey: "scope", generationRequestKey: "request", topic: "Useful habits", goal: "Build a durable practice", currentKnowledge: "", writingTone: "warm", language: "en", chapterCount: 25, narratorVoiceKey: "clear", narrationPace: 1, archiveDocumentKeys: [] }).success).toBe(false);
  expect(client.createBookRequestSchema.safeParse({ teamKey: "team", scopeKey: "scope", generationRequestKey: "request", topic: "Useful habits", goal: "Build a durable practice", currentKnowledge: "", writingTone: "warm", language: "en", narratorVoiceKey: "clear", narrationPace: 1, archiveDocumentKeys: [] }).success).toBe(true);
  expect(client.createBookRequestSchema.safeParse({ teamKey: "team", scopeKey: "scope", generationRequestKey: "request", topic: "Useful habits", goal: "Build a durable practice", currentKnowledge: "", writingTone: "warm", language: "en", narratorVoiceKey: "clear", narrationPace: 1, archiveDocumentKeys: Array.from({ length: 11 }, (_, index) => `document-${index}`) }).success).toBe(false);
...
test("parses the final lifecycle and media DTO strictly", () => {
  ...
  expect(client.chapterProgressRequestSchema.safeParse({ teamKey: "team", scopeKey: "scope", progressSeconds: 1.5, isCompleted: false }).success).toBe(false);
});
...
test("asks Core to create books on the scoped book workspace", async () => {
  ...
  expect(calls[0]?.body).toEqual({ teamKey: "team-key", scopeKey: "scope-key", input: { surface: "book-workspace", requestKey: "request-key", message: "Create a short book about useful habits", currentNote: { title: "", content: "" } } });
});
...
test("sends scoped favorite, retry, cancel, and delete lifecycle requests", async () => {
  ...
  expect(calls.map(({ body }) => body)).toEqual([
    { teamKey: "team-key", scopeKey: "scope-key", isFavorite: true },
    { teamKey: "team-key", scopeKey: "scope-key", requestKey: "retry-key" },
    { teamKey: "team-key", scopeKey: "scope-key", requestKey: "cancel-key" },
    { teamKey: "team-key", scopeKey: "scope-key", requestKey: "delete-key" },
  ]);
});
