...
test("onboarding uses one scripted Core conversation before auth and plans", () => {
  ...
  expect(conversation).toContain("void recordOnboardingEvent(event, appKeyForStep(step, apps))");
...
test("Core requests only required permissions conversationally without automatic Gallery imports", () => {
  const nativeConfig = JSON.parse(appConfig);
  ...
  expect(onboarding).toContain('onComplete={() => setPhase(postDeletion ? "profile-badge" : "reward")}');
  ...
  expect(reward).toContain("<SparksIcon");
  ...
  expect(conversation).not.toContain('permission: "photos"');
  ...
  expect(conversation).not.toContain("ImagePicker.requestMediaLibraryPermissionsAsync");
  expect(nativeConfig.expo.plugins.find((plugin: unknown) => Array.isArray(plugin) && plugin[0] === "expo-image-picker")?.[1].photosPermission).toBe(false);
  expect(nativeConfig.expo.android.blockedPermissions).toContain("android.permission.READ_EXTERNAL_STORAGE");
  expect(nativeConfig.expo.android.blockedPermissions).toContain("android.permission.WRITE_EXTERNAL_STORAGE");
  ...
  expect(conversation).not.toContain("Allow access to your phone's photo library");
  ...
  for (const source of [conversation, reward, stepLayout, paywall]) {
    for (const button of source.replaceAll("=>", "ARROW").match(/<Button\b[^>]*>/g) ?? []) expect(button).toContain(button.includes('accessibilityLabel="How Sparks are billed"') ? 'size="sm"' : 'size="md"');
  }
...
test("onboarding completion is durable and step analytics use fixed dotted slugs", () => {
  ...
  expect(onboardingState).toContain('setItemAsync(INTRO_SEEN_KEY, "true")');
  expect(authState).toContain("markOnboardingComplete()");
  expect(layout).toContain("localOnboarding.introSeen");
  ...
  expect(conversation).toContain("recordOnboardingEvent(event, appKeyForStep(step, apps))");
  expect(conversation).toContain('step.kind === "app" ? step.app.slug');
  expect(conversation).toContain('step.permission === "notifications" ? "signal" : "core"');
  ...
  for (const permission of ["camera", "notifications"]) {
  ...
  expect(onboardingEvents).not.toContain('"onboarding.photos"');
...
test("production onboarding completion persists and uses the authenticated profile", () => {
  ...
  expect(onboardingState).toContain("markPostDeletionOnboarding");
  expect(onboarding).toContain("getLocalOnboardingState().postDeletion");
  expect(onboarding).not.toContain("includeReferral=");
  expect(authState).toContain("clearPendingReferralCode()");
