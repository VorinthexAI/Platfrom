...
test("selects onboarding apps in explicit order using server presentation fields", () => {
  ...
  expect(stages[0]).toMatchObject({ key: app("vorinthex-ai", 6).key, name: "vorinthex-ai server name", description: "vorinthex-ai server description", logoUrl: "https://vorinthex.com/logos/vorinthex-ai.png" });
});
