...
test("maps every audited Gallery slug to invalidation", () => {
  for (const slug of ["collection.index.changed", "collection.content.changed", "image.changed", "upload.changed", "subject.changed", "highlight.changed", "memory.created", "memory.deleted"]) expect(invalidatesGalleryQueries(slug)).toBe(true);
