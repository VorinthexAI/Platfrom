...
test("loads a one-plus-three portrait media set independently from country text", () => {
  ...
  expect(workspace).toContain("countryImages.images.slice(1)");
  ...
  expect(workspace).toContain("Images sourced from the web");
  expect(workspace).toContain("countryDetail.sources.map");
  expect(workspace).toContain("IMAGE SOURCES");
