...
export const AiTextEditor = forwardRef<HTMLTextAreaElement, AiTextEditorProps>(function AiTextEditor({ "aria-label": ariaLabel, onOpenActions, onOpenHistory, style, transformation, value, ...props }, ref) {
  ...
  return <div style={{ position: "relative", width: "100%" }}>
    {transformation ? <div aria-label={`${transformation === "enhance" ? "Enhancing" : "Translating"} ${label.toLocaleLowerCase()}`} role="progressbar" style={{ minHeight: 400, width: "100%" }} /> : <textarea {...props} aria-label={ariaLabel} ref={ref} style={{ minHeight: 400, paddingBottom: 64, width: "100%", ...style, backgroundColor: "var(--vui-color-page, #030507)" }} value={value} />}
