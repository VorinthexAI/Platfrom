...
export function FileViewer({ blocks, error, loading = false, onBack, onMenu, onRead, onRenderError, pdfUri, readLoading = false, reading = false, title }: FileViewerProps) {
  ...
  return <View style={[styles.root, { paddingBottom: insets.bottom + 78 + spacing.md }]}><View style={styles.scene}>
    <View style={styles.header}><Button accessibilityLabel="Back" contentMode="raw" onPress={onBack} size="sm" variant="icon"><ChevronLeftIcon size="sm" /></Button><View style={styles.headerSpacer} />{onRead ? <Button accessibilityLabel={reading ? "Pause reading" : "Read document"} contentMode="raw" loading={readLoading} onPress={onRead} size="sm" variant="icon">{reading ? <PauseIcon size="sm" /> : readLoading ? <VolumeIcon size="sm" /> : <PlayIcon size="sm" />}</Button> : null}<Button accessibilityLabel={`Manage ${title}`} contentMode="raw" onPress={onMenu} size="sm" variant="icon"><MoreHorizontalIcon size="sm" /></Button></View>
