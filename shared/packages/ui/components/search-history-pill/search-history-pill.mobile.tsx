...
export function SearchHistoryPill({ count, disabled, onPress, onRemove, query, removing = false, style, ...props }: SearchHistoryPillProps) {
  return (
    <View style={[styles.root, style]} {...props}>
      <Button accessibilityLabel={`Remove ${query} from search history`} contentMode="raw" disabled={disabled} hitSlop={8} loading={removing} onPress={onRemove} size="xs" style={styles.remove} variant="secondary"><CloseIcon size="sm" /></Button>
      <Button accessibilityLabel={`Search for ${query}`} contentMode="raw" disabled={disabled || removing} onPress={onPress} size="sm" style={styles.queryButton} variant="ghost">
...
const styles = StyleSheet.create({
  ...
  remove: { width: 28, height: 28, paddingHorizontal: 0, paddingVertical: 0 },
  queryButton: { minWidth: 0, height: 32, flex: 1, gap: 8, paddingLeft: 4, paddingRight: 6, flexDirection: "row", justifyContent: "space-between", overflow: "hidden" },
