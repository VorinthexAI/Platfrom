...
export async function POST(request: Request, context: { params: Promise<{ artifactKey: string }> }) {
  ...
  const result = await backendFetch(`/founders/artifacts/${encodeURIComponent(artifactKey)}/resolve`, { method: "POST", headers: await foundersAuthHeaders(), body: await request.text() });
