      ...
      <AnimatePresence>
        {open ? <Dialog.Portal forceMount>
          ...
          <Dialog.Content asChild forceMount>
            <motion.section className="fixed inset-x-4 top-1/2 z-50 max-h-[calc(100svh-2rem)] w-auto -translate-y-1/2 overflow-y-auto rounded-2xl border border-white/12 bg-[#090b0d] p-5 shadow-[0_28px_100px_rgba(0,0,0,0.7)] outline-none sm:left-1/2 sm:right-auto sm:w-full sm:max-w-2xl sm:-translate-x-1/2" initial={reducedMotion ? { opacity: 0 } : { y: "-46%", opacity: 0, scale: 0.98 }} animate={reducedMotion ? { opacity: 1 } : { y: "-50%", opacity: 1, scale: 1 }} exit={reducedMotion ? { opacity: 0 } : { y: "-46%", opacity: 0, scale: 0.98 }} transition={{ duration: reducedMotion ? 0.12 : 0.32, ease: [0.16, 1, 0.3, 1] }}>
