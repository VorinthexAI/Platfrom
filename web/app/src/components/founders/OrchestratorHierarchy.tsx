...
function stationPosition(entity: GalaxyEntity): [number, number, number] {
  if (entity.slug === "atlas") return [0, 0.42, 0];
  const depth = depthFor(entity);
  const radius = LAYER_RADII[depth];
  const angle = THREE.MathUtils.degToRad(NODE_ANGLES[entity.slug] ?? 0);
  const height = depth === 1 ? 0.28 : depth === 2 ? 0 : -0.3;
...
function StationRing({ radius, layer, paused }: { radius: number; layer: number; paused: boolean }) {
  ...
  useFrame((_, delta) => {
    if (!paused && rotating.current) rotating.current.rotation.y += delta * (layer % 2 === 0 ? -0.012 : 0.009);
  });

  return (
    <group ref={rotating} position={[0, layer === 1 ? 0.18 : layer === 2 ? -0.06 : -0.32, 0]}>
      <mesh castShadow receiveShadow rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[radius, 0.14, 10, 192]} />
        <meshStandardMaterial color="#151719" metalness={0.94} roughness={0.3} />
      </mesh>
      <mesh position={[0, -0.24, 0]} castShadow rotation={[Math.PI / 2, 0, 0]}>
      ...
      <mesh position={[0, -0.1, 0]} rotation={[Math.PI / 2, Math.PI / segments, 0]}>
...
function DockingPod({ entity, selected, onSelect, onNavigate }: { entity: GalaxyEntity; selected: boolean; onSelect: (entity: GalaxyEntity) => void; onNavigate: NavigateOrchestrator }) {
  ...
  const depth = depthFor(entity);
  const podScale = depth === 0 ? 1.35 : depth === 1 ? 1 : depth === 2 ? 0.88 : 0.8;
  ...
  return (
    <group position={stationPosition(entity)} scale={podScale}>
      ...
      <Html center position={[0, -0.58, 0.35]} distanceFactor={10.5} zIndexRange={[20, 0]}>
        <button
      ...
      {selected ? <pointLight color="#ff7b18" intensity={depth === 0 ? 8 : 5} distance={depth === 0 ? 4 : 2.7} decay={2} position={[0, 0.45, 0]} /> : null}
    </group>
  );
}
...
function StationDebris({ paused }: { paused: boolean }) {
  ...
  const debris = useMemo(() => Array.from({ length: 72 }, (_, index) => {
    ...
    return {
      position: [Math.cos(angle) * radius, ((index * 43) % 100) / 24 - 2.1, Math.sin(angle) * radius] as [number, number, number],
      scale: 0.025 + ((index * 17) % 10) / 120,
      rotation: [angle, angle * 0.7, angle * 1.3] as [number, number, number],
    };
  }), []);

  useFrame((_, delta) => { if (!paused && field.current) field.current.rotation.y -= delta * 0.004; });
  return (
    <group ref={field}>
...
function CameraRig({ paused }: { paused: boolean }) {
  ...
  useFrame((_, delta) => {
    ...
    current.position.x = THREE.MathUtils.damp(current.position.x, pointer.x * 0.48, 2.5, delta);
    current.position.y = THREE.MathUtils.damp(current.position.y, 7.6 + pointer.y * 0.3, 2.5, delta);
...
function CommandStation({ selectedSlug, paused, onSelect, onNavigate }: { selectedSlug: string; paused: boolean; onSelect: (entity: GalaxyEntity) => void; onNavigate: NavigateOrchestrator }) {
  ...
  useEffect(() => {
    ...
    if (paused) station.current.rotation.y = rotationTarget.current;
  }, [paused, selectedSlug]);

  useFrame((_, delta) => {
    if (!paused && station.current) {
      station.current.rotation.y = THREE.MathUtils.damp(station.current.rotation.y, rotationTarget.current, 3.2, delta);
    }
  });

  return (
    <group ref={station} rotation={[0, -0.015, 0]}>
      <StationSpokes />
      ...
      {ORCHESTRATORS.map((entity) => <DockingPod key={entity.id} entity={entity} selected={entity.slug === selectedSlug} onSelect={onSelect} onNavigate={onNavigate} />)}
...
export default function OrchestratorHierarchy({ selectedSlug, onSelect }: OrchestratorHierarchyProps) {
  ...
  return (
    <div className="relative h-full min-h-[540px] w-full overflow-hidden" aria-label="Nexus command station">
      <Canvas dpr={[1, 1.35]} shadows camera={{ position: [0, 7.6, 13.8], fov: 44, near: 0.1, far: 90 }} gl={{ alpha: true, antialias: true, powerPreference: "high-performance" }} className="!absolute !inset-0">
