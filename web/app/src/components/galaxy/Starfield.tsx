...
/** Three depth layers of monochrome stars plus a faint drifting dust belt. */
export function Starfield({ paused, dense }: { paused: boolean; dense: boolean }) {
  const scale = dense ? 1 : 0.4;
  return (
    <group>
      <StarLayer
        count={Math.round(1600 * scale)}
        radius={[40, 90]}
        size={0.14}
        opacity={0.75}
        rotationSpeed={0.004}
