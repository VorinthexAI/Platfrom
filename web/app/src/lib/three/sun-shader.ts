/**
 * The Nexus sun's surface shader — plasma breathing, convection-cell
 * churn, golden limb. One source of truth shared by the star in the
 * galaxy overview (NexusSun) and the full-bleed founders-gate backdrop
 * (/nexus), so the gate shows the EXACT same surface, colors, and
 * animations as the outside of the sun.
 */

export const SUN_SURFACE_VERTEX_SHADER = /* glsl */ `
  uniform float uTime;
...
export const SUN_SURFACE_FRAGMENT_SHADER = /* glsl */ `
  uniform float uTime;
  uniform float uPulse;
  varying vec3 vNormal;
  varying vec3 vPosition;
...
/**
 * The star's heartbeat, sampled per frame into the uPulse uniform: a deep
 * primary pulse with a double-beat on top. Shared so the gate backdrop
 * beats in exact time with the galaxy sun.
 */
export function sunHeartbeat(time: number): number {
  return 0.5 + 0.46 * Math.sin(time * 0.9) + 0.16 * Math.sin(time * 2.3 + 1.2);
}
