...
// Mirrors src/shared/lib/api-client.ts's base-URL/version-path/header
 ...
 * session validation; we just relay whatever `vx_session` etc. the browser
 * sent us) and injects the server-only `x-vorinthex-api-key` header.
 */
 ...
 * Defaults to a JSON `Content-Type` whenever a body is present, since almost
 * every auth/chat/universe endpoint speaks JSON — override per-call for
 * anything else (e.g. multipart).
 */
 ...
 * for streaming call sites (chat SSE, universe binary tiles) per the plan's
 * file manifest — those callers read `response.body` as a `ReadableStream`
 * and should not have a JSON `Content-Type` assumed onto their (often
 * bodyless GET, or non-JSON POST) requests.
 */
